...
Add capabilities by extending the `runners` map in `plugins/sevens-core/scripts/witness/perf.mjs`. The framework supports any synchronous benchmark that throws on failure.
...
node plugins/sevens-core/scripts/witness/perf.mjs

# Run with baseline comparison (median of last 5 entries per capability)
node plugins/sevens-core/scripts/witness/perf.mjs --baseline
...
- name: Performance verification
  run: |
    node plugins/sevens-core/scripts/witness/perf.mjs --baseline --json > /tmp/perf.json
    node -e "
...
- **CLI startup time** — `sevens --version` is the obvious metric, but currently dominated by node startup + module graph; not stable enough as a regression signal until the cli-core split (PR #1764) lands.
...
- [`plugins/sevens-core/scripts/witness/perf.mjs`](../plugins/sevens-core/scripts/witness/perf.mjs) — benchmark runner
