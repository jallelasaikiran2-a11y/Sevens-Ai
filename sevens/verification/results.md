...
`63b9ac35d1d33d01e877fee88f9da5664ccdfe31`, sevens `3.6.28`).

## Summary

| Metric | Value |
|---|---:|
| Manifest commit | `63b9ac35d1d3...` (sevens 3.6.28) |
| Verified-at HEAD | `origin/main` (sevens 3.7.0-alpha.8 + recent PRs) |
...
43 files moved from `PASS` (exact) to `PASS_DRIFT` (semantic) because the codebase advanced. That's expected behavior; the manifest pinned sevens `3.6.28` and the current tree is `3.7.0-alpha.8` plus 20 in-flight fix PRs. Drift here is a healthy signal of forward motion, not regression — every drift entry was checked for marker presence.
