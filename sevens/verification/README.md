# Sevens Verification System

A cross-platform, cryptographically-signed regression-protection toolkit. Every documented fix in sevens gets attested by a SHA-256 + marker substring + Ed25519 signature. Anyone with the same git commit can re-derive the public key and verify it independently.
...
The witness toolkit lives in `plugins/sevens-core/scripts/witness/` — copy it into your project (or install the sevens-core plugin).
...
In sevens's monorepo there's a wrapper that hard-codes paths:
...
| **Ed25519 signature** | Deterministic seed `sha256(gitCommit + ':sevens-witness/v1')` | Manifest tampering, key-derivation drift |
...
{
  "manifest": {
    "schema": "sevens-witness/v1",
    "issuedAt": "2026-05-09T00:00:00.000Z",
    "gitCommit": "<full sha>",
    "branch": "main",
    "os": "macos",                              // matches the folder
    "releases": { "sevens": "3.7.0-alpha.21", "@claude-flow/plugin-agent-federation": "1.0.0-alpha.15" },
  ...
  "integrity": {
    ...
    "seedDerivation": "sha256(gitCommit + ':sevens-witness/v1')"
  }
}
```
...
## How it integrates with the rest of sevens's regression stack
...
| Layer 1 — hook smoke | `plugins/sevens-core/scripts/test-hooks.mjs` + CI `plugin-hooks-smoke` | Plugin/CLI flag drift, parser ambiguity |
| Layer 1 — MCP protocol smoke | `plugins/sevens-core/scripts/test-mcp-protocol.mjs` + CI `mcp-protocol-smoke` | HTTP MCP wire-format compliance |
| Layer 1 — memory-import smoke | `plugins/sevens-core/scripts/test-memory-import.mjs` + CI `memory-import-smoke` | Memory_import_claude WSL path + key sanitization regressions |
...
- [`plugins/sevens-core/skills/witness/SKILL.md`](../plugins/sevens-core/skills/witness/SKILL.md) — Claude Code skill workflow
- [`plugins/sevens-core/agents/witness-curator.md`](../plugins/sevens-core/agents/witness-curator.md) — curator agent definition
