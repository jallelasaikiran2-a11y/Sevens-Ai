 ...
 *
 * The bash shim (sevens-hook.sh) works on Mac/Linux but fails on native
 ...
 *
 * Mac/Linux continue to use sevens-hook.sh via the plugin hooks.json files
 * (unchanged). On Windows, sevens init writes a .claude/settings.json that
 * overrides those entries with node-based equivalents pointing here.
 *
 * Behaviour mirrors sevens-hook.sh:
 *   1. Reads hook JSON payload from stdin.
 *   2. Prefers a locally installed `sevens` or `claude-flow` binary.
 *   3. Falls back to `npx --prefer-offline sevens@latest`.
 *   4. Always exits 0 — hook subcommands are best-effort telemetry.
 *   5. Swallows all stderr — nothing should surface to Claude Code.
 *
 * Usage: node sevens-hook.cjs <hook-subcommand> [args...]
 *   e.g. node sevens-hook.cjs post-edit --file "x.ts" --train-patterns
 */
...
/** Build the argv for the sevens/claude-flow/npx invocation */
function buildArgs(subcommand, extraArgs) {
  // The `hooks` word is prepended here, matching sevens-hook.sh convention.
...
function main() {
  ...
  // Priority 1: locally installed sevens binary
  if (commandExists('sevens')) {
    invokeHook('sevens', [], hookArgs, stdinData);
    done();
  }
  ...
  //
  // SKIP this when SEVENS_HOOK_SKIP_NPX=1 — used by CI smokes that test
