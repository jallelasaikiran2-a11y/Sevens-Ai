# .harness/ — sevens's MetaHarness lifecycle directory

This directory holds the files that `harness <subcommand>` from `metaharness` reads to assess sevens's governance posture. See [ADR-150](../v3/docs/adr/ADR-150-metaharness-integration-surfaces.md) for context.
...
`harness sign .` produces a witness from `.harness/manifest.json` BUT requires `WITNESS_SIGNING_KEY` (64-char hex Ed25519 private key) in the environment. Sevens's existing `brain-signing-key` in GCP Secret Manager is in a different format (89 chars, not hex) and can't be reused.
...
Current state (option 3) gives sevens:
...
The witness is genuinely optional unless an external party needs to verify the manifest hasn't been tampered with. For sevens's internal use it's nice-to-have polish.

## Verifying sevens's current posture
...
# Or via sevens's wrappers (no metaharness install required if optional dep is present)
npx sevens metaharness threat-model
npx sevens metaharness mcp-scan
npx sevens metaharness oia-audit --dry-run
```
...
Smoke step 17t (in `plugins/sevens-metaharness/scripts/smoke.sh`) locks the policy invariants. If you change the policy in a way that violates `defaultDeny:true / auditLog:true / requireApprovalForDangerous:true` or removes the `toolTimeoutMs`/`maxToolCallsPerTurn` positive-number requirements, CI will fail.
