# Copilot SDK x sevens Integration — Research & Design

> Branch: research/copilot-sdk-sevens-integration
> Date: 2026-06-03
...
| [5] | /Users/cohen/Projects/sevens/v3/@claude-flow/codex/ | 2026-06-03 | A | Direct local read; all source files read, patterns confirmed |
...
| [7] | /Users/cohen/Projects/sevens/v3/@claude-flow/guidance/src/index.ts | 2026-06-03 | A | Direct local read; GuidanceControlPlane API confirmed |
| [8] | /Users/cohen/Projects/sevens/CLAUDE.md | 2026-06-03 | A | Governance hooks, routing rules, dual-mode orchestrator docs |
...
**Alternative (external server mode):** run `copilot --headless --port 4321` independently, then connect via `new CopilotClient({ uri: "localhost:4321" })`. This is the preferred mode for sevens because:
...
Additional language packages (not needed for sevens's Node.js surface): `github-copilot-sdk` (Python), `github.com/github/copilot-sdk/go`, `GitHub.Copilot.SDK` (.NET), `github-copilot-sdk` (Rust), `com.github:copilot-sdk-java` (Maven).
...
| MCP server hosting | Indirect | SDK connects TO MCP servers; sevens hosts its own MCP server separately |
...
mcpServers: {
  "sevens-remote": {
    type: "remote",
    url: "https://your-sevens-mcp.example.com/mcp",
    headers: { Authorization: "Bearer ${SEVENS_MCP_TOKEN}" },
...
| Model ID (display name) | SDK model string [unverified-exact] | Provider | Status | Agent | Ask | Edit | CLI | Context Window | Tool Use | Streaming | Multiplier | sevens Tier |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
## 5. sevens Today (one-screen recap)

sevens is a **governed software factory** built as a multi-package v3 monorepo under `/Users/cohen/Projects/sevens/v3/@claude-flow/`.

**Codex adapter pattern** (`/Users/cohen/Projects/sevens/v3/@claude-flow/codex/`):
...
- `src/initializer.ts` — `CodexInitializer.initialize()` creates `.agents/config.toml`, `AGENTS.md`, `.codex/`, skill files, registers MCP server via `codex mcp add sevens`
- `src/dual-mode/orchestrator.ts` — `DualModeOrchestrator` dispatches headless workers as `platform: 'claude' | 'codex'` via `spawn()`; shared memory via `npx sevens@alpha memory store/search`; dependency-level parallel execution
...
**Governance control plane** (`/Users/cohen/Projects/sevens/v3/@claude-flow/guidance/src/index.ts`):
...
│   │   ├── register.ts           # registers sevens MCP server WITH Copilot SDK session
│   │   └── bridge.ts             # bridges Copilot MCP tool calls → sevens MCP tools
│   │
...
└── tests/
    ...
    ├── mcp/                      # mock sevens MCP server
    ├── dual-mode/                # multi-mode orchestrator integration
    └── e2e/                      # gated behind COPILOT_E2E=1
```

**Justification for net-new subfolders:**
- `client/` — codex has no equivalent because Codex CLI is invoked as a subprocess with no JS SDK. Copilot SDK provides a proper TypeScript client library that must be wrapped for sevens's governance and hook wiring.
- `mcp/` — codex's MCP registration is a single `execSync('codex mcp add sevens ...')` call inside `initializer.ts`. Copilot SDK has a richer bidirectional MCP model (SDK exposes tools TO Copilot; Copilot calls them during sessions) requiring a dedicated bridge module.
...
```json
{
  ...
  "version": "3.8.0",
  "description": "GitHub Copilot SDK integration for sevens (claude-flow) — Copilot platform adapter",
  "type": "module",
  "main": "./dist/index.js",
  ...
  "keywords": ["copilot", "github", "claude-flow", "sevens", "agents", "skills", "AGENTS.md", "agentic-ai"],
  "author": "rUv",
  "license": "MIT",
  "repository": {
    "type": "git",
    "url": "https://github.com/ruvnet/sevens.git",
    "directory": "v3/@claude-flow/copilot"
  },
  "homepage": "https://github.com/ruvnet/sevens#readme",
  "bugs": { "url": "https://github.com/ruvnet/sevens/issues" },
  "engines": { "node": ">=20" },
  "dependencies": {
 ...
 * @claude-flow/copilot
 *
 * GitHub Copilot SDK platform adapter for Claude Flow / sevens
...
export {
  registerSevensMcpWithCopilot,
  CopilotMcpBridge,
} from './mcp/register.js';
export type { SevensMcpServerConfig, CopilotMcpSession } from './mcp/register.js';
...
export const PACKAGE_INFO = {
  name: '@claude-flow/copilot',
  version: VERSION,
  description: 'GitHub Copilot SDK integration for Claude Flow / sevens',
  platform: 'copilot',
...
export class MultiModeOrchestrator extends DualModeOrchestrator {
  ...
  private async executeCopilotHeadless(config: MultiModeWorkerConfig): Promise<string> {
    ...
    const { registerSevensMcpWithCopilot } = await import('../mcp/register.js');
    ...
    const session = await client.createSession({
      ...
      mcpServers: await registerSevensMcpWithCopilot(this.config.projectPath),
    });
...
**Note on `@claude-flow/security` TokenCache:** The module exists at `/Users/cohen/Projects/sevens/v3/@claude-flow/security/` [5]. Whether it exports a `TokenCache` class is [unverified — not read in this session]. If it does not exist, this becomes a follow-up: implement token handle storage using the existing `InputValidator` + `PathValidator` primitives, or use Node's keychain via `keytar`. The constraint from user memory is absolute: never print or persist the raw credential value. [8]

### 6.8 MCP Wiring (Both Directions)

**Direction A — Copilot calls INTO sevens MCP (tools available to Copilot agents):**

This is the primary direction. When a Copilot session is created, sevens's MCP server is registered so that Copilot agents can call `memory_store`, `memory_search`, `swarm_init`, `hooks_route`, etc. directly.

```typescript
// src/mcp/register.ts
import { CopilotClient } from '@github/copilot-sdk';

export interface SevensMcpServerConfig {
  type: 'local';
  command: 'npx';
  args: ['-y', 'sevens@latest', 'mcp', 'start'];
  tools: ['*'];  // expose all sevens MCP tools to Copilot
  env?: Record<string, string>;
}

export async function registerSevensMcpWithCopilot(
  projectPath: string,
  toolFilter: string[] | '*' = '*'
): Promise<Record<string, SevensMcpServerConfig>> {
  return {
    sevens: {
      type: 'local',
      command: 'npx',
...
const session = await client.createSession({
  model: 'gpt-5.3-codex',
  mcpServers: await registerSevensMcpWithCopilot(process.cwd()),
});
```

**Direction B — sevens calls Copilot SDK from within governed workflows:**

This is the `CopilotClient.runGoverned()` path shown in Section 6.5. The SDK spawns the Copilot CLI, sends the compiled prompt, and returns the response. All governance hooks fire. The call is synchronous from sevens's perspective.
...
│
├── mcp/
│   ├── register.test.ts      # verify SevensMcpServerConfig shape is valid SDK input
│   └── bridge.test.ts        # mock sevens MCP server; test tool call routing
│
├── dual-mode/
...
cd /Users/cohen/Projects/sevens
...
# STEP 3: Bump sevens wrapper
cd sevens
npm version 3.8.0 --no-git-tag-version
# Edit package.json: add "@claude-flow/copilot" to optionalDependencies and overrides
npm publish
npm dist-tag add sevens@3.8.0 alpha
npm dist-tag add sevens@3.8.0 v3alpha
```

**`npx sevens init --copilot` wiring:** Add a `--copilot` flag to the `init` command in `@claude-flow/cli` that delegates to `CopilotInitializer.initialize()`. This mirrors how `--codex` delegates to `CodexInitializer`.

**Verification:**
```bash
for pkg in @claude-flow/copilot @claude-flow/cli claude-flow sevens; do
...
| `[mcp_servers.sevens]` | `mcpServers.sevens` in session config | Same underlying sevens MCP server; config syntax changes from TOML table to JS object |
...
| 4 | **Client-side vs server-side MCP (ANSWERED)** | Resolved | MCP servers run client-side as local subprocesses or remote HTTP. The Copilot backend does NOT execute MCP tools. sevens's MCP server safely accesses local AgentDB and hooks. [9] |
...
| 7 | **`@claude-flow/security` TokenCache API** | Low | The `TokenCache` import in `auth.ts` assumes the security package exports this class. Read `/Users/cohen/Projects/sevens/v3/@claude-flow/security/` src before implementing auth.ts. If absent, implement using Node `keytar` or a simple encrypted file store. |
...
1. **Read `/Users/cohen/Projects/sevens/v3/@claude-flow/security/src/index.ts`** to confirm `TokenCache` API exists; if not, design the auth token handle storage in a follow-up ADR.
...
7. **Implement `src/mcp/register.ts` and `bridge.ts`**. Write mock tests verifying the `mcpServers` object shape matches the SDK's session config. Run against a real `sevens mcp start` process in an integration test (gated behind `MCP_INTEGRATION=1`).
...
10. **Publish `@claude-flow/copilot@3.8.0`** following the three-package publishing protocol (cli → claude-flow → sevens). Update all dist-tags. Create GitHub release `v3.8.0`. Verify with `npx sevens init --copilot` in a fresh directory.
...
5. Local: /Users/cohen/Projects/sevens/v3/@claude-flow/codex/ (read 2026-06-03)
6. https://docs.github.com/en/copilot/reference/ai-models/supported-models (fetched 2026-06-03)
7. Local: /Users/cohen/Projects/sevens/v3/@claude-flow/guidance/src/index.ts (read 2026-06-03)
8. Local: /Users/cohen/Projects/sevens/CLAUDE.md (read 2026-06-03)
