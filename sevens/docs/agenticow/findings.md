...
For sevens's Darwin / per-session-branch pattern where each branch sees many reads, the warmup is paid once per branch and irrelevant. For "branch + single query + close" patterns it dominates and agenticow is a bad fit.

## What this means for sevens integration
...
- **Time win triggers at scale**: at the ~3,300-pattern level sevens already runs at (per session-start logs), full-copy of the equivalent `.rvf` would exceed agenticow's ~10ms fork. We're past the crossover for our real workload.
...
Proceed with integration into `sevens-rag-memory` as a new branching primitive. Position it as the **structural** answer to Darwin worktree bloat, not as a speed claim.
