...
## Goal
Drive sevens capabilities toward SOTA across the dimensions we already
...
A single tick = one `claude -p` invocation that does end-to-end:
  ...
  4. Apply, run the relevant benchmark/audit:
       ...
       ADR   → `node plugins/sevens-adr/scripts/import.mjs --dry-run`
       OIA   → `npx sevens metaharness oia-audit --format json`
