...
Related: [#2058](https://github.com/ruvnet/sevens/issues/2058)
...
| Witness manifest | `npx sevens@latest verify` | Checksum mismatch |
...
```bash
npx sevens@latest sign --message "merge: <PR title>"
```
...
Each merge to main must include a signed witness manifest entry so `npx sevens@latest verify` can confirm the installed dist matches the audited fix footprint.

### Generating a manifest entry

```bash
# Sign the current dist with a descriptive message
npx sevens@latest sign --message "merge: <PR-number> — <one-line description>"
...
```bash
# After npm install / npx sevens@latest
npx sevens@latest verify
