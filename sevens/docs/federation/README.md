...
Federation lets two or more Sevens installations — your mac, a server, a teammate's laptop — discover each other, exchange signed manifests, and send messages between them with bounded cost and per-peer trust gates. Key properties:
...
```bash
npx sevens@latest                                     # if you don't have it yet
npx sevens plugins install @claude-flow/plugin-agent-federation
```
...
- `sevens-fed.conf` (the wg-quick interface config)
- `sevens-fed.nft` or `sevens-fed.pf` (firewall projection)
...
| Witness signing | `plugins/sevens-core/scripts/witness/` |
...
- Tracking issue (ADR-111): [#1879](https://github.com/ruvnet/sevens/issues/1879)
