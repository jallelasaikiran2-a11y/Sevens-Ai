...
```bash
cat /tmp/adr-111-stage/sevens-fed.conf
cat /tmp/adr-111-stage/sevens-fed.nft     # linux only
cat /tmp/adr-111-stage/sevens-fed.pf      # macos only
```

Checklist:
- [ ] `sevens-fed.conf` has exactly one `[Peer]` block (per peer expected)
...
If anything looks off, **stop and re-stage** with corrected inputs — `sevens-fed.conf` is what `wg-quick up` parses, and a misconfigured rule can drop ssh.
...
sudo install -m 0600 /tmp/adr-111-stage/sevens-fed.conf /etc/wireguard/sevens-fed.conf

# Load the firewall rules (atomic, scoped to the WG interface or pf anchor)
# Linux:
sudo nft -f /tmp/adr-111-stage/sevens-fed.nft
# macOS:
sudo pfctl -a sevens-fed -f /tmp/adr-111-stage/sevens-fed.pf

# Bring up the WG interface
sudo wg-quick up sevens-fed
```
...
sevens federation evict --node-id ruvultra
# Or via MCP: federation_evict

# Confirm WG layer responded
sudo wg show sevens-fed     # ruvultra peer's [Peer] line should be gone
...
```bash
sevens federation reactivate --node-id ruvultra
# A wg set sevens-fed peer ... allowed-ips ... command is emitted via the
# wgCommandSink the operator wired during plugin init.
sudo wg show sevens-fed     # ruvultra back in [Peer] list
...
```bash
sudo wg-quick down sevens-fed

# Linux:
sudo nft delete table inet sevens_fed

# macOS:
sudo pfctl -a sevens-fed -F all
```

The configs in `/tmp/adr-111-stage/` and `/etc/wireguard/sevens-fed.conf` stay on disk — re-run `wg-quick up sevens-fed` to reactivate. To fully tear down also delete `/etc/wireguard/sevens-fed.conf` (private key inside).
