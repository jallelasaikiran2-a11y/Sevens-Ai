# BEIR Public-Benchmark Results — sevens

This page tracks sevens's measured retrieval performance on BEIR datasets.
...
| **sevens + BGE-base (direct dense, no rerank)** | **0.352** | **0.626** | **0.489** |
| **sevens + BM25+BGE-base RRF k=60 (3.10.27, did NOT improve)** | 0.328 | 0.569 | 0.449 |
...
| **sevens Lucene RRF + CE rerank (3.10.28)** | **110M** | **0.521** | **0.358** | **0.683** |
| sevens multi-field RRF + CE rerank | 110M | 0.520 | 0.355 | 0.685 |
| sevens Lucene BM25 alone | — | 0.505 | 0.328 | 0.681 |
...
| sevens dense alone (BGE-base) | 110M | 0.489 | 0.352 | 0.626 |
...
| Dataset | Best sevens | Pipeline | Rank | Best Listed |
|---|---:|---|---:|---:|
...
| **sevens best (per-dataset)** | **110M** | **0.358** | **0.683** | **0.432** | **0.491** |
...
| Dataset | Best sevens | Pipeline | Rank |
|---|---:|---|---:|
...
| **sevens best (per-dataset)** | **110M** | **0.358** | **0.683** | **0.432** | **0.211** | **0.421** |
...
> MMR + opt-in rerank, ADRs 078-083) is what sevens uses internally for
...
| **sevens + BGE-base-en-v1.5** | **110M** | **0.352** | **this repo** |
...
node /path/to/sevens/v3/@claude-flow/cli/scripts/run-beir-bge.mjs
...
BEIR_DATA_DIR=/tmp/beir-scifact/scifact node /path/to/sevens/v3/@claude-flow/cli/scripts/run-beir-bge.mjs

# Paired bootstrap significance test (ADR-086)
node /path/to/sevens/v3/@claude-flow/cli/scripts/beir-bootstrap-significance.mjs \
  /path/to/sevens/docs/benchmarks/runs/beir-nfcorpus-bge-latest.json
```
...
- **Single-annotator labelled retrieval** for internal sevens bench
