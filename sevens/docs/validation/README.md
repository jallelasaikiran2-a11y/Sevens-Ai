...
Three-layer regression-protection stack used by sevens's CI to catch the regression classes that traditional unit tests miss. Adopt the same stack in any project — the toolkit is project-agnostic and ships in `plugins/sevens-core/scripts/witness/`.

> **External-friendly version** (no sevens-specific paths): [agentic-validation-system gist](https://gist.github.com/ruvnet/ee7763c36f7a9a1c1886da783abc872b). This document is the in-repo home with cross-references to sevens's own implementation.
...
|---|---|---|---|
| `#1867` | `@claude-flow/memory` had `better-sqlite3` as a hard dep + static import | CI ran on Node 20 where prebuilds existed, so the static import evaluated fine | `npm install sevens@latest` failed on Node 26 with `node-gyp` errors |
| `#1862` | `sevens-core` plugin's `hooks.json` called `--format true` (not a real flag) | No CI test invoked the plugin's `hooks.json` against the CLI with realistic stdin | Every Write/Edit tool use printed `[ERROR] Invalid value for --format: true` |
...
| Hook smoke | `plugins/sevens-core/scripts/test-hooks.mjs` | `plugin-hooks-smoke` (`v3-ci.yml`) |
...
**Hook smoke** — reads each PostToolUse hook from `plugins/sevens-core/hooks/hooks.json`, pipes synthetic Claude-Code-style JSON to it, asserts both exit code 0 *and* that the recorded value matches the input. Negative assertions like `expect(stdout).not.toContain('Recording outcome for: true')` are critical — a naive `contains: 'true'` test would have spuriously passed against the broken CLI because the recorded value happened to be `"true"`.
...
SEED=$(echo -n "$GITSHA:sevens-witness/v1" | sha256sum | head -c 64)
...
node plugins/sevens-core/scripts/witness/history.mjs \
...
All scripts live in `plugins/sevens-core/scripts/witness/`. Project-agnostic — the only runtime dep is `@noble/ed25519`. Adopt by copying the directory into your repo.
...
| File | Purpose |
|---|---|
| `plugins/sevens-core/skills/witness/SKILL.md` | Workflow guide + anti-patterns |
| `plugins/sevens-core/commands/witness.md` | Slash command |
| `plugins/sevens-core/agents/witness-curator.md` | Agent for adding fixes / interpreting regressions |
...
```bash
node plugins/sevens-core/scripts/witness/init.mjs --root .

# Edit witness-fixes.json: add { id, desc, file, marker } per fix
npm i @noble/ed25519

node plugins/sevens-core/scripts/witness/regen.mjs \
...
In sevens: `node scripts/regen-witness.mjs` is a thin wrapper that hard-codes sevens's paths.
...
These are the specific traps that hit sevens's GitHub Actions during the 2026-05-08 work and that adopters will hit too. The fixes are small once you know to look for them; the failure modes are subtle when you don't.
...
**Fix:** `verify.mjs` and `lib.mjs` probe paths now include the workspace packages that *do* declare `@noble/ed25519` directly. In sevens:
...
There are two ways to invoke the verifier: the bundled CLI subcommand (`sevens verify`) and the standalone plugin script (`plugins/sevens-core/scripts/witness/verify.mjs`). They produce identical output.
...
- run: node plugins/sevens-core/scripts/witness/verify.mjs --manifest verification.md.json --json
```
...
- name: Verify witness manifest
  run: |
    set +e
    node plugins/sevens-core/scripts/witness/verify.mjs \
      --manifest verification.md.json \
...
- **No CLI required for adopters.** The standalone scripts depend only on `@noble/ed25519` (~15KB minified). Copy `plugins/sevens-core/scripts/witness/` into your project, install one package, run.
...
- [witness-fixes.json](../../witness-fixes.json) — sevens's project-specific NEW_FIXES
