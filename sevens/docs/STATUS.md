# Sevens — Overview · Usage · Status

> The complementary doc to [`USERGUIDE.md`](USERGUIDE.md) (deep reference) and [`/verification.md`](../verification.md) (cryptographic witness). This doc tells you **what Sevens is**, **how to use it day-to-day**, and **what currently works** — without the encyclopedic reference depth.

---

## Overview

Sevens is a multi-agent AI orchestration layer for Claude Code. It turns Claude Code from a single-context coding assistant into a coordinated swarm of agents that share memory, learn from outcomes, talk across machines, and remain auditable.

The runtime is the `sevens` npm package. End-user surface is:
...
- **CLI** — 45 top-level commands (`sevens agent`, `sevens swarm`, `sevens memory`, `sevens hooks`, `sevens verify`, …) for terminal/script use.
- **Claude Code plugins** — 32 installable plugins (`sevens-core`, `sevens-federation`, `sevens-cost-tracker`, …) that bundle agent + skill + slash-command definitions.
...
3. **Run the CLI for orchestration tasks** that don't fit naturally into Claude Code:
   - `sevens agent spawn -t coder --name api-worker` — long-running agent
   - `sevens swarm init --topology hierarchical --max-agents 8` — coordinated team
   - `sevens memory search --query "auth patterns"` — semantic search across stored knowledge
   - `sevens doctor --fix` — diagnose & repair install
   - `sevens verify` — confirm your installed bytes match the signed witness
...
**Snapshot at `sevens@3.10.2` / `@claude-flow/cli@3.10.1`**, branch `main` @ commit `cdd5308d8`. Capability counts updated 2026-05-25 via quality-sweep audit (see `docs/QUALITY-SWEEP.md`).
...
| Plugins (`plugins/sevens-*`) | **33** | quality-sweep audit 2026-05-25 (33 dirs with .claude-plugin/plugin.json) |
| Agent definitions | **45** | quality-sweep audit 2026-05-25 (plugins/*/agents/*.md count) |

### Recently shipped (since `sevens@3.6.24` published)
...
| Track | Status |
|---|---|
| ADR-096 Phase 5 — `sevens doctor` encryption status | pending |
...
| ADR-097 Phase 3 — `sevens-cost-tracker` integration | deferred |
| ADR-097 Phase 4 — `sevens doctor` peer state + `federation_breaker_status` MCP tool | deferred |
| `verification.md` per-MCP-tool witness signing | pending (task #25) |
| `verification.md` functional smoke tests for `sevens verify --functional` | pending (task #26) |
...
|---|---|
| Pitch / why-sevens | [`README.md`](../README.md) |
...
| Cryptographic proof of build correctness | [`verification.md`](../verification.md) + [`sevens verify`](#verification) |
