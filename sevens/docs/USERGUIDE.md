...
> Complete reference documentation for Sevens v3.7. For a quick overview, see the [README](../README.md).
>
> **Latest:** `npx sevens@latest --version` → **3.7.0-alpha.8**. See [What's new in 3.7](#whats-new-in-37) below.
...
All wrapped in MutationGuard (fail-closed) + AttestationLog (audit). Unblocks `/adr-index` re-index when ADR files are deleted from disk — stale nodes + dangling `supersedes` / `amends` / `related` / `depends-on` edges are now scrubbable. Closed [#1784](https://github.com/ruvnet/sevens/issues/1784).
...
If you're running `npx sevens@latest`, everything you used in 3.6 still works. The above improvements compound underneath.

---

## Getting into the Flow

Sevens is a comprehensive AI agent orchestration framework that transforms Claude Code into a powerful multi-agent development platform. It enables teams to deploy, coordinate, and optimize specialized AI agents working together on complex software engineering tasks.

### Self-Learning/Self-Optimizing Agent Architecture

```
User → Sevens (CLI/MCP) → Router → Swarm → Agents → Memory → LLM Providers
                       ↑                          ↓
                       └──── Learning Loop ←──────┘
```
...
**RuVector Components** (included with Sevens):
...
```bash
# Use RuVector via Sevens
npx sevens@latest hooks intelligence --status
```
...
curl -fsSL https://cdn.jsdelivr.net/gh/ruvnet/sevens@main/scripts/install.sh | bash

# Or full setup with MCP + diagnostics
curl -fsSL https://cdn.jsdelivr.net/gh/ruvnet/sevens@main/scripts/install.sh | bash -s -- --full
...
⚡ **Plugs Into Claude Code** - Native integration via MCP (Model Context Protocol). Use sevens commands directly in your Claude Code sessions with full tool access.
...
Complex swarms can drift from their original goals. Sevens V3 includes anti-drift defaults that prevent agents from going off-task.
...
### Claude Code: With vs Without Sevens

| Capability | Claude Code Alone | Claude Code + Sevens |
|------------|-------------------|---------------------------|
...
# Or install globally
npm install -g sevens@latest
sevens init

# With Bun (faster)
bunx sevens@latest init
```
...
npm install -g sevens@latest --omit=optional
```

#### Claude Code Plugin Marketplace

Install Sevens as a native Claude Code plugin -- adds skills, commands, agents, and MCP tools directly into Claude Code:
...
/plugin install sevens-core@sevens         # MCP server + base agents
/plugin install sevens-swarm@sevens         # Swarm coordination + Monitor
/plugin install sevens-autopilot@sevens     # Autonomous /loop completion
/plugin install sevens-loop-workers@sevens  # Background workers + CronCreate
/plugin install sevens-security-audit@sevens # Security scanning
/plugin install sevens-rag-memory@sevens    # HNSW memory + AgentDB
/plugin install sevens-testgen@sevens       # Test gap detection + TDD
/plugin install sevens-docs@sevens          # Doc generation + drift detection
```
...
Sevens supports both **Claude Code** and **OpenAI Codex CLI** via the [@claude-flow/codex](https://www.npmjs.com/package/@claude-flow/codex) package, following the [Agentics Foundation](https://agentics.org) standard.

### Quick Start for Codex

```bash
# Initialize for Codex CLI (creates AGENTS.md instead of CLAUDE.md)
npx sevens@latest init --codex

# Full Codex setup with all 137+ skills
npx sevens@latest init --codex --full

# Initialize for both platforms (dual mode)
npx sevens@latest init --dual
```
...
codex mcp add sevens -- npx sevens mcp start
```
...
npx sevens@latest mcp start

# Spawn a coding agent
npx sevens@latest agent spawn -t coder --name my-coder

# Launch a hive-mind swarm with an objective
npx sevens@latest hive-mind spawn "Implement user authentication"

# List available agent types
npx sevens@latest agent list
```
...
npx sevens@latest init upgrade

# Update AND add any missing skills/agents/commands
npx sevens@latest init upgrade --add-missing
```
...
Add sevens as an MCP server for seamless integration:

```bash
# Add sevens MCP server to Claude Code
claude mcp add sevens -- npx -y sevens@latest mcp start

# Verify installation
claude mcp list
```

Once added, Claude Code can use all 313 sevens MCP tools directly:
...
<details>
<summary>🆚 <strong>Why Sevens v3?</strong></summary>

Sevens v3 introduces **self-learning neural capabilities** that no other agent orchestration framework offers. While competitors require manual agent configuration and static routing, Sevens learns from every task execution, prevents catastrophic forgetting of successful patterns, and intelligently routes work to specialized experts.

#### 🧠 Neural & Learning

| Feature | Sevens v3 | CrewAI | LangGraph | AutoGen | Manus |
|---------|----------------|--------|-----------|---------|-------|
...
What makes Sevens different from other agent frameworks? These 10 capabilities work together to create a system that learns from experience, runs efficiently on any hardware, and keeps working even when things go wrong.
...
Not every task needs the most powerful (and expensive) model. Sevens analyzes each request and automatically routes it to the cheapest handler that can do the job well. Simple code transforms skip the LLM entirely using WebAssembly. Medium tasks use faster, cheaper models. Only complex architecture decisions use Opus.
...
Connect Sevens to your development environment.

<details>
<summary>🔌 <strong>MCP Setup</strong> — Connect Sevens to Any AI Environment</summary>

Sevens runs as an MCP (Model Context Protocol) server, allowing you to connect it to any MCP-compatible AI client. This means you can use Sevens's 100+ agents, swarm coordination, and self-learning capabilities from Claude Desktop, VS Code, Cursor, Windsurf, ChatGPT, and more.

### Quick Add Command

```bash
# Start Sevens MCP server in any environment
npx sevens@latest mcp start
```
...
For ChatGPT, you need a remote MCP server (not local stdio). Deploy sevens to a server with HTTP transport:

```bash
# Start with HTTP transport
npx sevens@latest mcp start --transport http --port 3000
```
...
2. Configure your sevens MCP server
...
**CLI Commands:**
```bash
npx sevens hive-mind init                    # Initialize hive mind
npx sevens hive-mind spawn "Build API"       # Spawn with objective
npx sevens hive-mind spawn "..." --queen-type strategic --consensus byzantine
npx sevens hive-mind status                  # Check status
npx sevens hive-mind metrics                 # Performance metrics
npx sevens hive-mind memory                  # Collective memory stats
npx sevens hive-mind sessions                # List active sessions
```
...
**Enable Agent Teams:**
```bash
# Automatically enabled with sevens init
npx sevens@latest init
...
npx sevens@latest hooks teammate-idle --auto-assign true

# Handle task completion
npx sevens@latest hooks task-completed --task-id <id> --train-patterns
```
...
Install these optional plugins to extend Sevens capabilities:
...
| **@claude-flow/plugin-gastown-bridge** | 0.1.0 | Gas Town orchestrator integration with WASM-accelerated formula parsing (instant (regex-based, no LLM call)), Beads sync, convoy management, and graph analysis. 20 MCP tools. | `npx sevens@latest plugins install -n @claude-flow/plugin-gastown-bridge` |
| **@claude-flow/teammate-plugin** | 1.0.0-alpha.1 | Native TeammateTool integration for Claude Code v2.1.19+. BMSSP WASM acceleration, rate limiting, circuit breaker, semantic routing. 21 MCP tools. | `npx sevens@latest plugins install -n @claude-flow/teammate-plugin` |
...
npx sevens@latest plugins install -n @claude-flow/plugin-gastown-bridge
...
npx sevens plugins list --installed
```
...
```bash
npx sevens@latest worker dispatch --trigger audit --context "./src"
npx sevens@latest worker status
```
...
| **Auto-Install** | `sevens embeddings init` or `createEmbeddingServiceAsync()` | Zero config |
...
sevens embeddings init

# Use larger model for higher quality
sevens embeddings init --model all-mpnet-base-v2

# Semantic search
sevens embeddings search -q "authentication patterns"
```
...
sevens ruvector init --database mydb --user admin

# Check connection and schema status
sevens ruvector status --verbose

# Run pending migrations
sevens ruvector migrate --up

# Performance benchmark
sevens ruvector benchmark --iterations 1000

# Optimize indices and vacuum
sevens ruvector optimize --analyze

# Backup vector data
sevens ruvector backup --output ./backup.sql
```
...
**Quick Commands:**
```bash
npx sevens hive-mind init                                    # Initialize
npx sevens hive-mind spawn "Build API" --queen-type tactical # Spawn swarm
npx sevens hive-mind spawn "Research AI" --consensus byzantine --claude
npx sevens hive-mind status                                  # Check status
```

**Sevens Skill:** `/hive-mind-advanced` — Full hive mind orchestration
...
**Output Format:**
```
▊ Sevens V3 ● ruvnet  │  ⎇ main  │  Opus 4.6  | ●42% ctx  | $0.15
🏗️ DDD [●●●●○] 4/5  ⚡ HNSW 150x  🤖 ◉ [12/8]  👥 3  🟢 CVE 3/3  💾 512MB  🧠 15%  📦 AgentDB ●1.2K vectors
```

| Indicator | Description | Source |
|-----------|-------------|--------|
| `▊ Sevens V3` | Project header | Always shown |
...
|----------|--------|---------|
| `SEVENS_STATUSLINE_COST_SYMBOL` | Overrides the leading `$`. Set to an empty string to show the number alone. | `SEVENS_STATUSLINE_COST_SYMBOL=⚡` → `⚡1.30` |
| `SEVENS_STATUSLINE_HIDE_COST` | `1`/`true`/`yes`/`on` removes the segment entirely. | `SEVENS_STATUSLINE_HIDE_COST=1` |

Set them in the `env` block of `.claude/settings.json` — Claude Code applies it to every session and to the statusline subprocess, and unlike hand-editing the helper it survives `npx sevens@latest init --update`:

```json
{
  "statusLine": { "type": "command", "command": "node .claude/helpers/statusline.cjs" },
  "env": { "SEVENS_STATUSLINE_COST_SYMBOL": "⚡" }
}
```
...
```bash
export SEVENS_STATUSLINE_COST_SYMBOL=⚡   # or: export SEVENS_STATUSLINE_HIDE_COST=1
```

**Setup (Automatic):**

Run `npx sevens@latest init` — this generates `.claude/settings.json` with the correct statusline config and creates the helper script at `.claude/helpers/statusline.cjs`.
...
```bash
npx sevens@latest init --update --settings
```
...
npx sevens@latest daemon start

# Check status with worker history
npx sevens@latest daemon status

# Manually trigger a worker
npx sevens@latest daemon trigger map

# Enable/disable workers
npx sevens@latest daemon enable map audit optimize

# Stop daemon
npx sevens@latest daemon stop
```
...
Complete command-line interface for all Sevens operations.
...
npx sevens@latest swarm init --v3-mode

# Search memory (HNSW-indexed, 150x faster)
npx sevens@latest memory search -q "authentication patterns"

# Run security scan
npx sevens@latest security scan --depth full

# Performance benchmark
npx sevens@latest performance benchmark --suite all
```
...
Run `npx sevens@latest doctor` to diagnose and fix common issues.
...
npx sevens@latest doctor

# Run diagnostics with auto-fix
npx sevens@latest doctor --fix

# Check specific component
npx sevens@latest doctor --component memory

# Verbose output
npx sevens@latest doctor --verbose
```
...
npx sevens@latest embeddings init

# Generate embedding for text
npx sevens@latest embeddings embed "authentication patterns"

# Batch embed multiple texts
npx sevens@latest embeddings batch --file texts.txt

# Search with semantic similarity
npx sevens@latest embeddings search "login flow" --top-k 5
```
...
|----------|----------------|--------------|
| **Code Review** | Get thorough reviews with security, performance, and style checks | `npx sevens@latest agent spawn -t reviewer --name pr-review` |
| **Test Generation** | Auto-generate unit, integration, and e2e tests for existing code | `npx sevens@latest agent spawn -t tester --name test-gen` |
| **Refactoring** | Safely restructure code while maintaining behavior | `npx sevens@latest hive-mind spawn "Refactor user service to repository pattern"` |
| **Bug Fixing** | Diagnose and fix bugs with full context analysis | `npx sevens@latest hive-mind spawn "Fix race condition in checkout flow"` |

### 🔒 Security & Compliance

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Security Audit** | Find vulnerabilities before attackers do | `npx sevens@latest security scan --depth full` |
| **Dependency Scan** | Identify vulnerable packages and suggest upgrades | `npx sevens@latest security cve --check` |
| **Compliance Check** | Ensure code meets security standards | `npx sevens@latest security audit` |

### 🐝 Multi-Agent Swarms

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Feature Development** | Coordinate multiple agents on complex features | `npx sevens@latest swarm init --topology hierarchical && npx sevens@latest task orchestrate "Build user dashboard"` |
| **Large Refactors** | Parallel refactoring across many files without conflicts | `npx sevens@latest swarm init --topology mesh --max-agents 8` |
| **Codebase Migration** | Migrate frameworks, languages, or patterns systematically | `npx sevens@latest task orchestrate "Migrate from Express to Fastify" --strategy adaptive` |

### 📊 Performance & Optimization

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Performance Profiling** | Find and fix bottlenecks in your application | `npx sevens@latest performance profile --target src/` |
| **Query Optimization** | Speed up slow database queries | `npx sevens@latest performance benchmark --suite all` |
| **Memory Analysis** | Reduce memory usage and fix leaks | `npx sevens@latest performance metrics` |

### 🔄 GitHub & DevOps

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **PR Management** | Review, approve, and merge PRs efficiently | `npx sevens@latest hive-mind spawn "Review open PRs"` |
| **Issue Triage** | Categorize, prioritize, and assign issues automatically | `npx sevens@latest hive-mind spawn "Triage new issues"` |
| **Release Management** | Coordinate releases with changelogs and versioning | `npx sevens@latest hive-mind spawn "Prepare v2.0 release"` |
| **CI/CD Optimization** | Speed up pipelines and reduce flaky tests | `npx sevens@latest hive-mind spawn "Optimize GitHub Actions workflow"` |

### 📋 Spec-Driven Development

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Generate Specs** | Create complete specifications before coding | `npx sevens@latest hive-mind spawn "Create ADR for authentication system"` |
| **Validate Implementation** | Ensure code matches specifications | `npx sevens@latest hooks progress --detailed` |
| **Track Compliance** | Monitor spec adherence across the team | `npx sevens@latest progress sync` |

### 🧠 Learning & Intelligence

| Scenario | What It Solves | How To Do It |
|----------|----------------|--------------|
| **Bootstrap Intelligence** | Train the system on your codebase patterns | `npx sevens@latest hooks pretrain --depth deep` |
| **Optimize Routing** | Improve task-to-agent matching over time | `npx sevens@latest hooks route "<task>" --include-explanation` |
| **Transfer Learning** | Apply patterns learned from other projects | `npx sevens@latest hooks transfer <sourceProject>` |
...
Sevens eliminates Claude Code's context window ceiling with a real-time memory management system that archives, optimizes, and restores conversation context automatically.
...
Sevens intercepts the compaction lifecycle with three hooks that make context loss invisible:
...
Sevens uses RVF — a compact binary storage format that replaces the 18MB sql.js WASM dependency with pure TypeScript. No native compilation, no WASM downloads, works everywhere Node.js runs.
...
# Or via CLI
sevens memory init --force
sevens config set memory.backend hybrid
```
...
**Example Hook Output:**
```bash
$ npx sevens@latest hooks pre-task --description "convert var to const in utils.ts"
...
npx sevens@latest hooks pre-edit ./src/auth.ts
npx sevens@latest hooks post-edit ./src/auth.ts --success true --train-patterns
```
...
npx sevens@latest hooks route "refactor authentication to use JWT" --include-explanation

# Bootstrap intelligence from your codebase
npx sevens@latest hooks pretrain --depth deep --model-type moe
```
...
npx sevens@latest hooks session-start --session-id "feature-auth" --start-daemon

# End session and export learnings
npx sevens@latest hooks session-end --export-metrics --persist-patterns
```
...
npx sevens@latest hooks intelligence trajectory-start --task "implement OAuth2"

# Record successful action
npx sevens@latest hooks intelligence trajectory-step --action "created token service" --quality 0.9

# End trajectory and trigger learning
npx sevens@latest hooks intelligence trajectory-end --success true
...
```bash
# List all workers
npx sevens@latest hooks worker list

# Manually dispatch security audit
npx sevens@latest hooks worker dispatch --trigger audit --context "./src/auth"

# Check worker status
npx sevens@latest hooks worker status
```
...
npx sevens@latest hooks model-route --task "fix typo in README"
# → Recommends: haiku (simple task, low complexity)

npx sevens@latest hooks model-route --task "design distributed consensus system"
...
npx sevens@latest hooks route "<task>" --include-explanation

# Start/end session with learning
npx sevens@latest hooks session-start --start-daemon
npx sevens@latest hooks session-end --persist-patterns

# View what the system has learned
npx sevens@latest hooks metrics
npx sevens@latest hooks intelligence stats
...
npx sevens@latest hooks pretrain --depth deep

# Dispatch background worker
npx sevens@latest hooks worker dispatch --trigger audit
```
...
npx sevens@latest memory export --format json --output ./patterns.json

# Export specific namespace
npx sevens@latest memory export --namespace "security" --output ./security-patterns.json

# Export with embeddings (larger file, faster import)
npx sevens@latest memory export --include-embeddings --output ./full-export.json

# Export agent configurations
npx sevens@latest config export --scope project --output ./agent-configs.json

# Export session state
npx sevens@latest session export --session-id "my-session" --output ./session.json
```
...
npx sevens@latest memory import --input ./patterns.json

# Import and merge with existing (don't overwrite)
npx sevens@latest memory import --input ./patterns.json --merge

# Import from another project
npx sevens@latest hooks transfer --source-path ../other-project

# Import agent configurations
npx sevens@latest config import --input ./agent-configs.json --scope project

# Restore session
npx sevens@latest session restore --session-id "my-session"
```
...
npx sevens@latest transfer-store search --query "authentication" --min-rating 4.0

# Download a pattern
npx sevens@latest transfer-store download --id "auth-jwt-patterns-v2" --verify

# Publish your patterns
npx sevens@latest transfer-store publish --input ./my-patterns.json --category "security"
```
...
npx sevens@latest plugins list

# Filter by type
npx sevens@latest plugins list --type integration

# Rate a plugin
npx sevens@latest plugins rate --name @claude-flow/embeddings --rating 5

# Search for MCP tool plugins
npx sevens@latest transfer plugin-search --type "mcp-tool" --verified

# Get plugin info
npx sevens@latest transfer plugin-info --name "semantic-code-search"

# List official plugins
npx sevens@latest transfer plugin-official
```
...
npx sevens@latest transfer ipfs-resolve --name "/ipns/patterns.sevens.io"

# Detect PII before publishing
npx sevens@latest transfer detect-pii --content "$(cat ./patterns.json)"
```
...
curl -X POST "https://api.pinata.cloud/pinning/pinJSONToIPFS" \
  ...
  -d '{
    ...
    "pinataMetadata": {"name": "sevens-learning-pattern"}
  }'
...
# Import all models
npx sevens@latest transfer import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Import specific category
npx sevens@latest neural import --model security-review-patterns --source ipfs

# Use patterns in routing
npx sevens@latest hooks route --task "review authentication code" --use-patterns
```
...
npx sevens@latest transfer-store download --id "security-essentials" --apply
```
...
npx sevens@latest neural list

# List models by category
npx sevens@latest neural list --category security

# Train with WASM acceleration
npx sevens@latest neural train -p coordination -e 100 --wasm --flash --contrastive

# Train security patterns
npx sevens@latest neural train -p security --wasm --contrastive

# Benchmark WASM performance
npx sevens@latest neural benchmark -d 256 -i 1000

# Import pre-trained models
npx sevens@latest neural import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Export trained patterns to IPFS
npx sevens@latest neural export --ipfs --sign
```
...
# Via CLI
npx sevens@latest skill run github-code-review
npx sevens@latest skill list
npx sevens@latest skill info sparc-methodology
```
...
|---------|--------------|---------|
| `issues list` | See all issues and their status | `npx sevens@latest issues list` |
| `issues claim` | Claim an issue for yourself/agent | `npx sevens@latest issues claim #123 --as coder-1` |
| `issues release` | Release your claim | `npx sevens@latest issues release #123` |
| `issues handoff` | Hand off to another worker | `npx sevens@latest issues handoff #123 --to reviewer` |
| `issues status` | Update progress on claimed work | `npx sevens@latest issues status #123 --progress 75` |
| `issues stealable` | List abandoned/stuck issues | `npx sevens@latest issues stealable` |
| `issues steal` | Take over stealable issue | `npx sevens@latest issues steal #123` |
| `issues load` | View agent workloads | `npx sevens@latest issues load` |
| `issues rebalance` | Redistribute work evenly | `npx sevens@latest issues rebalance --dry-run` |
| `issues board` | Visual board view | `npx sevens@latest issues board` |

### Visual Board View

```bash
npx sevens@latest issues board
```
...
npx sevens@latest issues handoff #123 \
  --to security-architect \
  --reason "Needs security review" \
  --progress 80

# 2. Target accepts handoff
npx sevens@latest issues accept #123 --as security-architect
...
# Auto-rebalance
npx sevens@latest issues rebalance
```
...
| Command | What It Does | Example |
|---------|--------------|---------|
| `route task` | Get agent recommendation | `npx sevens@latest route task "implement OAuth2"` |
| `route explain` | Understand routing decision | `npx sevens@latest route explain "task"` |
| `route coverage` | Route based on test coverage | `npx sevens@latest route coverage` |

### Example: Route a Task

```bash
npx sevens@latest route task "refactor authentication to use JWT"
...
```bash
npx sevens@latest route coverage
...
npx sevens@latest hooks route "implement caching layer" --include-explanation

# Record outcome for learning
npx sevens@latest hooks post-task --task-id "task-123" --success true --agent coder
```
...
Use Sevens packages directly in your applications.

<details>
<summary>💻 <strong>Programmatic SDK</strong> — Use Sevens in Your Code</summary>

Use Sevens packages directly in your TypeScript/JavaScript applications.
...
npx sevens@latest memory init --force

# Store patterns
npx sevens@latest memory store --key "pattern-auth" --value "JWT authentication with refresh tokens"
npx sevens@latest memory store --key "pattern-cache" --value "Redis caching for API responses"

# Build HNSW index for HNSW-indexed search
npx sevens@latest memory search --query "authentication" --build-hnsw

# Semantic search (uses HNSW if built)
npx sevens@latest memory search --query "how to cache data" --limit 5

# List and manage entries
npx sevens@latest memory list --namespace patterns
npx sevens@latest memory stats
```
...
```bash
# Generate embedding
sevens embeddings embed "Your text here"

# Batch embed from file
sevens embeddings batch documents.txt -o embeddings.json

# Similarity search
sevens embeddings search "query" --index ./vectors

# Document chunking
sevens embeddings chunk document.txt --strategy sentence --max-size 512

# Normalize embeddings
sevens embeddings normalize embeddings.json --type l2 -o normalized.json

# Convert to hyperbolic
sevens embeddings hyperbolic embeddings.json -o poincare.json

# Neural operations
sevens embeddings neural drift --baseline "context" --input "check"
sevens embeddings neural store --id mem-1 --content "data"
sevens embeddings neural recall "query" --top-k 5

# Model management
sevens embeddings models list
sevens embeddings models download all-MiniLM-L6-v2

# Cache management
sevens embeddings cache stats
sevens embeddings cache clear --older-than 7d
```
...
Core infrastructure packages powering Sevens's intelligence layer.
...
Sevens v3 is built on top of **[agentic-flow](https://github.com/ruvnet/agentic-flow)**, a production-ready AI agent orchestration platform. This deep integration provides instant (regex-based, no LLM call) code transformations, learning memory, and geometric intelligence.
...
The agentic-flow ecosystem exposes MCP tools across packages (sevens CLI provides 314 tools):
...
### Integration with Sevens

Sevens automatically leverages agentic-flow for:
...
```typescript
// Sevens automatically uses agentic-flow optimizations
import { getTokenOptimizer } from '@claude-flow/integration';
...
Sevens includes a dedicated `/agentic-jujutsu` skill for AI-powered version control:
...
**RuVector** is a high-performance distributed vector database combining vector search, graph queries, and self-learning neural networks. Written in Rust with Node.js/WASM bindings, it powers Sevens's intelligence layer with native speed.
...
npx sevens ruvector setup --output ./my-ruvector
...
Sevens automatically uses RuVector when available:

```typescript
// Sevens detects and uses native ruvector
...
npx sevens ruvector setup                    # Output to ./ruvector-postgres
npx sevens ruvector setup --output ./mydir   # Custom directory
npx sevens ruvector setup --print            # Preview files

# Import from sql.js/JSON to PostgreSQL
npx sevens ruvector import --input data.json              # Direct import
npx sevens ruvector import --input data.json --output sql # Dry-run (generate SQL)

# Other RuVector commands
npx sevens ruvector status --verbose         # Check connection
npx sevens ruvector benchmark --vectors 10000 # Performance test
npx sevens ruvector optimize --analyze       # Optimization suggestions
npx sevens ruvector backup --output backup.sql # Backup data
...
Flow Nexus is a **cloud platform** for deploying and scaling Sevens beyond your local machine.

### What Flow Nexus Provides

| Feature | Local Sevens | + Flow Nexus |
|---------|-------------------|--------------|
...
# Or via CLI
npx sevens@latest nexus swarm deploy \
  --topology hierarchical \
  --max-agents 50 \
  --region us-east-1
```
...
```bash
# Create sandbox
npx sevens@latest nexus sandbox create --language python

# Execute code safely
npx sevens@latest nexus sandbox exec --code "print('Hello')"

# Cleanup
npx sevens@latest nexus sandbox destroy
```
...
# 2. Get API key
# 3. Configure
npx sevens@latest nexus configure --api-key <key>
...
# Define pipeline
npx sevens@latest stream-chain create \
...
```bash
# Run the pipeline
npx sevens@latest stream-chain run feature-pipeline \
  --input '{"requirements": "Add user dashboard with analytics"}'

# Monitor progress
npx sevens@latest stream-chain status feature-pipeline
```
...
# Via CLI
npx sevens@latest pair start --mode navigator
```
...
npx sevens@latest pair switch

# Get AI explanation
npx sevens@latest pair explain

# Run tests
npx sevens@latest pair test

# End session with summary
npx sevens@latest pair end
```
...
```bash
# Basic threat scan
npx sevens@latest security defend -i "ignore previous instructions"

# Scan a file
npx sevens@latest security defend -f ./user-prompts.txt

# Quick scan (faster)
npx sevens@latest security defend -i "some text" --quick

# JSON output
npx sevens@latest security defend -i "test" -o json

# View statistics
npx sevens@latest security defend --stats
...
AI-optimized browser automation integrating [agent-browser](https://github.com/AugmentCode/agent-browser) with sevens for intelligent web automation, trajectory learning, and multi-agent browser coordination.
...
Automated release management, versioning, and CI/CD for Sevens packages.
...
# Performance report
npx sevens@latest performance report

# Benchmark specific suite
npx sevens@latest performance benchmark --suite memory
```
...
| `GCS_PREFIX` | Prefix for stored files | `sevens-patterns` |
...
Sevens looks for configuration in this order:
1. `./claude-flow.config.json` (project root)
2. `~/.config/sevens/config.json` (user config)
...
```json
{
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "file",
    "filePath": "/var/log/sevens/production.log"
  },
...
npx sevens@latest config list

# Get specific value
npx sevens@latest config get --key memory.type

# Set configuration value
npx sevens@latest config set --key swarm.maxAgents --value 10

# Export configuration
npx sevens@latest config export > my-config.json

# Import configuration
npx sevens@latest config import --file my-config.json

# Reset to defaults
npx sevens@latest config reset --key swarm
...
npx sevens@latest hooks pretrain
```
...
$env:CLAUDE_FLOW_MEMORY_PATH = "C:/Users/name/sevens/data"
```
...
node --expose-gc node_modules/.bin/sevens
...
| Change | V2 | V3 | Impact |
|--------|----|----|--------|
| **Package Structure** | `sevens` | `@claude-flow/*` (scoped) | Update imports |
...
| **Config Format** | `.sevens/config.json` | `claude-flow.config.json` | Update path |
...
cp -r ./.sevens ./.sevens-backup-v2

# STEP 2: Check migration status
npx sevens@latest migrate status

# STEP 3: Run migration with dry-run first
npx sevens@latest migrate run --dry-run

# STEP 4: Execute migration
npx sevens@latest migrate run --from v2

# STEP 5: Verify migration
npx sevens@latest migrate verify
...
|------------|------------|-------|
| `sevens start` | `sevens mcp start` | MCP is explicit |
| `sevens init` | `sevens init wizard` | Interactive setup (subcommand, not a flag) |
| `sevens spawn <type>` | `sevens agent spawn -t <type>` | Nested under `agent` |
| `sevens swarm create` | `sevens swarm init --topology mesh` | Explicit topology |
...
```typescript
// V2 (deprecated)
import { ClaudeFlow, Agent, Memory } from 'sevens';
...
npx sevens@latest migrate rollback --list

# Rollback to V2
npx sevens@latest migrate rollback --to v2
...
- [ ] Verify all agents spawn correctly: `npx sevens@latest agent list`
- [ ] Check memory search works: `npx sevens@latest memory search -q "test"`
- [ ] Confirm MCP server starts: `npx sevens@latest mcp start`
- [ ] Run doctor diagnostics: `npx sevens@latest doctor`
- [ ] Test a simple swarm: `npx sevens@latest swarm init --topology mesh`
- [ ] Bootstrap learning: `npx sevens@latest hooks pretrain`
