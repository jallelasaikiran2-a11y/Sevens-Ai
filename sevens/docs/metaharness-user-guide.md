...
**MetaHarness integration in sevens `3.12.1+`.** Ten CLI subcommands, nine MCP tools, three CI workflows, and a dedicated `sevens eject` command — all wired to the upstream `metaharness` / `@metaharness/*` ecosystem with **graceful degradation** when those optional packages aren't installed.

Quick links: [Quick start](#quick-start) · [10 CLI subcommands](#cli-subcommands) · [9 MCP tools](#mcp-tools) · [Architectural constraints](#architectural-constraints-adr-150) · [Workflows](#common-workflows) · [Troubleshooting](#troubleshooting) · [ADR-152 similarity search](#adr-152-§31-genome-similarity-search) · [Eject](#sevens-eject)

---

## What is MetaHarness?

`metaharness` is a sibling agent-harness scaffolding system designed by the same author as sevens. Where sevens *is* a harness, metaharness analyzes harnesses — scoring readiness, mapping MCP surfaces, threat-modeling, fingerprinting genome characteristics, and detecting drift over time. ADR-150 integrates it as a first-class subsystem so you can audit and characterize sevens (or any harness) from the same CLI.

The integration is **strictly optional**. Per [ADR-150](../v3/docs/adr/ADR-150-metaharness-integration-surfaces.md) constraint #4, sevens remains fully operational even when every `@metaharness/*` package is uninstalled — every command degrades gracefully with a clear `degraded: true` payload instead of crashing.
...
npx sevens metaharness score --path .

# 7-section categorical genome report
npx sevens metaharness genome --path .

# Static security scan of the declared MCP surface
npx sevens metaharness mcp-scan --path . --fail-on high

# Composite audit (oia-manifest + threat-model + mcp-scan + score + genome)
npx sevens metaharness oia-audit --path . --alert-on-worst high

# Detect drift from the last audit
npx sevens metaharness drift-from-history --threshold 0.95

# Score two harnesses' similarity (ADR-152 §3.1)
npx sevens metaharness similarity --a harnessA.json --b harnessB.json
```
...
```
npx sevens metaharness <subcommand> [flags]
```
...
```bash
npx sevens metaharness score --path . --format json
npx sevens metaharness score --path . --alert-on-fit-below 70
```
...
```bash
npx sevens metaharness genome --path . --alert-on-risk-above 0.5
```
...
```bash
npx sevens metaharness threat-model --path . --fail-on high
```
...
```bash
npx sevens metaharness oia-audit --path . \
  --alert-on-worst high \
  --format json
```
...
```bash
npx sevens metaharness audit-list --limit 20 --since 30d --format json
```
...
```bash
npx sevens metaharness audit-trend \
...
```bash
npx sevens metaharness similarity \
...
npx sevens metaharness drift-from-history \
...
- name: composite audit
  run: |
    npx sevens metaharness oia-audit --path . --dry-run \
...
A dedicated CLI command (not under `metaharness`) that lifts a sevens project into a renamed standalone harness via `metaharness --from-existing`.

```bash
# Dry-run (default) — prints the plan and exits without writing
npx sevens eject --name my-harness

# Eject for real
npx sevens eject --name my-harness --confirm

# Eject to a specific dir (must be OUTSIDE the calling repo)
npx sevens eject --name my-harness --target /abs/path --confirm
```

**Safety gate:** refuses any `--target` inside the calling repo. The default target is `/tmp/sevens-eject-<ts>-<name>/` — a fresh location to prevent eject-on-top-of-source accidents.

Use case: you've prototyped agent workflows on top of sevens and want a renamed harness with its own identity, ready to publish or distribute independently.
...
```bash
npx sevens doctor --component metaharness
```
...
### "metaharness: plugins/sevens-metaharness/scripts/ not found"

Shipped fixed in **`sevens@3.12.1+`**. The CLI dispatcher locates its plugin scripts under `node_modules/@claude-flow/cli/plugins/sevens-metaharness/scripts/`. If you're on `3.12.0`, upgrade:
...
The optional `metaharness` / `@metaharness/*` packages aren't in `node_modules`. Per ADR-150 constraint #3 this is a **valid degraded mode** — sevens still works, you just won't get score/genome/etc. results. To enable them:

```bash
npm install -D metaharness@latest @metaharness/router@latest
```

(Or accept the degraded mode — sevens doesn't *require* metaharness for any non-metaharness command.)
...
```bash
AUDIT_LIST_NAMESPACE=my-custom-ns npx sevens metaharness audit-list
```
...
- **Source**: [`plugins/sevens-metaharness/`](../plugins/sevens-metaharness/) in the repo
- **Bundled location at runtime**: `node_modules/@claude-flow/cli/plugins/sevens-metaharness/scripts/`
...
- **Tracking issue**: [#2399](https://github.com/ruvnet/sevens/issues/2399)
