...
All notable changes to the Sevens project (formerly Claude Flow) are documented here.
...
- Configurable statusline cost segment via two environment variables (defaults unchanged):
  - `SEVENS_STATUSLINE_COST_SYMBOL` — override the leading `$` (e.g. `⚡`, `€`, `🌱`); empty string shows the number alone.
  - `SEVENS_STATUSLINE_HIDE_COST` — `1`/`true`/`yes`/`on` hides the segment. `cost.total_cost_usd` is a client-side estimate that may differ from the actual bill and is misleading on subscription plans.

## [3.5.0] - 2026-02-27

### Sevens v3.5 — First Major Stable Release

This release marks the official rebranding from **Claude Flow** to **Sevens** and represents the first major stable release after 5,800+ commits, 55 alpha iterations, and 10 months of development.

### Highlights

- **Rebranding**: Claude Flow → Sevens across all packages (`@claude-flow/cli`, `claude-flow`, `sevens`)
...
- Fixed CLI branding: show 'sevens' instead of 'claude-flow' when run via `npx sevens`
...
| **Sevens v3.5** | **v3.5.0** | **2026-02-27** | **First stable release, rebranding** |
