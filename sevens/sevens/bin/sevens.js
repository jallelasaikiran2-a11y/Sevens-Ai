#!/usr/bin/env node
// Sevens CLI - thin wrapper around @claude-flow/cli with sevens branding
...
// because the wrapper imports dist/src/index.js, bypassing bin/cli.js.)
{
  ...
  if (_argv.length === 1 && (_argv[0] === '--version' || _argv[0] === '-V')) {
    try {
      ...
      process.stdout.write(`sevens v${pkg.version || '0.0.0'}\n`);
    } catch {
      process.stdout.write('sevens v0.0.0\n');
    }
    process.exit(0);
  }
}

// Walk up from sevens/bin/ to find @claude-flow/cli in node_modules
...
if (isMCPMode) {
  ...
  // CLI mode: use sevens branding
  const { CLI } = await import(toImportURL(join(cliBase, 'dist', 'src', 'index.js')));
  const cli = new CLI({
    name: 'sevens',
    description: 'Sevens - AI Agent Orchestration Platform',
  });
  cli.run()
    .then(() => {
