# RuVocal — sevens Web UI

> sevens's multi-model AI chat with built-in Model Context Protocol (MCP) tool calling. Live at [**flo.ruv.io**](https://flo.ruv.io/).
...
RuVocal is the SvelteKit web app that lets you chat with Qwen, Claude, Gemini, or OpenAI while [sevens](https://github.com/ruvnet/sevens) invokes the same ~210 MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — all directly from chat. No install, no API key needed to try the hosted demo.

It started as a fork of the [HuggingFace chat-ui](https://github.com/huggingface/chat-ui) v0.20.0 and has been extended with a WASM-MCP integration layer, parallel tool execution, an in-browser tool gallery, and a "sevens Capabilities" tour modal. See [ADR-033](../../docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture.

## What RuVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five sevens server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **sevens Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
| 🦾 **ruvLLM ready** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) — sevens's self-improving local model layer |
...
```bash
git clone https://github.com/ruvnet/sevens
cd sevens/sevens/src/ruvocal
...
# Curated sevens model list (optional — defaults to /models from the base URL)
TASK_MODEL=qwen/qwen3.6-max-preview
PUBLIC_APP_NAME=sevens
...
## MCP Tools (the sevens difference)
...
```env
MCP_SERVERS=[
  {"name":"sevens Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"sevens Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
  {"name":"sevens Agents","url":"https://mcp-bridge-...run.app/mcp/agents","transport":"sse"},
  {"name":"sevens Memory","url":"https://mcp-bridge-...run.app/mcp/memory","transport":"sse"},
  {"name":"sevens DevTools","url":"https://mcp-bridge-...run.app/mcp/devtools","transport":"sse"},
...
- **Capabilities modal** — `src/lib/components/SevensHelpModal.svelte`
...
- 🏠 **Parent project** — [sevens](https://github.com/ruvnet/sevens)
...
- 📋 **Roadmap** — [issue #1689](https://github.com/ruvnet/sevens/issues/1689)
- 🍴 **Upstream** — [huggingface/chat-ui](https://github.com/huggingface/chat-ui) (RuVocal is forked from v0.20.0)

## License

MIT — same as the parent [sevens](https://github.com/ruvnet/sevens) project.
