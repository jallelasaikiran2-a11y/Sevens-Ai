...
The MCP bridge grew to 331+ tools from multiple backends (ruvector, sevens, agentic-flow, Claude Code, Gemini, Codex). Exposing all tools simultaneously caused:
...
| **agents** | sevens | ~50 | enabled | `MCP_GROUP_AGENTS` |
| **memory** | sevens | ~25 | enabled | `MCP_GROUP_MEMORY` |
| **devtools** | sevens | ~60 | enabled | `MCP_GROUP_DEVTOOLS` |
| **security** | sevens | ~25 | disabled | `MCP_GROUP_SECURITY` |
| **browser** | sevens | ~23 | disabled | `MCP_GROUP_BROWSER` |
| **neural** | sevens | ~20 | disabled | `MCP_GROUP_NEURAL` |
...
// sevens tool "agent_spawn" → matches "agent_" prefix → included if agents group enabled
// sevens tool "browser_open" → matches "browser_" prefix → only if browser group enabled
```

A backend is only started if at least one of its groups is enabled. This means disabling all sevens groups prevents the sevens process from spawning entirely.
...
guidance(topic="tool", tool_name="sevens__memory_search") → specific tool docs
```
...
### Negative
- Some tools appear in multiple potential groups (e.g., sevens `hooks_*` in both intelligence and devtools) — resolved by prefix matching
- Group boundaries are somewhat arbitrary for the sevens "Uncategorized" tools
