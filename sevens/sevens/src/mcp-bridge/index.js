...
const TOOL_GROUPS = {
  ...
  // --- Agents & Orchestration (sevens) ---
  agents: {
    enabled: process.env.MCP_GROUP_AGENTS !== "false",
    description: "Agent lifecycle, swarm coordination, task management, workflows (sevens)",
    source: "sevens",
    prefixes: ["agent_", "swarm_", "task_", "session_", "hive-mind_", "workflow_", "coordination_"],
  },

  // --- Memory & Knowledge (sevens) ---
  memory: {
    enabled: process.env.MCP_GROUP_MEMORY !== "false",
    description: "Vector memory, AgentDB, embeddings, semantic search (sevens)",
    source: "sevens",
  ...
  devtools: {
    ...
    description: "Hooks, code analysis, performance profiling, GitHub integration (sevens)",
    source: "sevens",
    prefixes: ["hooks_", "analyze_", "performance_", "github_", "terminal_", "config_", "system_", "progress_"],
  },

  // --- Security & Safety (sevens) ---
  security: {
    enabled: process.env.MCP_GROUP_SECURITY === "true",
    description: "AI defence, PII detection, claims management, pattern transfer (sevens)",
    source: "sevens",
    prefixes: ["aidefence_", "claims_", "transfer_"],
  },

  // --- Browser Automation (sevens) ---
  browser: {
    enabled: process.env.MCP_GROUP_BROWSER === "true",
    description: "Headless browser control — navigate, click, fill, screenshot (sevens)",
    source: "sevens",
    prefixes: ["browser_"],
  },
  ...
  neural: {
    ...
    description: "Neural network training, DAA autonomous agents, cognitive patterns (sevens)",
    source: "sevens",
...
const BACKEND_DEFS = [
  ...
  { name: "sevens",          command: "npx", args: ["-y", "sevens", "mcp", "start"],      groups: ["agents", "memory", "devtools", "security", "browser", "neural"] },
...
- To recall previously stored patterns or solutions`,
    ...
    agents: `# Agents & Orchestration Group (sevens)

Multi-agent lifecycle management, swarm coordination, and task workflows.

## Key Tools
- **sevens__agent_spawn** — Create a new agent with specific capabilities
- **sevens__agent_list** — List all active agents
- **sevens__swarm_init** — Initialize a swarm with a topology (mesh, hierarchical, ring, star)
- **sevens__task_create** — Create and assign tasks
- **sevens__workflow_create** — Define multi-step workflows
- **sevens__workflow_execute** — Execute a workflow
- **sevens__hive-mind_init** — Start collective intelligence coordination
- **sevens__coordination_orchestrate** — Multi-agent coordination
...
- Distributed task management`,
    ...
    memory: `# Memory & Knowledge Group (sevens)

Vector storage, semantic search, AgentDB pattern learning, and embeddings.

## Key Tools
- **sevens__memory_store** — Store a value with vector embedding for semantic search
- **sevens__memory_search** — Semantic search across stored memories (HNSW-indexed)
- **sevens__memory_list** — List stored memory entries
- **sevens__agentdb_pattern-store** — Store a reasoning pattern for learning
- **sevens__agentdb_pattern-search** — Search for similar reasoning patterns
- **sevens__agentdb_context-synthesize** — Synthesize context from stored memories
- **sevens__embeddings_generate** — Generate vector embeddings for text
- **sevens__embeddings_search** — Semantic similarity search
...
- Building semantic search over custom data`,
    ...
    devtools: `# Dev Tools Group (sevens)

Code analysis, performance profiling, GitHub integration, and terminal access.

## Key Tools
- **sevens__analyze_diff** — Analyze git diff for risk and change classification
- **sevens__performance_benchmark** — Run performance benchmarks
- **sevens__performance_bottleneck** — Detect performance bottlenecks
- **sevens__github_repo_analyze** — Analyze a GitHub repository
- **sevens__github_pr_manage** — Manage pull requests
- **sevens__terminal_execute** — Execute commands in a terminal session
...
- GitHub repository management`,
    ...
    security: `# Security & Safety Group (sevens)

AI defence, PII detection, and claims-based authorization.

## Key Tools
- **sevens__aidefence_scan** — Scan text for AI manipulation attempts
- **sevens__aidefence_has_pii** — Check for PII (emails, phones, SSNs)
- **sevens__aidefence_is_safe** — Quick safety check on input
- **sevens__claims_claim** — Claim an issue for work
- **sevens__claims_board** — Visual board of all claims
...
- Work item management across agents`,
    ...
    browser: `# Browser Automation Group (sevens)

Headless browser control for web interaction and testing.

## Key Tools
- **sevens__browser_open** — Navigate to a URL
- **sevens__browser_click** — Click elements by reference
- **sevens__browser_fill** — Fill form inputs
- **sevens__browser_screenshot** — Capture page screenshots
- **sevens__browser_snapshot** — Get accessibility tree for AI parsing
- **sevens__browser_eval** — Execute JavaScript in page context
...
- Form filling and web interaction`,
    ...
    neural: `# Neural & DAA Group (sevens)

Neural network operations and Decentralized Autonomous Agents.

## Key Tools
- **sevens__neural_train** — Train a neural model
- **sevens__neural_predict** — Make predictions
- **sevens__daa_agent_create** — Create an autonomous agent
- **sevens__daa_workflow_create** — Create autonomous workflows
- **sevens__daa_knowledge_share** — Share knowledge between agents
...
Your tools are organized into groups. Each tool name is prefixed with its backend (e.g., \`sevens__agent_spawn\`, \`ruvector__hooks_route\`). Always use the full prefixed name when calling tools.
...
- **guidance** — Get help on any tool group, specific tool usage, or capabilities overview.
  ...
  For specific tool help: \`guidance(topic='tool', tool_name='sevens__agent_spawn')\`
...
- **sevens__agent_status** — Check an agent's current state. \`{"agentId": "agent-xxx"}\`
- **sevens__agent_list** — List all active agents with their states.
- **sevens__agent_terminate** — Stop an agent. \`{"agentId": "agent-xxx"}\`
- **sevens__agent_health** — Health check across all agents.
- **sevens__agent_pool** — View the agent pool and available capacity.

### Swarm Coordination:
- **sevens__swarm_init** — Initialize a multi-agent swarm.
...
- **sevens__swarm_status** — Get swarm health, topology, and agent states.
- **sevens__swarm_health** — Detailed health metrics for the swarm.
- **sevens__swarm_shutdown** — Tear down a swarm and all its agents.

### Task Management:
- **sevens__task_create** — Create a tracked task.
  \`{"description": "what needs to be done", "priority": "low|normal|high|critical"}\`
- **sevens__task_status** — Check task progress. \`{"taskId": "task-xxx"}\`
- **sevens__task_list** — List all tasks with their statuses.
- **sevens__task_complete** — Mark a task as done. \`{"taskId": "task-xxx"}\`
- **sevens__task_update** — Update task details, status, or assignment.
- **sevens__task_cancel** — Cancel a task.

### Workflow Orchestration:
- **sevens__workflow_create** — Define a multi-step workflow with dependencies.
- **sevens__workflow_execute** — Run a workflow. \`{"workflowId": "wf-xxx"}\`
- **sevens__workflow_status** — Check workflow progress.
- **sevens__workflow_template** — Use a pre-built workflow template.
- **sevens__workflow_pause** / **sevens__workflow_resume** — Control workflow execution.

### Hive-Mind (Distributed Consensus):
- **sevens__hive-mind_init** — Start distributed consensus system.
- **sevens__hive-mind_spawn** — Add an agent to the hive.
- **sevens__hive-mind_consensus** — Run consensus vote across agents.
- **sevens__hive-mind_broadcast** — Send message to all hive agents.
- **sevens__hive-mind_memory** — Access shared hive memory.

### Coordination:
- **sevens__coordination_topology** — View/change coordination topology.
- **sevens__coordination_load_balance** — Distribute work across agents.
- **sevens__coordination_orchestrate** — Orchestrate complex multi-agent tasks.
- **sevens__coordination_sync** — Synchronize state across agents.

### Session Management:
- **sevens__session_save** — Save current session state.
- **sevens__session_restore** — Restore a previous session.
- **sevens__session_list** — List available sessions.

## Group 4: Memory & Knowledge (sevens)
Persistent memory, vector search, embeddings, and pattern storage. ${TOOL_GROUPS.memory.enabled ? "ACTIVE" : "DISABLED"}

### Memory Operations:
- **sevens__memory_store** — Store data in persistent memory.
  \`{"key": "my-key", "value": "data to store", "namespace": "default", "tags": ["tag1"]}\`
- **sevens__memory_retrieve** — Get stored data by key. \`{"key": "my-key"}\`
- **sevens__memory_search** — Semantic vector search across stored memories.
  \`{"query": "what to search for", "limit": 5, "namespace": "default"}\`
- **sevens__memory_list** — List all stored keys in a namespace.
- **sevens__memory_delete** — Remove a stored memory.
- **sevens__memory_stats** — View memory usage statistics.

### Embeddings:
- **sevens__embeddings_generate** — Generate vector embeddings for text.
- **sevens__embeddings_compare** — Compare semantic similarity of two texts.
- **sevens__embeddings_search** — Search embeddings database by similarity.
- **sevens__embeddings_neural** — Generate neural embeddings.
- **sevens__embeddings_hyperbolic** — Generate hyperbolic embeddings for hierarchical data.

### AgentDB (Advanced Pattern Storage):
- **sevens__agentdb_pattern-store** — Store a learned pattern with metadata.
  \`{"pattern": "description", "category": "code|debug|architecture", "confidence": 0.9}\`
- **sevens__agentdb_pattern-search** — Search patterns by similarity.
- **sevens__agentdb_route** — Route a query to the most relevant stored pattern.
- **sevens__agentdb_feedback** — Provide feedback on a pattern (reinforcement learning).
- **sevens__agentdb_context-synthesize** — Synthesize context from multiple sources.
- **sevens__agentdb_semantic-route** — Semantic routing based on stored knowledge.
- **sevens__agentdb_consolidate** — Consolidate and deduplicate stored patterns.
- **sevens__agentdb_batch** — Batch operations on patterns.
- **sevens__agentdb_session-start** / **sevens__agentdb_session-end** — Session tracking.
- **sevens__agentdb_hierarchical-store** / **sevens__agentdb_hierarchical-recall** — Hierarchical memory.

## Group 5: Dev Tools & Analysis (sevens)
Performance, system health, GitHub integration, code analysis, terminal. ${TOOL_GROUPS.devtools.enabled ? "ACTIVE" : "DISABLED"}

### System & Performance:
- **sevens__system_status** — System health overview.
- **sevens__system_metrics** — Detailed performance metrics.
- **sevens__system_health** — Health check across all subsystems.
- **sevens__performance_report** — Generate performance report.
- **sevens__performance_bottleneck** — Identify performance bottlenecks.
- **sevens__performance_benchmark** — Run benchmarks.
- **sevens__performance_optimize** — Get optimization recommendations.
- **sevens__performance_profile** — Profile specific operations.

### Code Analysis:
- **sevens__analyze_diff** — Analyze a code diff.
- **sevens__analyze_diff-risk** — Assess risk level of changes.
- **sevens__analyze_diff-classify** — Classify type of changes (feature, bugfix, refactor).
- **sevens__analyze_diff-reviewers** — Suggest code reviewers.
- **sevens__analyze_file-risk** — Assess risk of a specific file.

### GitHub Integration:
- **sevens__github_repo_analyze** — Analyze a GitHub repository.
  \`{"repo": "owner/repo", "analysis_type": "code_quality|performance|security"}\`
- **sevens__github_pr_manage** — Manage pull requests (create, review, merge).
- **sevens__github_issue_track** — Track and manage issues.
- **sevens__github_workflow** — Manage GitHub Actions workflows.
- **sevens__github_metrics** — Repository metrics and insights.

### Terminal Access:
- **sevens__terminal_create** — Create a terminal session.
- **sevens__terminal_execute** — Execute a command. \`{"command": "ls -la"}\`
- **sevens__terminal_list** — List active terminals.
- **sevens__terminal_history** — View command history.

### Development Hooks:
- **sevens__hooks_pre-task** / **sevens__hooks_post-task** — Task lifecycle hooks for learning.
- **sevens__hooks_pre-edit** / **sevens__hooks_post-edit** — File edit hooks.
- **sevens__hooks_session-start** / **sevens__hooks_session-end** — Session lifecycle.
- **sevens__hooks_worker-dispatch** — Dispatch background workers.
  Workers: \`optimize\`, \`audit\`, \`testgaps\`, \`document\`, \`map\`, \`deepdive\`, \`benchmark\`
- **sevens__hooks_model-route** — Route to optimal AI model for a task.
- **sevens__hooks_explain** — Explain a routing or intelligence decision.

### Configuration:
- **sevens__config_get** / **sevens__config_set** / **sevens__config_list** — Manage settings.

### Progress Tracking:
- **sevens__progress_check** — Check implementation progress.
- **sevens__progress_summary** — Summarize overall progress.
...
3. **Code task** ("write", "fix", "implement") → \`ruvector__hooks_route\` to find best approach, then \`sevens__agent_spawn\`
4. **Analysis request** ("analyze", "review", "audit") → spawn reviewer/security agents + analysis tools
5. **Multi-step project** → \`sevens__task_create\` for tracking, \`sevens__swarm_init\` for coordination
6. **Memory/recall** ("remember", "save", "what did we...") → \`sevens__memory_store\` / \`sevens__memory_search\`
7. **System question** ("what tools", "help") → \`guidance(topic='overview')\`
8. **Performance concern** → \`sevens__performance_bottleneck\` + \`sevens__performance_optimize\`
9. **GitHub task** → \`sevens__github_*\` tools
...
### Code Implementation
\`ruvector__hooks_route\` → \`sevens__agent_spawn(coder)\` → track with \`sevens__task_create\` → report

### Multi-Agent Analysis
\`sevens__swarm_init(hierarchical)\` → spawn agents → coordinate → synthesize results

### Learning & Memory
\`sevens__memory_search\` (check existing) → do work → \`sevens__memory_store\` (save results) → \`ruvector__hooks_learn\`
...
- The prefixes "sevens__" or "ruvector__" — just describe what you're doing naturally
