...
ADR-033 added ruvector (61 tools) and sevens (215 tools) as default MCP backends. Users also want access to additional AI agent capabilities:
...
Add three optional MCP backends that can be enabled via environment variables. Unlike ruvector/sevens (enabled by default), these are **disabled by default** and require explicit API keys.
...
| sevens | `ENABLE_SEVENS` | None | `npx sevens mcp start` | **enabled** |
...
sevens__agent_spawn         → sevens MCP
...
- [ADR-033: RuVector + Sevens MCP Integration](ADR-033-RUVECTOR-SEVENS-MCP-INTEGRATION.md)
