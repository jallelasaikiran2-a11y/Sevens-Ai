#!/usr/bin/env bash
#
# Sevens Installer (formerly Claude Flow)
# https://github.com/ruvnet/sevens
#
# Usage:
...
VERSION="${SEVENS_VERSION:-${CLAUDE_FLOW_VERSION:-latest}}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --version=*)
            ...
            ;;
        --help|-h)
            echo "Sevens Installer"
            echo ""
            echo "Usage: curl -fsSL .../install.sh | bash -s -- [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --global, -g     Install globally (npm install -g sevens)"
...
print_banner() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${BOLD}Sevens${NC} — AI Agent Orchestration for Claude Code     ${CYAN}║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
verify_installation() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        VERSION_OUTPUT=$(sevens --version 2>/dev/null || claude-flow --version 2>/dev/null || echo "")
        if [ -z "$VERSION_OUTPUT" ]; then
            print_warning "Global command not found in PATH"
            print_substep "Try: ${BOLD}npm install -g sevens@${VERSION}${NC}"
...
show_quickstart() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        ...
        echo -e "  ${BOLD}sevens init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}sevens doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add sevens -- sevens mcp start${NC}"
    else
        echo -e "  ${DIM}# Initialize project${NC}"
        echo -e "  ${BOLD}npx sevens@latest init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}npx sevens@latest doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add sevens -- npx -y sevens@latest mcp start${NC}"
    fi

    echo ""
    echo -e "${DIM}Documentation: https://github.com/ruvnet/sevens${NC}"
    echo -e "${DIM}Issues: https://github.com/ruvnet/sevens/issues${NC}"
    echo ""
}
...
setup_mcp_server() {
    ...
    if claude mcp list 2>/dev/null | grep -q "sevens\|claude-flow"; then
    ...
    if [ "$GLOBAL" = "1" ]; then
        claude mcp add sevens -e CLAUDE_FLOW_CWD="$HOME" -- sevens mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add sevens -e CLAUDE_FLOW_CWD=\"\$HOME\" -- sevens mcp start"
    else
        claude mcp add sevens -e CLAUDE_FLOW_CWD="$HOME" -- npx -y sevens@${VERSION} mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add sevens -e CLAUDE_FLOW_CWD=\"\$HOME\" -- npx -y sevens@latest mcp start"
    fi
    echo ""
}
...
run_doctor() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        sevens doctor 2>&1 || true
    else
        npx sevens@${VERSION} doctor 2>&1 || true
    fi
    echo ""
}
...
run_init() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        sevens init --yes 2>&1 || true
    else
        npx sevens@${VERSION} init --yes 2>&1 || true
    fi
    echo ""
}
...
main() {
    ...
    print_success "${BOLD}Sevens is ready!${NC}"
    echo ""
}
