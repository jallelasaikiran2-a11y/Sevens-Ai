#!/usr/bin/env node
/**
 * Auto-extract sevens's full capability surface — every MCP tool, CLI
...
function renderMarkdown({ mcp, cli, plugins, agents }) {
  ...
  lines.push('');
  lines.push(`Per-capability cryptographic witnesses (SHA-256 of the dist file containing each tool / command, signed with the existing Ed25519 manifest key) land in iteration 2 of task #24 — see \`v3/docs/adr/\` for the design ADR. Functional smoke tests (\`sevens verify --functional\`) that round-trip each MCP tool through the in-process server are iteration 3.`);
  lines.push('');
  ...
  lines.push('');
  lines.push(`Top-level command surface. Subcommands are documented per-command in the source file and in \`.claude-flow/CAPABILITIES.md\` after \`sevens init\`.`);
  lines.push('');
  lines.push(table(
    ['Command', 'Description', 'Source'],
    cli.map(c => [`\`sevens ${c.name}\``, c.description || '*(no description)*', `\`${c.sourceFile}\``]),
  ));
