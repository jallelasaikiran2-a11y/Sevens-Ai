...
if (!existsSync(ARTIFACT_TS)) {
  ...
  check(
    ...
    /interface\s+SignedBacktestArtifact/.test(src) &&
      /schema\s*:\s*'sevens-neural-trader-backtest\/v1'/.test(src) &&
...
if (ed && signBacktestArtifact && verifyBacktestArtifact) {
  ...
  check(
    ...
    signed.schema === 'sevens-neural-trader-backtest/v1' &&
...
if (!existsSync(TRADER_BACKTEST_MD)) {
  ...
  check(
    'trader-backtest skill documents SEVENS_WITNESS_KEY_PATH env var',
    /SEVENS_WITNESS_KEY_PATH/.test(skill),
