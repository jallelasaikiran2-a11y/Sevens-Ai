...
// surface that sevens depends on (via ADR-148/149). If the upstream ships
// a breaking change, this fails BEFORE a sevens release goes out with a
...
//      ADR-150 graceful degradation: sevens runs without it)
...
async function main() {
  ...
  try {
    if (typeof mod.Router?.fromExamples !== 'function') {
      ...
      // sevens wraps it as `predictAll(embedding)` via map (see neural-router.ts
      ...
      results.push({
        check: 'router.predict method present (sevens predictAll wrapper depends on it)',
        ok: hasPredict,
  ...
  try {
    if (typeof mod.TrainedRouter?.fromJSON !== 'function') {
      ...
      // bundled with sevens; we don't load that here to keep this
  ...
  if (ARGS.format === 'json') {
    ...
    if (failed.length) {
      ...
      console.log('');
      console.log('✓ Upstream API matches sevens\'s expectations.');
    }
  }
