#!/usr/bin/env node
/**
 * Codex ↔ Sevens integration audit (issue #1909).
 *
...
console.log('Codex ↔ Sevens integration audit (#1909)\n' + '─'.repeat(48));

// ── 1. Codex MCP backend uses the real `mcp-server` subcommand ──────────────
section('MCP backend registration (`codex` group):');
for (const p of ['sevens/src/mcp-bridge/index.js', 'sevens/src/ruvocal/mcp-bridge/index.js']) {
...
section('CLI references standardized to sevens:');
...
// ── 8. config.toml generator emits a working `sevens` MCP server ────────────
...
check(/getSevensMcpServerConfig/.test(cfgGen)
    && /SEVENS_MCP_SERVER_NAME\s*=\s*'sevens'/.test(mcpConfig)
    && /SEVENS_MCP_PACKAGE\s*=\s*'sevens@latest'/.test(mcpConfig)
    && /args:\s*\['\/c',\s*'npx',\s*\.\.\.args\]/.test(mcpConfig)
    && /SEVENS_MCP_STARTUP_TIMEOUT_SEC\s*=\s*120/.test(mcpConfig),
  `default MCP server is Windows-safe \`sevens@latest mcp start\` with 120s startup timeout`,
  `default MCP server must use the shared platform-aware Sevens definition (cmd /c npx on Windows)`);
