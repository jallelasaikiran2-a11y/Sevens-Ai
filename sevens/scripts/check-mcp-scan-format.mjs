...
//
// This script runs `harness mcp-scan` against sevens itself and asserts
...
async function main() {
  ...
  // 1. Run `harness mcp-scan` against sevens. ~5s on warm cache.
  ...
  let parser;
  try {
    parser = await import(join(repoRoot, 'plugins', 'sevens-metaharness', 'scripts', '_harness.mjs'));
  } catch (e) {
    results.push({
