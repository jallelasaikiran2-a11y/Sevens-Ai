#!/usr/bin/env node
/**
 * Smoke test for ruvnet/sevens#2132 — Windows-compatible init hook generation.
 *
 * Verifies that `sevens init` generates a .claude/settings.json containing
...
// Run sevens init in a temporary directory
const tmpDir = mkdtempSync(join(tmpdir(), 'sevens-smoke-win-init-'));
console.log(`Running sevens init in: ${tmpDir}`);
...
if (!existsSync(settingsPath)) {
  ...
  console.error('FAIL: settings.json not generated by sevens init');
  process.exit(1);
}
...
if (IS_WINDOWS) {
  ...
  // Check that sevens-hook.cjs was deployed (new in #2132)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'sevens-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/sevens-hook.cjs exists (#2132)');
  ...
  // sevens-hook.cjs is always deployed now (cross-platform shim always available)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'sevens-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/sevens-hook.cjs exists (#2132)');
}
