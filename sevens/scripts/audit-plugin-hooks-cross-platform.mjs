#!/usr/bin/env node
/**
 * Static guard for ruvnet/sevens#2132 — plugin `hooks.json` commands must be
 ...
 * verify that plugins/<name>/scripts/sevens-hook.cjs exists (the Node shim that
...
for (const file of walkForHooksJson(REPO_ROOT)) {
  ...
  if (json._platform === 'posix') {
    ...
    //             shim lives in <plugin>/scripts/sevens-hook.cjs
    const pluginDir = resolve(join(file, '..', '..'));
    const shimPath = join(pluginDir, 'scripts', 'sevens-hook.cjs');
    if (!existsSync(shimPath)) {
      violations.push({
        ...
        hint: 'Create plugins/<name>/scripts/sevens-hook.cjs (cross-platform Node port of sevens-hook.sh). See #2132.',
      });
      ...
      // shipped with plugins/sevens-cost-tracker/scripts/sevens-hook.cjs
      ...
      // the sevens-core/sevens-cost-tracker plugins this file's #2721 fix
      ...
      const referencesShim = Object.values(json?.hooks ?? {})
        ...
        .some((h) => typeof h?.command === 'string' && h.command.includes('sevens-hook.cjs'));
      if (!referencesShim) {
        violations.push({
          file: relFile,
          line: 0,
...
console.error('\nReference: ruvnet/sevens#2132 (plugin hooks broken on Windows).');
console.error('Cross-platform pattern: .claude/settings.json + .claude/helpers/hook-handler.cjs (node, no bash).');
console.error('POSIX-exempt pattern: add "_platform": "posix" to hooks.json + create scripts/sevens-hook.cjs sibling.');
process.exit(1);
