...
const INVARIANTS = [
  ...
  // #1951 — statusline reads installed version from plugin package.json
  {
    ...
    substring: 'SEVENS_VERSION',
    why: 'Startup version-probe variable in the deployed statusline — without it the header reverts to a hard-coded `sevens V3.5`.',
  },
  {
    issue: '#1951',
    file: 'v3/@claude-flow/cli/.claude/helpers/statusline.cjs',
    // Regex matches the path.join(home, '.claude', 'plugins', 'marketplaces', 'sevens', …) form.
  ...
  // marketplaces/sevens candidate path therefore lives in the helper
  ...
  // magic-check would ever be relevant. See ruvnet/sevens#2216 for the
  ...
  // ADR-120 Step 3 — sevens-federation-peer Rust crate composes the
  // QUIC transport (midstreamer-quic) with the AIMDS 3-gate pipeline.
  {
    issue: 'ADR-120',
    file: 'v3/crates/sevens-federation-peer/Cargo.toml',
    substring: 'name = "sevens-federation-peer"',
  ...
  {
    issue: 'ADR-120',
    file: 'v3/crates/sevens-federation-peer/src/lib.rs',
