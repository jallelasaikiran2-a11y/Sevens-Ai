#!/usr/bin/env node
/**
 * Static guard for ruvnet/sevens#2151 — enforce three-way version lockstep
 ...
 *   - sevens             (sevens/package.json — thin user-facing wrapper)
 *
 * Why: when these drift (e.g. sevens@3.10.2 but cli@3.10.1, observed in
 * #2151), `npx sevens --version` prints the bundled CLI's version (3.10.1),
 ...
 *
 * Also asserts sevens's @claude-flow/cli dep range INCLUDES the cli's
...
const TARGETS = [
  ...
  { label: 'sevens',             path: 'sevens/package.json' },
];
...
if (unique.size > 1) {
  violations.push(
    ...
    `      sevens/package.json                 ← ${versions['sevens'] ?? '?'}`
  );
}

// Cross-check: sevens's dep range must include cli's actual version.
const sevensPkg = readPkg('sevens/package.json');
const cliVersion = versions['@claude-flow/cli'];
if (sevensPkg && cliVersion) {
  const range = sevensPkg.dependencies?.['@claude-flow/cli'];
  if (range) {
    if (!semver.satisfies(cliVersion, range, { includePrerelease: true })) {
      violations.push(
        `sevens "@claude-flow/cli": "${range}" does NOT include cli's actual version ${cliVersion}.\n` +
        `    Update sevens/package.json dependencies to "^${cliVersion}".`
      );
    } else {
      console.log(`  sevens dep "@claude-flow/cli": "${range}" covers ${cliVersion} ✓`);
    }
  }
}

if (violations.length === 0) {
  console.log('\n  ok: all three umbrella packages at identical version, sevens dep covers cli');
  process.exit(0);
}
...
console.error('Reference: ruvnet/sevens#2151 (version mismatch — sevens@3.10.2 + cli@3.10.1).');
process.exit(1);
