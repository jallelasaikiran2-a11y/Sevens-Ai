#!/usr/bin/env node
/**
 * Static guard for ruvnet/sevens ADR-125 / ADR-130 env-var precedence pattern.
 *
 * Context
 * -------
...
const KNOWN_ESCAPE_HATCHES = new Set([
  ...
  'SEVENS_HOOK_SKIP_NPX',          // CI: suppress cold-install latency in smoke tests
  'SEVENS_HOOK_CLI_OVERRIDE',      // #2721 test-only: point plugins/sevens-core/scripts/sevens-hook.cjs at a local CLI build instead of the sevens/claude-flow/npx PATH probe. Hook scripts have no CLI-flag surface (invoked by hooks.json, never a user-typed command)
  'SEVENS_HOOK_DEBUG_STDOUT',      // #2721 test-only: surface the invoked CLI's stdout/stderr from sevens-hook.cjs instead of swallowing it, so test-hooks.mjs can assert on recorded values. Same no-CLI-surface reasoning as SEVENS_HOOK_CLI_OVERRIDE above — production never sets this
  'SEVENS_SUBLINEAR_NATIVE',       // Manual override for native vs WASM sublinear — CI/perf knob
  'SEVENS_METAHARNESS_CACHE_BASE', // CI/test seam: relocates the ~/.sevens pinned-cache root in metaharness smoke tests — intentionally env-only, plugin scripts have no CLI-flag surface
  'SEVENS_FUNNEL',                 // Read inside the generated hook-handler.cjs (ADR-312/313 rate-limit nudge), not a typed CLI invocation — no command surface to attach a flag to
  'SEVENS_STATUSLINE_HYPERLINKS',  // Terminal-capability opt-out for OSC 8 hyperlinks in the statusline hook, same pattern as NO_COLOR — the statusline runs as a hook script, never a user-typed CLI command
  'SEVENS_STATUSLINE_HYPERLINKS_TMUX', // Opt-in override of the tmux OSC 8 mangling workaround above — same hook-script context, no CLI surface
  'SEVENS_STATUSLINE_IDENTITY',    // Hook-only display mode (`project` default, `author` compatibility); statusline has no interactive CLI surface
  'SEVENS_ADVISOR_MAX_BUDGET_USD', // Advisor-tip spend cap read from generated hook code (funnel/advisor-tip.ts) — background nudge, no CLI invocation to attach a flag to
  'SEVENS_FUNNEL_CLICK_ENDPOINT',    // Staging/self-hosted override for the funnel click-redirect endpoint — deployment config, not a per-invocation CLI flag
  'SEVENS_FUNNEL_EVENTS_ENDPOINT',   // Staging/self-hosted override for the funnel events endpoint — same deployment-config pattern as CLICK_ENDPOINT above
  'SEVENS_FUNNEL_MESSAGES_ENDPOINT', // Staging/self-hosted override for the funnel messages endpoint — same deployment-config pattern as CLICK_ENDPOINT above
  'SEVENS_STATE_DIR',                // Test/CI isolation seam for the funnel state directory (defaults to ~/.sevens) — background hook state, no CLI command reads it
  'SEVENS_AI_DEDUP_DISABLE',         // #2661 — background daemon tuning knob (cross-worktree AI job dedup), no CLI command reads a running daemon's config
  'SEVENS_AI_DEDUP_WINDOW_SECS',     // #2661 — same background daemon tuning context as SEVENS_AI_DEDUP_DISABLE above
  'SEVENS_DAEMON_AI_WORKERS',        // #2661 — DOES have CLI-flag precedence (`daemon start --headless`), but it's wired via constructor-injected config in commands/daemon.ts, not a local check the audit's same-file heuristic can see from worker-daemon.ts where this read lives
  'SEVENS_AI_BUDGET_DIR',            // #2663 — repo-supervisor state directory relocation (services/global-ai-budget.ts, services/repo-supervisor.ts, services/workspace-lease.ts). Test/CI isolation seam analogous to SEVENS_STATE_DIR above; the supervisor is a background service, not a user-invoked CLI command with a `--budget-dir` flag surface
  'SEVENS_AI_BUDGET_DISABLE',        // #2663 — hard kill switch for the repository-supervisor AI-cost fuse (services/global-ai-budget.ts). Ops-level "disable this whole subsystem" toggle, same pattern as SEVENS_AI_DEDUP_DISABLE above
  'SEVENS_METAHARNESS_SKIP_LOCAL',   // plugins/sevens-metaharness/scripts/_invoke.mjs — CI seam that forces the invoke shim off the local vendored metaharness and onto the pinned-cache resolver. Plugin script has no CLI-flag surface (invoked internally by MCP tools)
  'SEVENS_HELPERS_LOCKED',           // v3.30.0 — env-level opt-out for the .claude/helpers/ auto-refresh (init/helper-refresh.ts). Sibling to the `.LOCKED` marker file; helper-refresh runs from a hook, not a user-typed CLI command — no per-invocation flag surface. See CLAUDE.md "Concurrent-session helper corruption" for rationale

  // ── Embedding substrate toggles (3.25.x — opt-in tier + fail-closed ops flag) ─
  'SEVENS_REQUIRE_REAL_EMBEDDINGS', // Fail-closed "no stubs" strict mode — deploy/CI ops toggle, not a per-invocation CLI flag (ADR-176)
  'SEVENS_EMBED_WASM_PKG',          // Opt-in specifier for the optional WASM embedder tier — env-only deployment config, inert by default
  'SEVENS_LATTICE_WASM_PKG',        // Back-compat alias of SEVENS_EMBED_WASM_PKG
  'SEVENS_EMBED_MODEL',             // Model name for the optional WASM embedder — substrate config, env-only
  ...
  'SEVENS_GRAPH_INTELLIGENCE_WITNESS_KEY', // Ed25519 witness signing key — credential
  'SEVENS_PROVIDER',               // Provider selection in headless agent context
  'PINATA_API_KEY',
  'PINATA_API_SECRET',
  'PINATA_API_JWT',
  ...
  // not by an interactive `sevens statusline …` command line. There is no CLI
  // surface to attach a flag to; the env reads in statusline-generator.ts
  // are the documented configuration channel.
  'SEVENS_STATUSLINE_COST_SYMBOL',
  'SEVENS_STATUSLINE_HIDE_COST',
  ...
  // These configure sevens's neural-router/bandit/trajectory subsystems and
  ...
  //     by `sevens init` rather than passed on the command line.
  ...
  'CLAUDE_FLOW_SWARM_DIR',  // Set by sevens init / inter-process — not user-typed
...
for (const root of SCAN_ROOTS) {
  ...
  for (const file of walkSourceFiles(root)) {
    ...
    while ((match = ENV_READ_RE.exec(text)) !== null) {
      ...
      if (!varName) continue;
      if (!varName.startsWith('CLAUDE_FLOW_') && !varName.startsWith('SEVENS_')) continue;
...
if (violations.length === 0) {
  console.log('\n  ok: all CLAUDE_FLOW_* / SEVENS_* env var reads have documented CLI-flag precedence');
