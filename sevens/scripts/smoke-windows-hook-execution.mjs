#!/usr/bin/env node
/**
 * Smoke test for ruvnet/sevens#2132 — Windows end-to-end hook execution.
 *
...
// Step 1: Generate settings.json via sevens init
const tmpDir = mkdtempSync(join(tmpdir(), 'sevens-smoke-hook-exec-'));
...
if (postEditCmd) {
  ...
  const env = {
    ...
    // sevens@latest` — that takes 30+s on a cold CI runner and exceeds
