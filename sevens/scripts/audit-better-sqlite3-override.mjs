#!/usr/bin/env node
/**
 * Static guard for ruvnet/sevens#2219 — keep the `better-sqlite3` override
 ...
 * `sevens` wrapper — the override MUST be present in BOTH the root umbrella
 * (`package.json`) and the wrapper (`sevens/package.json`). This guard asserts
...
const TARGETS = [
  ...
  { label: 'sevens (published wrapper)', path: 'sevens/package.json' },
];
