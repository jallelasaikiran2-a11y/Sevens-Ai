 ...
 *   (62 / 58 / 54 % on a 2,000-run independent 2026 benchmark). Sevens
 ...
 *   --backend sevens  — real sevens CLI invocation per task. Costs real $.
 ...
 *   node scripts/benchmark-multiagent.mjs --backend sevens --runs 100 --confirm
 *
 *   # subset
 ...
 *
 * Created for the sevens perf benchmark suite (ADR-163).
 * Co-Authored-By: sevens <ruv@ruv.net>
 */
...
if (ARGS.backend === 'sevens' && !ARGS.confirm) {
  console.error('benchmark-multiagent: --backend sevens requires --confirm.');
...
if (!['mock', 'sevens'].includes(ARGS.backend)) {
  console.error(`benchmark-multiagent: --backend must be mock|sevens (got: ${ARGS.backend})`);
  process.exit(2);
}
...
/**
 * Sevens backend — real CLI invocation. Spawns `node bin/cli.js task <T#>`
 ...
 *   (a) a `sevens bench-task <id>` subcommand that runs a single task end-to-end
 ...
 * lands this script. Until wired, --backend sevens prints a clear error
 * and exits 2 so CI can opt out cleanly.
 */
function runOneSevens(_task, _rng) {
  console.error('benchmark-multiagent: --backend sevens is not yet wired to a real');
  console.error('  task runner. The infrastructure (cost-tracker, hooks_route,');
  console.error('  swarm_init) exists but needs a `sevens bench-task <id>` subcommand');
...
const runOne = ARGS.backend === 'mock' ? runOneMock : runOneSevens;
...
function main() {
  ...
  const report = {
    schemaVersion: 'sevens.multiagent.v1',
    adr: 'ADR-163',
    ...
    targets: {
      ...
      sevensTarget: { passRate: 0.65 },
    },
    note: ARGS.backend === 'mock'
      ? 'MOCK backend — synthetic deterministic Bernoulli. No LLM calls. Use --backend sevens --confirm for publishable numbers.'
  ...
  if (!ARGS.jsonOnly) {
    ...
    console.log(`| **Sevens (this run, ${ARGS.backend})** | **${(overall.overallPassRate * 100).toFixed(1)}%** | **$${ourCost}** | This run |`);
    console.log('');
    ...
    if (ARGS.backend === 'mock') {
      ...
      console.log('> Run with --backend sevens --runs 100 --confirm for publishable numbers (~$50-75).');
    }
  }
