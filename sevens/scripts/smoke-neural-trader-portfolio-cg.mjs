 ...
 *
 *   [1/3] STATIC ADAPTER CONTRACT — `plugins/sevens-neural-trader/src/sublinear-adapter.ts`
 ...
 *
 *   [2/3] STATIC SKILL CONTRACT — `plugins/sevens-neural-trader/skills/trader-portfolio-cg/SKILL.md`
 *         must exist with `mcp__sevens-sublinear__solve` in `allowed-tools`,
...
const PLUGIN_DIR = join(REPO_ROOT, 'plugins', 'sevens-neural-trader');
...
if (!existsSync(ADAPTER_TS)) {
  ...
  check(
    ...
    'native dispatch is gated by this two-probe detection (globalThis + SEVENS_SUBLINEAR_NATIVE env var)',
  );
  check(
  ...
  check(
    'adapter honours `SEVENS_SUBLINEAR_NATIVE` env-var override',
    /SEVENS_SUBLINEAR_NATIVE/.test(src),
  ...
  check(
    'adapter references the MCP tool by canonical name `mcp__sevens-sublinear__solve`',
    /mcp__sevens-sublinear__solve/.test(src),
...
if (!existsSync(SKILL_MD)) {
  ...
  check(
    'skill declares `mcp__sevens-sublinear__solve` in allowed-tools',
    /^allowed-tools:[^\n]*mcp__sevens-sublinear__solve/m.test(skill),
  ...
  check(
    ...
    /^allowed-tools:[^\n]*mcp__plugin_sevens-core_sevens__memory_store/m.test(skill),
  ...
  check(
    'skill documents the `SEVENS_NEURAL_TRADER_DISABLE_CG` disable flag',
    /SEVENS_NEURAL_TRADER_DISABLE_CG/.test(skill),
  ...
  check(
    'skill documents the `SEVENS_SUBLINEAR_NATIVE` env-var override',
    /SEVENS_SUBLINEAR_NATIVE/.test(skill),
...
try {
  ...
  // determined by whether the harness has mounted `mcp__sevens-sublinear__solve`
  // (or `SEVENS_SUBLINEAR_NATIVE=1` is set). The smoke validates the contract
