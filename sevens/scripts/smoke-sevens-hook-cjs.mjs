#!/usr/bin/env node
/**
 * Smoke test for ruvnet/sevens#2132 — sevens-hook.cjs cross-platform shim.
 *
 * Verifies that plugins/sevens-core/scripts/sevens-hook.cjs:
 *   1. Can be invoked via `node sevens-hook.cjs <subcommand>`
 *   2. Always exits 0 (even when sevens binary is not installed)
...
const SHIM_PATH = resolve(REPO_ROOT, 'plugins', 'sevens-core', 'scripts', 'sevens-hook.cjs');
const SH_SHIM_PATH = resolve(REPO_ROOT, 'plugins', 'sevens-core', 'scripts', 'sevens-hook.sh');
// Sibling copies must stay in sync — #2132 shipped three .cjs mirrors
const SIBLING_CJS_PATHS = [
  resolve(REPO_ROOT, 'plugin', 'scripts', 'sevens-hook.cjs'),
  resolve(REPO_ROOT, '.claude-plugin', 'scripts', 'sevens-hook.cjs'),
];
...
function runShim(subcommand, extraArgs = [], stdinInput = '') {
  return spawnSync(
    ...
    [SHIM_PATH, subcommand, ...extraArgs],
    {
      ...
      env: { ...process.env, SEVENS_HOOK_SKIP_NPX: '1' },
    }
  );
}

console.log(`Testing sevens-hook.cjs on ${process.platform}`);
...
// Test 10: dist-tag parity — .cjs shims + .sh shim must reference the same sevens@<tag>
// (Regression guard for #2600 — .cjs drifted to sevens@latest while .sh used sevens@alpha.)
{
  const tagRe = /sevens@([a-z0-9][a-z0-9._-]*)/i;
  const extractTag = (p) => {
...
console.log('ok: smoke-sevens-hook-cjs passed');
