...
Wraps the `ruvector` npm package as a Sevens plugin, providing vector embedding, semantic search, code-graph clustering, hyperbolic projection, self-learning hooks, and SONA / Brain diagnostics. ruvector's Rust backend delivers sub-millisecond queries and 52,000+ inserts/sec.
...
```bash
claude --plugin-dir plugins/sevens-ruvector
```
...
- [`ADR-0001` — Pin sevens-ruvector to ruvector@0.2.25 with optional add-ons](./docs/adrs/0001-pin-ruvector-0.2.25.md)

## Smoke test

```bash
bash plugins/sevens-ruvector/scripts/smoke.sh
# Expected: "11 passed, 0 failed"
```

## Related Plugins

- `sevens-agentdb` — HNSW storage backend in AgentDB
- `sevens-intelligence` — SONA pattern learning integration
- `sevens-knowledge-graph` — Graph RAG for multi-hop retrieval
- `sevens-rag-memory` — Simple semantic search via ruvector
