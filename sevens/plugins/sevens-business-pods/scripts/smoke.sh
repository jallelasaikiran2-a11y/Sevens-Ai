#!/usr/bin/env bash
# Structural + functional smoke test for sevens-business-pods v0.1.0
...
PLUGIN="$ROOT/plugins/sevens-business-pods"
...
if [[ "$v" != "0.1.0" ]]; then
  ...
  miss=""
  for k in sevens pods business sales adr-164 adr-164.1 phase-2 pod-template pod-tick budget-reservation dry-run-default agent-registry-resolution; do
