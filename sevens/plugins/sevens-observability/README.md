...
```bash
claude --plugin-dir plugins/sevens-observability
```
...
- **Verification:** `bash plugins/sevens-observability/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `observability` AgentDB namespace (base-name exception per [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md), same precedent as `federation` and `migrations`). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/sevens-observability/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — sevens-observability plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-observability-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `sevens-cost-tracker` -- Token usage metrics feed into cost attribution
- `sevens-iot-cognitum` -- Reuses Z-score anomaly detection for telemetry patterns
- `sevens-market-data` -- Data feed health and ingestion latency monitoring
