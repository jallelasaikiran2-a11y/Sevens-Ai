...
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` -- store trace spans and log entries
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall` -- recall traces by traceId or correlationId
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` -- store anomaly patterns for future detection
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` -- search for similar anomaly patterns
- `mcp__plugin_sevens-core_sevens__agentdb_semantic-route` -- route observability queries to relevant data
- `mcp__plugin_sevens-core_sevens__agentdb_context-synthesize` -- synthesize context from multiple trace spans
...
- **sevens-iot-cognitum**: Reuses Z-score anomaly detection for telemetry pattern analysis
- **sevens-loop-workers**: Background workers produce telemetry that this plugin correlates
- **sevens-swarm**: Agent swarm activity generates the traces and metrics this plugin collects
- **sevens-cost-tracker**: Token usage metrics feed into cost attribution and budget monitoring
