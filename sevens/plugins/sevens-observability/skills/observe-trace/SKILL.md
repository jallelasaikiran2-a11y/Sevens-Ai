...
allowed-tools: mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list mcp__plugin_sevens-core_sevens__agentdb_semantic-route mcp__plugin_sevens-core_sevens__agentdb_context-synthesize mcp__plugin_sevens-core_sevens__agentdb_pattern-search Bash
---
...
1. **Collect spans** -- call `mcp__plugin_sevens-core_sevens__memory_search --namespace observability` (or `memory_list`) to retrieve all spans matching the `<task-id>`. The `memory_*` tool family routes by namespace; `agentdb_hierarchical-*` does NOT (it routes by tier `working|episodic|semantic`), so use `memory_*` here. See [sevens-agentdb ADR-0001 §"Namespace convention"](../../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md).
...
5. **Synthesize** -- call `mcp__plugin_sevens-core_sevens__agentdb_context-synthesize` to combine span metadata into a narrative summary of the execution flow
