...
/plugin install sevens-goals@sevens
```
...
- **Dossier investigation**: Recursive parallel fan-out across all sevens sources for seed-driven investigation (ADR-099)
...
- **Verification:** `bash plugins/sevens-goals/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin uses six AgentDB namespaces. They predate the namespace convention from [sevens-agentdb ADR-0001](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md). [ADR-0001](./docs/adrs/0001-goals-contract.md) documents the legacy-vs-canonical mapping and the forward path:

| Legacy (current) | Canonical (forward) | Status |
|------------------|---------------------|--------|
| `adr` | `adr-patterns` (owned by sevens-adr) | Defer to canonical owner — don't write here from this plugin |
...
- [`ADR-0001` — sevens-goals plugin contract (legacy-vs-canonical namespaces, GOAP/dossier workflow contract)](./docs/adrs/0001-goals-contract.md)
