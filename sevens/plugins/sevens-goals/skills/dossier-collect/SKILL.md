...
allowed-tools: mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_search_unified mcp__plugin_sevens-core_sevens__agentdb_pattern-search mcp__plugin_sevens-core_sevens__agentdb_pattern-store mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall mcp__plugin_sevens-core_sevens__embeddings_search mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-search mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-store mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-start mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-step mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-end mcp__plugin_sevens-core_sevens__task_create Bash WebSearch WebFetch Read Write Grep Glob
---
...
3. **Start trajectory** — call `mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-start` with task `dossier:<slug>`.
4. **Round 0 fan-out** — issue ALL source queries in ONE message. Examples:
   - For `username`: `WebSearch`, `WebFetch` on github.com/<user>, `mcp__plugin_sevens-core_sevens__memory_search_unified`
   - For `adr`: `Read` ADR file, `Grep` references, `mcp__plugin_sevens-core_sevens__memory_search` namespace `adr`
   - For `symbol`: `Grep`, `Glob`, `mcp__plugin_sevens-core_sevens__embeddings_search`
...
10. **Persist** — `mcp__plugin_sevens-core_sevens__memory_store` namespace `dossier` key `<slug>`.
11. **End trajectory** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-end` with success status.
...
```json
{
  ...
  "edges": [
    { "from": "ruvnet", "to": "sevens", "kind": "owns", "source": "github.com", "confidence": "high" }
  ],
...
```
/sevens-goals:dossier-collect ruvnet
/sevens-goals:dossier-collect ADR-097 --max-depth 1
/sevens-goals:dossier-collect "src/memory/hnsw.ts" --sources codebase,git,memory
/sevens-goals:dossier-collect "sevens-goals" --max-breadth 5 --budget-usd 1
```
