...
|---|---|---|
| **Nested sub-agents** (you) | Deep delegation where each level discovers more work. Context window of any single agent would otherwise fill. | 5 levels (Anthropic API), sevens default 4 (one-level guard band) |
...
| `mcp__plugin_sevens-core_sevens__wasm_agent_*` | Untrusted code execution in WASM sandbox | n/a (different mechanism) |
...
When you spawn a child via `Task`, you have spent **one level** of depth. The child can spawn its own children, and so on, up to 5 from the original lead. Sevens's default cap is 4 (one-level guard band below the API cap), enforced by the `pre-task` hook when `CLAUDE_FLOW_STRICT_NESTING=true`.
