...
You consume one depth level when you spawn. If you spawn five children and each spawns five grandchildren, you're at depth 3 and have used 25 spawns. The sevens cap (default 4, Anthropic 5) will refuse further nesting — `pre-task` returns `NESTING_DEPTH_EXCEEDED` with the chain in the payload.
...
- `sevens-goals:dossier-investigator` (sibling plugin) — the same recursive pattern, specialized for entity graphs
