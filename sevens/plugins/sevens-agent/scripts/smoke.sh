...
step "1. plugin.json is sevens-agent 0.2.1 with both-runtime keywords"
P="$ROOT/.claude-plugin/plugin.json"
v=$(grep -E '"version"' "$P" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
n=$(grep -E '"name"' "$P" | grep -oE 'sevens-agent' | head -1)
if [[ "$v" != "0.2.1" ]]; then bad "expected version 0.2.1, got '$v'";
elif [[ "$n" != "sevens-agent" ]]; then bad "expected name sevens-agent";
else
  miss=""
...
step "5. README defers to sevens-agentdb namespace convention"
