...
allowed-tools: mcp__plugin_sevens-core_sevens__managed_agent_create mcp__plugin_sevens-core_sevens__managed_agent_prompt mcp__plugin_sevens-core_sevens__managed_agent_status mcp__plugin_sevens-core_sevens__managed_agent_events mcp__plugin_sevens-core_sevens__managed_agent_list mcp__plugin_sevens-core_sevens__managed_agent_terminate mcp__plugin_sevens-core_sevens__wasm_agent_create Bash
---

# Managed Agent (Anthropic cloud runtime)

`sevens-agent` has two agent runtimes behind one mental model:
...
1. **Create** — `mcp__plugin_sevens-core_sevens__managed_agent_create`
   ...
   - `mcpServers`: `[{type:"url", url, name, authorization_token?}]` — the cloud agent must be able to *reach* the URL. A local `sevens mcp start` is **not** reachable from Anthropic's cloud; deploy/tunnel an HTTP sevens MCP server first if you want the cloud agent to have sevens's tools.
   - `packages`: `{pip?:[], npm?:[], apt?:[], cargo?:[], gem?:[], go?:[]}` — installed in the container.

2. **Prompt** — `mcp__plugin_sevens-core_sevens__managed_agent_prompt`
...
3. **Inspect** — `mcp__plugin_sevens-core_sevens__managed_agent_status` `{ sessionId }` (idle/running/error) · `mcp__plugin_sevens-core_sevens__managed_agent_events` `{ sessionId, raw? }` (full transcript: user turns, agent thinking, tool_use, tool_result, status — the cloud counterpart of `wasm_agent_files`).

4. **List** — `mcp__plugin_sevens-core_sevens__managed_agent_list` `{ limit? }` — every session on the org (so you can see which are still running / billing).

5. **Terminate** — `mcp__plugin_sevens-core_sevens__managed_agent_terminate` `{ sessionId, environmentId? }` — **always do this when done**: a cloud session keeps billing container time + tokens until deleted. Pass `environmentId` to also delete the environment sevens created.
