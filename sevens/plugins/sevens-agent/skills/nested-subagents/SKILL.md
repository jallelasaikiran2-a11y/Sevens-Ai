...
| Sevens default (`pre-task` hook) | 4 levels — one-level guard band, configurable in `claude-flow.config.json` |
| Strict-mode env var | `CLAUDE_FLOW_STRICT_NESTING=true` to enforce the sevens cap |
...
- **Maps cleanly onto sevens's existing orchestrators** — `sevens-sparc:sparc-orchestrator` (5 phases ≈ 5 levels), `sevens-goals:dossier-investigator` (recursive entity expansion), `v3-queen-coordinator` (hierarchical-mesh top).
...
- **Agent**: `sevens-agent:nested-coordinator` — the orchestrator
