...
Agent runtimes for sevens — one mental model, two backends (a third planned):
...
> Renamed from `sevens-wasm` (it only covered the local WASM runtime). The `wasm_agent_*` / `wasm_gallery_*` tool *names* are unchanged.

## Install

```
/plugin marketplace add ruvnet/sevens
/plugin install sevens-agent@sevens
```
...
> Beta API (`anthropic-beta: managed-agents-2026-04-01`); `multiagent` / `define-outcomes` on the agent config are research preview. `mcpServers` for a cloud agent must point at a **publicly reachable** URL — a local `sevens mcp start` is not reachable from Anthropic's cloud (deploy/tunnel an HTTP sevens MCP server first). Managed sessions cost LM tokens + container time and are rate-limited per org.
...
- **Verification:** `bash plugins/sevens-agent/scripts/smoke.sh` is the contract (12 structural checks). CI: `.github/workflows/sevens-agent-smoke.yml`. Behavioral guard for the cloud tools: `v3/@claude-flow/cli/__tests__/managed-agent-tools.test.ts` (no-network).
...
For prompt-injection defense on output flowing back to the host LLM, the [sevens-aidefence 3-gate pattern](../sevens-aidefence/docs/adrs/0001-aidefence-contract.md) applies.

## Namespace coordination

This plugin owns the `wasm-gallery` AgentDB namespace (kebab-case, per [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed. `wasm-gallery` indexes published WASM agents (manifest, version, signature, download count); accessed via `memory_*` (namespace-routed).

## Verification

```bash
bash plugins/sevens-agent/scripts/smoke.sh
# Expected: "12 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — sevens-agent plugin contract](./docs/adrs/0001-wasm-contract.md) (WASM runtime: 10-tool MCP surface, ADR-070 integration, sandbox isolation, smoke as contract)
...
- `sevens-agentdb` — namespace convention owner
- `sevens-aidefence` — 3-gate pattern applies to agent output flowing back to the host LLM
- `sevens-ruvector` — the ruvector substrate that ships `@ruvector/rvagent-wasm`
- `sevens-cost-tracker` — record completed Managed Agent sessions (LM tokens + container time)
