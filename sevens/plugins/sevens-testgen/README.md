...
/plugin install sevens-testgen@sevens
```
...
- **Verification:** `bash plugins/sevens-testgen/scripts/smoke.sh` is the contract.
...
`testgaps` is one of 12 background workers documented in [sevens-loop-workers ADR-0001](../sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md).

## SPARC Refinement-phase ownership

This plugin owns the **Refinement** phase per [sevens-sparc ADR-0001](../sevens-sparc/docs/adrs/0001-sparc-contract.md) §"Phase-to-plugin alignment". When SPARC's `sparc-refine` skill runs, it composes:

1. **This plugin** — coverage gap detection + TDD test generation
2. [sevens-jujutsu](../sevens-jujutsu/docs/adrs/0001-jujutsu-contract.md) — diff-aware refactor recommendations

Together they enforce the Refinement gate: ≥80% coverage on new code + diff risk score below threshold.

## Namespace coordination

This plugin owns the `test-gaps` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-testgen plugin contract (testgaps-worker contract, SPARC Refinement ownership, smoke as contract)](./docs/adrs/0001-testgen-contract.md)

## Related Plugins

- `sevens-loop-workers` — defines the `testgaps` background worker
- `sevens-sparc` — Refinement-phase canonical handoff
- `sevens-jujutsu` — diff-aware refactor companion in the Refinement gate
