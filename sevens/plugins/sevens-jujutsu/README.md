...
/plugin install sevens-jujutsu@sevens
```
...
- **Verification:** `bash plugins/sevens-jujutsu/scripts/smoke.sh` is the contract.

## ADR-compliance integration

This plugin's diff analysis is the substrate that [sevens-adr ADR-0001](../sevens-adr/docs/adrs/0001-adr-plugin-pattern.md)'s `/adr check` runs on. When ADR compliance is queried for a diff:
...
This plugin owns the `git-patterns` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-jujutsu plugin contract (ADR-compliance integration, smoke as contract)](./docs/adrs/0001-jujutsu-contract.md)

## Related Plugins

- `sevens-adr` — `/adr check` consumes this plugin's `analyze_diff` output
