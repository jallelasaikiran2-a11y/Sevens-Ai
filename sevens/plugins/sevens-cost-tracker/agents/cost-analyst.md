...
- `mcp__plugin_sevens-core_sevens__memory_store` — store usage records, budget config, optimization patterns (the `memory_*` family is namespace-routed; prefer this over `agentdb_hierarchical-*`)
- `mcp__plugin_sevens-core_sevens__memory_search` / `memory_list` / `memory_retrieve` — read cost-tracking + cost-patterns namespaces
- `mcp__plugin_sevens-core_sevens__memory_delete` — clean up stale config (used by budget upsert-via-timestamp pattern)
- `mcp__plugin_sevens-core_sevens__hooks_route` — invoked by `cost-booster-route`
- `mcp__plugin_sevens-core_sevens__hooks_model-outcome` — invoked by `cost-optimize` step 8 (auto-emits via `outcome.mjs`)
- `mcp__plugin_sevens-core_sevens__hooks_worker-status` — `cost workers` subcommand (consumes `optimize` + `benchmark` worker outputs)
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` / `_pattern-search` — ReasoningBank-routed (no namespace arg) for typed cost-optimization patterns
...
This plugin is the declared consumer of two `sevens-loop-workers` background workers (see [sevens-loop-workers ADR-0001 §"12-worker trigger map"](../../sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md)):

- **`optimize`** — periodically scans recent cost data and produces optimization recommendations. Consumed by the `cost-optimize` skill and surfaced via `cost workers` (see `commands/sevens-cost.md`).
...
Use `mcp__plugin_sevens-core_sevens__hooks_worker-status --worker optimize` and `--worker benchmark` to inspect last-run timestamps and outcomes. The worker scheduling itself is owned by `sevens-loop-workers`; this plugin only consumes outputs.
...
- **sevens-intelligence**: Model routing optimization data feeds cost analysis (3-tier routing reduces cost 75%).
- **sevens-autopilot**: Budget-aware autopilot mode uses cost data to throttle agent spawns.
- **sevens-observability**: Token usage metrics collected via observability instrumentation.
- **sevens-swarm**: Agent spawn/stop decisions informed by budget remaining.
- **sevens-federation**: Federation budget circuit breaker (ADR-097) — federation_send `maxTokens` / `maxUsd` enforcement complements local cost tracking.
