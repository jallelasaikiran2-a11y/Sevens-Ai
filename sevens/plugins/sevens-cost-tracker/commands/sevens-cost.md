...
1. Run `node plugins/sevens-cost-tracker/scripts/budget.mjs set <amount>` to write the config to the cost-tracking namespace
...
1. Run `node plugins/sevens-cost-tracker/scripts/budget.mjs get`
...
1. Run `node plugins/sevens-cost-tracker/scripts/budget.mjs check`
...
1. Invoke `node plugins/sevens-cost-tracker/scripts/track.mjs` (no flags = current cwd's most-recent session)
...
2. Runs `node plugins/sevens-cost-tracker/scripts/outcome.mjs "<task>" <model> <outcome>`
...
1. Run `node plugins/sevens-cost-tracker/scripts/summary.mjs --format json`
...
1. Run `node plugins/sevens-cost-tracker/scripts/federation.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/conversation.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/trend.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/projection.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/counterfactual.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/burn.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/anomaly.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/session.mjs`
...
1. Run `node plugins/sevens-cost-tracker/scripts/diff.mjs --baseline <path> --current <path>`
...
1. Run `node plugins/sevens-cost-tracker/scripts/health.mjs`
...
**`cost workers`** -- Inspect the `optimize` and `benchmark` background workers consumed from sevens-loop-workers.
1. Call `mcp__plugin_sevens-core_sevens__hooks_worker-status --worker optimize` -- report last-run timestamp, outcome, and any pending recommendations
2. Call `mcp__plugin_sevens-core_sevens__hooks_worker-status --worker benchmark` -- report last-run timestamp, outcome, and any pending benchmark deltas
3. Cross-link [sevens-loop-workers ADR-0001 §"12-worker trigger map"](../../sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md) — the contract this command honors
