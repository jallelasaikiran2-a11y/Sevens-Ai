...
```bash
claude --plugin-dir plugins/sevens-cost-tracker
```
...
| **Optimize + benchmark loop workers** | `hooks_worker-status --worker optimize / --worker benchmark` (declared by sevens-loop-workers) | `cost workers` command + `cost-analyst` agent |
...
## Federation budget circuit breaker pairing (sevens 3.6.25+)
...
- **Verification:** `bash plugins/sevens-cost-tracker/scripts/smoke.sh` is the contract.
...
Both follow the kebab-case `<plugin-stem>-<intent>` convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md). Both are accessed via the `memory_*` tool family which routes by namespace.
...
- [`ADR-0001` — sevens-cost-tracker plugin contract (namespace-routing fix, federation budget pairing, smoke as contract)](./docs/adrs/0001-cost-tracker-contract.md)
...
- `sevens-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `sevens-observability` -- Token usage metrics collected via observability instrumentation
- `sevens-neural-trader` -- PnL tracking and cost-adjusted return calculation
- `sevens-federation` -- Budget circuit breaker on outbound federation_send (ADR-097)
