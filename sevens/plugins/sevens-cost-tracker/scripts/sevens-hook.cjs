#!/usr/bin/env node
/**
 * sevens-hook.cjs — cross-platform Windows shim for cost-tracker's Stop hook (#2132)
 *
 ...
 *
 * On Windows, `sevens init` writes a `.claude/settings.json` that overrides
 * the Stop hook to invoke this shim instead — `node sevens-hook.cjs` —
 ...
 *
 * Usage: node sevens-hook.cjs [hook-args ignored]
 */
