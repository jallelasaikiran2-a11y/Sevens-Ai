---
id: ADR-0002
title: sevens-cost-tracker — agentic-flow + Agent Booster integration, model-outcome feedback loop, optimize-worker consumption, tier-aware reporting
status: Accepted
date: 2026-05-04
authors:
  - planner (Claude Code)
...
`sevens-cost-tracker` (v0.2.2, post-ADR-0001) does three things well: it owns two namespaces (`cost-tracking`, `cost-patterns`) routed correctly via `memory_*`, it documents the federation budget circuit breaker pairing (ADR-097), and it ships a 10-check smoke contract. ADR-0001 fixed a real namespace-routing bug.
...
sevens-intelligence ADR-0001 §"Neutral" already calls out the equivalent intent: "Consumers of [the legacy `routing-outcomes` namespace] should migrate to `hooks_model-outcome` which is the typed equivalent." Cost-tracker is one such consumer.

### 4. Cost-tracker is named as consumer of the `optimize` worker — but doesn't know it

`plugins/sevens-loop-workers/README.md:46` lists `sevens-cost-tracker` (alongside `sevens-intelligence`) as the consumer of the `optimize` background worker, and line 55 lists it as a consumer of `benchmark`. Cost-tracker's own README, agent, skills, and commands make zero reference to either. This is a sibling-plugin contract drift: the substrate plugin (`sevens-loop-workers` ADR-0001 §"12-worker trigger map") has already declared the relationship; the consumer plugin hasn't honored it.
...
- **Allowed tools:** `mcp__plugin_sevens-core_sevens__hooks_route`, `mcp__plugin_sevens-core_sevens__memory_search`, `Bash`. No wildcard.
...
- **Behavior:** import `getTokenOptimizer` from `@claude-flow/integration` via a `Bash`-shelled Node one-liner (or via a new `mcp__plugin_sevens-core_sevens__token_optimize_compact` tool if/when one is added — see "Riskiest assumption" below). Report the bridge's `tokensSaved` figure plus the `agentBoosterAvailable` flag.
...
After the existing step 7 ("Store the optimization pattern"), add step 8: when a downgrade recommendation is *applied* (or its application is logged), call `mcp__plugin_sevens-core_sevens__hooks_model-outcome` with `{task, fromModel, toModel, outcome: 'success'|'escalated'|'failure'}`. This closes the routing loop documented in sevens-intelligence ADR-0001.

- **Allowed-tools update:** add `mcp__plugin_sevens-core_sevens__hooks_model-outcome`.
- **Cross-link:** sevens-intelligence ADR-0001 §"Neutral" (migration target).
...
sevens-loop-workers README already declares this. Honor it on the cost-tracker side:

- **`agents/cost-analyst.md`** — add a "Background workers" section listing `optimize` (consumed for cost-optimization recommendations) and `benchmark` (consumed for cost-per-benchmark reporting). Cross-link sevens-loop-workers ADR-0001 §"12-worker trigger map".
- **`commands/sevens-cost.md`** — add a `cost workers` subcommand that calls `mcp__plugin_sevens-core_sevens__hooks_worker-status --worker optimize` and `--worker benchmark` and reports last-run timestamps + outcomes.
- **No new skill** — the existing `loop-worker` skill from `sevens-loop-workers` is reused.
...
- Sibling-plugin contract honored: sevens-loop-workers' declared consumer relationship is now reciprocated.
...
13. `cost-optimize/SKILL.md` step list includes a `hooks_model-outcome` invocation step, and `allowed-tools` enumerates `mcp__plugin_sevens-core_sevens__hooks_model-outcome`.
14. `agents/cost-analyst.md` documents both `optimize` and `benchmark` workers in a "Background workers" section, with a cross-link to sevens-loop-workers ADR-0001.
15. `commands/sevens-cost.md` includes a `cost workers` subcommand referencing `hooks_worker-status` for both `optimize` and `benchmark`.
...
- `grep -q "agentic-flow" plugins/sevens-cost-tracker/README.md`
- `grep -q "AGENT_BOOSTER_AVAILABLE" plugins/sevens-cost-tracker/skills/cost-booster-route/SKILL.md`
- `grep -qE "Tier 1.+Tier 2.+Tier 3" plugins/sevens-cost-tracker/REFERENCE.md`
...
- `plugins/sevens-cost-tracker/docs/adrs/0001-cost-tracker-contract.md` — namespace-routing fix; this ADR builds on its smoke-as-contract pattern
- `plugins/sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — surfaces `hooks_route`, `hooks_model-route`, `hooks_model-outcome`, `hooks_transfer`; calls out the `routing-outcomes` → `hooks_model-outcome` migration
- `plugins/sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md` — declares cost-tracker as consumer of `optimize` and `benchmark` workers (the contract this ADR honors from the consumer side)
- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention; RaBitQ quantization (relevant for storing tier-classified cost records cheaply at scale)
