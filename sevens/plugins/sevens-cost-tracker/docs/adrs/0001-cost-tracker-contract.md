---
id: ADR-0001
title: sevens-cost-tracker plugin contract — pinning, namespace-routing fix, federation budget integration, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-cost-tracker` (v0.2.1) tracks token usage per agent/task/model, computes USD cost attribution, and recommends optimizations. It already documents:
...
The two skills (`cost-report`, `cost-optimize`) call `agentdb_hierarchical-recall` with a `namespace: 'cost-tracking'` argument and `agentdb_pattern-store` with `namespace: 'cost-patterns'`. Per [sevens-agentdb ADR-0001 §"Where namespace strings actually apply"](../../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md), neither tool family routes by namespace:
...
6. README references sevens-agentdb namespace convention.
...
```bash
bash plugins/sevens-cost-tracker/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Related

- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention; defines the routing contract this ADR fixes a violation of
...
- `plugins/sevens-aidefence/docs/adrs/0001-aidefence-contract.md`
- `plugins/sevens-autopilot/docs/adrs/0001-autopilot-contract.md`
- `plugins/sevens-core/docs/adrs/0001-core-contract.md`
- `v3/docs/adr/ADR-097-federation-budget-circuit-breaker.md` — federation budget envelope

## Implementation status

Plugin version v0.16.1 shipped and listed in marketplace.json. Source exists at `plugins/sevens-cost-tracker/`. Contract elements implemented: dual namespaces (`cost-tracking`, `cost-patterns`) with correct `memory_*` routing; ADR-097 budget circuit breaker Phase 1 (send-side enforcement) documented; namespace-routing bug fixed (switched from `agentdb_hierarchical-*` to `memory_*`); smoke-as-contract gate defined in `scripts/smoke.sh`.
