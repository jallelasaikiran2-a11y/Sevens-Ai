...
| commands/sevens-cost.md                     |  2,802 |   370 |    46 |
...
| 4 | sibling contract honored both ways | `grep "cost-tracker" sevens-loop-workers/README.md`        | declared at lines 46, 55 ✓ | none |
...
```bash
( cd v3 && node ../plugins/sevens-cost-tracker/scripts/bench.mjs )
```
...
Method to refresh: `( cd v3 && BENCH_LLM_BASELINE=1 BENCH_ANTHROPIC=1 node ../plugins/sevens-cost-tracker/scripts/bench.mjs )`.
...
```bash
cd plugins/sevens-cost-tracker
