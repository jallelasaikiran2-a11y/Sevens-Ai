...
1. **Pick a format** — textfile collector for Prometheus / Grafana, or webhook POST for everything else:
   ...
   node plugins/sevens-cost-tracker/scripts/export.mjs --prometheus /var/lib/node_exporter/textfile_collector/cost_tracker.prom

   # Webhook (POSTs JSON; add auth via env)
   EXPORT_WEBHOOK_HEADER='Authorization: Bearer $TOKEN' \
     node plugins/sevens-cost-tracker/scripts/export.mjs --webhook https://hooks.example.com/cost-tracker

   # Stdout JSON (default if no flag)
   node plugins/sevens-cost-tracker/scripts/export.mjs
   ```
