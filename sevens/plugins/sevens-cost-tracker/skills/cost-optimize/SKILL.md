...
argument-hint: ""
allowed-tools: mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__agentdb_pattern-search mcp__plugin_sevens-core_sevens__agentdb_pattern-store mcp__plugin_sevens-core_sevens__agentdb_semantic-route mcp__plugin_sevens-core_sevens__hooks_model-outcome Bash
---
...
1. **Load usage data** -- call `mcp__plugin_sevens-core_sevens__memory_search` on the `cost-tracking` namespace (last 7 days). The `memory_*` tools route by namespace; use them — not `agentdb_hierarchical-*` (which routes by tier).
...
6. **Search prior optimization patterns** -- call `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` (ReasoningBank-routed; **don't** pass a `namespace` argument — pattern-* tools ignore it).
7. **Store the optimization pattern** -- two paths:
   - **Pattern store (typed, recommended)**: `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` with `type: 'cost-optimization'`. Don't pass a `namespace` arg — ReasoningBank routes it; on bridge unavailability the fallback writes to the reserved `pattern` namespace with `controller: 'memory-store-fallback'` (see sevens-agentdb ADR-0001).
   - **Plain store (namespace-routable)**: `mcp__plugin_sevens-core_sevens__memory_store --namespace cost-patterns` — this DOES respect the `cost-patterns` namespace because `memory_*` is namespace-routed.
8. **Close the routing feedback loop — auto-emit `hooks_model-outcome`** -- for each downgrade recommendation, format the outcome-emit command as part of the recommendation table so it can be run directly:
   ...
   node plugins/sevens-cost-tracker/scripts/outcome.mjs "<task-description>" <model> success

   # escalated path (had to upgrade after downgrade attempt)
   node plugins/sevens-cost-tracker/scripts/outcome.mjs "<task-description>" <model> escalated
   ```

   The script wraps `npx @claude-flow/cli hooks model-outcome -t ... -m ... -o ...` with explicit-argv `spawnSync` so quoting is safe. Without this signal the router does not learn from cost-tracker's recommendations and the booster bypass rate (see `cost-booster-route` skill) does not improve over time. This is the typed equivalent of the legacy `routing-outcomes` namespace (see sevens-intelligence ADR-0001 §"Neutral").
