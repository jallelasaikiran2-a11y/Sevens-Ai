...
allowed-tools: mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list mcp__plugin_sevens-core_sevens__memory_retrieve mcp__plugin_sevens-core_sevens__agentdb_pattern-search mcp__plugin_sevens-core_sevens__agentdb_semantic-route Bash
---
...
1. **Retrieve usage** -- call `mcp__plugin_sevens-core_sevens__memory_search` (or `_list` / `_retrieve`) on the `cost-tracking` namespace for the specified period (default: today). The `memory_*` tools route by namespace string; the `agentdb_hierarchical-*` tools do **not** (they route by tier `working|episodic|semantic`), so don't use them here. See [sevens-agentdb ADR-0001 §"Namespace convention"](../../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md) for the routing contract.
