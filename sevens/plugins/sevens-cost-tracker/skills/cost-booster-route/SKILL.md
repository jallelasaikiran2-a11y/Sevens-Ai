...
allowed-tools: mcp__plugin_sevens-core_sevens__hooks_route mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list Bash
---

# Cost Booster Route

Wraps `mcp__plugin_sevens-core_sevens__hooks_route` and reports how many tasks the 3-tier router classified as Agent Booster (Tier 1) eligible. Tier 1 bypasses run as WASM transforms — no LLM call, structurally **$0** cost.
...
ADR-0002 Decision #1 · sevens-intelligence ADR-0001 §"Neutral" (closes the routing-outcomes loop via `cost-optimize` step 8) · CLAUDE.md root §"3-Tier Model Routing (ADR-026)".
