...
/plugin install sevens-workflows@sevens
```
...
- **Verification:** `bash plugins/sevens-workflows/scripts/smoke.sh` is the contract.
...
This plugin owns the `workflows-state` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-workflows plugin contract (10-tool MCP surface, lifecycle state machine, smoke as contract)](./docs/adrs/0001-workflows-contract.md)
...
- `sevens-loop-workers` — sibling automation surface (loops are recurring; workflows are stateful pipelines)
- `sevens-sparc` — SPARC phase transitions can be modeled as workflows
