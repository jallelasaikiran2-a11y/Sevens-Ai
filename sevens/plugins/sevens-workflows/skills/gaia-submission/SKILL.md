...
allowed-tools: Bash mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list mcp__plugin_sevens-core_sevens__hooks_post_task mcp__plugin_sevens-core_sevens__hooks_pre_task
---
...
```bash
/gaia submit --results=~/.cache/sevens/gaia/results-latest.json
```
...
signed *into* the witness manifest as an ADR-103 fix marker, so a sevens GAIA
...
Interpret the gap between sevens's score and the leaderboard top-10.
