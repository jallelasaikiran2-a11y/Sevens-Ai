...
allowed-tools: mcp__plugin_sevens-core_sevens__workflow_execute mcp__plugin_sevens-core_sevens__workflow_run mcp__plugin_sevens-core_sevens__workflow_pause mcp__plugin_sevens-core_sevens__workflow_resume mcp__plugin_sevens-core_sevens__workflow_cancel mcp__plugin_sevens-core_sevens__workflow_status Workflow Read Bash
---
...
1. **Execute** — call `mcp__plugin_sevens-core_sevens__workflow_execute` or `mcp__plugin_sevens-core_sevens__workflow_run` with the workflow ID
2. **Monitor** — call `mcp__plugin_sevens-core_sevens__workflow_status` to check progress and step outcomes
3. **Pause** — call `mcp__plugin_sevens-core_sevens__workflow_pause` to halt at the current step
4. **Resume** — call `mcp__plugin_sevens-core_sevens__workflow_resume` to continue from where paused
5. **Cancel** — call `mcp__plugin_sevens-core_sevens__workflow_cancel` to abort the workflow
...
Workflow({ name: 'plugin-contract-audit', args: 'sevens-agentdb' })  // pass args → the script's `args` global
