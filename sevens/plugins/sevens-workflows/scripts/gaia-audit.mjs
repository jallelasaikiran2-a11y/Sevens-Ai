...
const GRADER_SURFACE_PATTERNS = [
  /gaia-judge/i,
  /judgments/i,                       // ~/.cache/sevens/gaia/judgments/<hash>.json
  /judge[_-]?cache/i,
  /verify\.mjs/i,
  /regen\.mjs/i,
  /manifest\.md\.json/i,
...
// they let the audit pass clean against sevens's own runner (which legitimately
  ...
  if (auditable === 0) {
    return {
      ...
      harness_gap:
        ...
        'the judge cache (~/.cache/sevens/gaia/judgments) is an unsigned ' +
 ...
 * sevens's own `gcloud secrets …` execSync calls): comment lines skipped;
 ...
 * (plugins/sevens-workflows/scripts/gaia-audit.mjs → three levels up), falling
...
async function main() {
  ...
  // dataset dirs default to the HF + sevens cache locations gaia-loader uses.
  let runnerSources = [];
  let artifactScan = [];
  let datasetDirs = [];
  if (!args['skip-source-scan']) {
    ...
    datasetDirs = (args['dataset-dir'] && args['dataset-dir'].length)
      ...
      : [
          ...
          join(homedir(), '.cache', 'sevens', 'gaia', 'dataset'),
        ];
  }
