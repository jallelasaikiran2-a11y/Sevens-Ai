...
Run GAIA benchmark questions through the sevens agent loop.
...
2. **Load dataset** — downloads and caches the GAIA validation split from
   Hugging Face (`~/.cache/sevens/gaia/`).  Cached files are reused on
   subsequent runs.
...
6. **Write output** — results land in `~/.cache/sevens/gaia/results-<sha>.json`.
...
| sevens iter 23 | 20.8% | 53 Q, web_search restored |
| sevens iter 15 | 9.4% | 53 Q, broken web_search |
