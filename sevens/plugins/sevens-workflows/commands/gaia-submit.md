...
with the sevens Ed25519 witness manifest.
...
|------|---------|-------------|
| `--results` | `~/.cache/sevens/gaia/results-latest.json` | Path to the JSON results file from `/gaia run` |
...
1. Locate the results file — default `~/.cache/sevens/gaia/results-latest.json`;
...
5. Build `metadata.json`:
   ```json
   {
     ...
     "harness": "sevens@3.6.x / @claude-flow/cli@3.6.x",
     "model": "<model-id>",
     "gaia_level": 1,
