...
(default `~/.cache/sevens/gaia/results-latest.json`). Motivated by UC Berkeley
...
```bash
node plugins/sevens-workflows/scripts/gaia-audit.mjs \
  --results ~/.cache/sevens/gaia/results-latest.json \
...
`~/.cache/huggingface`, `~/.cache/sevens/gaia/dataset`). `dynamic-eval` is tuned
to pass sevens's own multi-line `gcloud secrets … execSync` calls (fixed-string
...
5. If `--audit` is set and a results file exists, run
   `plugins/sevens-workflows/scripts/gaia-audit.mjs` (ADR-167); treat a CRITICAL
