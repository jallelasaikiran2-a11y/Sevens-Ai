---
id: ADR-0001
title: sevens-workflows plugin contract — pinning, namespace coordination, 10-tool MCP surface, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-workflows` (v0.1.0) — workflow automation with templates, orchestration, and lifecycle management. 1 agent (`workflow-specialist`), 2 skills (`workflow-create`, `workflow-run`), 1 command (`/workflow`).
...
**Positive:** plugin joins the cadence. The 10-tool surface + lifecycle state machine are now contractually documented. **This ADR completes the plugin-contract retrofit across the entire sevens plugin family** — all 33 plugins now have ADR-0001 + smoke + namespace coordination.
...
```bash
bash plugins/sevens-workflows/scripts/smoke.sh
...
- `plugins/sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md` — sibling automation surface (loops vs workflows)
- `plugins/sevens-sparc/docs/adrs/0001-sparc-contract.md` — SPARC orchestration uses workflows for phase transitions
- `v3/@claude-flow/cli/src/mcp-tools/workflow-tools.ts` — 10 `workflow_*` tools

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/sevens-workflows/`. Contract elements implemented: all 10 `workflow_*` MCP tools covered; SPARC phase-transition orchestration cross-linked; namespace `workflows-state` claimed; smoke-as-contract gate defined in `scripts/smoke.sh` (11 checks).
