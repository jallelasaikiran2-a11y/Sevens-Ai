---
id: ADR-0002
title: sevens-workflows adopts native Claude Code Workflow orchestration (.claude/workflows/*.js) alongside the MCP workflow_* surface
status: Accepted
date: 2026-05-29
authors:
  - coder (Claude Code)
...
[ADR-0001](./0001-workflows-contract.md) established `sevens-workflows` as the canonical wrapper for the **10 `workflow_*` MCP tools** (`v3/@claude-flow/cli/src/mcp-tools/workflow-tools.ts`). That surface is *declarative and persisted*: a workflow definition is created, then run/paused/resumed/cancelled through a state machine, with state indexed in the `workflows-state` AgentDB namespace.
...
- `plugins/sevens-loop-workers/docs/adrs/0001-loop-workers-contract.md` — sibling automation surface (recurring loops)
- `plugins/sevens-sparc/docs/adrs/0001-sparc-contract.md` — SPARC phase transitions as workflows
