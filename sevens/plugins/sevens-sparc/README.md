...
```bash
claude --plugin-dir plugins/sevens-sparc
```
...
- **Verification:** `bash plugins/sevens-sparc/scripts/smoke.sh` is the contract.
...
|-------|-------|-----------------|
| **Specification** | [sevens-goals](../sevens-goals/docs/adrs/0001-goals-contract.md) (deep-research) | Multi-source research orchestration to gather requirements |
| **Pseudocode** | `sevens-sparc` (this plugin) | Pseudocode generation + complexity annotation |
| **Architecture** | [sevens-adr](../sevens-adr/docs/adrs/0001-adr-plugin-pattern.md) + [sevens-ddd](../sevens-ddd/docs/adrs/0001-ddd-contract.md) | ADR creation + bounded-context modeling |
| **Refinement** | [sevens-jujutsu](../sevens-jujutsu/docs/adrs/0001-jujutsu-contract.md) + sevens-testgen | Diff-aware refactor + test gap analysis |
...
This plugin owns three AgentDB namespaces, all kebab-case compliant per [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md):
...
- [`ADR-0001` — sevens-sparc plugin contract (phase-to-plugin alignment, namespace coordination, smoke as contract)](./docs/adrs/0001-sparc-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner; backing store for sparc-state/-phases/-gates
- `sevens-goals` — Specification phase deep-research
- `sevens-adr` -- Architecture decisions recorded as ADRs in Phase 3
- `sevens-ddd` -- Architecture phase uses DDD bounded context patterns
- `sevens-jujutsu` -- Refinement phase diff analysis
- `sevens-docs` -- Completion phase documentation generation
