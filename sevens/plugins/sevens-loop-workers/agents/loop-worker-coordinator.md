...
1. **Dispatch workers** via `mcp__plugin_sevens-core_sevens__hooks_worker-dispatch` with the correct trigger name
2. **Monitor health** via `mcp__plugin_sevens-core_sevens__hooks_worker-status` and report failures
...
- `mcp__plugin_sevens-core_sevens__hooks_worker-dispatch` — direct worker dispatch
- `mcp__plugin_sevens-core_sevens__hooks_worker-status` — worker health check
