...
[[ -f "$ROOT/commands/sevens-loop.md" ]] || miss="$miss missing-loop-cmd"
[[ -f "$ROOT/commands/sevens-schedule.md" ]] || miss="$miss missing-schedule-cmd"
[[ -z "$miss" ]] && ok || bad "$miss"
...
step "9. sevens-autopilot 270s heartbeat contract cross-reference"
F="$ROOT/README.md"
grep -q "sevens-autopilot" "$F" \
...
grep -q "sevens-docs" "$F" || miss="$miss docs-attribution"
grep -q "sevens-testgen" "$F" || miss="$miss testgen-attribution"
[[ -z "$miss" ]] && ok || bad "$miss"
