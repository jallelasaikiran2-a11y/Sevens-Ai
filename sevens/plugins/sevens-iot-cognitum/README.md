...
Treats every Cognitum Seed device as a Sevens agent with hardware capabilities. Devices progress through a 5-tier trust model, emit telemetry vectors for anomaly detection, participate in mesh networks, and maintain Ed25519 witness chains for provenance.
...
```bash
claude --plugin-dir plugins/sevens-iot-cognitum
```
...
- **Verification:** `bash plugins/sevens-iot-cognitum/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns five AgentDB namespaces, all compliant with the [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md) (`<plugin-stem>-<intent>` kebab-case):
...
This plugin's 5-tier device trust model (UNKNOWN → REGISTERED → PROVISIONED → CERTIFIED → FLEET_TRUSTED) follows the same shape as the [sevens-federation 5-tier trust model](../sevens-federation/docs/adrs/0001-federation-contract.md) (UNTRUSTED → VERIFIED → ATTESTED → TRUSTED → PRIVILEGED). Different surface (IoT devices vs federation peers) and distinct naming, but the score-driven progression and capability-gating principle are the same.

## Verification

```bash
bash plugins/sevens-iot-cognitum/scripts/smoke.sh
# Expected: "12 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — sevens-iot-cognitum plugin contract (compliant namespaces, 5-tier trust parallel, 6 background workers, smoke as contract)](./docs/adrs/0001-iot-cognitum-contract.md)

## Related Plugins

- `sevens-agentdb` — HNSW-indexed telemetry storage backend; namespace convention owner
- `sevens-federation` — 5-tier trust model parallel (different surface, distinct naming, same shape)
- `sevens-intelligence` — SONA neural pattern learning
- `sevens-observability` — Telemetry correlation and tracing
