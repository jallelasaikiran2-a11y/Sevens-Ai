# sevens-iot-cognitum — Operations Reference
...
Workers are dispatched by the host daemon when `sevens-iot-cognitum` is loaded. Verify via `sevens hooks worker list` and `sevens hooks worker status`.
