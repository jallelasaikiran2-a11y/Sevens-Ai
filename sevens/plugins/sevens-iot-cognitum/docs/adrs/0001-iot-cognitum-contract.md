---
id: ADR-0001
title: sevens-iot-cognitum plugin contract — pinning, namespace coordination (compliant), 5-tier device trust + 6 background workers, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-iot-cognitum` (v0.1.1) — IoT plugin for Cognitum Seed hardware. Surface:
...
Five AgentDB namespaces already used, **all compliant** with the kebab-case `<plugin-stem>-<intent>` rule from [sevens-agentdb ADR-0001](../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md):
...
4. No `device-coordinator` cross-reference to `sevens-federation`'s 5-tier trust model — they describe similar concepts but for different surfaces (federation peers vs. IoT devices). Worth cross-linking.

## Decision

1. Add this ADR (Proposed).
2. README augment: Compatibility (pin v3.6 + Cognitum Seed firmware compatibility note), explicit Namespace coordination block (already-compliant, just document the contract claim), Verification + Architecture Decisions sections, brief cross-link to sevens-federation 5-tier trust pattern (different surface, same shape).
...
**Positive:** plugin joins the cadence. Shared shape with sevens-federation's 5-tier trust model is now cross-referenced.
...
- `plugins/sevens-federation/docs/adrs/0001-federation-contract.md` — 5-tier trust model parallel
- `plugins/sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — SONA neural integration

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/sevens-iot-cognitum/`. Contract elements implemented: 5-tier device trust model; 6 background workers (HealthProbe, TelemetryIngest, AnomalyScan, MeshSync, FirmwareWatch, WitnessAudit); namespace coordination compliant; smoke-as-contract gate defined in `scripts/smoke.sh` (12 checks).
