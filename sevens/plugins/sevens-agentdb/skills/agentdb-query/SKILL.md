...
argument-hint: "<query>"
allowed-tools: mcp__plugin_sevens-core_sevens__agentdb_semantic-route mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store mcp__plugin_sevens-core_sevens__agentdb_context-synthesize mcp__plugin_sevens-core_sevens__agentdb_causal-edge mcp__plugin_sevens-core_sevens__agentdb_pattern-search mcp__plugin_sevens-core_sevens__agentdb_pattern-store mcp__plugin_sevens-core_sevens__agentdb_controllers mcp__plugin_sevens-core_sevens__agentdb_health mcp__plugin_sevens-core_sevens__agentdb_batch mcp__plugin_sevens-core_sevens__agentdb_feedback mcp__plugin_sevens-core_sevens__agentdb_consolidate mcp__plugin_sevens-core_sevens__agentdb_session-start mcp__plugin_sevens-core_sevens__agentdb_session-end Bash
---
...
1. **Check health** — `mcp__plugin_sevens-core_sevens__agentdb_health`. Sanity-check `available: true`.
2. **Start session** — `mcp__plugin_sevens-core_sevens__agentdb_session-start` if not already active.
3. **Store knowledge** — `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` for structured tier-keyed data (tiers: `working|episodic|semantic`).
4. **Recall knowledge** — `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall` with a query.
5. **Search patterns** — `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` for learned patterns (ReasoningBank-routed).
6. **Synthesize context** — `mcp__plugin_sevens-core_sevens__agentdb_context-synthesize` to combine multiple memories.
7. **Build causal graph** — `mcp__plugin_sevens-core_sevens__agentdb_causal-edge` to link related knowledge.

## Available controller groups

Call `mcp__plugin_sevens-core_sevens__agentdb_controllers` to list the runtime registry. Functional categories surfaced via the 15 MCP tools:
