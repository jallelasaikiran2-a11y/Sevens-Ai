...
allowed-tools: mcp__plugin_sevens-core_sevens__embeddings_generate mcp__plugin_sevens-core_sevens__embeddings_search mcp__plugin_sevens-core_sevens__embeddings_compare mcp__plugin_sevens-core_sevens__embeddings_init mcp__plugin_sevens-core_sevens__embeddings_status mcp__plugin_sevens-core_sevens__embeddings_hyperbolic mcp__plugin_sevens-core_sevens__embeddings_neural mcp__plugin_sevens-core_sevens__embeddings_rabitq_build mcp__plugin_sevens-core_sevens__embeddings_rabitq_search mcp__plugin_sevens-core_sevens__embeddings_rabitq_status mcp__plugin_sevens-core_sevens__ruvllm_hnsw_create mcp__plugin_sevens-core_sevens__ruvllm_hnsw_add mcp__plugin_sevens-core_sevens__ruvllm_hnsw_route mcp__plugin_sevens-core_sevens__memory_search_unified Bash
---
...
1. **Check status** — `mcp__plugin_sevens-core_sevens__embeddings_status` to verify the embedding engine.
2. **Initialize** — `mcp__plugin_sevens-core_sevens__embeddings_init` if not active.
3. **Generate** — `mcp__plugin_sevens-core_sevens__embeddings_generate` for text input.
4. **Search** — `mcp__plugin_sevens-core_sevens__embeddings_search` with the query.
5. **Compare** — `mcp__plugin_sevens-core_sevens__embeddings_compare` to measure similarity.
6. **Unified search** — `mcp__plugin_sevens-core_sevens__memory_search_unified` for cross-namespace.
...
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_create` — create the WASM index (cap ~11)
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_add` — add a pattern
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_route` — route an incoming query

This is **not** a corpus index. Treat it as a fast classifier over a curated set of patterns.

## Hyperbolic embeddings

For hierarchical data (code trees, org charts), use `mcp__plugin_sevens-core_sevens__embeddings_hyperbolic` which maps to Poincare ball space. Distance is geodesic, not cosine.
