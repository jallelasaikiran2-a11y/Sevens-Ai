...
1. Call `mcp__plugin_sevens-core_sevens__agentdb_health` to check database health
2. Call `mcp__plugin_sevens-core_sevens__agentdb_controllers` to list whatever the runtime reports — controller `total` and `active` counts. The canonical list of names lives at `v3/@claude-flow/memory/src/controller-registry.ts:34-73` (`ControllerName` union); the runtime tool is the source of truth.
