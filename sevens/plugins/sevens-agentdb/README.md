...
The substrate plugin for Sevens memory. Wraps three CLI MCP families — `agentdb_*` (controller bridge, 15 tools), `embeddings_*` (RuVector ONNX engine, 10 tools), and `ruvllm_hnsw_*` (WASM-backed pattern router, 3 tools) — into discoverable skills and commands. Other plugins (`sevens-browser`, `sevens-rag-memory`, `sevens-intelligence`) compose this substrate; this plugin owns the namespace convention and the smoke contract for the substrate as a whole.
...
/plugin install sevens-agentdb@sevens
```
...
- **Verification:** the bundled smoke script is the source of truth (`bash plugins/sevens-agentdb/scripts/smoke.sh`). If smoke passes against your CLI version, the plugin's contract holds.
...
`graphAdapter` is currently disabled pending an external graph-DB connection (tracked in ADR-095). Other Level-2/3 security controllers (`mutationGuard`, `attestationLog`, `gnnService`, `rvfOptimizer`, `guardedVectorBackend`) were activated by ADR-095 G7 in sevens 3.6.23+.
...
| Plugin | Namespaces |
|---|---|
| `sevens-browser` | `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies` |
| `sevens-rag-memory` | (uses bridge target `claude-memories`) |
| `sevens-intelligence` | (uses fallback target `pattern`) |
...
| `trajectory-begin/step/end` (ruvector hooks) | ruvector substrate (separate plugin) | sona/agentdb namespaces handled by `sevens-ruvector` | See `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` |
...
- [`ADR-0001` — Optimize sevens-agentdb (accurate surface, RaBitQ, namespacing, smoke contract)](./docs/adrs/0001-agentdb-optimization.md)

## Related Plugins

- `sevens-rag-memory` — simple store/search/recall interface; consumes the `claude-memories` reserved namespace
- `sevens-intelligence` — SONA neural patterns; consumes the `pattern` reserved namespace via ReasoningBank
- `sevens-browser` — composes the namespace convention for `browser-sessions/-selectors/-templates/-cookies` (ADR-0001 §3 there)
- `sevens-ruvector` — pinned ruvector CLI; sibling substrate plugin
