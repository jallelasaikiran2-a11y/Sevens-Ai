---
id: ADR-0001
title: Optimize sevens-agentdb — accurate surface, quantization opt-ins, controller-managed namespacing, smoke-as-contract
status: Proposed
date: 2026-05-04
authors:
  - planner (Claude Code)
...
### Today's `sevens-agentdb`

`sevens-agentdb` is a thin documentation wrapper around three MCP tool families exposed by `@claude-flow/cli`: `agentdb_*` (controller bridge), `embeddings_*` (RuVector ONNX engine), and `ruvllm_hnsw_*` (WASM-backed pattern router). The plugin ships six files:

- `.claude-plugin/plugin.json:2` — `name: "sevens-agentdb"`, `version: "0.2.0"`, keywords `agentdb, ruvector, hnsw, embeddings, vector-search`.
...
3. **Namespacing convention.** Every consumer plugin (`sevens-browser` defines `browser-sessions / browser-selectors / browser-templates / browser-cookies` in its ADR-0001 §3; `sevens-rag-memory` references `claude-memories / patterns / tasks / solutions`; `sevens-intelligence` writes to `pattern`) reinvents namespace naming. There is no contract from `sevens-agentdb` about how namespaces should be named, what they should contain, or how they are GC'd. The `agentdb_*` tools mostly do not even take a namespace parameter — they route to controllers (`reasoningBank`, `hierarchicalMemory`, `causalGraph`) — but the CLI fallback `memory_store` and `embeddings_search` *do* take namespace strings, so the surface is mixed and undocumented.
...
`sevens-agentdb` is the substrate plugin — `sevens-browser` (ADR-0001) explicitly depends on AgentDB namespaces and controller-managed sessions; `sevens-rag-memory` and `sevens-intelligence` route through the same controllers; the whole "memory-as-substrate" story in `CLAUDE.md` rests on it. Documentation drift in this plugin is contagious — every downstream plugin inherits the wrong tool count, the wrong controller count, and the missing quantization story. We just fixed the same class of drift in `sevens-ruvector` (ADR-0001 there); the same pattern applies here.

## Decision

We propose six changes to `sevens-agentdb`. None requires modifying AgentDB itself; all are plugin-local edits plus one new smoke script.
...
Add a new section to `README.md` titled **"Namespace convention"** that sevens-agentdb owns and downstream plugins consume. The contract:

- **Naming**: `<plugin-stem>-<intent>` kebab-case. Examples: `browser-sessions`, `browser-selectors`, `browser-cookies` (sevens-browser), `claude-memories` (sevens-rag-memory's bridge), `pattern` (sevens-intelligence ReasoningBank fallback).
...
- **GC posture**: sevens-agentdb does not GC namespaces. Consumer plugins that want lifecycle (e.g., `browser-sessions` after `purge`) own their own deletion via `memory_delete` + `agentdb_consolidate`. Document this so consumers don't expect cleanup we don't provide.
...
- RaBitQ goes from invisible to a documented quantization workflow — downstream plugins handling large corpora (`sevens-rag-memory`, future `sevens-knowledge-graph` integrations) get a 32× memory-reduction story by reference, not by re-discovery.
- HNSW becomes tunable instead of a magic number. Consumer plugins choose an operating point.
- A namespace convention exists. `sevens-browser`'s ADR §3 already implements it ad hoc; this ADR formalizes it so the next plugin author doesn't re-derive it.
...
```bash
bash plugins/sevens-agentdb/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

Plus three documentation invariants checked by a one-line `grep` each:

- `! grep -rn "19 AgentDB controllers\|all 19 controllers\|19 Controllers" plugins/sevens-agentdb/` — no occurrence remains.
- `grep -q "embeddings_rabitq_build" plugins/sevens-agentdb/skills/vector-search/SKILL.md` — quantization workflow is documented.
- `grep -q "Namespace convention" plugins/sevens-agentdb/README.md` — namespace contract section exists.

## Related

- `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent (this ADR mirrors §5 and §6).
- `plugins/sevens-browser/docs/adrs/0001-browser-skills-architecture.md` — `browser-*` namespace family that motivated §4 (namespace convention contract).
