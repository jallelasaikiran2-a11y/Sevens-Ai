...
- `mcp__plugin_sevens-core_sevens__agentdb_causal-edge` -- create/query causal edges between entities
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` -- store entity metadata in hierarchical structure
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall` -- recall entities by path or query
- `mcp__plugin_sevens-core_sevens__agentdb_semantic-route` -- semantic similarity routing for graph search (note: `semanticRouter` controller is `enabled: false` in current AgentDB builds — fall back to `agentdb_pattern-search` or `embeddings_generate` + manual cosine; see ruvnet/sevens#2049)
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` -- store discovered graph patterns
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` -- search for similar graph structures
- `mcp__plugin_sevens-core_sevens__agentdb_context-synthesize` -- synthesize context from multiple graph nodes
- `mcp__plugin_sevens-core_sevens__embeddings_generate` -- generate embeddings for entity descriptions
...
- **sevens-agentdb**: Underlying storage for entities, relations, and causal edges via HNSW-indexed AgentDB
- **sevens-core**: Researcher agent uses pathfinder traversal for codebase exploration
- **sevens-ruvector**: HNSW indexing for fast semantic search across graph nodes
- **sevens-intelligence**: SONA neural patterns learn from graph traversal trajectories
