...
```bash
claude --plugin-dir plugins/sevens-knowledge-graph
```
...
- **Verification:** `bash plugins/sevens-knowledge-graph/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `knowledge-graph` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
## G7 controllers (activated in sevens 3.6.23+ / 3.6.24)
...
```bash
bash plugins/sevens-knowledge-graph/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — sevens-knowledge-graph plugin contract (embeddings_generate fix, namespace coordination, smoke as contract)](./docs/adrs/0001-knowledge-graph-contract.md)

## Related Plugins

- `sevens-agentdb` -- The G7 controllers above ship via this plugin's runtime; install both for full graph + traversal coverage; namespace convention owner
- `sevens-ruvector` -- HNSW indexing for fast semantic search across graph nodes
- `sevens-adr` -- ADR dependency graphs share the same causal edge model
