...
argument-hint: "<path>"
allowed-tools: Read Glob Grep mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store mcp__plugin_sevens-core_sevens__agentdb_causal-edge mcp__plugin_sevens-core_sevens__agentdb_pattern-store mcp__plugin_sevens-core_sevens__embeddings_generate Bash
---
...
3. **Map relations** -- for each entity, determine its relations to other entities. **Critical: TypeScript `import type` and inline `type` specifiers (`import { type Foo, bar }`) are erased at compile time and MUST NOT be counted as value imports** -- they're a separate, weaker relation. Misclassifying them produces phantom runtime cycles (see ruvnet/sevens#2049).
   ...
   **Regex hint for classifying TS imports** (so a naive `from '...'` grep doesn't conflate the two):
   ...
   ```
4. **Store in AgentDB** -- call `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` for each entity with metadata (name, type, file, line, description)
5. **Create edges** -- call `mcp__plugin_sevens-core_sevens__agentdb_causal-edge` for each relation with source, target, relation type, and weight
