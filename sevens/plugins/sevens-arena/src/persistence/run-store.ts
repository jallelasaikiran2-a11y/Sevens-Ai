// Run persistence. A competition run is a reproducible artifact (Sevens ADR-147; the local
// stand-in for RuVector ADR-197's payoff/fitness/evolution storage).
//
// Two backends ship here:
//   - FileRunStore     — writes full artifacts to `.sevens/arena/<runId>.json` (exact replay).
//   - InMemoryRunStore — for tests / ephemeral use.
// AgentDB persistence is handled at the COMMAND layer (Sevens convention: plugin TS persists
// artifacts; commands call `mcp__plugin_sevens-core_sevens__memory_store` with `agentdbRecord(run)`).
...
export class FileRunStore implements RunStore {
  constructor(private readonly baseDir = join(process.cwd(), '.sevens', 'arena')) {}
...
/**
 * Shape a run for AgentDB persistence via `mcp__plugin_sevens-core_sevens__memory_store`.
