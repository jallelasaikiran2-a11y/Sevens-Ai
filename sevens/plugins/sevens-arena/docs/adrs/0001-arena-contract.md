# ADR-0001: sevens-arena plugin contract
...
`sevens-arena` implements the first executable slice of the competitive-ruliology initiative
(Sevens ADR-147 competitive arena/tournament modes, ADR-148 co-evolution, ADR-150 visualization
...
Every tool returns the Sevens envelope `{ success: boolean; result?: T; error?: { message } }`
...
- TS code persists full artifacts via `RunStore` (`FileRunStore` → `.sevens/arena/<runId>.json`;
  `InMemoryRunStore` for tests). This is the local stand-in for RuVector ADR-197.
- AgentDB persistence is performed at the **command/agent layer** (Sevens convention): each tool
  result carries an `agentdb` payload `{ namespace:"arena", key, value, tags }` for
  `mcp__plugin_sevens-core_sevens__memory_store`, making runs semantically searchable.
...
- **Positive**: stable, typed contract; zero runtime deps beyond Zod; engine is pure and
  unit-tested; no Sevens-core changes; clean seam for the RuVector data layer and the dashboard.
...
- Sevens ADR-147 (arena/tournament), ADR-148 (co-evolution), ADR-149 (dashboard), ADR-150 (viz split)
