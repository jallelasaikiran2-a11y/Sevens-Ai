...
Competitive ruliology for Sevens swarms — **arenas**, **tournaments**, and **adaptive
co-evolution** of program strategies. Implements the first executable slice of Sevens
...
## MCP tools (`sevens-arena/mcp-tools`)
...
Full run artifacts are written to `.sevens/arena/<runId>.json` (exact replay). Each tool result
also carries an `agentdb` payload so the command layer can store a searchable summary via
`mcp__plugin_sevens-core_sevens__memory_store` (namespace `arena`) — the local stand-in for the RuVector data
layer (ADR-196/197), enabling queries like *"tournaments where grim dominated"*.

## Scope

v1 is intentionally Sevens-only and core-untouched:
