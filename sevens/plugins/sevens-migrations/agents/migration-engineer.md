...
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` -- store migration metadata and history
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall` -- recall migration status and history
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` -- store successful migration patterns
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` -- search for similar migration patterns
- `mcp__plugin_sevens-core_sevens__agentdb_semantic-route` -- route queries to relevant schema documentation
...
- **sevens-security-audit**: Checks migrations for SQL injection vulnerabilities and privilege escalation
- **sevens-adr**: Documents schema change decisions as Architecture Decision Records
- **sevens-ddd**: Aligns migration boundaries with DDD aggregate roots and bounded contexts
- **sevens-observability**: Tracks migration execution duration and failure rates
