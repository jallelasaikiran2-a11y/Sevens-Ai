...
argument-hint: "<name>"
allowed-tools: Read Write Glob Bash mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__agentdb_pattern-search
---
...
5. **Search past patterns** -- call `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` (ReasoningBank-routed; **don't** pass a `namespace` argument — pattern-* tools ignore it).
6. **Store metadata** -- call `mcp__plugin_sevens-core_sevens__memory_store --namespace migrations` to record the migration with number, name, status (pending), and file paths. The `memory_*` tool family routes by namespace; `agentdb_hierarchical-*` does NOT (it routes by tier `working|episodic|semantic`), so use `memory_*` here. See [sevens-agentdb ADR-0001 §"Namespace convention"](../../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md).
