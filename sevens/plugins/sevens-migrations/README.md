...
```bash
claude --plugin-dir plugins/sevens-migrations
```
...
- **Verification:** `bash plugins/sevens-migrations/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `migrations` AgentDB namespace (kebab-case is implicit when the plugin name is the intent — same documented exception as `federation`). Follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-migrations plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-migrations-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `sevens-adr` -- Document schema change decisions as ADRs
- `sevens-ddd` -- Align migration boundaries with aggregate roots
- `sevens-observability` -- Track migration execution duration and failure rates
