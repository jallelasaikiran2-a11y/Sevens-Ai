...
allowed-tools: mcp__plugin_sevens-core_sevens__browser_open mcp__plugin_sevens-core_sevens__browser_close mcp__plugin_sevens-core_sevens__browser_session-list mcp__plugin_sevens-core_sevens__browser_screenshot mcp__plugin_sevens-core_sevens__browser_snapshot mcp__plugin_sevens-core_sevens__browser_wait mcp__plugin_sevens-core_sevens__aidefence_has_pii mcp__plugin_sevens-core_sevens__aidefence_scan Bash Read Write
---
...
1. **Allocate session id and RVF container**:
   ...
   ```
2. **Open the browser** via `mcp__plugin_sevens-core_sevens__browser_open` with the URL.
...
- Until the `browser_session_record` MCP tool ships (ADR-0001 §7), this skill drives the lifecycle from inside its own bash steps. Do not call `mcp__plugin_sevens-core_sevens__browser_open` directly without these wrappers.
- The session id format is fixed: `<YYYYMMDD-HHMMSS>-<task-slug>`. Downstream `/sevens-browser ls` parses this.
