---
id: ADR-0001
title: Adopt a session-as-skill architecture for sevens-browser, backed by RVF, ruvector trajectories, and AgentDB
status: Proposed
date: 2026-05-04
authors:
  - planner (Claude Code)
...
### Today's `sevens-browser`

The current plugin (v0.1.0) is a thin wrapper around 23 Playwright-backed MCP tools (`mcp__plugin_sevens-core_sevens__browser_*`). Surface inventory:
...
- `commands/sevens-browser.md` — one slash command that lists/screenshots/closes sessions via `browser_session-list`
...
What Browserbase does *not* offer that we need: persistent semantic memory of selectors and page structures, PII/prompt-injection scanning of scraped content, federated session sharing, or learning-from-trajectory beyond the file-based `strategy.md` loop. Those are exactly the gaps Sevens's existing subsystems already fill.

### Why act now

`sevens-browser` is the most-used surface for any agent that needs the open web, and it is the weakest at translating a one-off run into reusable knowledge. Adopting the Browserbase architectural shape — primitive + named sessions + traces + outer/inner loop — while wiring each artifact into Sevens's own substrates (RVF, ruvector trajectories, AgentDB, AIDefence) gives us replayability, learning, and safety without leaving Playwright behind and without depending on Browserbase's hosted backend.

## Decision

We propose to refactor `sevens-browser` around a **session-as-skill** architecture. Each browser session is a first-class, replayable, auditable RVF container with a recorded ruvector trajectory; skills are thin compositions over a stable Playwright primitive; the marketplace shape mirrors Browserbase's so the plugin can be split later if the surface grows.

### 1. Session model — RVF as the session container

Every browser session opened by sevens-browser is allocated an RVF container at session start (`rvf create --dimension 384 <session-name>.rvf`) and committed at session end (`rvf compact && rvf export`). The container holds:
...
|-----------|-----|-------|---------|
| `browser-sessions` | `<rvf-id>` | manifest summary + verdict + tags | session index for `/sevens-browser ls --query "logged into stripe"` |
...
Borrowed pattern: the *idea* of a deliberate sync boundary from `cookie-sync/SKILL.md`. Browserbase has no PII/injection layer; this is a Sevens-native addition.
...
### 6. Commands — `/sevens-browser` as a verb dispatcher

The single `commands/sevens-browser.md` is rewritten to dispatch by subcommand:

```
/sevens-browser ls                      # list sessions (RVF-indexed) with filters
/sevens-browser show <session-id>       # show manifest + trajectory + verdict
/sevens-browser replay <session-id>     # invoke browser-replay
/sevens-browser export <session-id>     # rvf export → tar.zst, optionally federated
/sevens-browser fork <session-id>       # rvf derive → new session with shared lineage
/sevens-browser purge <session-id>      # destroy session, keep redacted manifest
/sevens-browser doctor                  # check Playwright, MCP, AgentDB, AIDefence wiring
```
...
- `commands/sevens-browser.md` is replaced with the verb dispatcher in §6. Old behavior (list + screenshot + close) maps to `ls` + `show` + `purge`.
- `.claude-plugin/plugin.json` bumps to `0.2.0` with new keywords (`rvf`, `replay`, `trajectory`).
- A `marketplace.json` mirror is **not** introduced yet — sevens's plugin marketplace is centralized at the repo root. We will revisit only if the plugin grows beyond ~8 skills (Browserbase's split happened at 10).

### 9. Pinning and contract

Following the precedent of ADR-0001 in `sevens-ruvector`:
...
- AgentDB and AIDefence become hard dependencies of `sevens-browser`. Today the plugin runs against a stock `claude-flow` install; under this proposal it requires the AgentDB controllers and AIDefence to be initialized. This must be enforced by `sevens-browser doctor` and by `init-project` updates.
...
7. **Doctor:** `/sevens-browser doctor` returns non-zero on each of: missing AgentDB controller, AIDefence not initialized, ruvector CLI not pinned, Playwright version drift.

A `scripts/smoke.sh` materializes these as 7 numbered tests; the contract is "7 passed, 0 failed".

## Related

- `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning precedent and smoke-test-as-contract pattern.
...
- `https://github.com/browserbase/skills/blob/main/.claude-plugin/marketplace.json` — multi-plugin marketplace shape (deferred for sevens-browser).
- Playwright (`https://playwright.dev/`) — the underlying runner; pinning precedent applies.
- Sevens subsystems referenced: AgentDB controllers, RVF (`rvf` CLI), ruvector hooks (`trajectory-*`, `pattern-*`), AIDefence (`aidefence_has_pii`, `aidefence_is_safe`, `aidefence_scan`), Federation (zero-trust cross-installation sharing).
