...
Usage: /sevens-browser <verb> [args]
...
1. **ls [--query "<text>"] [--host <host>] [--verdict pass|fail|partial]**
   ...
   ```
   For active (live) sessions, also call `mcp__plugin_sevens-core_sevens__browser_session-list`.
...
4. **export `<session-id>` [-o <path>] [--federate]**
   ...
   ```
   With `--federate`, also push via the `sevens-federation` plugin.
...
7. **doctor**
   ...
   - Playwright runner reachable (`mcp__plugin_sevens-core_sevens__browser_open` against `about:blank`)
   - AgentDB controllers initialized (`mcp__plugin_sevens-core_sevens__agentdb_health`)
   - AIDefence loaded (`mcp__plugin_sevens-core_sevens__aidefence_stats`)
