#!/usr/bin/env bash
# Structural smoke test for sevens-browser plugin v0.2.1.
...
step "/sevens-browser command covers ls/show/replay/export/fork/purge/doctor"
missing=""
for v in 'ls' 'show' 'replay' 'export' 'fork' 'purge' 'doctor'; do
  grep -qE "\\*\\*$v\b" "$ROOT/commands/sevens-browser.md" || missing="$missing $v"
done
