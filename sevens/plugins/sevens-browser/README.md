...
Session-as-skill browser automation. Playwright-backed via 23 `mcp__plugin_sevens-core_sevens__browser_*` tools, with each session captured as a first-class **RVF cognitive container** holding manifest + trajectory + screenshots + sanitized cookies + findings, indexed in AgentDB and gated by AIDefence.
...
`/sevens-browser` is a verb dispatcher:

```bash
/sevens-browser ls [--query <text>]      # list sessions, AgentDB-indexed
/sevens-browser show <session-id>        # manifest + trajectory + verdict
/sevens-browser replay <session-id>      # re-drive trajectory
/sevens-browser export <session-id>      # rvf export → tar.zst
/sevens-browser fork <session-id>        # rvf derive → new lineage-tracked session
/sevens-browser purge <session-id>       # destroy, keep redacted manifest
/sevens-browser doctor                   # check Playwright, MCP, AgentDB, AIDefence
```
...
|-----------|-----|-------|---------|
| `browser-sessions` | `<rvf-id>` | manifest summary + verdict + tags | session index for `/sevens-browser ls` |
...
```bash
bash plugins/sevens-browser/scripts/smoke.sh
...
```bash
bash plugins/sevens-browser/scripts/replay-spike.sh
```
...
- [`ADR-0001` — Adopt session-as-skill architecture for sevens-browser](./docs/adrs/0001-browser-skills-architecture.md)

## Related Plugins

- `sevens-ruvector` — trajectory hooks, SONA pattern distillation, MCP tools
- `sevens-agentdb` — controllers backing `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies`
- `sevens-aidefence` — PII / prompt-injection gates
- `sevens-federation` — cross-installation session sharing via RVF export
