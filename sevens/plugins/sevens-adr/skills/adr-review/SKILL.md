...
allowed-tools: mcp__plugin_sevens-core_sevens__agentdb_hierarchical-query mcp__plugin_sevens-core_sevens__agentdb_causal-query mcp__plugin_sevens-core_sevens__memory_search Bash Read Grep Glob
---
...
2. **Find relevant ADRs** -- For each changed file:
   ...
   - Call `mcp__plugin_sevens-core_sevens__memory_search` with the file path and change summary to find semantically related ADRs
...
5. **Query relationship graph** -- Call `mcp__plugin_sevens-core_sevens__agentdb_causal-query` to check if any referenced ADRs have been superseded. If so, flag that the code references an outdated decision.
