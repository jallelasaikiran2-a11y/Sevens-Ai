...
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` / `agentdb_hierarchical-query` — ADR tree storage.
- `mcp__plugin_sevens-core_sevens__agentdb_causal-edge` / `agentdb_causal-query` — relationship tracking.
- `mcp__plugin_sevens-core_sevens__memory_store` / `memory_search` — semantic search.
...
- **sevens-jujutsu**: Use diff analysis on PRs to check ADR compliance before merge.
- **sevens-docs**: Trigger doc generation when ADRs change status.
