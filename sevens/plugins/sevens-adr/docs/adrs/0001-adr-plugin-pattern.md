---
id: ADR-0001
title: sevens-adr plugin contract — pinning, namespace coordination, smoke as contract, REFERENCE.md token-optimization pattern
status: Proposed
date: 2026-05-04
authors:
...
`sevens-adr` is the meta-plugin that manages ADR lifecycle for projects (this one included — every other plugin in this repo is documenting itself with an ADR-0001 by the very contract this plugin proposes). The plugin already shipped four useful pieces:
...
Gaps observed against the pattern established by `sevens-ruvector` ADR-0001, `sevens-agentdb` ADR-0001, `sevens-browser` ADR-0001, `sevens-intelligence` ADR-0001:
...
3. **Free-form namespace usage.** The agent + skills write to `adr-patterns` (kebab-case, follows the convention) but never cite the convention from `sevens-agentdb` ADR-0001.
...
- **Namespace coordination** — explicit deferral to `sevens-agentdb` ADR-0001 §"Namespace convention". `adr-patterns` is the canonical namespace this plugin owns; the reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
7. README has Namespace coordination section deferring to `sevens-agentdb` ADR-0001.
...
```bash
bash plugins/sevens-adr/scripts/smoke.sh
...
- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention this plugin defers to
- `plugins/sevens-browser/docs/adrs/0001-browser-skills-architecture.md` — session-as-skill precedent
- `plugins/sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — surface-completeness precedent
- `plugins/sevens-adr/REFERENCE.md` — heavy reference data (template, AgentDB paths) per ADR-098 Part 2 token-diet
