...
3. Store in AgentDB: `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` at path `adr/ADR-NNN`
...
3. Create causal edge: `mcp__plugin_sevens-core_sevens__agentdb_causal-edge` from old to new with relation "supersedes"
...
1. Query `mcp__plugin_sevens-core_sevens__agentdb_causal-query` for all ADR relationships
...
1. Call `mcp__plugin_sevens-core_sevens__memory_search` with the query in namespace `adr-patterns`
