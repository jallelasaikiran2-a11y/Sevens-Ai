...
1. Call `mcp__plugin_sevens-core_sevens__daa_learning_status` with `detailed: true` to get the swarm summary (total / active / learning / avg success rate / total adaptations) plus per-agent records (id, status, cognitive pattern, success rate, adaptation count)
2. Call `mcp__plugin_sevens-core_sevens__daa_performance_metrics` for efficiency and accuracy stats
