...
**init**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation init`
**join**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation join ENDPOINT`
**leave**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation leave`
**peers**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation peers`
**send**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation send NODE_ID MSG_TYPE PAYLOAD [--max-hops N] [--max-tokens N] [--max-usd N] [--hop-count N] [--spent-tokens N] [--spent-usd N]`
**status**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation status`
**audit**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation audit --compliance MODE --since DATE`
**trust**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation trust NODE_ID --review`
**config**: `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation config --pii-policy PATH`
