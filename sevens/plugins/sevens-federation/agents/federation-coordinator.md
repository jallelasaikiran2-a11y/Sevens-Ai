...
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation init` -- generate keypair, create config
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation join <endpoint>` -- connect to peer
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation peers` -- list peers with trust levels
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation status` -- health dashboard
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation audit --compliance hipaa` -- audit logs
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation trust <node-id> --review` -- trust breakdown
- `npx -y -p @claude-flow/plugin-agent-federation@latest sevens-federation send <node-id> <msg-type> <payload> [--max-hops N] [--max-tokens N] [--max-usd N]` -- delegate with budget guardrails
