...
allowed-tools: mcp__plugin_sevens-core_sevens__transfer_plugin-info mcp__plugin_sevens-core_sevens__transfer_plugin-search mcp__plugin_sevens-core_sevens__transfer_store-search Bash Read Write Edit
---
...
2. **Check for conflicts** — call `mcp__plugin_sevens-core_sevens__transfer_plugin-search` to ensure the name isn't taken
...
5. **Generate SKILL.md files** with proper frontmatter:
   ...
   allowed-tools: mcp__plugin_sevens-core_sevens__tool1 mcp__plugin_sevens-core_sevens__tool2 Bash
   ---
   ```
...
8. **Generate README.md** with install instructions, features, commands, skills, AND the canonical plugin-contract sections:
   ...
   - **Namespace coordination** — claim a kebab-case `<plugin-stem>-<intent>` namespace; defer to sevens-agentdb ADR-0001 §"Namespace convention"
...
11. **Update marketplace.json** if adding to the sevens marketplace.
...
Browse available tools: `mcp__plugin_sevens-core_sevens__transfer_plugin-info`
