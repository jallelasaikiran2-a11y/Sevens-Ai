---
id: ADR-0001
title: sevens-plugin-creator plugin contract — pinning, scaffold-the-canonical-contract, MCP-drift warnings, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-plugin-creator` (v0.1.0) — meta-plugin that scaffolds new Claude Code plugins. 1 agent + 2 skills + 1 command.
...
The `create-plugin` skill mentioned "19 AgentDB controllers" — a stale count that [sevens-agentdb ADR-0001 §"Today's `sevens-agentdb`"](../../../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md) declared mythical (real: 15 `agentdb_*` MCP tools, 29 `ControllerName` entries). Fixed in this pass.
...
```bash
bash plugins/sevens-plugin-creator/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Related

- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + "19 controllers" myth fix
- `plugins/sevens-cost-tracker/docs/adrs/0001-cost-tracker-contract.md` — namespace-routing bug class
- `plugins/sevens-knowledge-graph/docs/adrs/0001-knowledge-graph-contract.md` — `embeddings_embed` rename
- `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/sevens-plugin-creator/`. Contract elements implemented: scaffolding templates updated to include contract ADR, smoke test, Compatibility section, and namespace coordination block by default; MCP-drift warnings added to scaffolded skill templates; smoke-as-contract gate defined in `scripts/smoke.sh`.
