---
id: ADR-0001
title: sevens-aidefence plugin contract — pinning, namespace coordination, 3-gate pattern, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-aidefence` documents the AIDefence MCP family (`aidefence_scan`, `_analyze`, `_stats`, `_learn`, `_is_safe`, `_has_pii`) — 6 tools at `v3/@claude-flow/cli/src/mcp-tools/security-tools.ts:108, 191, 277, 329, 424, 479`. Plugin is at v0.2.0 with full surface coverage and a "Defence-in-depth pairing" block already documenting the host-level hardening (loader-hijack denylist, file mode 0600, encryption-at-rest opt-in).

Gaps observed against the pattern from `sevens-ruvector` / `sevens-agentdb` / `sevens-browser` / `sevens-intelligence` / `sevens-adr` ADRs:

1. **No plugin-level ADR.** Every other plugin updated this session has one.
2. **No smoke test.**
3. **Free-form `security-patterns` namespace.** The agent writes there without referencing `sevens-agentdb` ADR-0001's namespace convention.
4. **No Compatibility section.**
5. **The sevens-browser ADR-0001 §4** mandates three AIDefence gates (PII pre-storage, cookie sanitization, prompt-injection check on returned content) — `sevens-aidefence` should canonicalize this 3-gate pattern so other plugins consume it the same way.
...
- **Namespace coordination** — `security-patterns` as the canonical namespace this plugin owns; defer to `sevens-agentdb` ADR-0001 §"Namespace convention".
- **The 3-gate pattern** — formalize the gates `sevens-browser` ADR-0001 §4 already uses. Three gates, every consumer plugin handling untrusted content should apply them in this order:
...
5. README defers to `sevens-agentdb` ADR-0001 namespace convention.
...
**Negative:**
- One downstream plugin (`sevens-browser`) embeds the 3-gate pattern in its own ADR §4. Updating it to defer here is a separate, mechanical task (the gates remain identical; only the canonical home changes).
...
```bash
bash plugins/sevens-aidefence/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Related

- `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md`
- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention
- `plugins/sevens-browser/docs/adrs/0001-browser-skills-architecture.md` — §4 codifies the 3-gate pattern this ADR canonicalizes
- `plugins/sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md`
- `plugins/sevens-adr/docs/adrs/0001-adr-plugin-pattern.md`
- `v3/@claude-flow/cli/src/mcp-tools/security-tools.ts` — 6 `aidefence_*` tool definitions

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/sevens-aidefence/`. Contract elements implemented: 3-gate pattern (PII pre-storage gate via `aidefence_has_pii`, sanitization gate via `aidefence_scan`, prompt-injection gate via `aidefence_is_safe`); ADR-097 budget integration deferred (Phase 3); smoke-as-contract gate defined in `scripts/smoke.sh`.
