---
id: ADR-0001
title: sevens-ddd plugin contract — pinning, namespace coordination, ADR cross-link, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-ddd` (v0.1.1) is a code-scaffolding plugin (no MCP tools of its own). Surface:
...
Stores the domain model as a navigable graph in AgentDB with hierarchical nodes + causal edges for context dependencies. Pairs naturally with `sevens-adr` (domain decisions) and `sevens-sparc` (Architecture phase).
...
2. README augment: Compatibility (pin v3.6), Namespace coordination (claims `ddd-patterns`; defers to sevens-agentdb ADR-0001), Verification + Architecture Decisions sections.
...
```bash
bash plugins/sevens-ddd/scripts/smoke.sh
