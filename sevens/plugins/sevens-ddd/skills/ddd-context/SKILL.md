...
allowed-tools: Bash Read Write Edit Grep Glob mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store
---
...
4. **Store in domain model graph**:
   ```
   mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store --parent "domain" --child "context:<context-name>" --relation "contains"
   mcp__plugin_sevens-core_sevens__memory_store --key "ddd-context-<context-name>" --value "Created bounded context" --namespace tasks
   ```
