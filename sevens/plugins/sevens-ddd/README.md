...
```bash
claude --plugin-dir plugins/sevens-ddd
```
...
- **Verification:** `bash plugins/sevens-ddd/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `ddd-patterns` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-ddd plugin contract](./docs/adrs/0001-ddd-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner; backing store for the domain graph
- `sevens-adr` -- Document domain decisions as Architecture Decision Records
- `sevens-sparc` -- Architecture phase leverages DDD bounded context patterns
- `sevens-migrations` -- Align migration boundaries with aggregate roots
