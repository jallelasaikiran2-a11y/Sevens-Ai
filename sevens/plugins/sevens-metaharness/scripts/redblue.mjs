...
function printHelp() {
  console.log('npx sevens metaharness redblue <subcommand> [options]');
  console.log('');
...
function main() {
  ...
  if ((ARGS.sub === 'run' || ARGS.sub === 'patch') && !ARGS.out) {
    const dir = mkdtempSync(join(tmpdir(), 'sevens-redblue-'));
  ...
  if (ARGS.format === 'json') {
    if (enriched !== null) {
      ...
      console.log(JSON.stringify({
        ...
        durationMs: r.durationMs,
        stdout: r.stdout,
        _sevens: { safetyBoundary: 'enforced-by-upstream-redblue' },
      }, null, 2));
    }
  } else {
