...
//   1. oia-audit --dry-run --path .         (real run against sevens)
...
const REPO_ROOT = dirname(dirname(dirname(SCRIPTS_DIR))); // up out of plugins/sevens-metaharness/scripts
...
try {
  // ──────────────────────────────────────────────────────────────
  // STAGE 1: oia-audit --dry-run against sevens repo itself
  // ──────────────────────────────────────────────────────────────
  console.log('Stage 1 — oia-audit --dry-run against sevens repo');
  ...
  // (real sevens audit has 1 INFO finding → introduced=1)
  ...
  assert(stage12.alert?.triggered === true,
    'Stage 12: --alert-on-new-severity info triggers on sevens current INFO finding');
