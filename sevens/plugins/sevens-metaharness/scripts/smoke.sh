#!/usr/bin/env bash
# Structural smoke test for sevens-metaharness v0.1.1 (ADR-150 Phase 1).
set -u
...
if [[ "$v" != "0.1.1" ]]; then
  ...
  miss=""
  for k in sevens metaharness harness scorecard genome mcp-scan threat-model router adr-150 adr-148 adr-149 optional-dependency graceful-degradation subprocess phase-1-mvp; do
...
F="$ROOT/commands/sevens-metaharness.md"
miss=""
...
step "16. sevens wrapper keeps metaharness out of hard dependencies (ADR-150 constraint #2)"
F="$ROOT/../../sevens/package.json"
...
# sevens to depend on metaharness at all. Historically this smoke asserted
# the presence of metaharness in sevens optionalDeps, which conflicted with
# the #2561 "guard" budget (SEVENS_MAX=0 for optionalDependencies) that was
# added to protect the cold `npx -y sevens@alpha --version` startup path
# from a heavy optional-dep install. #2561 is stronger evidence: metaharness
# is on its FORBIDDEN list because installing it during sevens's postinstall
# caused a measured `npx --version` timeout. Correct reading of ADR-150 #2:
# if metaharness IS present anywhere in sevens's deps, it MUST be optional;
# absence is fine (sevens delegates to @claude-flow/cli, which itself keeps
...
" 2>/dev/null && ok || bad "metaharness leaked into sevens wrapper hard dependencies"
...
EXPECTED_JOBS=(
  "score"            # iter 1 — score harness against sevens
...
#   - `npm install sevens` → uses sevens/package.json
...
SEVENSPJ="$ROOT/../../sevens/package.json"
...
for pkg in "metaharness" "@metaharness/router" "@metaharness/kernel" "@metaharness/redblue"; do
  ...
  SEVENS_PIN=$(node -e "console.log(JSON.parse(require('fs').readFileSync('$SEVENSPJ')).optionalDependencies?.['$pkg'] || '')" 2>/dev/null)
  ...
  if [[ -z "$ROOT_PIN" && -z "$SEVENS_PIN" && -z "$CLI_PIN" ]]; then
    continue
  fi
  # Compare non-empty pins — they must all match.
  PINS=$(echo "$ROOT_PIN $SEVENS_PIN $CLI_PIN" | tr ' ' '\n' | grep -v '^$' | sort -u | wc -l | tr -d ' ')
  if [[ "$PINS" != "1" ]]; then
    miss="$miss ${pkg}-pin-drift:root=${ROOT_PIN},sevens=${SEVENS_PIN},cli=${CLI_PIN}"
  fi
done
[[ -z "$miss" ]] && ok || bad "$miss"
...
#   plugins/sevens-metaharness/skills/harness-foo/SKILL.md
...
# SUBCOMMANDS map should appear as `npx sevens metaharness <X>` for
# discoverability.
DISP="$ROOT/../../v3/@claude-flow/cli/src/commands/metaharness.ts"
CMD="$ROOT/../../CLAUDE.md"
...
for k in $KEYS; do
  COUNT=$((COUNT + 1))
  grep -qE "npx sevens metaharness ${k}([ \\\\]|$)" "$CMD" 2>/dev/null \
...
# but forgets the map (or vice-versa), `sevens metaharness X` errors at
...
# Runtime: tripwire passes on sevens's own audit
node "$F" >/dev/null 2>&1 || miss="$miss tripwire-fails-locally"
[[ -z "$miss" ]] && ok || bad "$miss"
...
# (real sevens audit has 1 INFO finding; baseline file with no findings → introduced=1)
BC=$(mktemp)
...
# as a sevens skill or MCP tool. It clones arbitrary git URLs which is too
# powerful to expose to agents. (Upstream supports it; sevens deliberately
...
# static import of @metaharness/* breaks the "sevens works without the dep"
...
grep -q "sevens metaharness drift-from-history" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
step "17z8. sevens doctor checks the integration layer, not just upstream (iter 45)"
miss=""
DOC="$ROOT/../../v3/@claude-flow/cli/src/commands/doctor.ts"
...
grep -q "sevens metaharness similarity" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
grep -q "Sevens remains operational if every MetaHarness package is removed" "$F" || miss="$miss no-constraint-quote"
# All 4 rules documented
grep -q "no-metaharness-smoke.yml" "$F" || miss="$miss no-ci-gate-ref"
# Command surface + tool surface enumerated
grep -q "npx sevens metaharness score" "$F" || miss="$miss no-cli-example"
# audit-allow: standalone-mcp-prefix — this checks the repository CLAUDE.md, not a plugin skill.
...
grep -q "sevens metaharness oia-audit" "$HOOKS" 2>/dev/null || miss="$miss hooks-tip-missing"
[[ -z "$miss" ]] && ok || bad "$miss"
