...
async function main() {
  ...
  for (const tool of tools) {
    ...
    const isChainTool = tool.name === 'metaharness_drift_from_history'
      ...
      // gepa: one-time `npm install --prefix ~/.sevens/darwin-cache-*`
  ...
  if (scanTool) {
    // Run against sevens itself — guaranteed to produce at least the
  ...
  if (trendTool) {
    ...
    if (driftTool) {
      ...
      // through. baselineFile has no findings; current sevens audit has
