...
//     a temp dir under /tmp/sevens-mint-<timestamp>/ if not given — never the
//     project root (sevens behavioral rule "never save to root folder").
...
//     confirmation" pattern in sevens's CLAUDE.md.
//
// USAGE
...
function safetyChecks() {
  ...
  if (!ARGS.target) {
    ARGS.target = resolve(tmpdir(), `sevens-mint-${Date.now()}-${ARGS.name}`);
  }
