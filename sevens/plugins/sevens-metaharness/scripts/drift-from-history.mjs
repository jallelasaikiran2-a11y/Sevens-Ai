...
//   $ npx sevens metaharness audit-list --format json
//   $ # ... pick a key by hand from the listing
//   $ npx sevens metaharness oia-audit --format json > /tmp/curr.json
//   $ npx sevens metaharness audit-trend --baseline-key X --current-file /tmp/curr.json
// Now it's:
...
async function main() {
  ...
  if (records.length === 0) {
    ...
    emitAndExit({
      ...
      hint: 'Run `sevens metaharness oia-audit` at least once to seed history.',
    }, 2);
  }
