...
//     layer — defense in depth at the sevens boundary.
//   - Default --generations 3 --children 3 (small) — anything larger is opt-in.
//     Real evolutions are minutes-to-hours; sevens's default sandbox config
...
function safetyChecks() {
  ...
  if (ARGS.generations < 1 || ARGS.generations > 50) {
    console.error('evolve: --generations must be 1..50 (sevens cap; upstream supports more)');
    process.exit(2);
  }
  if (ARGS.children < 1 || ARGS.children > 20) {
    console.error('evolve: --children must be 1..20 (sevens cap)');
    process.exit(2);
  }
  if (ARGS.concurrency < 1 || ARGS.concurrency > 8) {
    console.error('evolve: --concurrency must be 1..8 (sevens cap)');
    process.exit(2);
  }
...
async function main() {
  ...
  // error the sevens layer should remap.
  if (r.exitCode === 99) {
    const payload = {
      success: false,
