...
//
// CONNECTION TO SEVENS ADR-155
// ===========================
// sevens's ADR-155 (#2417) proposes a nightly self-learning security harness
...
// gives us the empirical baseline that sevens's Phase 2 loop A needs before
...
function safetyChecks() {
  if (ARGS.population < 1 || ARGS.population > 20) {
    console.error('security-bench: --population must be 1..20 (sevens cap)');
    process.exit(2);
  }
  if (ARGS.cycles < 1 || ARGS.cycles > 100) {
    console.error('security-bench: --cycles must be 1..100 (sevens cap)');
    process.exit(2);
  }
}
