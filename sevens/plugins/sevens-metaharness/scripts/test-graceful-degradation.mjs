...
// ~/.sevens/<pkg>-cache-<pin> or a locally installed metaharness would satisfy
...
const EMPTY_CACHE_BASE = mkdtempSync(join(tmpdir(), 'sevens-degradation-drill-'));
...
function runWithUnreachableRegistry(scriptName, extraArgs) {
  const r = spawnSync('node', [join(SCRIPTS_DIR, scriptName), ...extraArgs], {
    env: {
      ...
      NPM_CONFIG_REGISTRY: UNREACHABLE_REGISTRY,
      SEVENS_METAHARNESS_CACHE_BASE: EMPTY_CACHE_BASE,
      SEVENS_METAHARNESS_SKIP_LOCAL: '1',
    },
