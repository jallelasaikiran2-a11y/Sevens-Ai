...
//
// Workaround: install the package once into a sevens-owned cache dir
// (~/.sevens/redblue-cache-<pin>, via _invoke.ensureCachedInstall) and invoke
...
// @claude-flow/cli/package.json + sevens/package.json.
...
export const REDBLUE_CACHE_DIR = join(
  homedir(), '.sevens', `redblue-cache-${REDBLUE_PIN_VERSION.replace(/[~^]/g, '')}`,
);
