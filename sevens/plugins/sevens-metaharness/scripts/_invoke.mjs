...
//   - ensureCachedInstall()   one-time `npm install --prefix ~/.sevens/<name>-cache-<pin>`
...
//   - SEVENS_METAHARNESS_CACHE_BASE   overrides ~/.sevens as the cache root
//   - SEVENS_METAHARNESS_SKIP_LOCAL=1 disables local node_modules resolution
...
// npx shim means "upstream unavailable", never a sevens bug.
...
/** Cache root — ~/.sevens unless the test seam overrides it. */
export function cacheBaseDir() {
  return process.env.SEVENS_METAHARNESS_CACHE_BASE || join(homedir(), '.sevens');
}
 ...
 * user's sevens install shipped with). Returns the package dir or null.
...
export function findLocalPackageDir(pkg, pinVersion) {
  if (process.env.SEVENS_METAHARNESS_SKIP_LOCAL === '1') return null;
 ...
 * rule #3: sevens continues to function when MetaHarness is absent.
 */
