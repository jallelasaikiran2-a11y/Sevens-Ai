...
1. Invoke the pinned `harness` binary (`metaharness@~0.3.0`, resolved from a
   local install or the one-time `~/.sevens/metaharness-cache-<pin>` cache —
...
- name: MCP static scan
  run: node plugins/sevens-metaharness/scripts/mcp-scan.mjs --fail-on high
```
...
and exits 0. Sevens continues — ADR-150 architectural constraint.
