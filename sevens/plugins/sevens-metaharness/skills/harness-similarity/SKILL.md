...
```bash
# File inputs
npx sevens metaharness similarity --a a.json --b b.json

# Memory inputs (records persisted by oia-audit.mjs)
npx sevens metaharness similarity --a-key harness-X --b-key harness-Y

# Per-dimension breakdown (used by ADR-151 §3.2 Recommender)
npx sevens metaharness similarity --a a.json --b b.json --per-dimension

# Alert when too-dissimilar (used by ADR-151 §3.3 Drift Detection)
npx sevens metaharness similarity --a a.json --b b.json --alert-below 0.5
```
...
MCP tool: `mcp__plugin_sevens-core_sevens__metaharness_similarity` (registered in `v3/@claude-flow/cli/src/mcp-tools/metaharness-tools.ts`)
