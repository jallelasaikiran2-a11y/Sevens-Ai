...
Surfaces the upstream `metaharness score` CLI as a sevens skill. Use when
Claude Code needs to assess whether a repo is ready for harness adoption
before recommending the user run `npx sevens init` or `harness-mint`.
...
## Phase-0 baseline (sevens's own scorecard, measured 2026-06-16)
...
Sevens passes its own readiness check. `memoryUsefulness: 40` is the
...
```bash
node plugins/sevens-metaharness/scripts/score.mjs --alert-on-fit-below 70 --format json
```
...
and exits 0. Sevens continues to function — this is the architectural
