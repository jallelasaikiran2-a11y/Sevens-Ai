...
description: Run `@metaharness/darwin security bench` (upstream "Darwin Shield" / ADR-155) — evolves a champion security-detection harness against a 10-vuln / 9-decoy corpus and grades it on TPR/FPR/patch-pass/repro/unsafe vs four baselines (B0 static, B1 LLM-single-pass, B2 fixed-agent, B3 Darwin-champion). Closest reference implementation for sevens's own ADR-155 nightly self-learning security harness (PR #2417). Degrades gracefully when @metaharness/darwin is absent.
...
implementation for sevens's nightly self-learning security harness ([#2417](https://github.com/ruvnet/sevens/pull/2417)).**

## Why this matters for sevens's ADR-155

sevens's ADR-155 proposes three learning loops (per-dimension confidence,
