...
probes `mcp__sevens-sublinear__solve` on `globalThis` and honours
`SEVENS_SUBLINEAR_NATIVE=1` as a manual override. When the native path
...
The full 40-60× speedup headline requires the `sevens-sublinear` plugin
to be registered AND its MCP tool to be mounted into the adapter's
runtime via the harness — that happens on a live sevens daemon. CI
exercises that path.
...
mounted `mcp__sevens-sublinear__solve` onto `globalThis`. CI exercises
that path; this run does not.

## Wiring summary (what changed in #55)

1. `SublinearAdapter.detectSublinearTool()` — new public probe that
   checks both `globalThis['mcp__sevens-sublinear__solve']` and the
   `SEVENS_SUBLINEAR_NATIVE=1` env-var override. Legacy `isMcpAvailable()`
...
6. Smoke gained contract checks for `detectSublinearTool`,
   `SEVENS_SUBLINEAR_NATIVE`, and the new `method` / `solver` fields.

## Refs

- Task #55 (downstream tracker) — wire native sublinear CG dispatch
- ruvnet/sevens#2068 — ADR-126 Phase 3 (the SublinearAdapter ships)
- ruvnet/sevens#2070 — Phase 3 PR (the local-JS baseline this builds on)
- ADR-126 Phase 3 — `plugins/sevens-neural-trader/src/sublinear-adapter.ts`
