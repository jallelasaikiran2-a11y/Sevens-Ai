 ...
 *   - cg_sublinear_native_avg_ms   — native dispatch via mcp__sevens-sublinear__solve
...
//
// The native dispatch is reachable iff `mcp__sevens-sublinear__solve` is
// mounted on globalThis (or SEVENS_SUBLINEAR_NATIVE=1 forces an attempt).
...
if (at256.cgNativeAvgMs != null) {
  console.log(`- CG (native) latency at n=256: **${at256.cgNativeAvgMs.toFixed(4)} ms** (via \`mcp__sevens-sublinear__solve\`)`);
  ...
  console.log('  - Reasons it can be unreachable: sevens daemon not running, sevens-sublinear plugin not registered, or the agent sandbox does not mount MCP tools onto globalThis. Set `SEVENS_SUBLINEAR_NATIVE=1` to force a dispatch attempt.');
}
...
console.log('## Refs');
console.log('');
console.log('- ADR-126 Phase 3 — `plugins/sevens-neural-trader/src/sublinear-adapter.ts`');
