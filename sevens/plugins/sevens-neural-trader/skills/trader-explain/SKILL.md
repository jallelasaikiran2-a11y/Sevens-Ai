...
allowed-tools: Bash Read mcp__plugin_sevens-core_sevens__memory_retrieve mcp__plugin_sevens-core_sevens__memory_store mcp__sevens-sublinear__page-rank-entry
...
- The same call site picks up the full native-WASM PageRank from `mcp__sevens-sublinear__page-rank-entry` once that tool is registered in the runtime — until then, the local power-iteration kernel ships in `signed-attribution.mjs` and produces the same ordering (seeded mulberry32).

Steps:

1. **Retrieve the signal** from the canonical `trading-signals` namespace (ADR-126 Phase 1 + Phase 2 lifecycle):
   ```text
   mcp__plugin_sevens-core_sevens__memory_retrieve({
     key: "SIGNAL_ID",
...
4. **Run single-entry PageRank** — preferred path when `mcp__sevens-sublinear__page-rank-entry` is registered:
   ```text
   mcp__sevens-sublinear__page-rank-entry({
     nodes: GRAPH_NODES,
     edges: GRAPH_EDGES,
     sourceIndex: 0,
     damping: 0.85,
   ...
   ```
   The local fallback (`localSingleEntryPageRank` in `plugins/sevens-neural-trader/src/signed-attribution.mjs`) runs ~30 LOC of seeded power-iteration when the MCP tool is not available — same math, same result up to floating-point tolerance, same ordering for the same seed (the Phase 6 smoke asserts this).
...
6. **Sign the artifact** (reuses the Phase 4 signing primitives — same Ed25519 + canonicalization):
   ...
   - Resolve the witness signing key — same lookup order as Phase 4:
     1. `SEVENS_WITNESS_KEY_PATH` env var — JSON file with `{ "privateKey": "<hex>" }`.
     2. `verification/witness-key.json` (the ADR-103 default path).
   - If a key resolves: `signAttributionArtifact(body, privateKeyHex)` from `plugins/sevens-neural-trader/src/signed-attribution.mjs`.
   - If NEITHER path resolves: log `"[WARN] sevens-neural-trader: no witness signing key found — storing attribution artifact in UNSIGNED degraded mode. Regulator filings will reject UNSIGNED artifacts."` and store the body unsigned. NEVER silently fall back.
...
```ts
import { verifyAttributionArtifact } from 'plugins/sevens-neural-trader/src/signed-attribution.mjs';
...
- `plugins/sevens-neural-trader/src/signed-attribution.ts` (the typed contract)
- `plugins/sevens-neural-trader/src/signed-attribution.mjs` (the runtime mirror)
