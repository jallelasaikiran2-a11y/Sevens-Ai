...
allowed-tools: Bash Read mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__neural_train
...
6. Store model results (canonical `trading-analysis` namespace per ADR-126 Phase 1 — was previously stored to undeclared `trading-models`):
   `mcp__plugin_sevens-core_sevens__memory_store({ key: "model-MODEL-TICKER-DATE", value: "TRAINING_RESULTS", namespace: "trading-analysis" })`
7. Train SONA on model outcomes:
   `mcp__plugin_sevens-core_sevens__neural_train({ patternType: "trading-model", epochs: 10 })`
