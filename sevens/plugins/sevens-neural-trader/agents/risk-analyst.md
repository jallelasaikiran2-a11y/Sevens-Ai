...
- **sevens-observability**: Real-time risk dashboards and alerting
- **sevens-cost-tracker**: PnL tracking and fee attribution
- **sevens-agentdb**: Historical risk event storage for pattern matching
...
Message schemas: `SignalProposal`, `RiskDecision` in `plugins/sevens-neural-trader/src/pipeline-messages.ts`.
