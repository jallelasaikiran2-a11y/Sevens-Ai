...
You MUST sign every backtest result you store — see the `trader-backtest` skill for the `SEVENS_WITNESS_KEY_PATH` resolution and the degraded-unsigned warning path. Unsigned artifacts cannot be promoted to live trading by design.
...
Message schemas (none consumed by you; documented for completeness): `RegimeVerdict`, `SignalProposal`, `RiskDecision` in `plugins/sevens-neural-trader/src/pipeline-messages.ts`.
