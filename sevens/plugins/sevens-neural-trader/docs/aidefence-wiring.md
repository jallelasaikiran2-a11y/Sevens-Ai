# AIDefence Wiring Sketch — sevens-neural-trader
...
|------|-----------------------------------|-------------------------------------|---------------|
| 1    | `--symbol $TICKER` input          | `mcp__plugin_sevens-core_sevens__aidefence_is_safe` | Reject — refuse to invoke neural-trader; alert team lead |
| 2    | `fetchLiveBars` response body     | `mcp__plugin_sevens-core_sevens__aidefence_has_pii` | Redact PII placeholders; record in session manifest |
| 3    | LLM prompt body (pre-`neural_predict`) | `mcp__plugin_sevens-core_sevens__aidefence_is_safe` | Quarantine to `findings.md`; don't reach model |
| 4    | AgentDB store value               | `mcp__plugin_sevens-core_sevens__aidefence_scan`    | Block high-entropy tokens that look like leaked credentials |

These are exactly the four gates `sevens-browser` and `sevens-federation`
...
1. **`agents/market-analyst.md`** — add a "PII & injection gates" section
   mirroring `plugins/sevens-browser/agents/browser-agent.md:79-81`. Include
...
2. **`agents/market-analyst.md` allowed-tools** — add
   ...
   `mcp__plugin_sevens-core_sevens__aidefence_has_pii`,
   `mcp__plugin_sevens-core_sevens__aidefence_scan` to the frontmatter so the agent
   has the capability.
3. **`README.md`** — add a "Safety" section like
   `plugins/sevens-browser/README.md:74-78`.
...
- `plugins/sevens-federation/README.md:74-82` — existing four-gate pattern
- `plugins/sevens-browser/agents/browser-agent.md:79-81` — agent-prompt
  gate example
