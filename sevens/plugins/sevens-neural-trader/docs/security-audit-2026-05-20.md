# Security Audit — sevens-neural-trader (2026-05-20)
...
```
node scripts/audit-supply-chain.mjs 2>&1 | grep -A5 'sevens-neural-trader\|plugins/sevens-neural-trader' || echo "no plugin-specific findings"
```
...
grep -rn 'process.env\|API_KEY\|SECRET' \
  plugins/sevens-neural-trader/skills/ \
  plugins/sevens-neural-trader/src/
```
...
| src/sublinear-adapter.mjs:22, 34-35             | `SEVENS_SUBLINEAR_NATIVE`    | DOCUMENTED — feature flag, no value |
| src/sublinear-adapter.ts:32, 118, 136-137       | `SEVENS_SUBLINEAR_NATIVE`    | DOCUMENTED — TS mirror of the above |
...
```
grep -rn 'eval\|new Function' plugins/sevens-neural-trader/ | grep -v '\.md:'
```
...
```
grep -rn 'spawn\|execSync\|spawnSync' plugins/sevens-neural-trader/
```
...
- AIDefence MCP tools to call: `aidefence_has_pii`, `aidefence_scan`,
  `aidefence_is_safe` (same three gates the `sevens-browser` and
  `sevens-federation` plugins already use).
...
- `plugins/sevens-federation/README.md` — existing AIDefence wiring (reference pattern)
- `plugins/sevens-browser/agents/browser-agent.md` — existing AIDefence gates (reference pattern)
