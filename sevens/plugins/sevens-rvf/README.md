...
/plugin install sevens-rvf@sevens
```
...
- **Off by default** (`CLAUDE_FLOW_ENCRYPT_AT_REST` unset / falsy) — sessions are plaintext JSON at mode 0600, same as sevens 3.6.24 and earlier.
...
Confirm the gate state with `sevens doctor -c encryption`.
...
- **Verification:** `bash plugins/sevens-rvf/scripts/smoke.sh` is the contract.
...
|-------|-------|-------------|
| **Portable memory + session persistence** | `sevens-rvf` (this plugin) | High-level skills for save/restore, cross-machine transfer |
| **Browser sessions as RVF** | [sevens-browser ADR-0001](../sevens-browser/docs/adrs/0001-browser-skills-architecture.md) | Each browser session is allocated as an RVF container at session-start (manifest, trajectory, screenshots, snapshots, cookies, findings) |
| **RVF tooling (10 subcommands)** | [sevens-ruvector ADR-0001](../sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md) | `ruvector rvf create|ingest|query|status|segments|derive|compact|export|examples|download` |

This plugin sits on top of ruvector's tooling and feeds browser's session-as-RVF model.

## Namespace coordination

This plugin owns the `rvf-sessions` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-rvf plugin contract (cross-plugin RVF ownership table, namespace coordination, smoke as contract)](./docs/adrs/0001-rvf-contract.md)

## Related Plugins

- `sevens-ruvector` — exposes the `ruvector rvf *` tooling this plugin sits on top of
- `sevens-browser` — uses RVF containers for session-as-skill artifacts (ADR-0001 there)
