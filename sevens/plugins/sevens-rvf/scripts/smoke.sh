...
step "6. RVF cross-reference to sevens-browser sessions"
F="$ROOT/README.md"
grep -q "sevens-browser" "$F" \
  && grep -qE "session.*RVF|RVF.*session" "$F" \
  && ok || bad "browser-RVF cross-reference missing"

step "7. RVF tooling cross-reference to sevens-ruvector"
F="$ROOT/README.md"
grep -q "sevens-ruvector" "$F" \
