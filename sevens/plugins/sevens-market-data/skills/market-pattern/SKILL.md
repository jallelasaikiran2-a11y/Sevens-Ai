...
allowed-tools: mcp__plugin_sevens-core_sevens__memory_search mcp__plugin_sevens-core_sevens__memory_list mcp__plugin_sevens-core_sevens__memory_store mcp__plugin_sevens-core_sevens__agentdb_pattern-store mcp__plugin_sevens-core_sevens__agentdb_pattern-search mcp__plugin_sevens-core_sevens__ruvllm_hnsw_route Bash
---
...
1. **Load candles** -- call `mcp__plugin_sevens-core_sevens__memory_search` (or `memory_list`) on the `market-data` namespace to retrieve normalized OHLCV data for the symbol and period. The `memory_*` tool family routes by namespace; the `agentdb_hierarchical-*` family does NOT (it routes by tier), so use `memory_*` for namespaced reads.
...
5. **Store** -- two paths (per sevens-cost-tracker ADR-0001 dual-path pattern):
   - **Pattern-store (typed, recommended)**: `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` with `type: 'market-pattern'`. Don't pass a `namespace` arg — ReasoningBank routes it; on bridge unavailability the fallback writes to the reserved `pattern` namespace with `controller: 'memory-store-fallback'` (see sevens-agentdb ADR-0001).
   - **Plain store (namespace-routable)**: `mcp__plugin_sevens-core_sevens__memory_store --namespace market-patterns` — this DOES respect the `market-patterns` namespace because `memory_*` is namespace-routed.
