...
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-store` -- store normalized OHLCV data and pattern metadata
- `mcp__plugin_sevens-core_sevens__agentdb_hierarchical-recall` -- recall historical market data by symbol/period
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-store` -- store detected candlestick patterns with vectors
- `mcp__plugin_sevens-core_sevens__agentdb_pattern-search` -- search for similar patterns via HNSW
- `mcp__plugin_sevens-core_sevens__agentdb_semantic-route` -- route queries to relevant market data sources
- `mcp__plugin_sevens-core_sevens__embeddings_generate` -- generate embeddings for pattern descriptions
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_create` -- create HNSW index for pattern vectors
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_add` -- add pattern vectors to HNSW index
- `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_route` -- nearest-neighbor search in pattern index
...
- **sevens-neural-trader**: Consumes market data patterns as strategy signals for trading decisions
- **sevens-ruvector**: HNSW indexing engine for fast pattern similarity search
- **sevens-agentdb**: Persistent storage for OHLCV data and pattern vectors
- **sevens-observability**: Metrics dashboards for data feed health and ingestion latency
