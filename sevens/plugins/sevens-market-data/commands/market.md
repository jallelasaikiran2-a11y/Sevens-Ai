...
4. Store normalized data via `mcp__plugin_sevens-core_sevens__memory_store --namespace market-data` (the `memory_*` family is namespace-routed; `agentdb_hierarchical-*` routes by tier and ignores namespace strings — see skills/market-ingest/SKILL.md)
5. Add vectors to HNSW index via `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_add`
...
1. Search HNSW index via `mcp__plugin_sevens-core_sevens__ruvllm_hnsw_route` for vectors matching the pattern
