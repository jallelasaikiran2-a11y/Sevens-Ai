...
```bash
claude --plugin-dir plugins/sevens-market-data
```
...
- **Verification:** `bash plugins/sevens-market-data/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns two AgentDB namespaces (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)):
...
- [`ADR-0001` — sevens-market-data plugin contract (3 functional bug fixes + namespace coordination + smoke as contract)](./docs/adrs/0001-market-data-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes violations of
- `sevens-neural-trader` -- Consumes market patterns as strategy signals
- `sevens-ruvector` -- HNSW indexing engine for pattern similarity search
- `sevens-observability` -- Data feed health and ingestion latency dashboards
