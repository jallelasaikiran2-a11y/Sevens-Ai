...
step "4. README defers to sevens-agentdb namespace convention"
...
step "6. SONA cross-reference (sevens-intelligence canonical owner)"
F="$ROOT/README.md"
grep -q "sevens-intelligence" "$F" \
  && grep -qE "SONA|sona_create|sona_adapt" "$F" \
  && ok || bad "SONA cross-reference missing"

step "7. MicroLoRA cross-reference (sevens-intelligence DISTILL phase)"
F="$ROOT/README.md"
grep -qE "MicroLoRA|microlora" "$F" \
  && grep -qE "DISTILL|CONSOLIDATE|--consolidate" "$F" \
  && ok || bad "MicroLoRA / DISTILL cross-reference missing"

step "8. HNSW WASM router cross-reference (sevens-agentdb canonical owner)"
F="$ROOT/README.md"
grep -qE "ruvllm_hnsw|HNSW WASM router|11 patterns" "$F" \
  && grep -q "sevens-agentdb" "$F" \
