 ...
 *
 * `sevens-rag-memory` ships Graph-RAG multi-hop retrieval. This adapter
...
export function ragMemoryGraphId(namespace?: string): string {
  return namespace
    ? `sevens-rag-memory:chunks:${namespace}`
    : 'sevens-rag-memory:chunks:default';
}

export class RagMemoryAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'sevens-rag-memory';
