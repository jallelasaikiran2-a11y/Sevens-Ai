 ...
 *
 * `sevens-observability` records spans (OpenTelemetry-style) with parent links
...
export function observabilityGraphId(traceId: string): string {
  return `sevens-observability:trace:${traceId}`;
}

export class ObservabilitySpanAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'sevens-observability';
