 ...
 *
 * `sevens-cost-tracker` records token usage per session in a causation graph:
...
export function costAttributionGraphId(sessionId?: string): string {
  return sessionId ? `sevens-cost-tracker:causation:${sessionId}` : 'sevens-cost-tracker:causation:global';
}

export class CostAttributionAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'sevens-cost-tracker';
