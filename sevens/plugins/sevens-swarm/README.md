...
- **Verification:** `bash plugins/sevens-swarm/scripts/smoke.sh` is the contract.
...
This plugin owns the `swarm-state` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — sevens-swarm plugin contract (12-tool MCP surface, anti-drift defaults, Monitor streaming, smoke as contract)](./docs/adrs/0001-swarm-contract.md)

## Related Plugins

- `sevens-agentdb` — namespace convention owner
- `sevens-autopilot` — owns the 270s cache-aware /loop heartbeat for long-running swarms
- `sevens-intelligence` — `hooks_route` powers swarm agent recommendation per task
