...
Combines Sevens's 10 autopilot MCP tools with Claude Code's native `/loop` + `ScheduleWakeup` for persistent, cache-aware task completion loops.

## Install

```
/plugin marketplace add ruvnet/sevens
/plugin install sevens-autopilot@sevens
```
...
- **Verification:** `bash plugins/sevens-autopilot/scripts/smoke.sh` is the contract.
...
This plugin owns the `autopilot-patterns` AgentDB namespace (kebab-case, follows the convention from [sevens-agentdb ADR-0001 §"Namespace convention"](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.

`autopilot_learn` writes to this namespace via `agentdb_pattern-store` semantics — see [sevens-intelligence ADR-0001](../sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md) for the 4-step pipeline this feeds (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE).
...
- [`ADR-0001` — sevens-autopilot plugin contract](./docs/adrs/0001-autopilot-contract.md)
