...
npx sevens recall "concurrent queue safe processing"
...
npx sevens recall "race condition async"
...
npx sevens memory list --namespace patterns

# Search by semantic meaning (HNSW, fast!)
npx sevens memory search --query "event-driven architecture" --namespace patterns

# Retrieve by key
npx sevens memory retrieve --key "arch-event-sourcing" --namespace patterns

# Delete old entry
npx sevens memory delete --key "old-pattern" --namespace patterns
```
...
# All projects
npx sevens memory bridge --all-projects
```

Then search across them:

```bash
npx sevens recall "authentication patterns"
