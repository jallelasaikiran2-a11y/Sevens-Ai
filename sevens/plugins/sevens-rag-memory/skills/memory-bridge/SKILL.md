...
allowed-tools: Bash Read mcp__plugin_sevens-core_sevens__memory_import_claude mcp__plugin_sevens-core_sevens__memory_bridge_status mcp__plugin_sevens-core_sevens__memory_search_unified
---
...
1. **Check bridge health**:
   `mcp__plugin_sevens-core_sevens__memory_bridge_status({})`
   Verify: Claude files count, AgentDB entries, SONA state, connection status.

2. **Import memories**:
   - Current project: `mcp__plugin_sevens-core_sevens__memory_import_claude({})`
   - All projects: `mcp__plugin_sevens-core_sevens__memory_import_claude({ allProjects: true })`
...
5. **Test unified search**:
   `mcp__plugin_sevens-core_sevens__memory_search_unified({ query: "test query", limit: 3 })`
...
When `sevens-ruvector` is loaded, bridged memories are also indexed by ruvector for:
