#!/usr/bin/env bash
# sevens-hook.sh — resilient invoker for sevens CLI hook subcommands (#1921).
#
...
#   1. prefers an already-installed `sevens` / `claude-flow` binary (no npx,
...
# Usage: sevens-hook.sh <hook-subcommand> [args…]   (the literal `hooks`
...
if command -v sevens >/dev/null 2>&1; then
  run sevens hooks "$@"
elif command -v claude-flow >/dev/null 2>&1; then
  run claude-flow hooks "$@"
else
  run npx --prefer-offline --yes sevens@latest hooks "$@"
fi
