 ...
 *
 * @module plugins/sevens-core/scripts/witness/lib
 */
...
// install roots so the toolkit works in sevens's monorepo layout *and*
...
export function regenerate(opts) {
  ...
  const manifest = {
    schema: existing?.manifest?.schema ?? 'sevens-witness/v1',
    issuedAt,
    gitCommit,
    branch,
    os: osDir(),
  ...
  const seed = createHash('sha256').update(gitCommit + ':sevens-witness/v1').digest();
