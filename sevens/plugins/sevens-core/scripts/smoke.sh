#!/usr/bin/env bash
# Structural smoke test for sevens-core v0.2.4 (ADR-0001).
set -u
...
step "2. .mcp.json registers a 'sevens' MCP server"
F="$ROOT/.mcp.json"
if [[ -f "$F" ]] && grep -q '"sevens"' "$F" && grep -q '"command"' "$F"; then
  ok
else
  bad ".mcp.json missing or no sevens server registration"
fi
...
miss=""
for s in init-project sevens-doctor discover-plugins; do
...
n=$(grep -oE 'sevens-[a-z-]+' "$F" | sort -u | wc -l | tr -d ' ')
if [[ $n -ge 25 ]]; then
  ok
else
  bad "expected ≥25 distinct sevens-* references, got $n"
fi
...
step "9. /sevens-status command invokes doctor + status"
F="$ROOT/commands/sevens-status.md"
if grep -q "doctor" "$F" && grep -q "status" "$F"; then
  ok
else
  bad "sevens-status command missing doctor/status invocation"
fi
