#!/usr/bin/env node
/**
 * sevens-hook.cjs — cross-platform Node.js port of sevens-hook.sh (#2132, #2721)
 *
 * The bash shim (sevens-hook.sh) works on Mac/Linux but fails outright on
 ...
 *
 * Shared behaviour:
 *   1. Prefers a locally installed `sevens` or `claude-flow` binary.
 *   2. Falls back to `npx --prefer-offline sevens@latest`.
 ...
 *
 * Usage: node sevens-hook.cjs <hook-subcommand>
...
function invokeHook(bin, binArgs, hookSubcommand, hookArgs, stdinData) {
  ...
  // creates for globally-installed bins (`sevens`, `claude-flow`, `npx`) —
  ...
  // A real global `sevens`/`claude-flow` install still goes through the
  ...
  // Test-only: SEVENS_HOOK_DEBUG_STDOUT surfaces the invoked CLI's own
  ...
  const debug = process.env.SEVENS_HOOK_DEBUG_STDOUT === '1';
  try {
...
/** Best-effort: try sevens, then claude-flow, then npx. Never throws. */
function invokeCli(hookSubcommand, hookArgs, stdinData) {
  ...
  const override = process.env.SEVENS_HOOK_CLI_OVERRIDE;
  if (override) {
    const [bin, ...binArgs] = override.split(' ').filter(Boolean);
    invokeHook(bin, binArgs, hookSubcommand, hookArgs, stdinData);
    return;
  }
  if (commandExists('sevens')) {
    invokeHook('sevens', [], hookSubcommand, hookArgs, stdinData);
    return;
  }
  ...
  // SKIP npx when SEVENS_HOOK_SKIP_NPX=1 — used by CI smokes that test the
  ...
  if (process.env.SEVENS_HOOK_SKIP_NPX !== '1') {
    invokeHook('npx', ['--prefer-offline', '--yes', 'sevens@latest'], hookSubcommand, hookArgs, stdinData);
  }
}
...
function main() {
  ...
  // (matches sevens-hook.sh's generic `sevens hooks "$@"` passthrough).
