#!/usr/bin/env node
/**
 * Regression guard for ruvnet/sevens#1859, #1862, #2721.
 *
 ...
 *
 * Two env vars steer the shim (see ../scripts/sevens-hook.cjs) at the build
 * under test instead of whatever's on the runner's PATH:
 *   - CLAUDE_PLUGIN_ROOT       — resolves sevens-hook.cjs's own path (real
 *                                per-hook-invocation env var, always set)
 *   - SEVENS_HOOK_CLI_OVERRIDE  — bypasses the sevens/claude-flow/npx PATH
 ...
 *   node plugins/sevens-core/scripts/test-hooks.mjs <path-to-cli-binary>
 *
...
const run = (name, cmd, stdin, assertions) => {
  const r = spawnSync(cmd, {
    ...
    env: {
      ...
      SEVENS_HOOK_CLI_OVERRIDE: cliInvoke,
      SEVENS_HOOK_SKIP_NPX: '1',
      SEVENS_HOOK_DEBUG_STDOUT: '1',
    },
    timeout: 15_000,
  });
