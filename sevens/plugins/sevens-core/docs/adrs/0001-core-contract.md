---
id: ADR-0001
title: sevens-core plugin contract — pinning, MCP server contract, plugin-catalog discovery, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`sevens-core` is the **foundation plugin**. Every other plugin (`sevens-ruvector`, `sevens-agentdb`, `sevens-browser`, `sevens-intelligence`, `sevens-adr`, `sevens-aidefence`, `sevens-autopilot`, plus 25 others) depends on the MCP server it registers via `.mcp.json` and the orchestration patterns it documents.

Today's plugin (v0.1.0):

- `.claude-plugin/plugin.json:4` — `version: "0.1.0"`, keywords `mcp, orchestration, claude-code`
- `.mcp.json` — registers `sevens` MCP server via `npx -y @claude-flow/cli@latest`
- `agents/` — 3 generalists (`coder`, `researcher`, `reviewer`)
- `skills/` — 3 first-run helpers (`init-project`, `sevens-doctor`, `discover-plugins`)
- `commands/sevens-status.md` — system status one-liner
...
- **MCP server contract** — the registered `sevens` MCP server exposes 300+ tools across families: `memory_*`, `agentdb_*`, `embeddings_*`, `ruvllm_*`, `hooks_*`, `aidefence_*`, `neural_*`, `autopilot_*`, `browser_*`, `agent_*`, `swarm_*`, `system_*`, etc. Runtime truth via `mcp tool call mcp_status`.
...
2. `.mcp.json` exists and registers a `sevens` MCP server.
3. All 3 agents present (`coder`, `researcher`, `reviewer`) with valid frontmatter.
4. All 3 skills present (`init-project`, `sevens-doctor`, `discover-plugins`) with valid frontmatter.
...
9. `commands/sevens-status.md` invokes `doctor` and `status`.
...
- `plugins/sevens-browser/docs/adrs/0001-browser-skills-architecture.md`
- `plugins/sevens-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — 4-step pipeline
- `plugins/sevens-adr/docs/adrs/0001-adr-plugin-pattern.md`
- `plugins/sevens-aidefence/docs/adrs/0001-aidefence-contract.md` — 3-gate pattern
- `plugins/sevens-autopilot/docs/adrs/0001-autopilot-contract.md` — 270s cache-aware /loop
- `v3/@claude-flow/cli/` — the MCP server source backing this plugin

## Implementation status

Plugin version v0.2.1 shipped and listed in marketplace.json. Source exists at `plugins/sevens-core/`. Contract elements implemented: `.mcp.json` registers `sevens` server via `npx -y @claude-flow/cli@latest`; plugin-catalog discovery skill present; 3 generalist agents shipped; smoke-as-contract gate defined in `scripts/smoke.sh`.
