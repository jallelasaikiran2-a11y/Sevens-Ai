...
`sha256(gitCommit + ':sevens-witness/v1')` — no committed private key.
...
Toolkit lives in `plugins/sevens-core/scripts/witness/`:
...
3. Run `node plugins/sevens-core/scripts/witness/regen.mjs --dry-run` first
