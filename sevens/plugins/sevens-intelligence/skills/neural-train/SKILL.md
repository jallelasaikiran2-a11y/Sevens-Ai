...
allowed-tools: mcp__plugin_sevens-core_sevens__neural_train mcp__plugin_sevens-core_sevens__neural_status mcp__plugin_sevens-core_sevens__neural_patterns mcp__plugin_sevens-core_sevens__neural_predict mcp__plugin_sevens-core_sevens__neural_optimize mcp__plugin_sevens-core_sevens__neural_compress mcp__plugin_sevens-core_sevens__hooks_pretrain mcp__plugin_sevens-core_sevens__hooks_build-agents mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-start mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-step mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-end mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-store mcp__plugin_sevens-core_sevens__hooks_intelligence_learn mcp__plugin_sevens-core_sevens__hooks_intelligence-reset mcp__plugin_sevens-core_sevens__ruvllm_sona_create mcp__plugin_sevens-core_sevens__ruvllm_sona_adapt mcp__plugin_sevens-core_sevens__ruvllm_microlora_create mcp__plugin_sevens-core_sevens__ruvllm_microlora_adapt mcp__plugin_sevens-core_sevens__agentdb_consolidate Bash
---
...
1. **Check current neural status** — `mcp__plugin_sevens-core_sevens__neural_status`.
2. **Start a trajectory** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-start` with the task context.
3. **Record steps** — for each significant action, `mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-step`.
4. **End trajectory** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_trajectory-end` with `verdict: pass|fail|partial`.
5. **Learn from the trajectory** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_learn`.
6. **Train patterns** — `mcp__plugin_sevens-core_sevens__neural_train` with `--pattern-type coordination --epochs 10`.
7. **Store patterns** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-store`.
8. **Verify** — `mcp__plugin_sevens-core_sevens__neural_patterns` to confirm.
...
`hooks_pretrain` writes to the `patterns` (plural) namespace — distinct from the `pattern` (singular) ReasoningBank target. See `sevens-agentdb` ADR-0001 for the namespace convention.
