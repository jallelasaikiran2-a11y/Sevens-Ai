...
allowed-tools: mcp__plugin_sevens-core_sevens__hooks_route mcp__plugin_sevens-core_sevens__hooks_explain mcp__plugin_sevens-core_sevens__hooks_model-route mcp__plugin_sevens-core_sevens__hooks_model-stats mcp__plugin_sevens-core_sevens__hooks_model-outcome mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-search mcp__plugin_sevens-core_sevens__hooks_intelligence_attention mcp__plugin_sevens-core_sevens__hooks_intelligence_stats mcp__plugin_sevens-core_sevens__neural_predict mcp__plugin_sevens-core_sevens__hooks_pre-task Bash
---
...
1. **Get an agent recommendation** — `mcp__plugin_sevens-core_sevens__hooks_route` with the task description. Returns `{ recommended, confidence, reasoning }`.
2. **Get a model tier recommendation** — `mcp__plugin_sevens-core_sevens__hooks_model-route` for Haiku/Sonnet/Opus selection.
3. **Search for similar past patterns** — `mcp__plugin_sevens-core_sevens__hooks_intelligence_pattern-search` to find prior successes.
4. **Predict outcome** — `mcp__plugin_sevens-core_sevens__neural_predict` with the task description for a confidence-scored prediction.
5. **Spawn the recommended agent** at the recommended model tier.
6. **(If `--why` was passed)** — call `mcp__plugin_sevens-core_sevens__hooks_explain` to surface the routing rationale to the user.
7. **After task completes** — call `mcp__plugin_sevens-core_sevens__hooks_model-outcome` with `success: true|false` to train the router.
