---
id: ADR-0001
title: Optimize sevens-intelligence — surface completeness, 4-step pipeline, IPFS pattern transfer, namespace coordination with sevens-agentdb
status: Proposed
date: 2026-05-04
authors:
...
- **`hooks_intelligence_attention`** — attention-mechanism dispatch (the same primitive `sevens-ruvector` surfaces via `attention list`). Plugin makes no mention.
...
`sevens-agentdb` ADR-0001 (just landed) introduced a namespace convention with three reserved namespaces: `pattern` (ReasoningBank fallback), `claude-memories` (Claude Code bridge), `default`. **The intelligence pipeline writes to `pattern`** (via `agentdb_pattern-store` and the `hooks post-task --train-neural` path). The plugin should explicitly cite the convention so consumers don't reinvent it.

There's also a pluralization gotcha worth surfacing here: ruvector's neural/pretrain hooks write to `patterns` (plural), distinct from ReasoningBank's `pattern`. `sevens-agentdb` documents this; `sevens-intelligence` should reference the documentation.

### Why now

`sevens-intelligence` is the **user-facing** surface for the self-learning system that the rest of the plugin family relies on. Documentation drift here means:
...
4. Downstream plugins (`sevens-browser` ADR-0001 references trajectory recording; `sevens-agentdb` references `pattern` namespace writes) lose their anchor doc.

We just fixed analogous drift in `sevens-ruvector` (ADR-0001) and `sevens-agentdb` (ADR-0001). Same pattern applies here.
...
`plugin.json` moves `0.1.0 → 0.3.0` (matches the sevens-agentdb cadence and reflects that this is a **substantial scope expansion**, not a patch). Keywords gain `microlora`, `ewc`, `attention`, `moe`, `pattern-transfer`, `model-routing`, `mcp`. Description rewrites to explicitly enumerate the three families and the IPFS transfer.
...
- **Hook integration** — table mapping `post-task --train-neural`, `pretrain`, `pre-task` to which intelligence tools fire. References sevens-agentdb's reserved namespaces (`pattern` for ReasoningBank, `patterns` for neural-train).
- **Namespace coordination** — explicit deferral to `sevens-agentdb` ADR-0001 §"Namespace convention"; reproduces the pluralization gotcha (`pattern` vs `patterns`).
...
- **Compatibility** — pinned to `@claude-flow/cli` v3.6 (matches sevens-agentdb).
...
10. Pluralization gotcha referenced (`pattern` vs `patterns`) — defers to sevens-agentdb.
...
- `grep -q "4-step intelligence pipeline" plugins/sevens-intelligence/README.md`
- `grep -q "hooks_transfer" plugins/sevens-intelligence/skills/intelligence-transfer/SKILL.md`
- `grep -qE "RETRIEVE.+JUDGE.+DISTILL.+CONSOLIDATE" plugins/sevens-intelligence/README.md`
...
- Namespace coordination with `sevens-agentdb` is explicit; downstream plugins inherit one consistent story.
...
```bash
bash plugins/sevens-intelligence/scripts/smoke.sh
# Expected: "16 passed, 0 failed" (13 contract checks + 3 doc invariants)
```

## Related

- `plugins/sevens-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent
- `plugins/sevens-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`claude-memories`/`default`); pluralization gotcha; hook integration table
- `plugins/sevens-browser/docs/adrs/0001-browser-skills-architecture.md` — uses ruvector trajectory hooks (a primary learning input for this plugin)
