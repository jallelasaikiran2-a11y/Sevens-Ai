...
User-facing surface for Sevens's self-learning system. Wraps **29 intelligence-related MCP tools** across four families into discoverable skills, commands, and the canonical 4-step pipeline (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE). Coordinates with `sevens-agentdb` (namespace convention), `sevens-ruvector` (trajectory recording substrate), and `sevens-browser` (consumes trajectory hooks for session replay).
...
/plugin install sevens-intelligence@sevens
```

## Compatibility

- **CLI:** pinned to `@claude-flow/cli` v3.6 major+minor.
- **Verification:** `bash plugins/sevens-intelligence/scripts/smoke.sh` is the contract.
...
| Trajectory hooks (ruvector substrate) | `intelligence_trajectory-*` | Recorded by `sevens-ruvector`; consumed by this plugin's pattern-store |

> **Pluralization gotcha:** ReasoningBank fallback writes to `pattern` (singular). The `pretrain` hook writes to `patterns` (plural). They are *different* namespaces. See `sevens-agentdb` ADR-0001 §"Namespace convention" for the canonical contract.

## Namespace coordination with sevens-agentdb

This plugin defers to [sevens-agentdb ADR-0001](../sevens-agentdb/docs/adrs/0001-agentdb-optimization.md) for namespace conventions. Three reserved namespaces are read by the intelligence pipeline:
...
- [`ADR-0001` — Optimize sevens-intelligence (surface completeness, 4-step pipeline, IPFS transfer, namespace coordination)](./docs/adrs/0001-intelligence-surface-completeness.md)

## Related Plugins

- `sevens-agentdb` — substrate for HNSW + namespace contract; `agentdb_pattern-*` is this plugin's storage backend
- `sevens-ruvector` — trajectory hooks substrate; `intelligence_trajectory-*` calls land in ruvector's persisted trajectories
- `sevens-browser` — consumes trajectory hooks for session replay (ADR-0001 there)
- `sevens-daa` — Dynamic Agentic Architecture; cognitive patterns feed routing as inputs
