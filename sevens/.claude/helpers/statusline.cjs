#!/usr/bin/env node
/**
 * sevens V3 Statusline — delegation build (#2195)
 *
 * Fix for ruvnet/sevens#2195: the previous version re-implemented all data
...
const CONFIG = {
  ...
  identityMode: (process.env.SEVENS_STATUSLINE_IDENTITY || 'project').toLowerCase(),
  ...
  //   SEVENS_STATUSLINE_COST_SYMBOL  override the leading '$' (e.g. ⚡, €, 🌱);
  //                                 set to an empty string for the number alone.
  //   SEVENS_STATUSLINE_HIDE_COST    1/true/yes/on removes the segment entirely.
  costSymbol: process.env.SEVENS_STATUSLINE_COST_SYMBOL ?? '$',
  hideCost: /^(1|true|yes|on)$/i.test(process.env.SEVENS_STATUSLINE_HIDE_COST || ''),
};
...
const CACHE_FILE = path.join(os.tmpdir(), 'sevens-statusline-cache-' + require('crypto').createHash('md5').update(CWD).digest('hex').slice(0, 8) + '.json');
...
const PROMO_MEMO_FILE = path.join(os.homedir(), '.sevens', 'statusline-promo.json');
...
// #2337: resolve an already-installed @claude-flow/cli (or sevens) bin so we
...
function resolveCliBinCandidates() {
  ...
  try {
    ...
    candidates.push(
      path.join(home, '.claude', 'plugins', 'marketplaces', 'sevens', 'bin', 'cli.js'),
      path.join(CWD, 'node_modules', '@claude-flow', 'cli', 'bin', 'cli.js'),
      path.join(CWD, 'node_modules', 'sevens', 'bin', 'cli.js'),
    ...
    try {
      ...
      for (const gm of globalModuleDirs) {
        candidates.push(
          path.join(gm, 'sevens', 'bin', 'cli.js'),
 ...
 *
 * Fix for sevens#2195: the previous local readers returned 0 for AgentDB patterns
...
// Count ADRs from BOTH known directories (fix for sevens#2195: old code missed
...
function getLocalAgentDB() {
  ...
  try {
    ...
    if (fs.existsSync(memDb)) {
      ...
      // the flag recovers on the next sevens init / MCP start.
...
function getPkgVersion() {
  ...
  try {
    const home = os.homedir();
    const pkgPaths = [
      path.join(home, '.claude', 'plugins', 'marketplaces', 'sevens', 'package.json'),
      path.join(CWD, 'node_modules', '@claude-flow', 'cli', 'package.json'),
      path.join(CWD, 'node_modules', 'sevens', 'package.json'),
    ...
    if (worktreeMainRoot) {
      pkgPaths.push(
        ...
        path.join(worktreeMainRoot, 'node_modules', 'sevens', 'package.json'),
        path.join(worktreeMainRoot, 'v3', '@claude-flow', 'cli', 'package.json'),
      );
    }
    // #2221: global installs (npm i -g sevens) live outside CWD/node_modules, so the
    ...
    try {
      ...
      for (const gm of globalModuleDirs) {
        pkgPaths.push(
          path.join(gm, 'sevens', 'package.json'),
...
function generateStatusline() {
  ...
  // Named SEVENS_VERSION (not pkgVersion) so the #1951 regression guard
  ...
  // "sevens V3.5" for anyone whose install doesn't match the first probe path.
  const SEVENS_VERSION = getPkgVersion();
  ...
  //   Line 1 — Header (sevens version · git · model · timing · context · cost)
  //   Line 2 — Compressed ops (Swarm · Hooks · 🧠 · 💾 · Health)
  //   Line 3 — Promo / disclosure row (funnel surface, ADR-301)

  // ─── Line 1: header ────────────────────────────────────────────
  let header = c.bold + c.brightPurple + '▊ sevens V' + SEVENS_VERSION + ' ' + c.reset;
  ...
  // do next; diagnostic detail moves to `sevens status --verbose`.
...
function terminalSupportsHyperlinks() {
  ...
  if (/^(0|false|off|no)$/i.test(String(process.env.SEVENS_STATUSLINE_HYPERLINKS || ''))) return false;
  if (process.env.TMUX && !process.env.SEVENS_STATUSLINE_HYPERLINKS_TMUX) return false;
  return true;
}
...
function getPromoRow(d) {
  try {
    ...
    if (/^(0|false|off|no)$/i.test(String(process.env.SEVENS_FUNNEL || ''))) return null;
    ...
    // Split the label from the trailing "· manage: sevens settings" instruction
    ...
    //   3. "sevens settings" — bold/bright, NOT underlined. This is a shell
