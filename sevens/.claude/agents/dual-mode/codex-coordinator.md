...
```bash
npx sevens@latest swarm init --topology hierarchical --max-agents 6
```
...
```bash
npx sevens@latest memory list --namespace results
```
...
# Initialize
npx sevens@latest swarm init --topology hierarchical --max-agents 4
