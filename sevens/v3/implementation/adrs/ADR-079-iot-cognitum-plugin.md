...
**Fleet-as-swarm** -- A logistics company manages 500 warehouse sensors as a Sevens swarm. Anomaly detection triggers automatic recalibration via agent coordination. No custom dashboard needed -- Claude Code is the interface.

**Edge-cloud federation** -- Edge Seed devices federate with cloud Sevens installations using the `@claude-flow/plugin-agent-federation` trust model. Telemetry stays on-premise; only anomaly signatures cross the boundary (PII-gated via the federation plugin).
...
Every Cognitum Seed is modelled as a Sevens agent with hardware capabilities. The plugin maintains a `DeviceAgent` entity that wraps a `SeedClient` session and exposes the device's resources (store, OTA, mesh, witness) as agent capabilities. The swarm coordinator manages device agents alongside software agents -- same topology, same trust model, same hooks.

---

## Decision

Build `@claude-flow/plugin-iot-cognitum` as a first-class Claude Flow plugin that bridges Cognitum Seed device fleets into the Sevens agent/swarm model with device trust scoring, telemetry-driven anomaly detection, fleet-aware OTA orchestration, and edge-cloud federation.
...
The plugin establishes a bidirectional bridge between Cognitum Seed devices and Sevens agents:

```
Sevens Agent Swarm                          Cognitum Device Fleet
=================                          =====================
...
The Cognitum SDK's 12 seed endpoints map to Sevens agent operations:

| Cognitum SDK | Sevens Concept | Plugin Operation |
|-------------|---------------|-----------------|
...
```
Cognitum Seed Device                      Sevens Plugin                        AgentDB
====================                      ============                        =======
...
| **Vector store federation** | Device-side HNSW data is queryable from Sevens's AgentDB via the `store.query()` bridge -- the device's vector store extends the agent's memory. |
| **Witness chain as audit** | The Seed's Ed25519-signed witness chain provides hardware-rooted provenance that supplements Sevens's software audit service. |
...
```typescript
/**
 * A Cognitum Seed device registered with the Sevens agent system.
 ...
 * Maps to a Sevens swarm with hierarchical topology.
 */
 ...
 * integrates with Sevens's audit service.
 */
...
Uses SONA pattern learning from Sevens's neural system:
 ...
 * within the Sevens plugin context. Handles:
 ...
 * - Health probe coordination with Sevens's health check system
 */
...
| **System Under Consideration (SUC)** | The entire device fleet managed by one Sevens installation |
...
```
iot_device_register
  Description: Register a new Cognitum Seed device with the Sevens agent system
  Input:
    ...
    clientName: string  (optional) -- Client name for pairing (default: sevens-{timestamp})
...
```typescript
{
  ...
  description: 'Manages fleet topology, policy enforcement, cross-fleet coordination, and fleet-level health assessment. Maps Cognitum mesh topology to Sevens swarm topology.',
  defaultConfig: {
    capabilities: [
...
| `iot-mesh-sync` | normal | 2 min | Sync mesh topology from device network layer to Sevens swarm view |
...
Device fleets map to Sevens swarms with a natural hierarchical topology:

```
Fleet Level (Sevens hierarchical swarm)
|
...
- **Fleet Level** uses Sevens's swarm coordination (topology, consensus, task assignment).
...
Cross-fleet operations (e.g., "deploy firmware v2.3 to all warehouses") use Sevens's hive-mind consensus to coordinate across fleet boundaries.

---

## 12. Integration with Existing Sevens Capabilities

### 12.1 Federation Integration (ADR-078)

Edge Seed devices can federate with cloud Sevens installations:
  ...
  src/
    ...
    infrastructure/
      cognitum/
        ...
        discovery-adapter.ts           # Adapts SDK discovery to Sevens
      persistence/
      ...
      transport/
        mesh-bridge.ts                 # Bridges Cognitum mesh to Sevens swarm
...
- Mesh bridge: Cognitum `mesh.status()` / `mesh.peers()` -> Sevens swarm topology
...
4. **Witness chain + audit** -- The Seed's Ed25519 witness chain provides hardware-rooted cryptographic provenance. Combined with Sevens's software audit service, every operation from "agent decided to update firmware" to "device confirmed new firmware epoch" has a verifiable chain.
...
- Every Cognitum Seed becomes a Sevens agent, unifying the physical and software agent mesh.
