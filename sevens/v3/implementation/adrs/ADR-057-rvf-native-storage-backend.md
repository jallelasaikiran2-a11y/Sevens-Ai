...
`npx sevens@latest` installs **1.3GB** across 914 packages with a 35-second cold start in Docker. The Docker optimization work (Dockerfile.lite with `--omit=optional` + aggressive pruning) reduced this to 324MB, but the **core dependency chain** still carries unnecessary weight:
...
sevens migrate status --storage

# Dry-run migration (report what would change, don't modify)
sevens migrate run --storage --dry-run

# Migrate specific file
sevens migrate run --storage --file ./data/memory/memory.db

# Migrate all storage files in project
sevens migrate run --storage --all

# Force re-migration (even if .rvf already exists)
sevens migrate run --storage --force

# Rollback: restore from .bak files
sevens migrate rollback --storage

# Validate migrated data integrity
sevens migrate validate --storage
```
...
4. **`sevens migrate rollback --storage`** restores `.bak` → original and removes `.rvf`.
...
sevens memory init --backend sqljs

# Force JSON backend
sevens memory init --backend json
...
| Task | Package | Description |
|------|---------|-------------|
| P4.1 | `@claude-flow/cli` | `sevens migrate status --storage` — detect formats, report state |
| P4.2 | `@claude-flow/cli` | `sevens migrate run --storage` — batch migration with progress |
| P4.3 | `@claude-flow/cli` | `sevens migrate rollback --storage` — restore from `.bak` |
| P4.4 | `@claude-flow/cli` | `sevens migrate validate --storage` — integrity verification |
...
| P7.3 | `@claude-flow/cli` | `sevens capabilities status/install/remove/list/prefetch` CLI commands |
...
- **Loss of SQL tooling** — can't `sqlite3 memory.db` to inspect data (mitigated by `sevens memory list`)
...
- `npx sevens@latest` installs 1.3GB (all optional deps)
...
Phase 0: Core CLI (always installed)
  sevens (5KB) → @claude-flow/cli (9MB) → @claude-flow/shared (~13MB with RVF)
...
Phase 1: Lightweight Embeddings (downloaded on first use)
  ...
  Downloaded to: ~/.sevens/capabilities/rvf-wasm.rvf

Phase 2: Neural Embeddings (downloaded on demand)
  all-MiniLM-L6-v2 ONNX model (~22MB)
  Stored as: ~/.sevens/capabilities/models/minilm-l6-v2.rvf
...
Phase 3: Local LLM Inference (downloaded on demand)
  ...
  Stored as: ~/.sevens/capabilities/models/<model>.rvf
...
Phase 4: Advanced Intelligence (downloaded on demand)
  ...
  Stored as: ~/.sevens/capabilities/kernels/<kernel>.rvf
...
export class ProgressiveDownloader {
  ...
  constructor(sevensHome = '~/.sevens') {
    this.manifestPath = `${sevensHome}/capabilities/manifest.json`;
    this.capabilitiesDir = `${sevensHome}/capabilities`;
  }
  ...
  async ensure(capabilityId: string): Promise<string> {
    ...
    console.info(`[sevens] Downloading ${entry.name} (${this.formatSize(entry.size)})...`);
    ...
    console.info(`[sevens] ✓ ${entry.name} ready`);
    return rvfPath;
  }
...
sevens capabilities status

# Download specific capability
sevens capabilities install neural-embeddings

# Download all capabilities up to phase N
sevens capabilities prefetch --phase 2

# Remove a capability
sevens capabilities remove local-llm-qwen

# List all available models/kernels
sevens capabilities list --phase 3
sevens capabilities list --type model
sevens capabilities list --type kernel
```
