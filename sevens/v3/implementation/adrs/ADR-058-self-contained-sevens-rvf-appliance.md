# ADR-058: Self-Contained Sevens RVF Appliance — Linux Kernel + Claude Code + ruvLLM
...
Sevens v3.5 requires the user to have Node.js 20+, npm, Claude Code CLI, API keys, and a properly configured OS environment. This means:
...
A **single `sevens.rvf` file** that contains everything needed to run the full Sevens platform:

```
sevens.rvf (self-contained appliance)
├── Linux microkernel (Alpine-based, ~5MB)
├── Node.js 22 runtime (~30MB stripped)
├── Claude Code CLI
├── Sevens v3.5+ (all packages)
...
Extending RVF to `RVFA` (RuVector Format Appliance) creates a unified format that Sevens already understands natively.
...
|------|---------|-------------|
| **Run** | `sevens-appliance run sevens.rvf` | Boot and enter interactive CLI |
| **MCP** | `sevens-appliance mcp sevens.rvf` | Boot as MCP server (stdio) |
| **Verify** | `sevens-appliance verify sevens.rvf` | Run full capability test suite |
| **Extract** | `sevens-appliance extract sevens.rvf ./out/` | Unpack all sections |
| **Build** | `sevens-appliance build --profile offline` | Create new appliance |
| **Update** | `sevens-appliance update sevens.rvf --section SEVENS` | Hot-patch one section |
| **Inspect** | `sevens-appliance inspect sevens.rvf` | Show header + section manifest |
...
sevens-appliance run sevens.rvf
# Equivalent to:
# docker run --rm -it sevens:self-contained
```
...
The appliance includes a built-in verification suite that tests **every capability** of Sevens + Claude Flow. This runs automatically at boot (`Section 5: VERIFY`) and can be triggered manually.
...
| 35 | Hot Update | patch SEVENS section → verify new version | Live update |

### 3.3 Test Output Format

```
╔══════════════════════════════════════════════════════════╗
║  Sevens Appliance v3.5.2 — Full Capability Verification  ║
...
Stage 3: SEVENS
  ├── npm pack sevens@latest --omit=optional
...
|---------|----------|-----------------|
| `cloud` | Kernel + Runtime + Sevens + Data + Verify | ~60MB |
...
| `sevens-appliance build` | Multi-stage builder with profile selection |
| `sevens-appliance inspect` | Header + section manifest viewer |
| `sevens-appliance extract` | Unpack all sections to directory |
| Cloud profile | Kernel + Runtime + Sevens + encrypted keys |

### Phase 2: Runtime + Verification (Week 3-4)

| Task | Description |
|------|-------------|
| `sevens-appliance run` | Boot sequence with container isolation |
| `sevens-appliance verify` | 95-check capability suite |
| `sevens-appliance mcp` | MCP server mode (stdio + SSE) |
...
| Task | Description |
|------|-------------|
| `sevens-appliance update` | Hot-patch individual sections |
...
- **Fast boot**: <5s from cold start (vs 35s for `npx sevens@latest`)
