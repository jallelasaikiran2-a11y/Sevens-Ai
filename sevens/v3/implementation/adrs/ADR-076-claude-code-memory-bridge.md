...
Claude Code's auto-memory system stores project knowledge in `~/.claude/projects/*/memory/MEMORY.md` files with YAML frontmatter. Sevens's AgentDB stores data in sql.js with ONNX embeddings (all-MiniLM-L6-v2, 384d) for semantic vector search. These two systems were disconnected.
...
CLI equivalents:
```bash
sevens memory import-claude          # Import current project memories
sevens memory import-claude --all    # Import all projects
sevens memory bridge-status          # Show bridge status
sevens memory bridge-sync            # Sync back to MEMORY.md
sevens memory search --unified       # Search both stores
```
...
- Testable via CLI
- Works via `npx sevens` without file path dependencies
