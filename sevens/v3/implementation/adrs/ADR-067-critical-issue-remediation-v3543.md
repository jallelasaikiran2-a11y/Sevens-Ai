...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over Sevens MCP |
...
Audit all `type: 'array'` properties and add appropriate `items` schema. This is partially addressed in PR #73 (sevens repo).

**4.2 — Enforce Sevens MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer Sevens MCP tools
- **B**: Add system prompt injection that instructs Claude to use Sevens MCP tools for orchestration
...
2. Integration test: `npx sevens daemon start` → workers execute successfully
3. Integration test: `npx sevens memory init` → process exits cleanly
