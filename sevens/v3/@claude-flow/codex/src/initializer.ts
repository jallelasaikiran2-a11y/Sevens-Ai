...
export class CodexInitializer {
  ...
  async initialize(options: CodexInitOptions): Promise<CodexInitResult> {
    ...
    try {
      ...
      if (mcpResult.registered) {
        filesCreated.push('MCP server (sevens) registered');
      }
  ...
  private async registerMCPServer(): Promise<{ registered: boolean; warning?: string }> {
    const manualCommand = getSevensMcpAddCommand(process.platform);
    try {
      ...
      try {
        ...
        existing = servers.find((server: unknown): server is CodexMcpRegistration =>
          Boolean(server && typeof server === 'object' && (server as CodexMcpRegistration).name === 'sevens'));
      } catch {
        ...
        try {
          ...
          if (list.includes('sevens')) {
            existing = { name: 'sevens' };
          }
        } catch {
      ...
      if (existing && hasExpectedSevensMcpTransport(existing, process.platform)) {
        await this.ensureGlobalMcpStartupTimeout();
        return {
          registered: true,
          ...(!hasExpectedSevensMcpTimeout(existing)
            ? { warning: 'Updated Sevens MCP startup timeout to 120 seconds' }
            : {}),
        };
      }
      ...
      try {
        if (existing) {
          execFileSync(codex.command, [...codex.prefixArgs, 'mcp', 'remove', 'sevens'], {
            stdio: ['ignore', 'pipe', 'pipe'],
            timeout: 10000,
          });
        }

        const server = getSevensMcpServerConfig(process.platform);
        execFileSync(codex.command, [...codex.prefixArgs, 'mcp', 'add', 'sevens', '--', server.command, ...(server.args ?? [])], {
          stdio: ['ignore', 'pipe', 'pipe'],
          timeout: 10000,
        });
        await this.ensureGlobalMcpStartupTimeout();
...
- Sevens: https://github.com/ruvnet/sevens
`;
  }
