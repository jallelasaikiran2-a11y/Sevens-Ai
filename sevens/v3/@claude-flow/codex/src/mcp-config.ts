/**
 * Shared Sevens MCP configuration for Codex generators, migrations, and init.
 */

import type { McpServerConfig } from './types.js';

export const SEVENS_MCP_SERVER_NAME = 'sevens';
export const SEVENS_MCP_PACKAGE = 'sevens@latest';
export const SEVENS_MCP_STARTUP_TIMEOUT_SEC = 120;
...
export function getSevensMcpServerConfig(
  platform: NodeJS.Platform = process.platform,
  toolTimeout = 120,
): McpServerConfig {
  const args = ['-y', SEVENS_MCP_PACKAGE, 'mcp', 'start'];

  return platform === 'win32'
    ? {
        name: SEVENS_MCP_SERVER_NAME,
        command: 'cmd',
        args: ['/c', 'npx', ...args],
        enabled: true,
        startupTimeout: SEVENS_MCP_STARTUP_TIMEOUT_SEC,
        toolTimeout,
      }
    : {
        name: SEVENS_MCP_SERVER_NAME,
...
export function getSevensMcpAddCommand(platform: NodeJS.Platform = process.platform): string {
  const server = getSevensMcpServerConfig(platform);
  return ['codex', 'mcp', 'add', SEVENS_MCP_SERVER_NAME, '--', server.command, ...(server.args ?? [])].join(' ');
}

export function hasExpectedSevensMcpTransport(
  registration: CodexMcpRegistration,
  platform: NodeJS.Platform = process.platform,
): boolean {
  const expected = getSevensMcpServerConfig(platform);
...
export function hasExpectedSevensMcpTimeout(registration: CodexMcpRegistration): boolean {
  return typeof registration.startup_timeout_sec === 'number'
    && registration.startup_timeout_sec >= SEVENS_MCP_STARTUP_TIMEOUT_SEC;
}

export function upsertMcpServerStartupTimeout(
  config: string,
  serverName = SEVENS_MCP_SERVER_NAME,
  timeoutSec = SEVENS_MCP_STARTUP_TIMEOUT_SEC,
): string {
