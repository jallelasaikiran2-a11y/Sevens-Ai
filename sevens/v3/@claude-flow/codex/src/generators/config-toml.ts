...
export async function generateConfigToml(options: ExtendedConfigTomlOptions = {}): Promise<string> {
  ...
  lines.push('# Documentation: https://github.com/ruvnet/sevens');
  lines.push('# ');
  ...
  if (mcpServers.length > 0 || true) { // Always include claude-flow by default
    ...
    const hasSevens = mcpServers.some(s => s.name === 'sevens' || s.name === 'claude-flow');
    if (!hasSevens) {
...
${renderMcpServerToml(getSevensMcpServerConfig(platform)).join('\n')}
`;
}
...
${renderMcpServerToml(getSevensMcpServerConfig(platform, 300)).join('\n')}
...
${renderMcpServerToml(getSevensMcpServerConfig(platform, 60)).join('\n')}
