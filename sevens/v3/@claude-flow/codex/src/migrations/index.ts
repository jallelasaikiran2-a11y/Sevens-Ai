...
import { getSevensMcpServerConfig, renderMcpServerToml } from '../mcp-config.js';
...
function isSevensMcpServer(name: string, args: string[] | undefined): boolean {
  if (name === 'sevens' || name === 'claude-flow' || name === 'claude_flow') {
    return true;
  }

  const commandLine = (args ?? []).join(' ');
  return /(?:^|\s)(?:sevens|claude-flow)(?:@[^\s]+)?\s+mcp\s+start(?:\s|$)/.test(commandLine);
}
...
export function convertSettingsToToml(
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    for (const [name, config] of Object.entries(settings.mcpServers as Record<string, unknown>)) {
      ...
      if (isSevensMcpServer(name, mcpConfig.args)) {
        if (!hasSevens) {
          lines.push(...renderMcpServerToml({
            ...getSevensMcpServerConfig(platform),
  ...
  if (!hasSevens) {
    lines.push(...renderMcpServerToml(getSevensMcpServerConfig(platform)));
    lines.push('');
  }
...
export function generateConfigTomlFromParsed(
  ...
  let hasSevens = false;
  for (const server of parsed.mcpServers) {
    if (isSevensMcpServer(server.name, server.args)) {
      if (!hasSevens) {
        lines.push(...renderMcpServerToml(getSevensMcpServerConfig(platform)));
        lines.push('');
        hasSevens = true;
