...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: sevens-bot');
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx sevens swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# Documentation: https://github.com/ruvnet/sevens');
    });
    ...
    it('should include default sevens MCP server', async () => {
      const result = await generateConfigToml({ platform: 'linux' });

      expect(result).toContain('[mcp_servers.sevens]');
      expect(result).toContain('command = "npx"');
      expect(result).toContain('args = ["-y", "sevens@latest", "mcp", "start"]');
...
describe('generateCIConfigToml', () => {
  ...
  it('should generate a Windows-safe Sevens MCP command with startup headroom', async () => {
    const result = await generateCIConfigToml('win32');

    expect(result).toContain('command = "cmd"');
    expect(result).toContain('args = ["/c", "npx", "-y", "sevens@latest", "mcp", "start"]');
