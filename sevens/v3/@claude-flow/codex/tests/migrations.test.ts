...
describe('convertSettingsToToml', () => {
  ...
  it('adds Sevens alongside unrelated custom MCP servers', () => {
    const result = convertSettingsToToml({
  ...
  it('should add default sevens server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default sevens server when none specified
    expect(result).toContain('[mcp_servers.sevens]');
