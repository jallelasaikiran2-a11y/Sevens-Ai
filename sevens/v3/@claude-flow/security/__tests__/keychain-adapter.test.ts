...
describe('createKeychainAdapter', () => {
  ...
  it('round-trips a canary secret through whatever backend was selected', async () => {
    const adapter = await createKeychainAdapter();
    const account = `sevens-test-${Date.now()}`;
    await adapter.setSecret('sevens-security-test', account, 'round-trip-value');
    try {
      expect(await adapter.getSecret('sevens-security-test', account)).toBe('round-trip-value');
    } finally {
      await adapter.deleteSecret('sevens-security-test', account);
    }
  });
});
