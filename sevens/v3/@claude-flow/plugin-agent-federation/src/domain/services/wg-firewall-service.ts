 ...
 *   - `pf` for macOS — `pfctl -a sevens-fed -f <file>`, anchor-scoped so
 *     sevens's rules don't collide with the operator's main pf ruleset.
 *
...
export interface WgFirewallServiceConfig {
  ...
  /** WG interface name to scope rules to. Defaults to `sevens-fed`. */
  readonly interfaceName?: string;
  /** Where the operator should write the rule file. Used for the load-command string. */
  readonly rulePath?: string;
  /** pf anchor (macOS only). Defaults to `sevens-fed`. */
...
const DEFAULT_INTERFACE = 'sevens-fed';
const DEFAULT_PF_ANCHOR = 'sevens-fed';
...
export class WgFirewallService {
  ...
  private renderNftables(projections: ReadonlyArray<{
    ...
    lines.push('# Generated by sevens federation plugin — ADR-111 Phase 4.');
    ...
    lines.push('');
    lines.push('table inet sevens_fed {');
