 ...
 * Maps `sevens-federation <subcommand> [args]` -> CLICommandDefinition handlers.
 *
...
function printHelp(commandNames: string[]): void {
  console.log('sevens-federation — Cross-installation agent federation CLI');
  console.log('');
  console.log('Usage: sevens-federation <subcommand> [options]');
  console.log('');
  ...
  console.log('');
  console.log('Run "sevens-federation <subcommand> --help" for command-specific options.');
}
...
async function main(): Promise<void> {
  ...
  if (!resolved) {
    ...
    console.error('Run "sevens-federation --help" to list available subcommands.');
    process.exitCode = 1;
    return;
  }
