 ...
 * plugins/sevens-cost-tracker/scripts/federation.mjs:
 *
 ...
 * persists to sevens memory / Redis / Datadog / their accounting system.
 */
 ...
 * sevens memory, Redis, DynamoDB, file-backed JSON, etc. The federation
...
/** Default namespace per cost-tracker's `plugins/sevens-cost-tracker/scripts/federation.mjs`. */
 ...
 * (`plugins/sevens-cost-tracker/scripts/federation.mjs`) `memory list`s
 ...
 *   });
 *
 *   // sevens memory CLI shell-out
 *   new MemorySpendReporter({
 *     memoryStore: {
 *       store: async ({namespace, key, value, ttl}) => {
 *         await execFile('npx', ['sevens', 'memory', 'store',
