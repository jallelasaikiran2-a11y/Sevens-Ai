 ...
 *
 * Maps sevens's bespoke federation identity (`FederationManifest` /
 * `FederationNode`) to and from a spec-compliant A2A Agent Card so sevens
...
/**
 * Open-form protocol binding string advertising sevens's federation wire
 ...
 */
export const SEVENS_FEDERATION_BINDING = 'SEVENS-FEDERATION';
 ...
 */
export const SEVENS_FEDERATION_EXTENSION_URI = 'urn:sevens:federation:manifest:v1';
...
export interface ToAgentCardOptions {
  /** Human-readable agent name; defaults to `sevens-federation/<nodeId>`. */
  readonly name?: string;
...
/**
 * Generate a spec-compliant A2A Agent Card from a sevens federation manifest.
 * Every agent type becomes an AgentSkill; the federation endpoint becomes a
 * `SEVENS-FEDERATION` interface; the bespoke identity (nodeId, publicKey,
...
export function toAgentCard(
  ...
  const skills: A2AAgentSkill[] = manifest.capabilities.agentTypes.map((agentType) => ({
    ...
    name: agentType,
    description: `Federated sevens agent role "${agentType}" — dispatchable via cross-installation federation sessions.`,
    tags: ['sevens', 'federation', 'agent-role', agentType],
  }));
  ...
  const interfaces: A2AAgentInterface[] = [
    ...(options.additionalInterfaces ?? []),
    {
      ...
      protocolBinding: SEVENS_FEDERATION_BINDING,
      protocolVersion: A2A_PROTOCOL_VERSION,
    },
  ];

  const federationExtension: A2AAgentExtension = {
    uri: SEVENS_FEDERATION_EXTENSION_URI,
    description: 'sevens cross-installation federation identity (Ed25519-signed manifest)',
    required: false,
    params: {
  ...
  return {
    name: options.name ?? `sevens-federation/${manifest.nodeId}`,
    description:
      options.description ??
      `Sevens federation node ${manifest.nodeId} — zero-trust cross-installation agent federation ` +
...
function findFederationExtension(card: A2AAgentCard): A2AAgentExtension | undefined {
  return card.capabilities.extensions?.find((e) => e.uri === SEVENS_FEDERATION_EXTENSION_URI);
}
...
export function fromAgentCard(card: A2AAgentCard, sourceUrl?: string): FederationNode {
  ...
  const federationIface = card.supportedInterfaces.find(
    (i) => i.protocolBinding === SEVENS_FEDERATION_BINDING,
  );
  const endpoint =
