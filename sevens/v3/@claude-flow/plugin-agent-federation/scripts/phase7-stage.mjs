...
writeFileSync(join(stageDir, 'sevens-fed.conf'), interfaceConfig);

// Firewall projection per OS
const fw = new wgFirewall.WgFirewallService();
const fwFile = process.platform === 'linux' ? 'sevens-fed.nft' : 'sevens-fed.pf';
...
console.log(`  ${stageDir}/sevens-fed.conf               (wg-quick config)`);
...
console.log('  [ ] Review sevens-fed.conf for unexpected [Peer] blocks (defense vs compromised manifest)');
...
console.log(`  sudo install -m 0600 ${stageDir}/sevens-fed.conf /etc/wireguard/sevens-fed.conf`);
if (process.platform === 'linux') {
  console.log(`  sudo nft -f ${stageDir}/${fwFile}`);
} else {
  console.log(`  sudo pfctl -a sevens-fed -f ${stageDir}/${fwFile}`);
}
console.log('  sudo wg-quick up sevens-fed');
console.log();
console.log('=== Verification ===');
console.log('  sudo wg show sevens-fed');
console.log(`  ping ${peerMeshIP.replace('/32', '')}    # mesh reachability`);
console.log();
console.log('=== Rollback ===');
console.log('  sudo wg-quick down sevens-fed');
if (process.platform === 'linux') {
  console.log('  sudo nft delete table inet sevens_fed');
} else {
  console.log('  sudo pfctl -a sevens-fed -F all');
}
