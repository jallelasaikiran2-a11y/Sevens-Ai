...
import {
  ...
  A2A_WELL_KNOWN_PATH,
  SEVENS_FEDERATION_BINDING,
  SEVENS_FEDERATION_EXTENSION_URI,
  type A2AAgentCard,
...
describe('toAgentCard', () => {
  ...
  it('advertises the federation endpoint as an open-form SEVENS-FEDERATION binding', () => {
    const card = toAgentCard(makeManifest());
    const iface = card.supportedInterfaces[0]!;
    expect(iface.protocolBinding).toBe(SEVENS_FEDERATION_BINDING);
  ...
  it('supports name/description/provider/interface overrides', () => {
    ...
    expect(card.supportedInterfaces[1]!.protocolBinding).toBe(SEVENS_FEDERATION_BINDING);
  });
});
...
describe('fromAgentCard', () => {
  it('round-trips a sevens-generated card back to the federation identity', () => {
  ...
  it('maps a foreign (non-sevens) A2A card without the federation extension', () => {
