...
function warnMemoryUnavailable() {
  ...
  const line2 = '             Fix: npm i -D @claude-flow/memory   (or re-run: npx sevens@latest init, then npx sevens@latest doctor --fix)';
  console.log(`${YELLOW}${line1}${RESET}`);
  console.log(`${YELLOW}${line2}${RESET}`);
  process.stderr.write(`${line1}\n${line2}\n`);
}

const DEBUG = !!(process.env.SEVENS_DEBUG || process.env.DEBUG);
...
async function loadMemoryPackage() {
  ...
  // documented `npx sevens` path @claude-flow/memory (an optionalDependency of
...
async function doStatus() {
  ...
  console.log(`  Resolver:       ${hasSidecar ? '✅ .claude-flow/memory-package.json' : '⏸ no sidecar (run: npx sevens@latest doctor --fix)'}`);
...
// reason under SEVENS_DEBUG/DEBUG so genuine async bugs aren't silently hidden
