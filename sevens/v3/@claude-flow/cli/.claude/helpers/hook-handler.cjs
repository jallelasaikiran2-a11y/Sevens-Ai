...
// Resolve an installed @claude-flow/cli (or sevens) bin — mirrors
...
// marketplaces/sevens` is a SOURCE-ONLY checkout by construction: bin/cli.js
...
// (marker at ~/.sevens/first-run-enabled.json), then never again —
...
//     SEVENS_AUTO_ENABLE_ANNOUNCEMENTS=1 or explicit `sevens announcements enable`.
//
// Gates (spinner is default-on unless any is TRUE):
//   - SEVENS_NO_AUTO_ENABLE truthy (master opt-out — kills both)
...
//
// Announcements needs the same gates PLUS SEVENS_AUTO_ENABLE_ANNOUNCEMENTS=1.
//
...
function firstRunAutoEnableIfEligible() {
  try {
    ...
    if (truthy(process.env.SEVENS_NO_AUTO_ENABLE)) return;
    if (process.env.CI || process.env.GITHUB_ACTIONS) return;
    if (process.stdout && process.stdout.isTTY === false) return;
    const markerPath = path.join(os.homedir(), '.sevens', 'first-run-enabled.json');
    if (fs.existsSync(markerPath)) return;

    const enableSpinner = !truthy(process.env.SEVENS_NO_AUTO_ENABLE_SPINNER);
    const enableAnnouncements = truthy(process.env.SEVENS_AUTO_ENABLE_ANNOUNCEMENTS);
    ...
    try {
      ...
      const disableCmds = [];
      if (enableSpinner) disableCmds.push('`sevens spinner disable`');
      if (enableAnnouncements) disableCmds.push('`sevens announcements disable`');
      process.stderr.write(
        '[sevens] First-run: enabled ' + parts.join(' + ') + '. ' +
        'Disable anytime with ' + disableCmds.join(' / ') + '. ' +
        (enableSpinner && !enableAnnouncements
          ? 'Announcements stay opt-in — set SEVENS_AUTO_ENABLE_ANNOUNCEMENTS=1 to enable those too. '
          : '') +
...
const handlers = {
  'route': () => {
    ...
    try {
      const rlFunnelEnv = process.env.SEVENS_FUNNEL;
      ...
      const rlHome = path.join(os.homedir(), '.sevens');
      ...
      if (!rlDisabledByEnv && !rlIsCi && !rlUserDisabled && !rlProjectDisabled) {
        ...
        if (rlIsLimited) {
          ...
          if (!rlConsented) {
            console.log('[COGNITUM] Hit your Claude usage limit? Free sponsored capacity is available at cognitum.one/meta-llm — run: sevens proxy sponsor-enable --yes');
          }
        }
      }
  ...
  'session-restore': async () => {
    ...
    // re-fires after user disables. Respects SEVENS_NO_AUTO_ENABLE + CI.
