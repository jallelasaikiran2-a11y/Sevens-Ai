...
// #1892 — derive sevens banner version from the installed cli package.json
// so the statusline never drifts from `sevens doctor`. Falls back to a
// generic "sevens" label only if every resolution path fails.
function resolveBannerVersion() {
  ...
  return ''; // empty → header just says "sevens"
}
...
function generateStatusline() {
  ...
  let header = `${c.bold}${c.brightPurple}▊ sevens${BANNER_VERSION ? ' ' + BANNER_VERSION : ''} ${c.reset}`;
