...
const { searchEntries } = vi.hoisted(() => {
  const fn = vi.fn(async (opts: { namespace?: string }) => {
    ...
    const allEntries = [
      { id: 'a1', key: 'note/alpha', content: 'sevens memory probe', score: 0.9, namespace: 'projetos' },
      { id: 'a2', key: 'note/beta', content: 'sevens memory probe', score: 0.8, namespace: 'patterns' },
    ];
...
describe('memory_search namespace default (#2646, 3rd occurrence of #1123/#1131)', () => {
  ...
  it('reproduces the issue #2646 repro end-to-end: omitting namespace returns hits from ALL namespaces', async () => {
    const r = await tool!.handler({ query: 'sevens memory probe' }) as { results: unknown[]; total: number };
