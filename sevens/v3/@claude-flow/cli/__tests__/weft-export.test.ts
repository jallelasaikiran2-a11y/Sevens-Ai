 ...
 *
 * Covers:
 *   1. Archive-builder: sevens run records → DarwinTrajectory[] mapping + the
...
describe('buildArchiveFromRecords', () => {
  it('maps sevens run records to DarwinTrajectory[] preserving the contract fields', () => {
...
describe('buildRemoteTrainInvocation (pure command construction)', () => {
  it('builds correct ssh/rsync/ruvllm argv for host + model + paths', () => {
    const plan = buildRemoteTrainInvocation({
      ...
      runId: 'run42', adapterPrefix: 'sevens-weft',
    });
    ...
    expect(sftStep.argv.join(' ')).toContain('sevens-weft-run42-sft');
    // ruvllm dpo initializes from the sft adapter (on-policy)
    const dpoStep = plan.steps.find((s) => s.label.startsWith('ruvllm dpo'))!;
    expect(dpoStep.argv.join(' ')).toContain('ruvllm microlora dpo --base Qwen/Qwen2.5-Coder-7B-Instruct --init-from sevens-weft-run42-sft');
    // adapter fetched back into the local dir
    const back = plan.steps.find((s) => s.label === 'rsync adapter back')!;
    expect(back.argv[back.argv.length - 1]).toContain('/local/adapters/sevens-weft-run42-dpo');
