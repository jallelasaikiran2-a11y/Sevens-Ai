 ...
 *
 *   SEVENS_STATUSLINE_COST_SYMBOL  override the leading '$' ('' => number alone)
 *   SEVENS_STATUSLINE_HIDE_COST    1/true/yes/on => omit the segment
 *
...
function renderHeader(env: Record<string, string> = {}): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'sevens-statusline-'));
...
describe('statusline cost display — generator contract', () => {
  it('reads both env vars and keeps "$" as the default', () => {
    expect(SCRIPT).toContain('SEVENS_STATUSLINE_COST_SYMBOL');
    expect(SCRIPT).toContain('SEVENS_STATUSLINE_HIDE_COST');
    // Default must be the dollar sign (?? '$') so existing setups are unchanged.
    expect(SCRIPT).toContain("process.env.SEVENS_STATUSLINE_COST_SYMBOL ?? '$'");
  });
...
describe('statusline cost display — runtime behavior', () => {
  ...
  it('replaces the symbol when SEVENS_STATUSLINE_COST_SYMBOL is set', () => {
    const header = renderHeader({ SEVENS_STATUSLINE_COST_SYMBOL: '⚡' });
    expect(header).toContain('⚡1.30');
    expect(header).not.toContain('$1.30');
  });

  it('omits the segment when SEVENS_STATUSLINE_HIDE_COST is truthy', () => {
    const header = renderHeader({ SEVENS_STATUSLINE_HIDE_COST: '1' });
    expect(header).not.toContain('1.30');
  });
...
describe('statusline trailing newline — breathing room before the input prompt', () => {
  ...
  it('the rendered statusline output ends with a real newline character', () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'sevens-statusline-nl-'));
    const scriptPath = path.join(dir, 'statusline.cjs');
    writeFileSync(scriptPath, SCRIPT, 'utf-8');
    const payload = JSON.stringify({
      model: { display_name: 'Opus 4.8' },
...
describe('getPkgVersion() — highest candidate wins, not first-found', () => {
  ...
  it('renders the HIGHER of two real candidate package.json versions', () => {
    const home = mkdtempSync(path.join(tmpdir(), 'sevens-statusline-ver-home-'));
    const cwd = mkdtempSync(path.join(tmpdir(), 'sevens-statusline-ver-cwd-'));
    const scriptPath = path.join(cwd, 'statusline.cjs');
    writeFileSync(scriptPath, SCRIPT, 'utf-8');
    try {
      // Marketplace candidate (checked first) — deliberately the STALE one.
      const marketplaceDir = path.join(home, '.claude', 'plugins', 'marketplaces', 'sevens');
