...
const CLEAN_ENV_KEYS = [
  'SEVENS_FUNNEL', 'SEVENS_ENTERPRISE_POLICY', 'CI', 'GITHUB_ACTIONS', 'GITLAB_CI',
...
beforeEach(() => {
  ...
  process.env.SEVENS_STATE_DIR = stateDir;
...
const TEST_DISCLOSURE_POOL = [
  { id: 'disclosure-1', schemaVersion: 1, class: 'disclosure', text: '✨ Tips, features and Cognitum updates here · manage: sevens settings', url: 'https://cognitum.one/sevens' },
  { id: 'disclosure-2', schemaVersion: 1, class: 'disclosure', text: '✨ Additional AI capabilities from Cognitum · manage: sevens settings', url: 'https://cognitum.one/sevens' },
  { id: 'disclosure-3', schemaVersion: 1, class: 'disclosure', text: '✨ Tips and Cognitum updates appear here · manage: sevens settings', url: 'https://cognitum.one/sevens' },
];
...
describe('message content boundaries (ADR-301)', () => {
  ...
  it('URL allowlist: exact hosts only, https only, no lookalikes', () => {
    ...
    expect(isAllowedUrl('https://github.com/ruvnet/sevens')).toBe(true);
  ...
  it('a disclosure-class message without the manage tail is rejected, never repaired', () => {
    ...
    expect(isValidMessage({ ...base, text: '✨ Has it · manage: sevens settings' })).toBe(true);
  });
...
describe('control precedence (ADR-305)', () => {
  ...
  it('SEVENS_FUNNEL=0 disables at the top of the chain', () => {
    for (const v of ['0', 'false', 'off', 'no', 'FALSE']) {
      expect(resolveFunnelEnabled(stateDir, { ...process.env, SEVENS_FUNNEL: v }).decidedBy).toBe('env');
    }
  });
  ...
  it('enterprise policy disables below env', () => {
    ...
    const decision = resolveFunnelEnabled(stateDir, { ...process.env, SEVENS_ENTERPRISE_POLICY: policyFile });
  ...
  it('a lower-precedence source never re-enables a higher-precedence disable', () => {
    ...
    const decision = resolveFunnelEnabled(stateDir, { ...process.env, SEVENS_FUNNEL: '0' });
...
describe('promo orchestrator (getFunnelPromo)', () => {
  ...
  it('renders nothing when disabled by any precedence source', () => {
    expect(getFunnelPromo({ interactive: true, env: { ...process.env, SEVENS_FUNNEL: '0' } })).toBeNull();
  });
  ...
  it('first interactive render is the disclosure, never a promotion', () => {
    ...
    // real cognitum.one/sevens target rides in the `to` query param.
    ...
    expect(parsed.origin + parsed.pathname).toBe('https://cognitum.one/sevens');
    expect(parsed.searchParams.get('utm_source')).toBe('sevens');
...
describe('credit-error classifier (ADR-303, fail-closed)', () => {
  ...
  it('recovery screen distinguishes signed-in vs signed-out', () => {
    expect(renderCreditRecovery(false)).toContain('sevens auth login');
    expect(renderCreditRecovery(true)).toContain('sevens proxy enable');
  });
});
...
describe('enrollment gates (ADR-302)', () => {
  it('never offers in CI, with --no-signup, or when funnel is disabled', () => {
    ...
    expect(
      shouldOfferEnrollment({ noSignup: false, cwd: stateDir, env: { ...process.env, SEVENS_FUNNEL: '0' } }),
    ).toBe(false);
  });
...
describe('generated statusline promo row', () => {
  ...
  it('embeds the promo renderer with CI and SEVENS_FUNNEL gates', () => {
    expect(script).toContain('getPromoRow');
    expect(script).toContain('process.env.CI');
    expect(script).toContain('SEVENS_FUNNEL');
  });
  ...
  it('validates a resolved CLI bin candidate actually has a compiled dist, not just a bin/cli.js on disk', () => {
    ...
    // with no build step, so ~/.claude/plugins/marketplaces/sevens is a
...
describe('attributionUrl (ADR-305 measurement, no runtime network)', () => {
  ...
  it('appends UTM params and preserves any query already on the base URL', () => {
    const out = attributionUrl('https://cognitum.one/sevens?foo=1', {
  ...
  it('does NOT append fid when telemetry consent is absent (privacy default)', () => {
    ...
    const out = attributionUrl('https://cognitum.one/sevens', {
  ...
  it('emits no network call — attribution is a pure link builder', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      attributionUrl('https://cognitum.one/sevens', {
...
describe('getFunnelPromo — API-down fallback discipline', () => {
  it('generated statusline styles the label as a link and the command as bold-not-underlined', () => {
    ...
    // supported.
    //
    // "manage: sevens settings" is a shell command, not a URL -- a terminal can
    ...
    // underlined or OSC-8-wrapped. Instead "sevens settings" renders bold so it
  ...
  it('generated statusline emits exactly 3 lines: header, ops, promo', () => {
    ...
    // line. The 3-line design puts sevens header on line 1, then ops, then
...
describe('credit-notifier (ADR-303 out-of-band signal)', () => {
  ...
  it('creditExhaustedNotice renders humanized "since" copy', () => {
    ...
    expect(notice).toContain('sevens funnel signup');
...
describe('rate-limit notifier (ADR-312 Phase 0 — manual, self-reported)', () => {
  ...
  it('rateLimitNotice humanizes age and points at the sponsor command', () => {
    ...
    expect(notice).toContain('sevens proxy sponsor-enable');
  });
});
...
describe('power-saver notifier (ADR-314 §A — manual, self-reported, mirrors rate-limit)', () => {
  ...
  it('quotaLowNotice humanizes age and points at power-saver-disable', () => {
    ...
    expect(notice).toContain('5m ago');
    expect(notice).toContain('sevens proxy power-saver-disable');
  });
...
describe('sponsored-downtime priority override (ADR-313)', () => {
  ...
  it('shows the enable-CTA when rate-limited without sponsored consent', () => {
    ...
    expect(row!.text).toContain('manage: sevens proxy sponsor-enable');
  ...
  it('shows the active-status line when rate-limited WITH sponsored consent granted', () => {
    ...
    expect(row!.text).toContain('manage: sevens proxy sponsor-disable');
    expect(displayWidth(row!.text)).toBeLessThanOrEqual(MAX_MESSAGE_COLUMNS);
  });
