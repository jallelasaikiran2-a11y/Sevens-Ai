 ...
 *
 * All three bugs are the same shape: the rename from `claude-flow` to `sevens`
...
describe('#2369 — stale MCP key handling in init', () => {
  ...
  it('detectExistingSevensMCP recognises the legacy `claude-flow@alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'claude-flow@alpha'\s+in\s+servers/);
  });

  it('detectExistingSevensMCP recognises the legacy `claude-flow@v3alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'claude-flow@v3alpha'\s+in\s+servers/);
  });

  it('detectExistingSevensMCP also recognises legacy keys in project-scoped registrations', () => {
...
describe('#2370 — swarm.ts MCP-down hint references the current package', () => {
  ...
  it('points users at `sevens@latest` with the `-y` flag (forces fresh fetch)', () => {
    expect(swarmSrc).toMatch(/npx -y sevens@latest/);
  });

  it('uses the `--` separator before the npx invocation (avoids claude-mcp flag ambiguity)', () => {
    expect(swarmSrc).toMatch(/claude mcp add claude-flow -- npx -y sevens@latest/);
  });
});
...
describe('#2371 — ContainerWorkerPool spawns workers with the current package', () => {
  ...
  it('spawns workers via `sevens@latest` with `-y` so npx never falls back to a stale local install', () => {
    ...
    expect(poolSrc).toMatch(/['"]npx['"]\s*,\s*['"]-y['"]\s*,\s*['"]sevens@latest['"]/);
  });
});
...
describe('Sanity — no other runtime source references the deprecated dist-tags', () => {
  ...
  it('no runtime .ts/.js code references `claude-flow@v3alpha` (comments and legacy-key recognition lists are OK)', () => {
    ...
    for (const file of walk(srcRoot)) {
      ...
      // (a) the legacy-key recognition list in detectExistingSevensMCP
