/** Bare `sevens proxy` should be an actionable lifecycle dashboard, not the
...
describe('bare proxy console guidance', () => {
  it('tells a user with an installed but stopped proxy exactly how to start it', async () => {
    ...
    expect(lines).toContain('npx sevens@latest proxy start --service');
    expect(lines).toContain('npx sevens@latest auth login');
  ...
  it('gives installation guidance before the proxy exists', async () => {
    ...
    expect(lines).toContain('npx sevens@latest proxy install --yes');
