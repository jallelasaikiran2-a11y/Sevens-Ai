 ...
 * shipped manifest is both signed by sevens's key and matches the shipped
...
import {
  ...
  SEVENS_HELPERS_PUBKEY, HELPERS_MANIFEST_FILE,
...
describe('shipped helpers manifest — real signature + real content', () => {
  ...
  it('exists and verifies against the baked sevens public key', () => {
    expect(existsSync(manifestPath)).toBe(true);
    const trusted = verifyHelpersManifest(readFileSync(manifestPath, 'utf-8'), SEVENS_HELPERS_PUBKEY);
