 ...
 *
 * Validates the sevens Docker-based deployment stack without running Docker.
...
const SEVENS_DIR = join(ROOT, 'sevens');
const COMPOSE_PATH = join(SEVENS_DIR, 'docker-compose.yml');
const NGINX_CONF_PATH = join(SEVENS_DIR, 'src', 'nginx', 'nginx.conf');
const NGINX_DOCKERFILE = join(SEVENS_DIR, 'src', 'nginx', 'Dockerfile');
const MCP_BRIDGE_INDEX = join(SEVENS_DIR, 'src', 'mcp-bridge', 'index.js');
const MCP_BRIDGE_DOCKERFILE = join(SEVENS_DIR, 'src', 'mcp-bridge', 'Dockerfile');
const CLI_DOCKERFILE = join(CLI_DIR, 'docker', 'Dockerfile');
const ENV_EXAMPLE = join(SEVENS_DIR, '.env.example');
...
describe('Nginx Configuration', () => {
  ...
  it('injects sevens welcome.js script via sub_filter', () => {
    expect(nginxContent).toMatch(/sub_filter\s+'<\/head>'\s+'<script src="\/sevens\/welcome\.js"/);
  });
  ...
  it('serves /sevens/ static assets from /etc/nginx/static/', () => {
    expect(nginxContent).toMatch(/location\s+\/sevens\//);
  });
...
describe('CLI Dockerfile (sevens:lite)', () => {
  let dockerContent: string;
  ...
  it('installs sevens globally in the build stage', () => {
    expect(dockerContent).toContain('npm install -g sevens@latest');
  });
  ...
  it('creates a non-root user (sevens)', () => {
    expect(dockerContent).toContain('adduser');
    expect(dockerContent).toContain('sevens');
    expect(dockerContent).toContain('USER sevens');
  });
  ...
  it('has a HEALTHCHECK using sevens doctor', () => {
    expect(dockerContent).toContain('HEALTHCHECK');
    expect(dockerContent).toContain('sevens doctor');
  });

  it('default CMD starts MCP server', () => {
    expect(dockerContent).toContain('CMD ["sevens", "mcp", "start"]');
  });
});
...
describe('Security Checks', () => {
  ...
  it('CLI Dockerfile runs as non-root', () => {
    ...
    expect(dockerfile).toContain('USER sevens');
  });
