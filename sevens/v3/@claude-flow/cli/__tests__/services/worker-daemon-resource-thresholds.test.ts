...
describe('WorkerDaemon resource thresholds', () => {
  ...
  // (SEVENS_DAEMON_TTL_SECS / SEVENS_DAEMON_IDLE_SECS) > built-in default.
  // =========================================================================
  describe('self-terminating lifecycle (ttl/idle)', () => {
    const TTL_ENV = 'SEVENS_DAEMON_TTL_SECS';
    const IDLE_ENV = 'SEVENS_DAEMON_IDLE_SECS';
    ...
    it('honors SEVENS_DAEMON_TTL_SECS env override (seconds → ms)', () => {
      process.env[TTL_ENV] = '3600';
      const config = new WorkerDaemon(tempDir).getStatus().config;
      expect(config.ttlMs).toBe(3600 * 1000);
    });

    it('honors an explicit SEVENS_DAEMON_TTL_SECS=0 as "disabled" (not falling back to default)', () => {
    ...
    it('honors SEVENS_DAEMON_IDLE_SECS env override', () => {
