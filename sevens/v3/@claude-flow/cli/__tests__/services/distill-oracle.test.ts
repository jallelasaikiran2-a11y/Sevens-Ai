...
describe('helpers', () => {
  ...
  describe('resolveRemote', () => {
    const saved = process.env.SEVENS_DISTILL_REMOTE;
    beforeEach(() => { delete process.env.SEVENS_DISTILL_REMOTE; });
    afterEach(() => { if (saved === undefined) delete process.env.SEVENS_DISTILL_REMOTE; else process.env.SEVENS_DISTILL_REMOTE = saved; });

    it('prefers explicit opt, then env, then null', () => {
      expect(resolveRemote('explicit')).toBe('explicit');
      process.env.SEVENS_DISTILL_REMOTE = 'from-env';
      expect(resolveRemote()).toBe('from-env');
      delete process.env.SEVENS_DISTILL_REMOTE;
