 ...
 *  - the SEVENS_DAEMON_NO_DISTILL opt-out skips distillation without crashing
...
describe.skipIf(!haveNative)('WorkerDaemon consolidate worker — ADR-174 M3 wiring', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.SEVENS_DAEMON_NO_DISTILL;
  });
  ...
  it('honors SEVENS_DAEMON_NO_DISTILL=1 and skips distillation entirely', async () => {
    seedDb(dbPath);
