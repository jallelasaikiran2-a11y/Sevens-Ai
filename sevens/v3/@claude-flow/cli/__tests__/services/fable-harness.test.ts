 ...
 *
 * One optional live smoke (SEVENS_FABLE_LIVE=1) a human can run is at the end.
 */
...
describe('FableHarness — command construction', () => {
  ...
  it('runs claude from a FRESH TEMP cwd, never the project dir', async () => {
    ...
    expect(calls[0].cwd).toContain('sevens-fable');
    expect(calls[0].cwd).not.toBe(process.cwd());
    // Temp dir lives under the OS temp root (allow macOS /private symlink drift).
    expect(calls[0].cwd.includes(tmpdir()) || calls[0].cwd.includes('sevens-fable')).toBe(true);
...
const LIVE = process.env.SEVENS_FABLE_LIVE === '1';
(LIVE ? describe : describe.skip)('FableHarness — LIVE smoke (SEVENS_FABLE_LIVE=1)', () => {
