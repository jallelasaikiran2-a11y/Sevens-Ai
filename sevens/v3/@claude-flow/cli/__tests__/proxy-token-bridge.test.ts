...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'sevens-token-bridge-'));
  process.env.SEVENS_STATE_DIR = dir;
});

afterEach(() => {
  clearSessionToken('default');
  delete process.env.SEVENS_STATE_DIR;
...
describe('proxy injected-token bridge', () => {
  it('atomically writes an access-token-only envelope and config path', async () => {
    ...
    expect(readFileSync(join(dir, 'proxy-config.toml'), 'utf8')).toContain('sevens_injected_token_path');
