 ...
 *          (not 'sevens') so all plugins resolve as mcp__claude-flow__*.
 * #2207 — detectExistingSevensMCP must accept both 'claude-flow' and 'sevens' keys;
 *          a bare .claude/settings.json must NOT be a false positive.
 * #2208 — writeClaudeMd (--force) must back up an existing CLAUDE.md to
 *          CLAUDE.md.pre-sevens before overwriting it.
 */
...
describe('#2206 — mcp-generator server key', () => {
  it('registers the sevens MCP server under the claude-flow key', () => {
  ...
  it('does NOT register the server under the bare sevens key', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    expect(config.mcpServers).not.toHaveProperty('sevens');
  });

  it('still invokes sevens@latest mcp start as the command args', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    const entry = config.mcpServers['claude-flow'] as { command: string; args: string[] };
    expect(entry.args).toContain('sevens@latest');
  ...
  it('generateMCPCommands uses claude-flow as the registration name', () => {
    ...
    // Every command that adds the main server must use 'claude-flow', not 'sevens'
    const mainCmd = cmds.find(c => c.includes('sevens@latest'));
    expect(mainCmd).toBeDefined();
    expect(mainCmd).toMatch(/claude mcp add claude-flow/);
    expect(mainCmd).not.toMatch(/claude mcp add sevens\b/);
  });
});

// ─── #2207: detectExistingSevensMCP — both keys + no false positive ───────────
...
// calls writeMCPConfig → detectExistingSevensMCP.
...
describe('#2207 — init detector accepts both server keys', () => {
  ...
  it('detects a prior install registered under the new claude-flow key', async () => {
    ...
    try {
      writeFileSync(
        ...
        JSON.stringify({ mcpServers: { 'claude-flow': { command: 'npx', args: ['-y', 'sevens@latest', 'mcp', 'start'] } } }),
      );
  ...
  it('detects a prior install registered under the legacy sevens key', async () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), 'wizard-2207-legacy-'));
    try {
      writeFileSync(
        path.join(parentDir, '.mcp.json'),
        JSON.stringify({ mcpServers: { sevens: { command: 'npx', args: ['-y', 'sevens@latest', 'mcp', 'start'] } } }),
      );
      const subDir = path.join(parentDir, 'sub');
      mkdirSync(subDir, { recursive: true });
  ...
  it('does NOT false-positive on a bare .claude/settings.json with no MCP registration', async () => {
    ...
    try {
      ...
      try {
        const result = await executeInit(opts);
        // No pre-existing sevens/claude-flow key → init must NOT skip .mcp.json
...
describe('#2208 — CLAUDE.md backup before overwrite', () => {
  ...
  it('backs up existing CLAUDE.md to CLAUDE.md.pre-sevens when --force is used', async () => {
    ...
    const backupPath = path.join(tmp, 'CLAUDE.md.pre-sevens');
    expect(existsSync(backupPath)).toBe(true);
    expect(readFileSync(backupPath, 'utf-8')).toBe(sentinel);
  });

  it('uses a timestamped backup name when CLAUDE.md.pre-sevens already exists', async () => {
    writeFileSync(path.join(tmp, 'CLAUDE.md'), '# First content\n', 'utf-8');
    writeFileSync(path.join(tmp, 'CLAUDE.md.pre-sevens'), '# Previous backup\n', 'utf-8');
    ...
    // The original .pre-sevens must still have its old content
    expect(readFileSync(path.join(tmp, 'CLAUDE.md.pre-sevens'), 'utf-8')).toBe('# Previous backup\n');
    // A timestamped backup must have been created
    const entries = readdirSync(tmp);
    const timestamped = entries.find(e => e.startsWith('CLAUDE.md.pre-sevens.') && e !== 'CLAUDE.md.pre-sevens');
  ...
  it('does NOT create a backup when no pre-existing CLAUDE.md exists', async () => {
    ...
    expect(existsSync(path.join(tmp, 'CLAUDE.md.pre-sevens'))).toBe(false);
