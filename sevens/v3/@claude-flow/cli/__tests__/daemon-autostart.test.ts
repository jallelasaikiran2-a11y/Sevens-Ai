...
describe('ensureDaemonRunning', () => {
  const saved = process.env.SEVENS_DAEMON_AUTOSTART;
  afterEach(() => { if (saved === undefined) delete process.env.SEVENS_DAEMON_AUTOSTART; else process.env.SEVENS_DAEMON_AUTOSTART = saved; });

  it('starts (spawns) when no daemon is alive in a sevens project', () => {
    delete process.env.SEVENS_DAEMON_AUTOSTART;
    const cwd = project();
    let spawned = 0;
  ...
  it('respects the opt-out (SEVENS_DAEMON_AUTOSTART=0)', () => {
    process.env.SEVENS_DAEMON_AUTOSTART = '0';
    let spawned = 0;
  ...
  it('does not spawn in a non-sevens directory', () => {
    delete process.env.SEVENS_DAEMON_AUTOSTART;
    const cwd = mkdtempSync(join(tmpdir(), 'not-sevens-'));
    let spawned = 0;
    const r = ensureDaemonRunning(cwd, { isAlive: () => false, spawnFn: () => { spawned++; } });
    expect(r.started).toBe(false);
    expect(spawned).toBe(0);
  ...
  it('respects a project-local claude-flow.config.json opt-out (survives env vars not propagating)', () => {
    ...
    // guard skips it outright), so `export SEVENS_DAEMON_AUTOSTART=0` in one
