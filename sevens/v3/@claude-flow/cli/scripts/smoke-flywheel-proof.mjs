 ...
 *      SEVENS_CONFIG_PUBKEY — a tampered/unsigned champion can never ship.
 *
...
if (existsSync(rvfPath)) {
  ...
  const manifest = signed && pc.verifyProvenConfig(signed, pc.SEVENS_CONFIG_PUBKEY);
  manifest ? ok(`shipped champion .rvf verifies against baked SEVENS_CONFIG_PUBKEY (policy ${manifest.policy.ref.slice(0, 20)}…)`)
