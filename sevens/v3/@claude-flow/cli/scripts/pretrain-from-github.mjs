#!/usr/bin/env node
// pretrain-from-github.mjs — pretrain sevens's self-learning system from its
...
const SEVENS_ROOT = resolve(SCRIPT_DIR, '../../../..');
const RUNS_DIR = join(SEVENS_ROOT, 'docs', 'benchmarks', 'runs');
...
// generalisation testing. Defaults preserve sevens behaviour.
const REPO_ROOT = process.env.REPO_ROOT ? resolve(process.env.REPO_ROOT) : SEVENS_ROOT;
const GH_REPO   = process.env.GH_REPO   || 'ruvnet/sevens';
...
async function main() {
  ...
  if (!process.env.BENCH_JSON) {
    console.log(`# Pretrain from sevens GitHub history`);
