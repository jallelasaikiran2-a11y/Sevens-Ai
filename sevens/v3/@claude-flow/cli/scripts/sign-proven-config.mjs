 ...
 *   1. GCP Secret Manager  — SEVENS_CONFIG_SIGNING_SECRET (default: sevens-config-signing-key)
 *                            + optional SEVENS_CONFIG_SIGNING_PROJECT
 *   2. SEVENS_CONFIG_SIGNING_KEY  — a local PEM path (air-gapped)
 *   3. ~/.sevens/config-signing.key  — dev default
 *
...
function loadPrivateKey() {
  const secret = process.env.SEVENS_CONFIG_SIGNING_SECRET || 'sevens-config-signing-key';
  if (process.env.SEVENS_CONFIG_SIGNING_SECRET || (!process.env.SEVENS_CONFIG_SIGNING_KEY && !existsSync(join(homedir(), '.sevens', 'config-signing.key')))) {
    const args = ['secrets', 'versions', 'access', 'latest', '--secret', secret];
    const project = process.env.SEVENS_CONFIG_SIGNING_PROJECT || 'ruv-dev';
  ...
  const keyPath = process.env.SEVENS_CONFIG_SIGNING_KEY || join(homedir(), '.sevens', 'config-signing.key');
