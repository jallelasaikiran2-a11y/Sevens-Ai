 ...
 * (SEVENS_HELPERS_PUBKEY).
 *
 * Private-key resolution (first that is set wins):
 *   1. GCP Secret Manager (PREFERRED for CI/publish):
 *        SEVENS_HELPERS_SIGNING_SECRET=<secret-name>   (e.g. sevens-helpers-signing-key)
 *        SEVENS_HELPERS_SIGNING_PROJECT=<gcp-project>  (optional; defaults to the
 *                                                       active gcloud project)
 *      Fetched via `gcloud secrets versions access latest`.
 *   2. SEVENS_HELPERS_SIGNING_KEY=<pem-file-path>       (local / air-gapped)
 *   3. ~/.sevens/helpers-signing.key                    (dev default)
 *
 * Usage:  SEVENS_HELPERS_SIGNING_SECRET=sevens-helpers-signing-key node scripts/sign-helpers.mjs
 */
...
function loadPrivateKey() {
  const secret = process.env.SEVENS_HELPERS_SIGNING_SECRET;
  if (secret) {
    const args = ['secrets', 'versions', 'access', 'latest', '--secret', secret];
    const project = process.env.SEVENS_HELPERS_SIGNING_PROJECT;
  ...
  const keyPath = process.env.SEVENS_HELPERS_SIGNING_KEY || join(homedir(), '.sevens', 'helpers-signing.key');
  if (!existsSync(keyPath)) {
    console.error(
      `[sign-helpers] no signing key. Set SEVENS_HELPERS_SIGNING_SECRET (GCP) ` +
      `or SEVENS_HELPERS_SIGNING_KEY (PEM path); tried ${keyPath}.`,
    );
    process.exit(1);
  }
