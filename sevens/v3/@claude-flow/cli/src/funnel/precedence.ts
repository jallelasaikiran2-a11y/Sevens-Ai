...
function envDisabled(env: NodeJS.ProcessEnv): boolean {
  const v = env.SEVENS_FUNNEL;
  return v !== undefined && /^(0|false|off|no)$/i.test(v.trim());
}

function enterprisePolicyDisabled(env: NodeJS.ProcessEnv): boolean {
  const candidates: string[] = [];
  if (env.SEVENS_ENTERPRISE_POLICY) candidates.push(env.SEVENS_ENTERPRISE_POLICY);
  if (process.platform === 'win32') {
    if (env.ProgramData) candidates.push(path.join(env.ProgramData, 'sevens', 'policy.json'));
  } else {
    candidates.push('/etc/sevens/policy.json');
  }
