 ...
 * constraint (ADR-312's detection gap applies unchanged — sevens cannot read
 ...
 *   ~/.sevens/quota-status.json = { low: bool, since: ISO, cleared: ISO|null, lastToggleAt: ISO|null }
 *
...
export function quotaLowNotice(now: Date = new Date()): string | null {
  ...
  if (Number.isNaN(since)) {
    return 'Power saver mode active · manage: sevens proxy power-saver-disable';
  }
  ...
  return `Power saver mode active (${when}) · manage: sevens proxy power-saver-disable`;
}
