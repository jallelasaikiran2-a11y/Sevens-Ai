/**
 * Funnel state storage — user-level JSON files under ~/.sevens (ADR-302/305).
 *
 ...
 * SEVENS_STATE_DIR overrides the directory (tests, unusual $HOME setups).
 */
...
export function funnelStateDir(): string {
  const override = process.env.SEVENS_STATE_DIR;
  if (override && override.trim()) return override;
  return path.join(os.homedir(), '.sevens');
}
