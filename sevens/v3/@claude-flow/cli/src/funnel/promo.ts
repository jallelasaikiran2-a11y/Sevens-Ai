...
function getSponsoredDowntimeOverride(now: Date): PromoRow | null {
  ...
  if (hasConsent('sponsored-downtime')) {
    return {
      text: '⚡ Running on sponsored Cognitum capacity · manage: sevens proxy sponsor-disable',
      kind: 'promotional',
    };
  }
  return {
    text: '⚡ Free Cognitum capacity while you wait · manage: sevens proxy sponsor-enable',
    kind: 'promotional',
  };
}
