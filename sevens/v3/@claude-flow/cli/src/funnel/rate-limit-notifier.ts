 ...
 * MANUAL, self-reported alternative: the user tells sevens they've hit a
 * limit, sevens remembers it, and the funnel promo row can react.
 *
 * State is cheap and durable:
 *   ~/.sevens/rate-limit-status.json = { limited: bool, since: ISO, cleared: ISO|null }
 *
...
/**
 * Clear rate-limited status. Called via `sevens settings notices
...
export function rateLimitNotice(now: Date = new Date()): string | null {
  ...
  if (Number.isNaN(since)) {
    return 'Claude usage limit flagged · run: sevens proxy sponsor-enable';
  }
  ...
  return `Claude usage limit flagged (${when}) · run: sevens proxy sponsor-enable`;
}
