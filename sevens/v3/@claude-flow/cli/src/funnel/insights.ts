 ...
 * populates ~/.sevens/flywheel-status.json (a future detached, session-start
...
function securityInsight(ctx: LocalInsightContext): LocalInsight | null {
  ...
  if (s.status === 'ISSUES' || findings > 0) {
    return {
      ...
      text: `⚠ ${findings} security finding${findings === 1 ? '' : 's'} — Review the latest sevens security scan`,
      priority: 90,
    };
  }
  if (s.status === 'PENDING') {
    return { id: 'insight-scan-pending', text: '🛡 Security scan pending — Run sevens security scan --depth full', priority: 70 };
  }
  return null;
}
...
/**
 * ADR-316 — a co-pilot tip from sevens's Fable Advisor Harness
