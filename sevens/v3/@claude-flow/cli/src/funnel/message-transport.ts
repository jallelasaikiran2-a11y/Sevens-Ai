 ...
 *   5. **Kill switch.** `SEVENS_FUNNEL_MESSAGES=0` (or `SEVENS_FUNNEL=0`)
...
export const DEFAULT_MESSAGES_ENDPOINT =
  process.env.SEVENS_FUNNEL_MESSAGES_ENDPOINT ?? 'https://funnel.ruv.io/v1/messages';
    ...
    const req = https.request(
      {
        ...
        method: 'GET',
        headers: {
          'User-Agent': 'sevens-funnel/messages',
...
function killSwitched(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  return kill(env.SEVENS_FUNNEL_MESSAGES) || kill(env.SEVENS_FUNNEL);
}
