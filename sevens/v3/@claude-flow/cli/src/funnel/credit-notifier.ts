 ...
 * user-visible notification in `~/.sevens/credit-status.json`.
 *
 ...
 *   ~/.sevens/credit-status.json = { exhausted: bool, since: ISO, cleared: ISO|null }
 *
 * Cleared automatically when the user opens the enrollment / signup
 * surface, or explicitly via `sevens funnel credit-clear`.
 */
 ...
 * runs `sevens funnel credit-clear`. `cleared` is stamped so the previous
...
export function creditExhaustedNotice(now: Date = new Date()): string | null {
  ...
  if (Number.isNaN(since)) {
    return 'Cognitum credits exhausted · run: sevens funnel signup';
  }
  ...
  return `Cognitum credits exhausted (${when}) · run: sevens funnel signup`;
}
