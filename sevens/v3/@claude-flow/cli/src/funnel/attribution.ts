...
export interface AttributionInput {
  ...
  content: string;
  /** Static UTM `source` — always 'sevens' but overridable for tests. */
  source?: string;
...
const CLICK_ENDPOINT_BASE =
  process.env.SEVENS_FUNNEL_CLICK_ENDPOINT ?? 'https://funnel.ruv.io/v1/click';
...
export function attributionUrl(url: string, input: AttributionInput): string {
  ...
  params.set('utm_source', input.source ?? 'sevens');
