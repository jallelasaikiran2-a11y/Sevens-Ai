 ...
 * validation split, caches it under ~/.cache/sevens/gaia/, and exposes
...
export interface LoadGaiaOptions {
  ...
  smokeOnly?: boolean;
  /** Override the cache directory (default: ~/.cache/sevens/gaia). */
  cacheDir?: string;
}
...
function defaultCacheDir(): string {
  return path.join(os.homedir(), '.cache', 'sevens', 'gaia');
}
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'sevens-gaia-loader/1.0',
        },
      },
      (res) => {
