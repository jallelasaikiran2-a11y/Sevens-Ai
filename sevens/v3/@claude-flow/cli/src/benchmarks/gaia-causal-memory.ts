 ...
 *
 * This is one of sevens's 6 architectural primitives distinguishing it
 * from HAL: HAL is stateless across runs; sevens accumulates causal memory.
 *
 * Storage: JSONL file at ~/.cache/sevens/gaia/causal-edges.jsonl
...
export interface CausalMemoryOptions {
   ...
   * Default: ~/.cache/sevens/gaia/causal-edges.jsonl
   */
  storePath?: string;
  /**
...
const DEFAULT_STORE_SUFFIX = path.join('.cache', 'sevens', 'gaia', 'causal-edges.jsonl');
const DEFAULT_MAX_EDGES = 5;

/** Resolve the JSONL store path from options, defaulting to ~/.cache/sevens/…  */
