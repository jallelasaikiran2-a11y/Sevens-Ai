 ...
 *
 * Creates a single binary containing kernel, runtime, Sevens CLI, models/keys,
...
const SEVENS_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'sevens' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      sevensVersion: options.sevensVersion || detectSevensVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'sevens',   raw: this.buildSevensSection(),   label: 'Sevens CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> sevens-init (PID 1)',
  ...
  private buildSevensSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack sevens@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'sevens', version: this.opts.sevensVersion,
      package: packageMeta ?? { name: 'sevens', version: this.opts.sevensVersion },
      commands: SEVENS_COMMANDS, commandCount: SEVENS_COMMANDS.length,
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'SEVENS_CMD="${SEVENS_CMD:-sevens}"',
        'echo "Running basic verification..."',
        '$SEVENS_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$SEVENS_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      entrypoint: '/opt/sevens/bin/cli.js',
    ...
    return {
      magic: 'RVFA', version: 1, name: 'sevens-appliance', appVersion: this.opts.sevensVersion,
...
function detectSevensVersion(): string {
  try {
