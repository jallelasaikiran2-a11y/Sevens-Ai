/**
 * RVFA Runner -- Boot and run self-contained Sevens appliances.
 *
...
export class RvfaRunner {
  ...
  /**
   * Run natively via Node.js: extract SEVENS section to a temp dir,
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const sevens = tryExtract(this.reader, 'sevens');
      if (!sevens) return fail('RVFA appliance does not contain a "sevens" section');

      const entryFile = join(workDir, 'sevens-bundle.js');
      await writeFile(entryFile, sevens);
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const sevens = tryExtract(this.reader, 'sevens');
      if (!sevens) return fail('RVFA appliance does not contain a "sevens" section');
      await writeFile(join(workDir, 'sevens-bundle.js'), sevens);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY sevens-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "sevens-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
