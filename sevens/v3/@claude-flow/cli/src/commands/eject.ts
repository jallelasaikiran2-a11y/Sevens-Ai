 ...
 *
 * Lifts the calling sevens/claude-flow project into a renamed standalone
 * harness using `metaharness --from-existing <dir>`. Attribution to
 * sevens is preserved via the upstream's `<!-- sevens-attribution-block -->`
 * convention.
 *
 *   npx sevens eject --name my-harness                  # dry-run plan
 *   npx sevens eject --name my-harness --confirm        # actually eject
 *   npx sevens eject --name my-harness --target /abs/out --confirm
 *
 ...
 *      is `/tmp/sevens-eject-<ts>-<name>/`. Writing to the calling repo
 *      is refused with exit 2 — protects the user from `cwd: $sevens`
 *      eject accidents.
 ...
 *   exits 0 with a structured "feature not available" message. Sevens
...
export const ejectCommand: Command = {
  name: 'eject',
  description:
    'Lift the calling sevens project into a renamed standalone harness via metaharness --from-existing (ADR-150 Phase 2). Dry-run by default; --confirm required to write.',
  options: [
    {
      name: 'name',
    ...
    {
      name: 'target',
      description: 'Absolute output dir (default: /tmp/sevens-eject-<ts>-<name>/); refused if inside the calling repo',
      type: 'string' as const,
    },
    {
      name: 'confirm',
  ...
  examples: [
    { command: 'npx sevens eject --name my-harness', description: 'Dry-run; prints what would happen' },
    {
      command: 'npx sevens eject --name my-harness --confirm',
      description: 'Eject to /tmp/sevens-eject-<ts>-my-harness/',
    },
    {
      command: 'npx sevens eject --name my-harness --target /abs/out --confirm',
  ...
  async action(context: CommandContext): Promise<CommandResult> {
    ...
    if (!opts.name) {
      ...
      output.writeln('');
      output.writeln('Example: npx sevens eject --name my-harness');
    ...
    const target = opts.target
      ...
      : resolvePath(tmpdir(), `sevens-eject-${Date.now()}-${opts.name}`);
    ...
    if (!opts.confirm) {
      if (opts.format === 'json') {
        ...
        output.writeln(output.bold('# sevens eject (dry-run)'));
        output.writeln('');
    ...
    // Actually run.
    output.writeln(output.bold('# sevens eject — running'));
    output.writeln('');
    ...
    if (r.degraded) {
      ...
      output.writeln(output.dim('(ADR-150 graceful degradation: sevens runs without it; install with `npm i -D metaharness`.)'));
