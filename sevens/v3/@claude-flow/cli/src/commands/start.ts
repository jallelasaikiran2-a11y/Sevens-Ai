...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('sevens is not initialized in this directory');
    output.printInfo('Run "sevens init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting sevens V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('sevens V3 is running!');
    output.writeln();
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the sevens system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping sevens'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop sevens?',
        default: false
      });
    ...
    try {
      ...
      output.writeln();
      output.printSuccess('sevens stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the sevens system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting sevens'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the sevens orchestration system',
