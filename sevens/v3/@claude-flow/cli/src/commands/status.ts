...
// missed projects that were initialized via `sevens memory init` (writes
...
// and a running MCP, yet `sevens status` reported "not initialized".
//
// Now: any of these signals counts as initialized:
//   - `.claude-flow/config.yaml`   (the canonical `sevens init` output)
//   - `.claude-flow/config.json`   (same, alt format)
//   - `.swarm/memory.db`           (the `sevens memory init` output)
...
function displayStatus(status: Awaited<ReturnType<typeof getSystemStatus>>): void {
  ...
  output.writeln(`${output.bold('sevens V3')} ${statusIcon}`);
  output.writeln();
