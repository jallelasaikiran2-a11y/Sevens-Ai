/**
 * `sevens advisor` — Fable co-pilot advisor tip in the statusline insight
 * ticker (ADR-316).
 *
...
const ADVISOR_DISCLOSURE = [
  ...
  '',
  'At most once per day, sevens will send a small STRUCTURAL snapshot of your',
  ...
  'refresh, at most once/day). Disable anytime: sevens advisor disable',
].join('\n');
...
const enableSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    if (!ctx.flags.yes) {
      output.writeln('Re-run with --yes to confirm: sevens advisor enable --yes');
    ...
    output.writeln('Disable anytime: sevens advisor disable');
...
export const advisorCommand: Command = {
  ...
  examples: [
    { command: 'sevens advisor enable --yes', description: 'Opt into the co-pilot advisor tip' },
    { command: 'sevens advisor status', description: 'Show consent state + current cached tip' },
    { command: 'sevens advisor disable', description: 'Revoke consent and stop generating tips' },
  ],
