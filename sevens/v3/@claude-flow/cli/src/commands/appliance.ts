...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained sevens.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'sevens.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'sevens.rvf';
    ...
    const steps = [
      ...
      'Packaging sevens CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'sevens'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'sevens', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'sevens appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'sevens appliance inspect -f sevens.rvf', description: 'Inspect appliance contents' },
    { command: 'sevens appliance verify -f sevens.rvf', description: 'Verify integrity' },
    { command: 'sevens appliance extract -f sevens.rvf', description: 'Extract sections' },
    { command: 'sevens appliance run -f sevens.rvf', description: 'Boot and run appliance' },
    { command: 'sevens appliance sign -f sevens.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'sevens appliance publish -f sevens.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'sevens appliance update -f sevens.rvf -s sevens -d ./new-sevens.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Sevens Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full Sevens platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained sevens.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "sevens appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
