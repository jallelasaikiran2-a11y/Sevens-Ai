...
const workerDispatchCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!trigger) {
      ...
      output.writeln(output.dim('Tip: `oia-audit` (ADR-150) also runs as `sevens metaharness oia-audit` for direct invocation.'));
      return { success: false, exitCode: 1 };
    }
...
const statuslineCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    function getUserInfo() {
      const identityMode = (process.env.SEVENS_STATUSLINE_IDENTITY || 'project').toLowerCase();
    ...
    // gates (SEVENS_FUNNEL, enterprise policy, config, CI, disclosure,
    ...
    // Generate lines
    let header = `${c.bold}${c.brightPurple}▊ sevens V3 ${c.reset}`;
