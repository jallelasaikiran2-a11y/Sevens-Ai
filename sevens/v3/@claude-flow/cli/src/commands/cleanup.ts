 ...
 * Removes project artifacts created by claude-flow/sevens
 *
 * Created with ruv.io
 */
...
/**
 * Sevens-owned subdirectories within .claude/ that are safe to delete.
 ...
 * See: https://github.com/ruvnet/sevens/issues/1557
 */
const CLAUDE_OWNED_SUBDIRS = [
  { path: join('.claude', 'helpers'), description: 'Sevens hook scripts' },
];

/**
 * Artifact directories and files that claude-flow/sevens may create
 */
const ARTIFACT_DIRS = [
...
export const cleanupCommand: Command = {
  name: 'cleanup',
  description: 'Remove project artifacts created by claude-flow/sevens',
  aliases: ['clean'],
  options: [
    {
      name: 'dry-run',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Scan sevens-owned subdirs within .claude/ (surgical — preserves Claude Code files)
    ...
    // Check if .claude/settings.json has sevens hooks/claudeFlow blocks to clean
    const settingsPath = join(cwd, '.claude', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.claude', 'settings.json'), description: 'Remove sevens hooks/claudeFlow blocks (preserves rest)', size: 0, type: 'file' });
    }
