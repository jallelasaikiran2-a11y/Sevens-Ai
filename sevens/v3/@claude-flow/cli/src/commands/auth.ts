/**
 * `sevens auth` — Cognitum identity (ADR-306).
 *
...
function refuseNonInteractive(hasTokenStdin: boolean): CommandResult | null {
  ...
  const message =
    'sevens auth login refuses to run interactively in a non-TTY/CI environment. ' +
...
const statusCommand: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    if (shown.length === 0) {
      const message = filterName ? `No such profile: ${filterName}` : 'Not logged in. Run: sevens auth login';
      if (ctx.flags.json) {
...
export const authCommand: Command = {
  ...
  examples: [
    { command: 'sevens auth login', description: 'Sign in via the browser PKCE flow' },
    { command: 'sevens auth login --no-browser', description: 'Sign in via the headless OOB copy-paste flow' },
    { command: 'sevens auth status', description: 'Show signed-in profile(s)' },
    { command: 'sevens auth status --check', description: 'Validate or silently refresh credentials now' },
    { command: 'sevens auth logout', description: 'Sign out of the default profile' },
  ],
