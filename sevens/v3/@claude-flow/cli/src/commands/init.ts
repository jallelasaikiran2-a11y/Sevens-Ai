...
async function offerCapabilityEnrollment(ctx: CommandContext): Promise<void> {
  try {
    ...
    if (accepted) {
      ...
      output.writeln(output.dim('  CLI sign-in (`sevens auth login`) ships with the ADR-306 auth release.'));
    } else {
...
// silent skip (auto-detect during a plain `sevens init`).
...
// Keep Codex out of the CLI dependency graph so cold `npx sevens --version`
...
// #2666-adjacent — quietly wire up Codex too when a plain `sevens init` (no
...
async function maybeAutoDetectCodex(
  ...
  try {
    ...
    if (!CodexInitializer) {
      ...
      output.writeln(output.dim('  npm install @claude-flow/codex && sevens init --codex'));
      return;
    }
...
// ruvnet/sevens --skill sevens --yes` so the *single* canonical sevens skill
// (SKILL.md at the ruvnet/sevens repo root — describes the platform + entry
...
// can run `npx skills add ruvnet/sevens --all` themselves. Best-effort — never
// fails init. Opt-out: --no-skills-sh flag OR SEVENS_NO_SKILLS_SH=1. Skipped
...
async function maybeInstallSkillsSh(ctx: CommandContext): Promise<void> {
  try {
    ...
    if (/^(1|true|on|yes)$/i.test(String(process.env.SEVENS_NO_SKILLS_SH || ''))) return;

    // Idempotency gate: if this project has already registered sevens with
    // skills.sh, don't re-clone the repo + re-fire a fresh install telemetry
    // event on every `sevens init --force` / `init upgrade` / etc. Each install
    // pings skills.sh's leaderboard AND clones the whole ruvnet/sevens repo
    ...
    const marker = nodePath.join(ctx.cwd, '.agents', 'skills', 'sevens');
    if (nodeFs.existsSync(marker)) {
      output.writeln();
      output.writeln(output.dim('  skills.sh registration already present at .agents/skills/sevens — skipping'));
      return;
    }
    ...
    output.writeln();
    output.printInfo('Registering the core `sevens` skill with skills.sh (cross-agent catalog)…');

    const { spawnSync } = await import('child_process');
    const result = spawnSync(
      npxCmd,
      ['--yes', 'skills', 'add', 'ruvnet/sevens', '--skill', 'sevens', '--yes'],
      { cwd: ctx.cwd, stdio: 'pipe', timeout: 60_000, windowsHide: true, encoding: 'utf-8' },
    );

    if (result.status === 0) {
      output.writeln(output.success('  ✓ sevens registered via skills.sh — the platform skill is available to any agent in this project'));
      output.writeln(output.dim('    Want all 267 plugin skills? npx skills add ruvnet/sevens --all'));
      output.writeln(output.dim('    Opt out next time: --no-skills-sh or SEVENS_NO_SKILLS_SH=1'));
    } else {
      ...
      output.writeln(output.dim('  skills.sh registration skipped (network or npx cache) — retry with: npx skills add ruvnet/sevens --skill sevens --yes'));
    }
  } catch {
...
async function initCodexAction(
  ...
  output.writeln();
  output.writeln(output.bold('Initializing sevens V3 for OpenAI Codex'));
  output.writeln();
...
// Check if project is already initialized with sevens.
// #2207: .claude/settings.json alone is NOT a sevens marker — it's created by
// Claude Code itself and exists in every Claude Code project. We require a
// sevens-specific signal: either a claudeFlow section in settings.json, OR a
// .mcp.json with a 'claude-flow' or 'sevens' server key, OR the sevens-only
...
function isInitialized(cwd: string): { claude: boolean; claudeFlow: boolean } {
  ...
  // Check .claude-flow/config.yaml — sevens-specific, always reliable
  const hasClaudeFlow = fs.existsSync(claudeFlowPath);

  // Check .claude/settings.json for sevens-specific content (claudeFlow section)
  let hasSevensSettings = false;
  ...
  // Check .mcp.json for sevens/claude-flow server key
  let hasSevensMcp = false;
    ...
    try {
      ...
      hasSevensMcp =
        ...
        ('claude-flow' in (parsed.mcpServers as Record<string, unknown>) ||
         'sevens' in (parsed.mcpServers as Record<string, unknown>));
    } catch { /* malformed — ignore */ }
  }

  return {
    claude: hasSevensSettings || hasSevensMcp,
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (hasExisting && !force) {
    output.printWarning('sevens appears to be already initialized');
  ...
  output.writeln();
  output.writeln(output.bold('Initializing sevens V3'));
  output.writeln();
  ...
  // #1744 — opt-out of the user-global ~/.claude/CLAUDE.md "Sevens Integration"
  ...
  try {
    ...
    spinner.succeed('sevens V3 initialized successfully!');
    output.writeln();
    ...
    if (!startDaemon && !startAll) {
      const bin = (process.argv[1] || '').includes('sevens') ? 'sevens' : 'claude-flow';
      output.writeln(output.bold('Next steps:'));
      output.printList([
...
const wizardCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('sevens V3 Setup Wizard'));
...
const checkCommand: Command = {
  name: 'check',
  description: 'Check if sevens is initialized',
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (result.initialized) {
      output.printSuccess('sevens is initialized');
      ...
      output.printWarning('sevens is not initialized in this directory');
      output.printInfo('Run "sevens init" to initialize');
    }
...
const upgradeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Upgrading sevens'));
...
export const initCommand: Command = {
  name: 'init',
  description: 'Initialize sevens in the current directory',
  ...
  options: [
    ...
    {
      name: 'no-global',
      description: 'Skip the ~/.claude/CLAUDE.md "Sevens Integration" pointer block (#1744)',
      type: 'boolean',
      default: false,
    },
    {
    ...
    {
      name: 'no-skills-sh',
      description: 'Skip the post-init `npx skills add ruvnet/sevens` registration (also honored via SEVENS_NO_SKILLS_SH=1)',
      type: 'boolean',
      default: false,
    },
    {
