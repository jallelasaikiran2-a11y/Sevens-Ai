 ...
 *
 * Run via: sevens verify [--branch <branch>] [--manifest <local-path>]
 *
...
const DEFAULT_MANIFEST_URL = 'https://raw.githubusercontent.com/ruvnet/sevens/{branch}/verification.md.json';
...
async function verifySignature(witness: Witness): Promise<{
  ...
  const seed = createHash('sha256').update(witness.manifest.gitCommit + ':sevens-witness/v1').digest();
...
export const verifyCommand: Command = {
  ...
  examples: [
    { command: 'sevens verify', description: 'Fetch latest manifest from main branch + verify' },
    { command: 'sevens verify --branch main', description: 'Verify against a specific branch' },
    { command: 'sevens verify --manifest ./verification.md.json', description: 'Use a local manifest copy' },
    { command: 'sevens verify --json', description: 'Machine-readable output for CI' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!asJson) {
      output.writeln();
      output.writeln(output.bold('Sevens Verification'));
