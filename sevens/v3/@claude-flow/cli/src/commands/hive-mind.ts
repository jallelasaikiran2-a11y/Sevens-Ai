...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__sevens__hive-mind_consensus    - Democratic decision making
   mcp__sevens__hive-mind_memory       - Share knowledge across the hive
   mcp__sevens__hive-mind_broadcast    - Broadcast to all workers
   mcp__sevens__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__sevens__hive-mind_status       - Monitor swarm health
   mcp__sevens__task_create            - Create and delegate tasks
   mcp__sevens__coordination_orchestrate - Orchestrate task distribution
   mcp__sevens__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__sevens__agent_list             - List all active agents
   mcp__sevens__agent_status           - Check agent status
   mcp__sevens__agent_health           - Check worker health
   mcp__sevens__hive-mind_join         - Add agent to hive
   mcp__sevens__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__sevens__task_assign            - Assign tasks to workers
   mcp__sevens__task_status            - Track task progress
   mcp__sevens__task_complete          - Mark tasks complete
   mcp__sevens__workflow_create        - Create workflows

5️⃣ **MEMORY & LEARNING**
   mcp__sevens__memory_store           - Store collective knowledge
   mcp__sevens__memory_retrieve        - Access shared memory
   mcp__sevens__memory_search          - Search memory patterns
   mcp__sevens__neural_train           - Learn from experiences
   mcp__sevens__hooks_intelligence_pattern-store - Store patterns
...
• You MUST use Sevens MCP tools (mcp__sevens__*) for ALL orchestration tasks
• Do NOT use Claude native Task/Agent tools for swarm coordination — use mcp__sevens__agent_spawn, mcp__sevens__task_assign, etc.
• Native Claude tools (Read, Write, Edit, Bash, Grep, Glob) should ONLY be used for file operations and shell commands
• All agent spawning, task assignment, memory, and coordination MUST go through mcp__sevens__* tools
• If a Sevens MCP tool exists for an operation, always prefer it over any native equivalent

💡 COORDINATION TIPS:
• Use mcp__sevens__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__sevens__hive-mind_status
• Store important decisions in shared memory for persistence
• Use consensus for any decisions affecting multiple workers
• Use mcp__sevens__task_assign to assign tasks to workers, then mcp__sevens__task_complete when done
  ...
  try {
    ...
    if (claudeAvailable && !dryRun) {
      ...
      if (mcpConfigPath) {
        ...
        output.printWarning('No .mcp.json or ~/.claude.json found — spawned worker will not have mcp__sevens__* tools (#1748 Issue 2). Pass --mcp-config <path> or run "sevens init" to generate one.');
      }
...
const taskCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // server (the `mcp__sevens__hive-mind_*` surface only exposes init,
