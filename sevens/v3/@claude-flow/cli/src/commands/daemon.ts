...
const startCommand: Command = {
  ...
  options: [
    ...
    // Without it (or daemon.aiWorkers.enabled / SEVENS_DAEMON_AI_WORKERS=1),
    ...
    // keep dispatching headless worker sweeps. Default 12h (or SEVENS_DAEMON_TTL_SECS); 0 = run until stopped.
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (noDistill) {
      process.env.SEVENS_DAEMON_NO_DISTILL = '1';
    }
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean, forwarded: ForwardedDaemonFlags = {}): Promise<CommandResult> {
  ...
  // child; its own `start` action sets SEVENS_DAEMON_NO_DISTILL from this flag.
  if (noDistill === true) {
  ...
  if (!quiet) {
    ...
    try {
      ...
      if (fleet.length > 1) {
        output.writeln();
        output.printWarning(
          `Found ${fleet.length} sevens daemons running across workspaces/worktrees.`
        );
        output.printInfo('Scheduled AI workers are off by default and every AI launch is capped by the user-global budget.');
        output.printInfo('Inspect:  sevens daemon status --all');
        output.printInfo('Stop all: sevens daemon stop --all');
      }
...
const stopCommand: Command = {
  ...
  options: [
    { name: 'quiet', short: 'Q', type: 'boolean', description: 'Suppress output' },
    // #2661: emergency stop for worktree-daemon fleets. Stops every sevens
    // daemon owned by the current user across ALL workspaces/worktrees.
    { name: 'all', short: 'a', type: 'boolean', description: 'Stop sevens daemons in ALL workspaces/worktrees (not just the current one)' },
  ],
  examples: [
    { command: 'claude-flow daemon stop', description: 'Stop the daemon in this workspace' },
    { command: 'claude-flow daemon stop --all', description: 'Stop sevens daemons in every workspace/worktree' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
    ...
    // Git worktrees. Only processes positively identified as sevens daemons
...
/**
 * #2661: stop every running sevens daemon across all workspaces/worktrees.
 *
 ...
 * self-identifying as a sevens daemon (`daemon start --foreground` +
 * claude-flow markers). Interactive Claude sessions and non-sevens processes
 ...
 * Only sevens-owned registry entries (each workspace's daemon.pid) are removed.
 */
async function stopAllDaemons(quiet: boolean): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    if (!quiet) {
      output.printInfo('No sevens daemons are running in any workspace.');
    }
  ...
  for (const d of daemons) {
    ...
    // Remove the sevens-owned PID file for that workspace — but only when it
  ...
  if (!quiet) {
    output.printSuccess(`Stopped ${stopped} sevens daemon(s) across all workspaces.`);
  }
...
/**
 * #2356: enumerate every running sevens daemon across ALL workspaces. Reuses
 ...
 * file or SEVENS_DAEMON_AI_WORKERS=1) and already has multiple worktree
...
export async function maybeShowMultiDaemonMigrationWarning(opts?: {
  ...
  try {
    ...
    if (anyAiEnabled) {
      output.writeln();
      output.printWarning(`Sevens found ${fleet.length} worktree daemons. Scheduled AI workers are now supervisor-gated.`);
      output.printInfo('Inspect:                      sevens daemon status --all');
      output.printInfo('Stop all:                      sevens daemon stop --all');
      output.printInfo('Pause autonomous launches:     sevens daemon budget pause');
      output.writeln();
    }
...
async function renderAllDaemonsStatus(): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    output.printBox(
      'No sevens daemons are running in any workspace.',
      'sevens Daemons (all workspaces)'
    );
  ...
  if (staleCount > 0) {
    output.printWarning(
      `${staleCount} daemon(s) have outlived their TTL (or have run >12h). Stop one with: cd <workspace> && sevens daemon stop`
    );
  } else {
    output.printInfo(`${daemons.length} daemon(s) running, all within their TTL.`);
  }
  if (daemons.length > 1) {
    output.printInfo('Stop all daemons across workspaces with: sevens daemon stop --all');
  }
...
const statusCommand: Command = {
  ...
  options: [
    ...
    // leaked in another project is invisible. --all scans every running sevens
    // daemon across all workspaces (the global view that surfaces leaks).
    { name: 'all', short: 'a', type: 'boolean', description: 'List sevens daemons across ALL workspaces (global view — surfaces leaked daemons)' },
  ],
  examples: [
...
const installSupervisorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (platform === 'win32') {
      ...
      output.printInfo('Use Task Scheduler manually, or follow this issue: https://github.com/ruvnet/sevens/issues/1565');
      return { success: false, exitCode: 1 };
    }
    ...
    if (platform === 'darwin') {
      ...
      const plistPath = join(plistDir, 'io.ruv.sevens.daemon.plist');
      const logDir = join(projectRoot, '.claude-flow', 'logs');
      const plist = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key><string>io.ruv.sevens.daemon</string>
...
`;
    ...
    const unitPath = join(unitDir, 'sevens-daemon.service');
    const unit = `[Unit]
Description=sevens background worker daemon
...
`;
    ...
    if (load) {
      try {
        ...
        execFileSync('systemctl', ['--user', 'enable', '--now', 'sevens-daemon.service'], { encoding: 'utf-8', timeout: 10000 });
        ...
        output.printInfo(`Run manually: systemctl --user daemon-reload && systemctl --user enable --now sevens-daemon.service`);
      }
    } else {
      output.printInfo(`Run when ready:  systemctl --user daemon-reload && systemctl --user enable --now sevens-daemon.service`);
    }
    return { success: true };
  },
};
...
const uninstallSupervisorCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      const plistPath = join(home, 'Library', 'LaunchAgents', 'io.ruv.sevens.daemon.plist');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('launchctl', ['unload', plistPath], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
    ...
    if (platform === 'linux') {
      const unitPath = join(home, '.config', 'systemd', 'user', 'sevens-daemon.service');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('systemctl', ['--user', 'disable', '--now', 'sevens-daemon.service'], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
...
// independently scriptable surface (e.g. `sevens daemon budget pause` before
...
const budgetPauseCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printSuccess('Autonomous AI worker launches paused across all daemons. Resume with: sevens daemon budget resume');
    return { success: true };
  },
};
...
export const daemonCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('sevens Daemon - Background Task Management'));
    output.writeln();
