/**
 * `sevens version` — ANV (Agent-Native Versioning) Phase 1.
 * https://gist.github.com/ruvnet/0d858ad440a4439b4a2281a40c39b1a0
 *
 * Plain `sevens --version` / `-V` (index.ts's showVersion()) is UNCHANGED —
 * it stays bare semver so scripts parsing that output never see a surprise
 * suffix. This is a separate subcommand: `sevens version` prints the same
...
export const versionCommand: Command = {
  ...
  examples: [
    { command: 'sevens version', description: 'Print the installed semver' },
    { command: 'sevens version --explain', description: 'Print the full ANV catalog/benchmark breakdown' },
    { command: 'sevens version --require-catalog-gte 40', description: 'Gate a script on a minimum catalog generation' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!manifest) {
      output.writeln(`Installed: sevens@${version}`);
    ...
    output.writeln(`Installed: ${output.bold(`sevens@${version}${suffix}`)}`);
    output.writeln();
