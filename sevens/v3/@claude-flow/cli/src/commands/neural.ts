...
//
// Turns sevens's captured run transcripts into AUDITED TRAINING DATA + a
...
const distillPlanCommand: Command = {
  ...
  options: [
    ...
    { name: 'adapter-prefix', type: 'string', description: 'Adapter output prefix', default: 'sevens-weft' },
    { name: 'format', short: 'f', type: 'string', description: 'Output format: table, json', default: 'table' },
  ],
  examples: [
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const res = await runPlan({ base, sftPath, dpoPath, adapterPrefix: String((ctx.flags['adapter-prefix'] ?? ctx.flags.adapterPrefix) || 'sevens-weft') });
    if (res.degraded) {
    ...
    if (fmt === 'json') { output.writeln(JSON.stringify(payload, null, 2)); return { success: true, data: payload }; }
    output.writeln();
    output.writeln(output.bold(`weight-eft GPU training plan ($0 dry-run — no tune runs from sevens)`));
    ...
    output.writeln();
    output.writeln(output.dim('These commands run on a GPU host; sevens does not execute them. See `neural distill train --remote` for a spend-gated remote path.'));
    return { success: true, data: payload };
  },
};
...
const distillTrainCommand: Command = {
  ...
  options: [
    { name: 'remote', short: 'r', type: 'string', description: 'SSH host or tailscale name (default: $SEVENS_DISTILL_REMOTE). Never hard-coded.' },
    ...
    { name: 'remote-workdir', type: 'string', description: 'Remote working dir (default: ~/.sevens-weft/<runId>)' },
  ...
  examples: [
    ...
    { command: 'SEVENS_DISTILL_REMOTE=gpu-box claude-flow neural distill train --execute --yes', description: 'Run the real remote tune (spends GPU time on your host)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const path = await import('node:path');
    const { runRemoteTrain } = await import('../services/weight-eft.js');
    const fmt = (ctx.flags.format as string) || 'table';
    const host = (ctx.flags.remote as string | undefined) || process.env.SEVENS_DISTILL_REMOTE;
    if (!host) {
      output.printError('No remote host. Pass --remote <host> or set SEVENS_DISTILL_REMOTE.');
      return { success: false, exitCode: 2 };
    }
    ...
    output.writeln();
    output.writeln(output.dim('Honesty: sevens does not train locally or at $0. This is an explicit, user-triggered remote-GPU spend. resolved-gold in the SFT data is still a proxy.'));
...
export const neuralCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('sevens Neural System'));
