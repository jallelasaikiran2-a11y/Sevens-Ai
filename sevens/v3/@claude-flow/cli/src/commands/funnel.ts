/**
 * `sevens funnel` — user control surface for the Cognitum lifecycle funnel
 * (ADR-301/305/309/317).
 *
...
const acceptSub: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (!decision.enabled) {
      output.printWarning(
        `Funnel is currently disabled by: ${decision.decidedBy}. Run 'sevens funnel enable' first, then re-run accept.`
      );
    ...
    if (current.state === 'disclosed_disabled') {
      output.printWarning(
        "Disclosure is in a declined state. Run 'sevens funnel enable' first, then re-run accept."
      );
...
function readCurrentPromo(): { text: string; url?: string; kind?: string } | null {
  try {
    const memoPath = path.join(os.homedir(), '.sevens', 'statusline-promo.json');
...
const openSub: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (!promo) {
      output.printWarning(
        "No promo has been shown yet. Wait for the statusline to render one, then re-run 'sevens funnel open'."
      );
...
const enrollSub: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (!decision.enabled) {
      output.printWarning(
        `Funnel is currently disabled by: ${decision.decidedBy}. Enable it (sevens funnel enable) before enrolling — there's nothing to earn from until the funnel is on.`
      );
      return { success: false, data: decision };
    }
    const disclosure = getDisclosure();
    if (disclosure.state !== 'disclosed_enabled') {
      output.printWarning(
        "The Cognitum disclosure hasn't been shown/accepted yet. Run 'sevens funnel accept' first, then re-run enroll."
      );
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const summary = {
      ...
      note: rec
        ...
        : 'Not enrolled. Run `sevens funnel enroll` to opt in.',
    };
    if (ctx.flags.json) {
...
export const funnelCommand: Command = {
  ...
  examples: [
    { command: 'sevens funnel status', description: 'Effective state + deciding source' },
    { command: 'sevens funnel accept', description: 'Skip the 24h grace so rotation starts now' },
    { command: 'sevens funnel open', description: 'Open the current promo URL in the browser' },
    { command: 'sevens funnel enroll', description: 'Opt in to 50/50 rev share (ADR-317)' },
    { command: 'sevens funnel earnings', description: 'Show accrued and paid balance' },
    { command: 'sevens funnel disable', description: 'Turn off every funnel surface' },
  ],
