 ...
 * and never re-ran `sevens init` still have it.
 *
 * Detection only — does not modify settings. Fix path is `sevens init` (the
...
async function checkStaleSettingsNpx(): Promise<HealthCheck> {
  ...
  return {
    ...
    fix: 'Re-run `npx sevens init` to migrate (the v3.13.3+ init migrator regenerates these to local-helper form). On macOS this prevents the process-storm / kernel-panic class reported in #2448.',
  };
}
...
//
// Embedding storage varies across sevens installs (agentdb migrations, HNSW
...
// the SAME way the SessionStart auto-memory hook does. On the documented `npx sevens`
...
async function checkLearningBridge(): Promise<HealthCheck> {
  ...
  if (!existsSync(hookPath)) {
    return {
      ...
      status: 'pass',
      message: 'auto-memory hook not installed (run: npx sevens@latest init)',
    };
  }
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  // parsed.projects[<projectPath>].mcpServers.sevens, in addition to any
  ...
  const isSevensKey = (k: string) =>
    k === 'sevens' || k === 'sevens_alpha' || k === 'claude-flow' || k === 'claude-flow_alpha';
  // Canonical MCP-server key is `claude-flow` — matches the `mcp__claude-flow__*`
  // prefix that ~166 plugin tool references depend on (#2206). A `sevens`-keyed
  ...
  const isCurrentSevensKey = (k: string) => k === 'claude-flow' || k === 'claude-flow_alpha';
  const isLegacySevensKey = (k: string) => k === 'sevens' || k === 'sevens_alpha';
  const isSevensServer = (server: unknown): boolean => {
    ...
    return /\b(?:sevens|claude-flow|@claude-flow\/cli)(?:@[\w.-]+)?\b/.test(haystack);
  };

  let totalServersSeen = 0;
  const sevensLocations: string[] = [];
  ...
  const inspectServers = (
    servers: unknown,
    location: string
  ): { total: number; hasSevens: boolean } => {
    if (!servers || typeof servers !== 'object') return { total: 0, hasSevens: false };

    const entries = Object.entries(servers as Record<string, unknown>);
    const activeSevensEntries = entries.filter(([key, server]) => isSevensKey(key) && isSevensServer(server));
    const hasLegacy = activeSevensEntries.some(([key]) => isLegacySevensKey(key));
    const hasCurrent = activeSevensEntries.some(([key]) => isCurrentSevensKey(key));

    if (activeSevensEntries.length > 0) {
      sevensLocations.push(`${location}: ${activeSevensEntries.map(([key]) => key).join(', ')}`);
    }
    if (hasLegacy && hasCurrent) {
      duplicateLocations.push(`${location}: ${activeSevensEntries.map(([key]) => key).join(' + ')}`);
    }
    for (const [key] of activeSevensEntries) {
      if (isLegacySevensKey(key)) legacyLocations.push(`${location}: ${key}`);
      if (isCurrentSevensKey(key)) currentLocations.push(`${location}: ${key}`);
    }

    return { total: entries.length, hasSevens: activeSevensEntries.length > 0 };
  };
  ...
  for (const configPath of mcpConfigPaths) {
    ...
    try {
      ...
      // Project-scoped (Claude Code shape): projects[*].mcpServers.sevens
  ...
  if (duplicateLocations.length > 0 || (legacyLocations.length > 0 && currentLocations.length > 0)) {
    ...
    return {
      name: 'MCP Servers',
      status: 'warn',
      message: `Duplicate Sevens MCP registrations found (${locations}) — Claude Code will start both tool schemas`,
      fix: 'Remove the legacy `sevens`-keyed MCP registration (pre-rename duplicate) and keep the canonical `claude-flow` entry: `claude mcp add claude-flow -- npx -y sevens@latest mcp start`. The canonical key stays `claude-flow` so the ~166 `mcp__claude-flow__*` plugin tool references keep resolving (#2206).',
    };
  }

  if (sevensLocations.length > 0) {
    return {
      name: 'MCP Servers',
      status: 'pass',
      message: `${totalServersSeen} servers (sevens configured: ${sevensLocations.join('; ')})`,
    };
  }
  ...
  if (totalServersSeen > 0) {
    return {
      name: 'MCP Servers',
      status: 'warn',
      message: `${totalServersSeen} servers (sevens not found)`,
      fix: 'claude mcp add claude-flow -- npx -y sevens@latest mcp start',
    };
  }
  ...
  return {
    ...
    fix: 'claude mcp add sevens -- npx -y sevens@latest mcp start',
  };
}
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@claude-flow/cli' || pkg.name === 'claude-flow' || pkg.name === 'sevens')
            ) {
...
// the standard sevens doctor flow. Graceful degradation: when metaharness
// is not installed (no network, optionalDep skipped), the check returns
// `warn` with a hint instead of `fail` — sevens continues to function.
/**
 * iter 45 — verify the sevens-side MetaHarness integration is intact.
 *
 ...
 * layer is shipped with sevens — missing files mean sevens's install is
 ...
 *   - plugins/sevens-metaharness/scripts/_harness.mjs                (subprocess bridge)
 *   - plugins/sevens-metaharness/scripts/_similarity.mjs             (ADR-152 §3.1 module, iter 36)
 *   - plugins/sevens-metaharness/scripts/similarity.mjs              (CLI skill, iter 36)
 *   - plugins/sevens-metaharness/scripts/_spike-similarity.mjs       (regression anchor, iter 35)
 *   - plugins/sevens-metaharness/scripts/drift-from-history.mjs      (1-command primitive, iter 53)
 *   - plugins/sevens-metaharness/skills/harness-similarity/SKILL.md
 *   - plugins/sevens-metaharness/skills/harness-drift-from-history/SKILL.md  (iter 53)
 */
/**
  ...
  try {
    ...
    if (rateLimited.limited && !consented) {
      return {
        ...
        fix: 'sevens proxy sponsor-enable --yes',
      };
    }
    ...
    return {
      ...
      status: 'warn',
      message: `state unreadable: ${err instanceof Error ? err.message : String(err)}`,
      fix: 'sevens proxy sponsor-status',
    };
  }
}
...
async function checkProxyBinary(): Promise<HealthCheck> {
  ...
  try {
    ...
    if (!existsSync(binPath)) {
      return { name: NAME, status: 'warn', message: 'not installed', fix: 'sevens proxy install' };
    }
    ...
    if (!existsSync(manifestPath)) {
      return {
        name: NAME,
        status: 'warn',
        message: 'binary present but no install-manifest.json — provenance unknown (installed outside `sevens proxy install`?)',
        fix: 'sevens proxy update --release <x.y.z>',
      };
    }
...
async function checkProxyProcess(): Promise<HealthCheck> {
  ...
  try {
    ...
    if (!existsSync(pidPath)) {
      return { name: NAME, status: 'warn', message: 'not running (no PID file)', fix: 'sevens proxy start' };
    }

    const pidRaw = readFileSync(pidPath, 'utf-8').trim();
    const pid = parseInt(pidRaw, 10);
    if (!Number.isFinite(pid)) {
      return { name: NAME, status: 'warn', message: `PID file is malformed: ${JSON.stringify(pidRaw)}`, fix: 'sevens proxy stop && sevens proxy start' };
    }

    try {
      process.kill(pid, 0); // signal-0 liveness probe — throws if the process is dead
    } catch {
      return { name: NAME, status: 'warn', message: `PID file points at ${pid}, which is not running — stale PID file`, fix: 'sevens proxy start' };
    }
      ...
      if (existsSync(proxyInstallManifestPath())) {
        ...
        if (manifest.version && body.version && manifest.version !== body.version) {
          return {
            ...
            fix: 'sevens proxy stop && sevens proxy start',
          };
        }
...
/** `sevens auth` health (ADR-306). Warn (never fail) on absence — auth is never required for core functionality. */
async function checkAuth(): Promise<HealthCheck> {
  ...
  try {
    ...
    if (profiles.length === 0) {
      return { name: NAME, status: 'warn', message: 'not logged in', fix: 'sevens auth login' };
    }
    ...
    if (violations.length > 0) {
      return {
        ...
        fix: 'sevens auth logout && sevens auth login',
      };
    }
...
async function checkMetaharnessIntegration(): Promise<HealthCheck> {
  ...
  // (dev) until we find a directory containing `plugins/sevens-metaharness/`.
  const candidates: string[] = [];
  ...
  try {
    ...
    let q = selfDir;
    for (let i = 0; i < 8; i++) {
      candidates.push(join(q, 'plugins', 'sevens-metaharness'));
      q = dirname(q);
    }
  } catch {
  ...
  let p = process.cwd();
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'sevens-metaharness'));
    p = dirname(p);
  }

  // Strategy 3: explicit node_modules path relative to cwd — covers project-local install.
  candidates.push(join(process.cwd(), 'node_modules', '@claude-flow', 'cli', 'plugins', 'sevens-metaharness'));
  ...
  if (!pluginDir) {
    ...
    return {
      ...
      status: 'warn',
      message: 'plugins/sevens-metaharness/ not found — MetaHarness skills will degrade gracefully',
      fix: 'Optional: install via `npm i -D @metaharness/darwin metaharness` or run `sevens plugins install sevens-metaharness`',
    };
  }
  ...
  if (missing.length > 0) {
    return {
      ...
      fix: 'Reinstall sevens or restore from git: `git checkout HEAD -- plugins/sevens-metaharness/`',
    };
  }
  ...
  try {
    ...
    if (typeof mod.similarity !== 'function') {
      return {
        ...
        fix: 'Reinstall sevens or restore the file from git',
      };
    }
    ...
    if (typeof harnessMod.parseMcpScanText !== 'function') {
      return {
        ...
        fix: 'Reinstall sevens or restore _harness.mjs from git',
      };
    }
...
async function checkMetaharness(): Promise<HealthCheck> {
  try {
    ...
    return {
      name: 'MetaHarness (ADR-150)',
      status: 'pass',
      message: `v${versionMatch[1]} — run \`npx sevens metaharness score\` for the full scorecard`,
    };
  } catch {
    return {
      name: 'MetaHarness (ADR-150)',
      status: 'warn',
      message: 'Not installed — `npx sevens metaharness *` commands will degrade gracefully',
...
export const doctorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (fixHandles) {
      output.writeln();
      output.writeln(output.bold('sevens Doctor — fix-handles'));
    ...
    output.writeln();
    output.writeln(output.bold('sevens Doctor'));
    ...
    const allChecks: (() => Promise<HealthCheck>)[] = [
      ...
      checkMetaharnessIntegration, // iter 45 — sevens-side integration layer
      ...
      checkAuth, // ADR-306 — Cognitum identity (warn-only; never fails bare `sevens doctor`)
    ];
    ...
    const componentMap: Record<string, (() => Promise<HealthCheck>) | Array<() => Promise<HealthCheck>>> = {
      ...
      'metaharness-integration': checkMetaharnessIntegration, // iter 45 — sevens-side
    ...
    // recording the resolver sidecar. When doctor runs via `npx sevens`, the CLI
