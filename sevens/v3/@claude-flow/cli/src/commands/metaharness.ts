 ...
 * `plugins/sevens-metaharness/scripts/<name>.mjs` via spawnSync. This is
 ...
 *
 *   npx sevens metaharness score
 *   npx sevens metaharness mcp-scan --fail-on high
 *   npx sevens metaharness mint --name foo --template vertical:coding
 *
 * instead of:
 *
 *   node plugins/sevens-metaharness/scripts/score.mjs
 *
...
/**
 * Walk up from the current dirname to find the sevens repo root that
 * contains plugins/sevens-metaharness/. Handles three install layouts:
 *   1. sevens dev tree   (cwd / ../plugins/...)
 *   2. sevens wrapper    (sevens/node_modules/@claude-flow/cli/...)
 ...
 * `v3/@claude-flow/cli/plugins/sevens-metaharness/scripts/` getting picked
 * over the source `plugins/sevens-metaharness/scripts/` when the mirror
...
function locatePluginScripts(requiredScript?: string): string | null {
  ...
  // Also try from cwd (covers the "npx sevens" case where the user is
  // sitting in their own repo and `npx sevens metaharness score` should
...
export const metaharnessCommand: Command = {
  ...
  async action(context: CommandContext): Promise<CommandResult> {
    ...
    if (!subcommand || subcommand === 'help' || subcommand === '--help' || subcommand === '-h') {
      output.writeln(output.bold('npx sevens metaharness <subcommand> [options]'));
      output.writeln('');
    ...
    if (!scriptDir) {
      output.writeln(
        output.warning(
          'metaharness: plugins/sevens-metaharness/scripts/ not found. Install sevens with `npm i sevens` or run from the sevens repo.'
        )
      );
      output.writeln(
