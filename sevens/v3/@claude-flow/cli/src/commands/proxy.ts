/**
 * `sevens proxy` — Meta LLM Proxy control surface (ADR-304/307/313).
 *
 ...
 * BOTH the sevens-side consent receipt (source of truth) AND a mirror flag
 * into ~/.sevens/proxy-config.toml (the file the Rust proxy binary reads).
 * The proxy never writes this file — sevens does, exactly once per
 ...
 * field sevens needs to set (`sponsored_consent_granted`); every other field
...
const CLOUD_ROUTING_DISCLOSURE = [
  ...
  '',
  'Disable anytime: sevens proxy config --local-only',
].join('\n');
...
const configSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    if (!hasConsent('cloud-routing')) {
      ...
      output.writeln('');
      if (!ctx.flags.yes) {
        output.writeln('Re-run with --yes to confirm: sevens proxy config --cloud --yes');
    ...
    output.writeln('  Disable anytime: sevens proxy config --local-only');
...
const sponsorEnableSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    if (!ctx.flags.yes) {
      output.writeln('Re-run with --yes to confirm: sevens proxy sponsor-enable --yes');
      return { success: true, data: { confirmed: false } };
    }
    ...
    output.writeln('The proxy will use it automatically while sevens settings notices');
    output.writeln('rate-limited is flagged. Disable anytime: sevens proxy sponsor-disable');
...
const sponsorStatusSub: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (rateLimited.limited && !consented) {
      ...
      output.writeln('capacity. Enable it with: sevens proxy sponsor-enable --yes');
    }
...
const POWER_SAVER_DISCLOSURE = [
  ...
  '',
  'Disable anytime: sevens proxy power-saver-disable',
].join('\n');
...
const powerSaverEnableSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    if (!ctx.flags.yes) {
      output.writeln('Re-run with --yes to confirm: sevens proxy power-saver-enable --yes');
      return { success: true, data: { confirmed: false } };
    }
    ...
    output.writeln('Flag it active with: sevens settings notices quota-low');
    output.writeln('Disable anytime: sevens proxy power-saver-disable');
    return { success: true, data: { confirmed: true } };
  },
};
...
const powerSaverStatusSub: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (quotaLow.low && !consented) {
      ...
      output.writeln('mode. Enable it with: sevens proxy power-saver-enable --yes');
    }
...
const TRAINING_SHARE_DISCLOSURE = [
  ...
  '',
  'Disable anytime: sevens proxy training-share-disable',
].join('\n');
...
const trainingShareEnableSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    if (!ctx.flags.yes) {
      output.writeln('Re-run with --yes to confirm: sevens proxy training-share-enable --yes');
      return { success: true, data: { confirmed: false } };
    }
    ...
    output.writeln('  sevens proxy training-share-disable');
    return { success: true, data: { confirmed: true } };
  },
};
...
export const proxyCommand: Command = {
  ...
  examples: [
    { command: 'sevens proxy install --yes', description: 'Install the signed Meta-Proxy v0.4.0 binary' },
    { command: 'sevens proxy start', description: 'Start meta-proxy in the foreground' },
    { command: 'sevens proxy status', description: 'Show install + process status' },
    { command: 'sevens proxy config --cloud --yes', description: 'Enable cloud routing (ADR-304)' },
    { command: 'sevens proxy config --local-only', description: 'Revert to local-only routing' },
    { command: 'sevens proxy sponsor-status', description: 'Show current sponsored-mode state' },
    { command: 'sevens proxy sponsor-enable --yes', description: 'Opt into sponsored downtime capacity' },
    { command: 'sevens proxy power-saver-enable --yes', description: 'Opt into power saver mode' },
    { command: 'sevens proxy training-share-enable --yes', description: 'Opt into training-data sharing (ADR-315)' },
  ],
  action: async () => {
