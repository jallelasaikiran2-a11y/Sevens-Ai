/**
 * `sevens spinner` — manage sevens verbs in Claude Code's spinnerVerbs
 * settings (ADR-318).
 *
 * Claude Code exposes `spinnerVerbs.mode` + `spinnerVerbs.verbs[]` in
 * ~/.claude/settings.json. This command appends a curated sevens pool to
 * that array, tagged with a zero-width joiner marker so `disable` can
 * strip only sevens verbs without touching user-authored ones.
 *
...
// user-authored verb. Used to tag sevens-managed entries so `disable`
// removes ONLY ours.
const SEVENS_MARKER = '‍‍‍';
...
// gives roughly a 40% chance any spin is a sevens verb, and ~6% of ALL spins
// show a Cognitum-tagged verb.
const SEVENS_VERB_POOL_V0 = [
  // Memory & retrieval (7)
...
function isValidVerb(v: string): boolean {
  ...
  const stripped = v.replace(SEVENS_MARKER, '');
...
function markVerb(v: string): string {
  return v + SEVENS_MARKER;
}

function isSevensVerb(v: string): boolean {
  return typeof v === 'string' && v.includes(SEVENS_MARKER);
}
...
const enableSub: Command = {
  name: 'enable',
  description: "Add sevens's curated verb pool to Claude Code's spinner rotation",
  options: [
    { name: 'yes', description: 'Skip the confirmation prompt', type: 'boolean', default: false },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const validVerbs = SEVENS_VERB_POOL_V0.filter(isValidVerb);
    ...
    output.writeln('');
    output.writeln('Some verbs mention Cognitum, Sevens\'s sponsor. This is opt-in and reversible via');
    output.writeln("`sevens spinner disable`. Claude Code's default verbs are preserved (append-only).");
    if (!ctx.flags.yes) {
      output.writeln('');
    ...
    if (currentBlock.mode === 'replace') {
      output.printError(
        ...
        'Either change your mode to "append" manually and re-run, or accept sevens\'s pool as your entire set via manual edit.'
      );
      return { success: false, data: { currentMode: 'replace' } };
    }
    const currentVerbs = Array.isArray(currentBlock.verbs) ? currentBlock.verbs : [];
    // Strip any prior sevens entries so re-running enable is idempotent
    // instead of duplicating our pool every time.
    const preservedUserVerbs = currentVerbs.filter(v => !isSevensVerb(v));
...
const disableSub: Command = {
  name: 'disable',
  description: 'Remove sevens verbs from Claude Code\'s spinner rotation (user-authored verbs preserved)',
  action: async (): Promise<CommandResult> => {
    ...
    data.spinnerVerbs.verbs = data.spinnerVerbs.verbs.filter(v => !isSevensVerb(v));
    ...
    writeSettings(data);
    revokeConsent('spinner-verbs', 'cli-spinner-disable');
    output.printSuccess(`Disabled — removed ${before - after} sevens verbs (kept ${after} user-authored).`);
...
const listSub: Command = {
  name: 'list',
  description: 'Show sevens\'s verb pool and which verbs are currently installed',
  options: [
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const installedSevens = installed.filter(isSevensVerb).map(v => v.replace(SEVENS_MARKER, ''));
    const installedUser = installed.filter(v => !isSevensVerb(v));
    const pool = SEVENS_VERB_POOL_V0.filter(isValidVerb);
    const summary = {
      consent: hasConsent('spinner-verbs') ? 'granted' : 'not-granted',
      mode: data.spinnerVerbs?.mode ?? '(none)',
      pool_available: pool,
      installed_sevens: installedSevens,
    ...
    if (ctx.flags.json) {
      ...
      output.writeln('');
      output.writeln(`Sevens pool (${pool.length} verbs, available):`);
      for (const v of pool) output.writeln('  • ' + v);
      output.writeln('');
      output.writeln(`Currently installed sevens verbs (${installedSevens.length}):`);
      if (installedSevens.length === 0) output.writeln('  (none — run `sevens spinner enable --yes` to install)');
      else for (const v of installedSevens) output.writeln('  • ' + v);
      output.writeln('');
      output.writeln(`User-authored verbs (${installedUser.length}, untouched by sevens):`);
...
export const spinnerCommand: Command = {
  name: 'spinner',
  description: 'Manage sevens verbs in Claude Code\'s spinnerVerbs rotation (ADR-318)',
  subcommands: [enableSub, disableSub, listSub, resetSub],
  examples: [
    { command: 'sevens spinner list', description: 'Show the sevens pool + what\'s currently installed' },
    { command: 'sevens spinner enable --yes', description: 'Append sevens\'s verb pool to settings.json' },
    { command: 'sevens spinner disable', description: 'Remove sevens verbs, keep user-authored ones' },
    { command: 'sevens spinner reset --yes', description: 'Restore the most recent settings.json backup' },
  ],
