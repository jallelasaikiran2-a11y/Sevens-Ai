/**
 * `sevens proxy install|start|stop|status|logs|update|uninstall` (ADR-307) —
...
const PROXY_COMMAND = 'npx sevens@latest proxy';
const AUTH_COMMAND = 'npx sevens@latest auth';

/**
 * Human-oriented next steps for `sevens proxy` and `sevens proxy status`.
...
const INSTALL_DISCLOSURE = [
  ...
  'separate, explicit opt-in (`sevens proxy config --cloud`), never enabled',
  'by install alone.',
  '',
  'Uninstall anytime: sevens proxy uninstall',
].join('\n');
...
const installSub: Command = {
  ...
  options: [
    ...
    // `sevens --version` handling — confirmed the hard way in E2E testing.
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    if (!version) {
      output.printError(
        'sevens proxy install requires --release <x.y.z> — there is no version-discovery ' +
    ...
    if (!hasConsent('proxy-install')) {
      ...
      if (!confirmed) {
        output.writeln(`Re-run with --yes to confirm: sevens proxy install --yes (installs ${version})`);
...
const updateSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    if (!version) {
      output.printError('sevens proxy update requires --release <x.y.z>');
      return { success: false, exitCode: 1 };
    }
    try {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    try {
      if (service) {
        ...
        output.writeln('  Logs: sevens proxy logs');
