...
const commandLoaders: Record<string, CommandLoader> = {
  ...
  // MetaHarness integration (ADR-150) — dispatcher over plugins/sevens-metaharness/
  metaharness: () => import('./metaharness.js'),
  // Eject (ADR-150 Phase 2) — lift sevens project into a renamed standalone harness
  ...
  // Sevens verbs in Claude Code's spinnerVerbs rotation (ADR-318)
  spinner: () => import('./spinner.js'),
  // Sevens entries in Claude Code's companyAnnouncements startup rotation (ADR-319)
