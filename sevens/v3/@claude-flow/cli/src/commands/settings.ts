/**
 * `sevens settings` — user-facing preferences router (ADR-311 amendment).
 *
 * The name change from `sevens funnel …` is deliberate copy discipline:
 ...
 * existing `sevens funnel` primitives so behavior stays identical — this
 * is a friendlier wrapper, not a re-implementation.
 *
 * Subcommands:
 *   sevens settings                     Show effective settings + how to change them
 *   sevens settings notices status      Show whether the notices row is on
 *   sevens settings notices off         Turn off the notices row (persistent)
 *   sevens settings notices on          Re-enable the notices row
 *   sevens settings notices id          Show the pseudonymous notices id
 */
...
const rateLimitedSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    output.writeln('This is a manual, self-reported flag — sevens cannot detect Claude\'s');
    ...
    output.writeln('until your own limit resets: sevens proxy sponsor-enable');
    output.writeln('');
    output.writeln('Clear it any time: sevens settings notices rate-limited --clear');
...
const quotaLowSub: Command = {
  ...
  action: async (ctx): Promise<CommandResult> => {
    ...
    output.writeln('');
    output.writeln('This is a manual, self-reported flag — sevens cannot read your actual');
    ...
    output.writeln('  sevens proxy power-saver-enable');
    output.writeln('');
    output.writeln('Clear it any time: sevens settings notices quota-low --clear');
...
const overviewAction: Command['action'] = async (): Promise<CommandResult> => {
  ...
  output.writeln('sevens settings — user preferences');
  output.writeln('');
  output.writeln('Notices (statusline tips + product updates)');
  output.writeln('  sevens settings notices status    Show current state');
  output.writeln('  sevens settings notices off       Turn off all notices');
  output.writeln('  sevens settings notices on        Re-enable');
  output.writeln('  sevens settings notices id        Show pseudonymous notices id');
  output.writeln('');
...
export const settingsCommand: Command = {
  ...
  examples: [
    { command: 'sevens settings', description: 'Overview + current state' },
    { command: 'sevens settings notices off', description: 'Turn off statusline notices' },
    { command: 'sevens settings notices status', description: 'Show current notices state' },
  ],
