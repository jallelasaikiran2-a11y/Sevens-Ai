...
const startCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // `sevens@latest` and force a fresh fetch with `-y` so npx doesn't pick
      // a stale local install.
      output.writeln(output.dim('  The MCP server may not be running. Start it with: claude mcp add claude-flow -- npx -y sevens@latest mcp start'));
    }
