 ...
 * `plugins/sevens-business-pods/templates/` MUST conform to, and a hand-rolled
...
export interface PodAgent {
  ...
  role: string;
  /** Must resolve to a known sevens agent type (researcher / coder / ...). */
  agentType: string;
...
/**
 * Known sevens agent types — kept in sync with src/commands/agent.ts AGENT_TYPES.
...
export const KNOWN_AGENT_TYPES = [
  ...
  // inside plugins/sevens-business-pods/scripts/pod-tick.mjs.
