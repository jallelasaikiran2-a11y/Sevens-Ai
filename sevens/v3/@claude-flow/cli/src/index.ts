...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    this.name = options.name || 'sevens';
    this.description = options.description || 'sevens V3 - AI Agent Orchestration Platform';
    ...
    try {
      ...
      if (commandPath[0] !== 'init' && commandPath[0] !== 'update') {
        try {
          ...
          // last ran `sevens init` — pre-3.31.3 nothing refreshed it, so any
        ...
        // SEVENS_DAEMON_AUTOSTART=0. Skipped for `daemon` (no recursion). Detached
