 ...
 * official SWE-bench harness. Sevens has NO SWE-bench oracle. Every record
 ...
 *                          sevens today; reserved for an external caller)
 *   - 'output-verifier'  — sevens's structural output-verifier confidence,
...
/** Sevens's cascade tier. haiku → 'cheap' (first tier), sonnet/opus → 'frontier'. */
...
/** Map a sevens model tier label to the weight-eft policy tier. */
