 ...
 * Ollama routing via SEVENS_PROVIDER + key-presence precedence (#2042).
 *
...
async function attachJsModelProvider(agent: any, config: WasmAgentConfig): Promise<boolean> {
  ...
  const systemPrompt = config.instructions || 'You are a helpful coding assistant running in a Sevens WASM agent sandbox.';
...
export async function promptWasmAgent(agentId: string, input: string): Promise<string> {
  ...
  try {
    ...
    const systemPrompt = entry.info.config.instructions || 'You are a helpful coding assistant running in a Sevens WASM agent sandbox.';
 ...
 * composed agents can declare which of sevens's 314 MCP tools they need.
 */
