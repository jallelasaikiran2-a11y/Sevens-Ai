 ...
 * default. With SEVENS_REQUIRE_REAL_EMBEDDINGS truthy, any hash last-resort throws
...
export function requireRealEmbeddings(): boolean {
  return /^(1|true|yes|on|strict)$/i.test(process.env.SEVENS_REQUIRE_REAL_EMBEDDINGS ?? '');
}
...
export function enforceNoStub(where: string): void {
  if (requireRealEmbeddings()) {
    throw new Error(
      ...
      `SEVENS_REQUIRE_REAL_EMBEDDINGS is set — install a real embedder ` +
