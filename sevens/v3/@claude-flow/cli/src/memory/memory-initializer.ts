 ...
 * `initializeMemoryDatabase` (MCP start / `memory init`) and from `sevens init`.
 *
  ...
  try {
    ...
    if (qcVal.toLowerCase() !== 'ok') {
      ...
      if (opts.autoRecover) {
        ...
        // npx-deployed sevens self-repair a corrupt memory DB on init / MCP start.
...
export async function initializeMemoryDatabase(options: {
  ...
  try {
    ...
    if (fs.existsSync(dbPath) && !force) {
      ...
      // every MCP-server start and `memory init`, so any sevens repairs itself.
