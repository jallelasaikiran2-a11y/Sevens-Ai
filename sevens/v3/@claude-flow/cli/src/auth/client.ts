/**
 * `sevens auth login` flow orchestration (ADR-306) — composes
...
export class NotLoggedInError extends Error {
  constructor(profile: string) {
    super(`not logged in for profile "${profile}" — run: sevens auth login --profile ${profile}`);
  ...
  constructor(profile: string) {
    super(
      `profile "${profile}" has no persisted refresh token and its in-memory access token is ` +
        `absent or expiring — run: sevens auth login --profile ${profile}`,
    );
 ...
 * policy: local sevens functionality is never affected by auth being
...
export async function refreshAccessToken(refreshTokenValue: string): Promise<OAuthTokenResponse> {
  ...
  try {
    ...
    if (e instanceof sec.OAuthError) {
      if (e.code === 'network') {
        throw new Error(
          'Could not reach the Cognitum auth service. sevens core functionality is unaffected — ' +
