 ...
 *                      sevens, unmodified. (native node:crypto, zero deps —
...
/** Sevens config-signing PUBLIC key (safe to commit). Private half in GCP Secret Manager (sevens-config-signing-key). */
export const SEVENS_CONFIG_PUBKEY = `-----BEGIN PUBLIC KEY-----
...
export interface ProvenConfigManifest {
  schema: string;                        // 'sevens.proven-config/v1'
  policy: {
  ...
  compatibility?: Record<string, string>;// package -> semver range (e.g. { sevens: '>=3.24.0' })
...
export interface InstallEnv {
  ...
  versions?: Record<string, string>;     // package -> installed version (e.g. { sevens: '3.24.0' })
...
/**
 * Verify a signed manifest against sevens's config public key. Returns the
...
export function verifyProvenConfig(
  ...
  pubkeyPem: string = SEVENS_CONFIG_PUBKEY,
...
export function isSuitable(manifest: ProvenConfigManifest, env: InstallEnv): SuitabilityResult {
  ...
  // package compatibility ranges (sevens, metaharness, …)
