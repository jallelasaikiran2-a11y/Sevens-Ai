...
export const ATTRIBUTION_FOOTER =
  '🤖 Generated with [sevens](https://github.com/ruvnet/sevens)';
...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# Sevens Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running Sevens pre-commit checks..."
...
# Notify sevens of commit
npx sevens@latest hooks notify \\
...
export function generateHookHandler(): string {
  ...
  const lines = [
    '#!/usr/bin/env node',
    '/**',
    ' * Sevens Hook Handler (Cross-Platform)',
    ...
    '    try {',
    "      var rlFunnelEnv = process.env.SEVENS_FUNNEL;",
    ...
    '      });',
    "      var rlHome = path.join(os.homedir(), '.sevens');",
...
function warnMemoryUnavailable() {
  ...
  const l2 = '             Fix: npm i -D @claude-flow/memory   (or re-run: npx sevens@latest init, then npx sevens@latest doctor --fix)';
...
async function loadMemoryPackage() {
  ...
  // when installed as a transitive dependency via npx sevens / npx claude-flow)
  try {
...
export function generateWindowsDaemonManager(): string {
  return `# sevens V3 Daemon Manager for Windows
...
function Show-Status {
    Write-Host ""
    Write-Host "sevens V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM sevens V3 - Windows Batch Wrapper
...
export function generateCrossPlatformSessionManager(): string {
  return `#!/usr/bin/env node
/**
 * Sevens Cross-Platform Session Manager
...
/**
 * Generate cross-platform Node.js port of sevens-hook.sh (#2132).
 *
 ...
 *   - Both: .claude/helpers/sevens-hook.cjs available as a canonical cross-platform shim
 */
export function generateSevensHookCjs(): string {
  return `#!/usr/bin/env node
/**
 * sevens-hook.cjs — cross-platform Node.js port of sevens-hook.sh (#2132)
 *
 * Deployed to .claude/helpers/ during sevens init. On Windows, the
 * generated .claude/settings.json hooks point here instead of the
 * plugin's bash-only sevens-hook.sh.
 *
...
function main() {
  ...
  if (commandExists('sevens')) { invokeHook('sevens', [], hookArgs, stdinData); done(); }
  if (commandExists('claude-flow')) { invokeHook('claude-flow', [], hookArgs, stdinData); done(); }
  invokeHook('npx', ['--prefer-offline', '--yes', 'sevens@latest'], hookArgs, stdinData);
  done();
}
