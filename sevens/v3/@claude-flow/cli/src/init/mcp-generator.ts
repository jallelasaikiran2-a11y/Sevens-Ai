...
export function generateMCPConfig(options: InitOptions): object {
  ...
  // Sevens MCP server (core) — the registration KEY is intentionally
  // `claude-flow` (not `sevens`) because #2206 established that all ~166
  // plugin tool references use `mcp__claude-flow__*`. The invoked binary
  // is `sevens@latest` (the post-rename wrapper) — only the registration
  // name stays legacy so plugin tool resolution keeps working.
  // #2612 (duplicate `claude-flow` + `sevens` registrations after users
  // followed pre-rename setup docs) is healed by `sevens doctor`, which
  // detects the duplicate and instructs the operator to remove the
  // extra `sevens`-keyed entry — NOT by flipping the canonical key here.
  if (config.claudeFlow) {
    mcpServers['claude-flow'] = createMCPServerEntry(
      ['sevens@latest', 'mcp', 'start'],
      {
        ...npmEnv,
        CLAUDE_FLOW_MODE: 'v3',
...
export function generateMCPCommands(options: InitOptions): string[] {
  ...
  if (isWindows()) {
    if (config.claudeFlow) {
      ...
      commands.push('claude mcp add claude-flow -- cmd /c npx -y sevens@latest mcp start');
    }
    if (config.ruvSwarm) {
    ...
    if (config.claudeFlow) {
      // #2206: registration name must be `claude-flow` to match mcp__claude-flow__* plugin tool references
      commands.push("claude mcp add claude-flow -- npx -y sevens@latest mcp start");
    }
    if (config.ruvSwarm) {
