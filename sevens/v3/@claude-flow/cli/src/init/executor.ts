  ...
  try {
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use Sevens'
      };
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Sevens'
      };
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add sevens allow rules)
 ...
 * that already declares a `sevens`-keyed MCP server. We use this to skip
 ...
 * issue describes.
 *
 * Returns the path of the file that already declares `sevens`/`claude-flow` (so we can
 * surface it in the skipped-message), or null if none found.
 */
function detectExistingSevensMCP(targetDir: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // and the current generator would write `sevens` (#2612).
      ...
      //     existing `claude-flow` or `sevens` registration in this repo is
...
async function writeMCPConfig(
  ...
  // plugin tool refs); a stray `sevens`-keyed entry pointing at the same
  ...
  if (!options.force) {
    const existingSevensPath = detectExistingSevensMCP(targetDir);
    if (existingSevensPath) {
      result.skipped.push(`.mcp.json (existing sevens/claude-flow MCP registration found at ${existingSevensPath} — would create duplicate; pass --force to write anyway)`);
      return;
    }
  }
...
async function writeHelpers(
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    // #2132: Always generate sevens-hook.cjs regardless of source copy path.
    // The source helpers dir may not contain this file (it lives in
    // plugins/sevens-core/scripts/, not .claude/helpers/), but it must
    // always be present so Windows users can use the node-based shim.
    const sevensHookDest = path.join(helpersDir, 'sevens-hook.cjs');
    if (!fs.existsSync(sevensHookDest) || options.force) {
      fs.writeFileSync(sevensHookDest, generateSevensHookCjs(), 'utf-8');
      result.created.files.push('.claude/helpers/sevens-hook.cjs');
    } else {
      result.skipped.push('.claude/helpers/sevens-hook.cjs');
    }
...
async function writeRuntimeConfig(
  ...
  const config = `# sevens V3 Runtime Configuration
...
async function writeInitialMetrics(
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Sevens. Run: npx sevens@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
...
/**
 * Write CAPABILITIES.md - comprehensive overview of all Sevens features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilities = `# sevens V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
...
sevens V3 is a domain-driven design architecture for multi-agent AI coordination with:
...
# Swarm
npx sevens@latest swarm init --topology hierarchical --max-agents 8
npx sevens@latest swarm status
...
# Memory
npx sevens@latest memory search --query "patterns"

# Hooks
npx sevens@latest hooks pre-task --description "task"
npx sevens@latest hooks worker dispatch --trigger optimize
\`\`\`
...
async function writeClaudeMd(
  ...
  if (fs.existsSync(claudeMdPath) && !options.force) {
    ...
    if (fs.existsSync(claudeMdPath)) {
      const backupBase = `${claudeMdPath}.pre-sevens`;
      ...
      console.warn(`[sevens init] Existing CLAUDE.md backed up to ${path.basename(backupPath)} before overwrite`);
    }
  ...
  // Also write/append global ~/.claude/CLAUDE.md so sevens tools are used automatically (#1497).
  ...
  if (homeDir && !options.skipGlobalClaudeMd) {
    ...
    const sevensBlock = [
      '',
      '# Sevens Integration (auto-generated by sevens init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke sevens MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('Sevens Integration')) {
          fs.appendFileSync(globalClaudeMd, sevensBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended sevens block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, sevensBlock.trimStart(), 'utf-8');
