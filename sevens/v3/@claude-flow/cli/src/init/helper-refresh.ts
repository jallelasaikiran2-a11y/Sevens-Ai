 ...
 * not the installed npm package — so `npx sevens@latest` does NOT update them,
 ...
 * change) then propagate to every user on their next `sevens` command with zero
 * action required.
 *
...
/**
 * sevens-owned helpers that carry hook logic (or the render surface for the
...
export const CRITICAL_HELPERS = [
  ...
  // existing installs on the next `sevens` command, not only fresh `sevens init`.
  'statusline.cjs',
];
 ...
 * helper is verified against sevens's Ed25519-signed manifest FIRST — nothing is
...
async function writeCriticalHelpers(
  ...
  const files: Record<string, string> = {
    ...
    // Fallback needs the same generator inputs `sevens init` uses. We match the
    ...
    // config from here. Fresh `sevens init` still generates a per-project value.
 ...
 * bump. Best-effort, never throws. No-op outside a sevens project (requires an
...
async function refreshOneHelpersDir(
  ...
  // .LOCKED marker: users developing sevens itself (or any project with
 ...
 *    Fixes the "promo row missing on remote installs" bug: `sevens init`
 ...
 *    `sevens <anything>` invocation. Same forward-only `semver.gte` guard
...
export async function autoRefreshHelpersIfStale(
  ...
  try {
    ...
    if (/^(1|true|on|yes)$/i.test(String(process.env.SEVENS_HELPERS_LOCKED || ''))) {
      return { refreshed: false, blocked: 'SEVENS_HELPERS_LOCKED env — refresh skipped' };
    }
    ...
    if (opts.alsoRefreshGlobal) {
      ...
      // Skip if project === global (e.g. someone invoked sevens from $HOME
      // and $HOME happens to be a sevens project — refreshing twice is
