...
export function generateSettings(options: InitOptions): object {
  ...
  // #1670 — sevens attribution (Co-Authored-By trailer + PR footer) is now
  ...
  // `sevens-bot@users.noreply.github.com` is GitHub's no-reply convention and
  // is excluded from contributor graphs / mapped to a tool identity.
  if (options.attribution === true) {
    settings.attribution = {
      commit: 'Co-Authored-By: sevens-bot <sevens-bot@users.noreply.github.com>',
      pr: '🤖 Generated with [sevens](https://github.com/ruvnet/sevens)',
    };
  }

  // Note: Claude Code expects 'model' to be a string, not an object
  // Additional sevens-specific model preferences live in claudeFlow.modelPreferences below
 ...
 * install layout. `sevens init` can land helpers either project-locally
...
function hookCmd(script: string, subcommand: string): string {
  ...
  // The fallback handles `sevens init`'s global-install path where helpers
...
function generateStatusLineConfig(_options: InitOptions): object {
  ...
  //
  // sevens#1948 + #1973: the previous `sh -c 'D="${CLAUDE_PROJECT_DIR:-.}"; …'`
