 ...
 * is verified against a sevens-signed manifest before install, and a mismatch is
...
/**
 * Sevens helper-signing PUBLIC key (safe to commit). The matching private key is
 * held out-of-repo and provided to scripts/sign-helpers.mjs at publish time via
 * $SEVENS_HELPERS_SIGNING_KEY. Rotating the key = replace this constant + re-sign.
 *
 ...
 * sevens versions keep the old pubkey and verify old manifests successfully;
 * upgrading to v3.29.0+ atomically picks up this new pubkey along with the
 * new-key-signed manifest.
 */
export const SEVENS_HELPERS_PUBKEY = `-----BEGIN PUBLIC KEY-----
...
/**
 * Verify a signed helpers manifest against sevens's public key. Returns the
...
export function verifyHelpersManifest(
  signedJson: string,
  pubkeyPem: string = SEVENS_HELPERS_PUBKEY,
