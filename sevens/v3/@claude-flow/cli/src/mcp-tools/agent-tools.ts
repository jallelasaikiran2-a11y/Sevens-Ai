...
// sevens model routing: agent type → optimal tier/model assignment.
// Tier aliases are resolved to real model slugs via resolveOpenAICompatModel()
// when OpenRouter is the active provider (which is the sevens default).
//
//  opus   → deepseek/deepseek-r1        (deep reasoning, architecture)
//  sonnet → deepseek/deepseek-chat      (balanced coding, review)
//  haiku  → google/gemini-2.0-flash-001 (fast, research, testing)
//
// Override per-model via SEVENS_MODEL_OPUS / SEVENS_MODEL_SONNET / SEVENS_MODEL_HAIKU.
const AGENT_TYPE_MODEL_DEFAULTS: Record<string, ClaudeModel> = {
  // Complex reasoning → deepseek-r1 (opus tier)
  'architect': 'opus',
  'security-architect': 'opus',
  'system-architect': 'opus',
  'core-architect': 'opus',
  'test-architect': 'opus',
  // Code generation → deepseek-chat (sonnet tier)
  'coder': 'sonnet',
  'reviewer': 'sonnet',
  'analyst': 'sonnet',
  'optimizer': 'sonnet',
  ...
  'swarm-specialist': 'sonnet',
  'memory-specialist': 'sonnet',
  // Fast/research → gemini-flash (haiku tier)
  'researcher': 'haiku',
  'tester': 'haiku',
  'formatter': 'haiku',
  'linter': 'haiku',
...
export const agentTools: MCPTool[] = [
  {
    ...
    handler: async (input) => {
      ...
      const response: Record<string, unknown> = {
        ...
        note: 'Agent registered for coordination. Execution paths (sevens priority): ' +
          '(1) set OPENROUTER_API_KEY → agent_execute dispatches via OpenRouter (PRIMARY); ' +
          '(2) set GEMINI_API_KEY → agent_execute dispatches via Google Gemini (SECONDARY); ' +
          '(3) set OLLAMA_API_KEY → agent_execute dispatches via Ollama (LOCAL); ' +
          '(4) set ANTHROPIC_API_KEY → agent_execute dispatches via Anthropic (OPTIONAL).',
      };
  ...
  },
  {
    ...
    name: 'agent_execute',
    description: 'Run a task on a previously-spawned agent_spawn record via the configured LLM provider (OpenRouter PRIMARY, Gemini SECONDARY, Ollama LOCAL, Anthropic OPTIONAL). Use when native Task tool is wrong because (a) you need the spawned agent\'s persistent config (model, instructions, cost-tracking attribution) to apply to this turn, (b) the result needs to feed back into the agent\'s lifecycle (taskCount, lastResult, swarm-coordinated state), or (c) you want explicit model routing via the spawn record\'s `model` field instead of inheriting. For one-shot Claude prompts without a tracked agent, native Task is fine. Requires at least one of: OPENROUTER_API_KEY, GEMINI_API_KEY, OLLAMA_API_KEY, or ANTHROPIC_API_KEY.',
    category: 'agent',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    // #1916 — the `sevens agent logs <id>` CLI subcommand and the guidance
    ...
    handler: async (input) => {
      ...
      return {
        ...
        note: 'per-agent activity logging is not yet wired; entries are synthetic (ruvnet/sevens#1916)',
      };
    },
  },
];
