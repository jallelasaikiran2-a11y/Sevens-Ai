 ...
 *
 * Provider priority order (sevens production config):
 *   1. OpenRouter  — PRIMARY  (OPENROUTER_API_KEY  or SEVENS_PROVIDER=openrouter)
 *   2. Google Gemini — SECONDARY (GEMINI_API_KEY     or SEVENS_PROVIDER=gemini)
 *   3. Ollama      — LOCAL    (OLLAMA_API_KEY       or SEVENS_PROVIDER=ollama)
 *   4. Anthropic   — OPTIONAL (ANTHROPIC_API_KEY    or SEVENS_PROVIDER=anthropic)
 *
 ...
 *
 * OpenRouter is the PRIMARY provider for sevens. No provider is mandatory;
...
export async function callAnthropicMessages(input: AnthropicCallInput): Promise<AnthropicCallResult> {
  const explicitProvider = (process.env.SEVENS_PROVIDER || '').toLowerCase();
  ...
  if (useOpenRouter && openrouterKey) {
    return callOpenAICompat({
      ...
      // sevens default: DeepSeek V3 via OpenRouter (cost-efficient, highly capable)
  ...
  if (!useAnthropic || !anthropicKey) {
    return {
      ...
      error:
        ...
        'Or set SEVENS_PROVIDER=openrouter|gemini|ollama|anthropic to force a specific provider.',
    };
  }
...
/**
 * Google Gemini provider — SECONDARY provider for sevens.
 *
 * Endpoint: https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent
 * Auth: ?key=GEMINI_API_KEY
 *
 * Translates the Sevens/Anthropic-flavored input onto Gemini's generateContent
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://github.com/ruvnet/sevens',
        'X-Title': 'Sevens',
      },
      body: JSON.stringify({
        model,
...
function resolveOpenAICompatModel(input: string | undefined, fallback: string): string {
  ...
  // sevens model routing: logical tier names → cost-optimal OpenRouter slugs.
  ...
  if (input === 'haiku') return process.env.SEVENS_MODEL_HAIKU || 'google/gemini-2.0-flash-001';
  if (input === 'sonnet' || input === 'inherit') return process.env.SEVENS_MODEL_SONNET || 'deepseek/deepseek-chat';
  if (input === 'opus' || input === 'opus-4.7') return process.env.SEVENS_MODEL_OPUS || 'deepseek/deepseek-r1';
  return input;
}
...
export async function executeAgentTask(input: AgentExecuteInput): Promise<AgentExecuteResult> {
  ...
  const systemPrompt = input.systemPrompt ||
    `You are a ${agent.agentType} agent operating as part of a Sevens swarm. ` +
  ...
  try {
    ...
    // correct (sevens has no SWE-bench gold oracle). model_patch is '' because
  ...
  saveAgentStore(store);
  // No-provider-configured error → surface actionable remediation with sevens priority order.
  ...
  return {
    ...
    ...(noProvider && {
      remediation:
        'sevens Provider Setup: Set OPENROUTER_API_KEY (primary), GEMINI_API_KEY (secondary), ' +
        'OLLAMA_API_KEY (local), or ANTHROPIC_API_KEY (optional fallback). ' +
        'Or set SEVENS_PROVIDER=openrouter|gemini|ollama|anthropic to force a specific provider. ' +
        'Recommended: export OPENROUTER_API_KEY=<your-key> SEVENS_PROVIDER=openrouter',
    }),
  };
}
