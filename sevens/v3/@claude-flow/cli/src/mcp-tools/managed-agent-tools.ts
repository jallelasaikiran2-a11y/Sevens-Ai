 ...
 * agent runtime alongside sevens's local WASM-sandboxed agents (`rvagent` /
...
export const managedAgentTools: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        mcpServers: {
          type: 'array',
          description: 'MCP servers to expose to the agent — each {type:"url", url, name, authorization_token?}. NOTE: the cloud agent must be able to *reach* the URL (a local `sevens mcp start` is not reachable from Anthropic\'s cloud — deploy/tunnel it).',
          items: { type: 'object' },
        },
    ...
    handler: async (input) => {
      ...
      const agentBody: Record<string, unknown> = {
        name: maName('sevens-managed', input.name),
      ...
      const envBody: Record<string, unknown> = {
        name: maName('sevens-managed-env', input.name),
      ...
      const s = await maRequest<MaSession>('POST', '/sessions', {
        ...
        title: maName('sevens-managed session', input.title),
      });
