 ...
 *
 * Exposes `sevens-testgen` plugin scripts as first-class MCP tools so
 ...
 * Zero static `sevens-testgen/*` imports. All script invocation stays
...
/**
 * Locate the sevens-testgen plugin's scripts directory.
 *
...
function locateTestgenScripts(): string | null {
  ...
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'sevens-testgen', 'scripts'));
    candidates.push(join(p, '..', 'plugins', 'sevens-testgen', 'scripts'));
    p = dirname(p);
  }
  const cwd = getProjectCwd();
  candidates.push(join(cwd, 'plugins', 'sevens-testgen', 'scripts'));
  candidates.push(join(cwd, 'node_modules', '@claude-flow', 'cli', 'plugins', 'sevens-testgen', 'scripts'));
...
export const testgenTools: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        maxAttempts: { type: 'number', description: '1..5 (sevens cap). Each attempt gets budgetUsd/maxAttempts of the total budget.', default: 1 },
