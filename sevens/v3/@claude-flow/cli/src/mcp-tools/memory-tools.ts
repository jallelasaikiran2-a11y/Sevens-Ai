...
export const memoryTools: MCPTool[] = [
  ...
  {
    // #1916: `sevens status memory` (the detailed view) referenced an
  ...
  {
    // #1916: `sevens memory cleanup` referenced an unregistered `memory_cleanup`
  ...
  {
    // #1916: `sevens memory compress` referenced an unregistered tool. The
  ...
  {
    // #1916: `sevens memory export -o <file>` referenced an unregistered tool.
    ...
    handler: async (input) => {
      ...
      const payload = {
        schema: 'sevens-memory-export/v1',
  ...
  {
    // #1916: `sevens memory import <file>` referenced an unregistered tool.
    // Reads a sevens-memory-export JSON and re-stores each entry.
    name: 'memory_import',
