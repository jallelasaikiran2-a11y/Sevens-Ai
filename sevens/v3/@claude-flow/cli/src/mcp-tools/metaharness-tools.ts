 ...
 * (`plugins/sevens-metaharness/scripts/<X>.mjs`) via the same locator
...
/**
 * Walk up from this module to find plugins/sevens-metaharness/scripts/.
 ...
 * (`v3/@claude-flow/cli/plugins/sevens-metaharness/scripts/`, regenerated
...
function locatePluginScripts(requiredScript?: string): string | null {
  ...
  for (let i = 0; i < 8; i++) {
    candidates.push(join(p, 'plugins', 'sevens-metaharness', 'scripts'));
    candidates.push(join(p, '..', 'plugins', 'sevens-metaharness', 'scripts'));
    p = dirname(p);
  }
  const cwd = getProjectCwd();
  candidates.push(join(cwd, 'plugins', 'sevens-metaharness', 'scripts'));
  candidates.push(join(cwd, 'node_modules', '@claude-flow', 'cli', 'plugins', 'sevens-metaharness', 'scripts'));
 ...
 *      exits 0 so sevens stays operational. `success: true` because the
...
export const metaharnessTools: MCPTool[] = [
  ...
  // ───────────────────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        generations: { type: 'number', description: '1..50 (sevens cap)', default: 3 },
        children: { type: 'number', description: '1..20 (sevens cap) — variants per generation', default: 3 },
        concurrency: { type: 'number', description: '1..8 (sevens cap)', default: 2 },
  ...
  },
  {
    ...
    description: 'ADR-153 — upstream Darwin Shield (their own ADR-155): evolves a champion security-detection harness against a 10-vuln/9-decoy ground-truth corpus and grades on TPR/FPR/patch-pass/repro/unsafe vs four baselines (B0 static, B1 LLM-single-pass, B2 fixed-agent, B3 Darwin-champion). Closest reference implementation for sevens ADR-155 nightly self-learning security harness (#2417). Use when you need an empirical floor for Loop A reward-signal soundness; running this periodically gives baseline diversity and week-over-week champion-fitness drift. Bypassing this and just running the static MCP scan is wrong because static-only baseline (B0) reaches TPR=0.3/FPR=1 — proving static-alone has a measured detection ceiling. Parses overall PASS/FAIL + per-gate verdicts + baselines table from markdown. ' + MCP_SUCCESS_SEMANTIC,
    category: 'metaharness',
    inputSchema: {
      type: 'object',
      properties: {
        population: { type: 'number', description: '1..20 (sevens cap) — candidate detectors per cycle', default: 2 },
        cycles: { type: 'number', description: '1..100 (sevens cap) — evolution cycles', default: 1 },
        seed: { type: 'number', description: 'PRNG seed for reproducibility' },
