 ...
 * trajectory. Sevens has no SWE-bench oracle, so today `resolved` is
 ...
 *     SEVENS_DISTILL_REMOTE — never hard-coded). DRY-RUN by default: it prints
...
export interface LabelOptions {
  /** SSH host for Tier-1 execution. Falls back to env SEVENS_DISTILL_REMOTE. Never hard-coded. */
  remote?: string;
      ...
      if (opts.execute) {
        ...
        preflights.set(i, {
          ...
          fellThroughBecause: remote
            ...
            : 'dry-run and no remote configured (set remote / SEVENS_DISTILL_REMOTE)',
        });
      }
    }
...
export function resolveRemote(remote?: string): string | null {
  const r = (remote ?? process.env.SEVENS_DISTILL_REMOTE ?? '').trim();
...
export function buildOraclePlan(spec: TestSpec, remote: string | null): OraclePlan {
  ...
  const workdir = spec.workdir ?? (spec.repo ? `~/sevens-distill/${sanitize(spec.repo)}` : '~/sevens-distill/work');
