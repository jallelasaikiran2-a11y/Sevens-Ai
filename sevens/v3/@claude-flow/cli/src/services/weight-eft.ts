 ...
 * ---------------------------------------------------
 * This service turns sevens's captured run transcripts into AUDITED TRAINING
 ...
 * captured archive is a PROXY (sevens has no SWE-bench gold oracle) — the SFT
 * data-quality caveat stands. Nothing here claims sevens "trains a model" as a
 * $0/local capability.
 *
...
// ============================================================================
// Archive-builder: sevens run records → DarwinTrajectory[]  (unit-tested)
// ============================================================================
...
/**
 * Map captured sevens run transcripts to the DarwinTrajectory[] contract the
...
export function buildArchiveFromRecords(records: RunTranscriptRecord[]): ArchiveBuildResult {
  ...
  const proxyNote = hasGold
    ...
    : 'resolved is a PROXY — sevens has no SWE-bench gold oracle. `output-verifier`/`api-success` ' +
...
export async function runPlan(opts: {
  ...
  try {
    const plan = api.twoStagePlan(base, opts.sftPath, opts.dpoPath, opts.adapterPrefix ?? 'sevens-weft');
...
export interface RemoteTrainArgs {
  ...
  adapterDir?: string;
  /** Remote working dir. Default ~/.sevens-weft/<runId>. */
  ...
  runId?: string;
  /** Adapter name prefix. Default 'sevens-weft'. */
  adapterPrefix?: string;
}
...
export function buildRemoteTrainInvocation(args: RemoteTrainArgs): RemoteTrainPlan {
  ...
  const prefix = args.adapterPrefix ?? 'sevens-weft';
  const remoteWorkdir = args.remoteWorkdir ?? `~/.sevens-weft/${runId}`;
