 ...
 *
 * Every autonomous `claude --print` launch across ALL sevens daemons in ALL
...
export class GlobalAiBudget {
  ...
  constructor(options?: { baseDir?: string; limits?: Partial<AiBudgetLimits> }) {
    this.dir = options?.baseDir
      ?? process.env.SEVENS_AI_BUDGET_DIR
   ...
   * prevent. `SEVENS_AI_BUDGET_DISABLE=1` is the explicit escape hatch.
   */
  async reserve(req: AiBudgetRequest): Promise<AiBudgetPermit> {
    if (process.env.SEVENS_AI_BUDGET_DISABLE === '1') {
  ...
  /**
   * #2661 root-fix — manual pause, via `sevens daemon budget pause`. Distinct
  ...
  async pause(reason?: string): Promise<void> {
    ...
    try {
      ...
      ledger.pauseReason = (reason ?? 'manual pause (sevens daemon budget pause)').slice(0, 200);
      this.writeLedger(ledger);
  ...
  /** #2661 root-fix — `sevens daemon budget resume`. Clears ANY pause (manual or quota-triggered). */
...
/** Test hook: reset the singleton (e.g. after changing SEVENS_AI_BUDGET_DIR). */
