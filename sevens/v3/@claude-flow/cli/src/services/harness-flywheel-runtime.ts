 ...
 * (SEVENS_HARNESS_LOOP) and bounded; lazily imports neural-tools so the daemon
 ...
 * SEVENS_HARNESS_LOOP unset it is a no-op. Best-effort; never throws.
 */
export async function runFlywheelWorker(
  ...
  try {
    if (!(opts.optInOverride ?? harnessLoopOptedIn())) return { ran: false, reason: 'opt-in required (SEVENS_HARNESS_LOOP=1)' };
...
export async function runFlywheelGenerationWorker(
  ...
  try {
    if (!(opts.optInOverride ?? harnessLoopOptedIn())) return { ran: false, reason: 'opt-in required (SEVENS_HARNESS_LOOP=1)', generation: 0 };
    const neural = await import('../mcp-tools/neural-tools.js');
    const tool = neural.neuralTools.find((t) => t.name === 'neural_patterns');
