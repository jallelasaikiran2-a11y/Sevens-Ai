 ...
 * `sevens daemon start`. This ensures a daemon is running on CLI use, SAFELY:
 *
 ...
 *     SEVENS_DAEMON_TTL_SECS) — auto-start never means "runs forever",
 *   - opt-out: SEVENS_DAEMON_AUTOSTART=0|false|no disables it entirely, OR a
 ...
 *     a shell rc file per invocation, so `export SEVENS_DAEMON_AUTOSTART=0`
...
function autostartDisabled(projectRoot: string): boolean {
  if (/^(0|false|no|off)$/i.test(process.env.SEVENS_DAEMON_AUTOSTART ?? '')) return true;
...
export function ensureDaemonRunning(
  ...
  try {
    if (autostartDisabled(projectRoot)) return { started: false, reason: 'disabled (SEVENS_DAEMON_AUTOSTART=0 or project config)' };
    ...
    if (!fs.existsSync(path.join(projectRoot, '.claude-flow')) && !fs.existsSync(path.join(projectRoot, '.claude'))) {
      return { started: false, reason: 'not a sevens project' };
    }
