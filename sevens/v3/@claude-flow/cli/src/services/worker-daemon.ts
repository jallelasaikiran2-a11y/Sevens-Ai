 ...
 *   causal_edges (30 min interval, ADR-174; SEVENS_DAEMON_NO_DISTILL=1 / --no-distill to skip)
...
export interface DaemonConfig {
  ...
  // #2356 (carry-forward from pacphi/sevens-machine-ref token-leak findings):
  ...
  // .claude-flow/config.json, or SEVENS_DAEMON_AI_WORKERS=1.
...
const DEFAULT_WORKERS: WorkerConfigInternal[] = [
  ...
  { type: 'harness', intervalMs: 6 * 60 * 60 * 1000, offsetMs: 12 * 60 * 1000, priority: 'low', description: 'Self-optimizing harness loop (opt-in SEVENS_HARNESS_LOOP, $0-default)', enabled: true },
...
const NO_DISTILL_ENV = 'SEVENS_DAEMON_NO_DISTILL';
...
// pacphi/sevens-machine-ref kit's proven value) heals a forgotten daemon within
// half a day; set SEVENS_DAEMON_TTL_SECS=0 (or `--ttl 0`) to opt out. Idle
...
export class WorkerDaemon extends EventEmitter {
  ...
  // path, `sevens daemon trigger -w <worker>` runs before headlessAvailable
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    this.config = {
      ...
      // env (SEVENS_DAEMON_TTL_SECS) > built-in default. readEnvSecsAsMs folds
      // env-or-default and honors an explicit 0 (disable).
      ttlMs: config?.ttlMs ?? fileConfig.ttlMs ?? readEnvSecsAsMs('SEVENS_DAEMON_TTL_SECS', DEFAULT_DAEMON_TTL_MS),
      idleShutdownMs: config?.idleShutdownMs ?? fileConfig.idleShutdownMs ?? readEnvSecsAsMs('SEVENS_DAEMON_IDLE_SECS', DEFAULT_DAEMON_IDLE_SHUTDOWN_MS),
      ...
      aiWorkersEnabled: config?.aiWorkersEnabled
        ...
        ?? (process.env.SEVENS_DAEMON_AI_WORKERS === '1'),
  ...
  private async initHeadlessExecutor(): Promise<void> {
    ...
    if (!this.config.aiWorkersEnabled) {
      this.log(
        'info',
        'AI workers disabled (default) - all workers run local-only. Enable with `daemon start --headless`, daemon.aiWorkers.enabled=true, or SEVENS_DAEMON_AI_WORKERS=1 (#2661)'
      );
      return;
    }
    try {
   ...
   * Inspectable by hand or via `sevens daemon status` follow-ups.
   */
   ...
   * invisible to `npx sevens memory stats` or the metrics consumers.
   *
  ...
  private async runConsolidateWorker(): Promise<unknown> {
    ...
    // Opt-out: SEVENS_DAEMON_NO_DISTILL=1 (or `daemon start --no-distill`)
   ...
   * SEVENS_BACKUP_GCS (a gs:// prefix), retention via SEVENS_BACKUP_KEEP.
   */
  private async runBackupWorker(): Promise<unknown> {
    ...
    try {
      const keepEnv = Number(process.env.SEVENS_BACKUP_KEEP);
      const r = await backupMemoryDb({
        dbPath: defaultMemoryDbPath(this.projectRoot),
        keep: Number.isFinite(keepEnv) && keepEnv > 0 ? keepEnv : 7,
        gcs: process.env.SEVENS_BACKUP_GCS || undefined,
      });
      result = {
        timestamp: new Date().toISOString(),
        backedUp: r.backedUp,
   ...
   * OPT-IN (SEVENS_HARNESS_LOOP), $0-default (no optimizer/verifier wired => no
  ...
  private async runHarnessWorker(): Promise<unknown> {
    ...
    try {
      ...
      // one tick). Opt-in ($0 no-op without SEVENS_HARNESS_LOOP).
