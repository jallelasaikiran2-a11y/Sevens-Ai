 ...
 *   - OPT-IN: no-op unless SEVENS_HARNESS_LOOP is truthy. The self-optimizing
...
export function harnessLoopOptedIn(): boolean {
  return /^(1|true|yes|on)$/i.test(process.env.SEVENS_HARNESS_LOOP ?? '');
}
...
export async function runHarnessLoopWorker(
  ...
  try {
    ...
    if (!optedIn) return { ran: false, reason: 'opt-in required (SEVENS_HARNESS_LOOP=1)' };
