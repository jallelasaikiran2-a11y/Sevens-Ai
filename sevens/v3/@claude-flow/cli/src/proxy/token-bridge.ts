/** ADR-318 access-token-only bridge from sevens auth to meta-proxy. */
...
function configureInjectedTokenPath(): void {
  ...
  const line = `sevens_injected_token_path = ${JSON.stringify(proxyInjectedTokenPath())}`;
  const pattern = /^sevens_injected_token_path\s*=.*$/m;
