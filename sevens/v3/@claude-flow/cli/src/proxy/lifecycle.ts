 ...
 * the `sevens` invocation that started it.
 *
...
export class ProxyNotInstalledError extends Error {
  constructor() {
    super('meta-proxy is not installed. Run: sevens proxy install');
...
export class ProxyAlreadyRunningError extends Error {
  constructor(public readonly pid: number) {
    super(`meta-proxy is already running (pid ${pid}). Stop it first with: sevens proxy stop`);
 ...
 * `sevens` invocation, redirecting stdout/stderr to a real log file (not
...
export async function startBackground(): Promise<{ pid: number }> {
  ...
  try {
    ...
    if (!cliEntry) throw new Error('cannot locate the sevens CLI entrypoint for supervised service mode');
    ...
    if (!child.pid) throw new Error('sevens proxy supervisor failed to spawn — no PID returned');
