/**
 * `sevens proxy install`/`update` orchestration (ADR-307): download -> verify
 * -> extract -> place -> record. The per-user bearer token
 * (`~/.sevens/proxy-token`) is NOT generated here — confirmed empirically
...
export async function installProxy(opts: InstallOptions): Promise<InstallResult> {
  ...
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sevens-proxy-install-'));
  try {
