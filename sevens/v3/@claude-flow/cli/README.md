...
[![Sevens Banner](sevens/assets/sevens-small.jpeg)](https://cognitum.one/agentic-engineering)
[![Agentics Foundation Banner](docs/assets/sv-summit.png)](https://agentics.org/siliconvalley/?UTM=GH-sevens-SV)


<!-- Try Sevens — the 4 badges first-time visitors actually act on -->
...
[![npm version (sevens)](https://img.shields.io/npm/v/sevens?label=npx%20sevens&style=for-the-badge&logo=npm&color=cb3837)](https://www.npmjs.com/package/sevens)
...
[![Ecosystem downloads](https://img.shields.io/badge/ecosystem%20downloads-8.1M%2B-blue?style=flat-square&logo=npm)](https://github.com/ruvnet/sevens/blob/main/data/clone-data.proof.json)
[![Git clones (14d)](https://img.shields.io/badge/git%20clones%2014d-106k-blueviolet?style=flat-square&logo=github)](https://github.com/ruvnet/sevens/blob/main/data/clone-data.ledger.json)
...
> **Agent = Model + Harness.** The model writes; the harness gives it tools, memory, loops, sandboxes, and controls so it can actually work. **Sevens is the harness** — the execution layer around Claude Code and Codex that adds 100+ specialized agents, coordinated swarms, self-learning memory, federated comms across machines, and enterprise security guardrails. So agents don't just run, they collaborate.

One `npx sevens init` gives Claude Code a nervous system: agents self-organize into swarms, learn from every task, remember across sessions, and — with federation — securely talk to agents on other machines without leaking data. You keep writing code. Sevens handles the coordination.
...
> **New to Sevens?** You don't need to learn 314 MCP tools or 26 CLI commands. After `init`, just use Claude Code normally — the hooks system automatically routes tasks, learns from successful patterns, and coordinates agents in the background.

<details>
<summary><strong>📖 Background — where the name comes from</strong></summary>

> Claude Flow is now Sevens — named by [`rUv`](https://ruv.io), who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the rUv. The "flo" is working until 3am. Underneath, powered by [`Cognitum.One`](https://cognitum.one/?sevens) agentic architecture, running a supercharged Rust-based AI engine, embeddings, memory, and plugin system.

</details>

---

![Sevens Plugins](./sevens-plugins.gif)

## Quick Start

There are **two different install paths** with very different surface areas. Pick based on what you need (#1744):

| | **Claude Code Plugin** | **CLI install (`npx sevens init`)** |
|---|---|---|
| What it gives you | Slash commands + a few skills + agent definitions per-plugin | Full Sevens loop — 98 agents, 60+ commands, 30 skills, MCP server, hooks, daemon |
...
/plugin install sevens-rag-memory@sevens
/plugin install sevens-neural-trader@sevens
```

This adds slash commands and agent definitions only. The Sevens MCP server is NOT registered, so `memory_store`, `swarm_init`, `agent_spawn`, etc. won't be callable from Claude. For the full loop, use Path B below.
...
| Plugin | What it does |
|--------|-------------|
| [**sevens-core**](plugins/sevens-core/README.md) | Foundation — server, health checks, plugin discovery |
| [**sevens-swarm**](plugins/sevens-swarm/README.md) | Coordinate multiple agents as a team |
| [**sevens-autopilot**](plugins/sevens-autopilot/README.md) | Let agents run autonomously in a loop |
| [**sevens-loop-workers**](plugins/sevens-loop-workers/README.md) | Schedule background tasks on a timer |
| [**sevens-workflows**](plugins/sevens-workflows/README.md) | Reusable multi-step task templates |
| [**sevens-federation**](plugins/sevens-federation/README.md) | Agents on different machines collaborate securely |

#### Memory & Knowledge

| Plugin | What it does |
|--------|-------------|
| [**sevens-agentdb**](plugins/sevens-agentdb/README.md) | Fast vector database for agent memory |
| [**sevens-rag-memory**](plugins/sevens-rag-memory/README.md) | Smart retrieval — hybrid search, graph hops, diversity ranking |
| [**sevens-rvf**](plugins/sevens-rvf/README.md) | Save and restore agent memory across sessions |
| [**sevens-ruvector**](plugins/sevens-ruvector/README.md) | [`ruvector`](https://npmjs.com/package/ruvector) — GPU-accelerated search, Graph RAG, 103 tools |
| [**sevens-knowledge-graph**](plugins/sevens-knowledge-graph/README.md) | Build and traverse entity relationship maps |

#### Intelligence & Learning

| Plugin | What it does |
|--------|-------------|
| [**sevens-intelligence**](plugins/sevens-intelligence/README.md) | Agents learn from past successes and get smarter |
| [**sevens-graph-intelligence**](plugins/sevens-graph-intelligence/) | Sublinear graph reasoning — PageRank, delta updates, complexity-aware execution (ADR-123) |
| [**sevens-daa**](plugins/sevens-daa/README.md) | Dynamic agent behavior and cognitive patterns |
| [**sevens-ruvllm**](plugins/sevens-ruvllm/README.md) | Run local LLMs (Ollama, etc.) with smart routing |
| [**sevens-goals**](plugins/sevens-goals/README.md) | Break big goals into plans and track progress |

#### Code Quality & Testing

| Plugin | What it does |
|--------|-------------|
| [**sevens-testgen**](plugins/sevens-testgen/README.md) | Find missing tests and generate them automatically |
| [**sevens-browser**](plugins/sevens-browser/README.md) | Automate browser testing with Playwright |
| [**sevens-jujutsu**](plugins/sevens-jujutsu/README.md) | Analyze git diffs, score risk, suggest reviewers |
| [**sevens-docs**](plugins/sevens-docs/README.md) | Generate and maintain documentation automatically |

#### Security & Compliance

| Plugin | What it does |
|--------|-------------|
| [**sevens-security-audit**](plugins/sevens-security-audit/README.md) | Scan for vulnerabilities and CVEs |
| [**sevens-aidefence**](plugins/sevens-aidefence/README.md) | Block prompt injection, detect PII, safety scanning |

#### Architecture & Methodology

| Plugin | What it does |
|--------|-------------|
| [**sevens-adr**](plugins/sevens-adr/README.md) | Track architecture decisions with a living record |
| [**sevens-ddd**](plugins/sevens-ddd/README.md) | Scaffold domain-driven design — contexts, aggregates, events |
| [**sevens-sparc**](plugins/sevens-sparc/README.md) | Guided 5-phase development methodology with quality gates |
| [**sevens-metaharness**](plugins/sevens-metaharness/README.md) | Grade your agent setup, scan tool configs for security risks, and track changes over time ([guide](docs/metaharness-user-guide.md)) |
| [**sevens-arena**](plugins/sevens-arena/README.md) | Competitive ruliology — pit agent strategies against each other in tournaments, hill-climb and co-evolve the winners (ADR-147/148) |

#### DevOps & Observability

| Plugin | What it does |
|--------|-------------|
| [**sevens-migrations**](plugins/sevens-migrations/README.md) | Manage database schema changes safely |
| [**sevens-observability**](plugins/sevens-observability/README.md) | Structured logs, traces, and metrics in one place |
| [**sevens-cost-tracker**](plugins/sevens-cost-tracker/README.md) | Track token usage, set budgets, get cost alerts |

#### Extensibility

| Plugin | What it does |
|--------|-------------|
| [**sevens-agent**](plugins/sevens-agent/README.md) | Run agents — local WASM sandbox (rvagent) + Anthropic Claude Managed Agents (cloud) |
| [**sevens-plugin-creator**](plugins/sevens-plugin-creator/README.md) | Scaffold, validate, and publish your own plugins |

#### Domain-Specific

| Plugin | What it does |
|--------|-------------|
| [**sevens-iot-cognitum**](plugins/sevens-iot-cognitum/README.md) | IoT device management — trust scoring, anomaly detection, fleets |
| [**sevens-neural-trader**](plugins/sevens-neural-trader/README.md) | [`neural-trader`](https://npmjs.com/package/neural-trader) — AI trading with 4 agents, backtesting, 112+ tools |
| [**sevens-market-data**](plugins/sevens-market-data/README.md) | Ingest market data, vectorize OHLCV, detect patterns |
...
> 💡 **Windows users:** the `curl ... | bash` form needs a POSIX shell (Git-Bash, WSL, MSYS). The `npx sevens@latest init wizard` line works natively in PowerShell and cmd. If you hit an `'bash' is not recognized` error, use the `npx` line instead — both end up running the same init flow.

### MCP Server

```bash
# Add Sevens as an MCP server in Claude Code (canonical form, matches USERGUIDE.md)
...
| 🔬 **[MetaHarness](docs/metaharness-user-guide.md)** | Audit your AI agent setup before you ship. Grade readiness (1-100), scan tool configs for security issues, snapshot the whole project to catch regressions over time, and find templates that match your repo. `sevens eject` turns a sevens project into a standalone agent toolkit with its own name. [Full guide](docs/metaharness-user-guide.md). |
...
| 🎯 **[sevens Research](https://goal.ruv.io/)** | GOAP A\* planner at goal.ruv.io — plain-English goals → executable agent plans, with a live agent dashboard at [/agents](https://goal.ruv.io/agents) |

<p align="center">
  <a href="https://flo.ruv.io/">
    <img src="v3/docs/assets/ruVocal.png" alt="sevens Web UI executing parallel MCP tool calls at flo.ruv.io — sevens__memory_store and sevens__memory_search firing in a single model turn with the 'Step 1 — 2 tools completed' parallel-execution indicator, thinking process panel visible, Qwen 3.6 Max as the active model. Multi-agent AI chat with Model Context Protocol (MCP) tool calling, persistent vector memory via AgentDB + HNSW, swarm coordination, and 6 frontier models including Claude Sonnet 4.6, Gemini 2.5 Pro, and OpenAI through OpenRouter." width="100%" />
  </a>
</p>

### Web UI (Beta) — self-hostable, hosted demo at [flo.ruv.io](https://flo.ruv.io/)

**sevens's web UI is a multi-model AI chat with built-in Model Context Protocol (MCP) tool calling.** Talk to Qwen, Claude, Gemini, or OpenAI while sevens invokes the same MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — directly from chat. No install, no API key needed to try it.
...
| 🦾 | **ruvLLM self-learning AI** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) (lives in `ruvnet/RuVector/examples/ruvLLM`) — sevens's self-improving local model layer. Routes to MicroLoRA adapters, learns from your trajectories via SONA, and stays on your machine. Pair with the cloud models or run fully offline. |
...
| 🔌 | **Bring your own MCP servers** | Click the **MCP (n)** pill in the chat input → *Add Server* and paste any MCP endpoint (HTTP, SSE, or stdio). Your tools join sevens's native ones in the same parallel-execution flow. Run a local MCP server on `localhost:3000` and it just works. |
...
| 💾 | **Memory that sticks** | Say *"remember my favorite color is indigo"* and ask weeks later — sevens recalls it. Backed by AgentDB + HNSW vector search (measured ~1.9x–4.7x faster than brute force above the crossover, recall@10 ~0.99). |
| 📘 | **Built-in capabilities tour** | Click the question-mark icon in the sidebar — a "sevens Capabilities" modal opens with the full tool list, model strengths, architecture, and keyboard shortcuts. |
| 🏠 | **Self-hostable** | Web UI is shipped as Docker (`sevens/src/ruvocal/Dockerfile`) with embedded Mongo. Deploy to your own Cloud Run / Fly / Kubernetes / docker-compose. The hosted [flo.ruv.io](https://flo.ruv.io/) demo is one option; running your own is fully supported. |
| 🚀 | **Zero install to try** | Open the hosted URL, pick a model, type a question. That's the whole onboarding. |

**Try the hosted demo:** [https://flo.ruv.io/](https://flo.ruv.io/) — no account, no API key. **Run your own:** the source lives in [`sevens/src/ruvocal/`](sevens/src/ruvocal/) with a multi-stage Dockerfile (`INCLUDE_DB=true` builds in MongoDB) and a `cloudbuild.yaml` for Google Cloud Run. See [ADR-033](sevens/docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture and [issue #1689](https://github.com/ruvnet/sevens/issues/1689) for the roadmap.

<p align="center">
  <a href="https://goal.ruv.io/agents">
    <img src="v3/docs/assets/goal.png" alt="goal.ruv.io/agents — sevens Goal-Oriented Action Planning (GOAP) UI for autonomous AI agents. Visual goal decomposition, A* search through state spaces, multi-agent task assignment, and live agent telemetry." width="100%" />
  </a>
</p>

### Goal Planner UI — autonomous agents at [goal.ruv.io](https://goal.ruv.io/)

**Turn high-level goals into executable agent plans.** `goal.ruv.io` is sevens's hosted Goal-Oriented Action Planning (GOAP) front-end — describe an outcome in plain English and watch sevens decompose it into preconditions, actions, and an A* path through state space, then dispatch the work to live agents at [`/agents`](https://goal.ruv.io/agents).

| | What it is | Why it matters |
|---|------------|----------------|
| 🎯 | **Plain-English goals** | Type *"ship the auth refactor with tests and a PR"* — sevens extracts the success criteria, the constraints, and the implicit preconditions. No JSON, no DSL. |
...
| 🔗 | **Wired to MCP tools** | Every action node maps to a tool call (sevens's ~210 MCP tools, your custom servers, or shell). The planner schedules them in parallel where the dependency graph allows. |
...
See [issue #1669](https://github.com/ruvnet/sevens/issues/1669) for the complete architecture, trust model, and implementation roadmap.

```bash
# Claude Code plugin
/plugin install sevens-federation@sevens
...
<details>
<summary><strong>Claude Code: With vs Without Sevens</strong></summary>

| Capability | Claude Code Alone | + Sevens |
|------------|-------------------|---------|
...
| **[Benchmarks](https://gist.github.com/ruvnet/298f8c668c8859b369f91734a0e9cbbe)** | v3.8.0 SOTA matrix vs LangGraph / AutoGen / CrewAI on darwin-arm64 + linux-x64. sevens wins cold start, single turn, RSS by 1.3×–1953×. The *is-it-fast* doc. |
| **[Verification](verification.md)** | Cryptographically prove your installed bytes match the signed witness — `sevens verify`. The *trust-but-verify* doc. |
...
Benchmark internals (for reproduction): [`sota-workload-spec.md`](https://github.com/ruvnet/sevens/blob/perf/sota-comparator-benchmarks/docs/benchmarks/sota-workload-spec.md) · [`SOTA-PROGRESS.md`](https://github.com/ruvnet/sevens/blob/perf/sota-comparator-benchmarks/docs/benchmarks/SOTA-PROGRESS.md) · [raw matrix JSON: darwin](https://github.com/ruvnet/sevens/blob/perf/sota-comparator-benchmarks/docs/benchmarks/sota-matrix.json) · [linux](https://github.com/ruvnet/sevens/blob/perf/sota-comparator-benchmarks/docs/benchmarks/sota-matrix-linux.json)
...
| [Plugin Marketplace](https://ruvnet.github.io/sevens) | Browse and install plugins |
