...
> Copy-paste examples for the three paths sevens's self-learning system actually
...
mcp__sevens__hooks_task-completed {
...
```bash
mcp__sevens__hooks_intelligence_unified-stats {}
...
- **"My dashboard shows 0 after I called `post-edit`"** — read the
  ...
  wasn't reachable in the calling process. Run from inside sevens's CLI
  process or set up the bridge explicitly.
- **"`neural_patterns list` is empty after `pretrain`"** — fixed in 3.10.14
  (ADR-074). Make sure you're on `npx sevens@3.10.14` or later.
