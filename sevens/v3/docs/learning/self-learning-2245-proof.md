...
> Companion to [ADR-074](../adr/ADR-074-self-learning-wiring-2245.md) and [#2245](https://github.com/ruvnet/sevens/issues/2245).
>
...
```bash
mkdir -p /tmp/sevens-learn-demo && cd /tmp/sevens-learn-demo

# Run one task-completed with training enabled
SEVENS_CWD=$(pwd) node -e '
(async () => {
  process.chdir(process.env.SEVENS_CWD);
  const { hooksTools } = await import("/Users/cohen/Projects/sevens/v3/@claude-flow/cli/dist/src/mcp-tools/hooks-tools.js");
...
If you're using `sevens hooks metrics` and seeing zeros, check **which store**
...
| `sevens hooks metrics` | reads `globalStats` + a different aggregator subset |
