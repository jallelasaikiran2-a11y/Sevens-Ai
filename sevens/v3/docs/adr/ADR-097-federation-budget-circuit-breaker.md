...
**Version**: Phase 1 shipped in `@claude-flow/plugin-agent-federation` (v3.6.x); Phase 3 consumer in `sevens-cost-tracker@0.14.0`
**Supersedes**: nothing
...
Agent Federation (ADR-086) lets agents on one Sevens node delegate tasks to peer nodes across trust boundaries. The implementation in `@claude-flow/plugin-agent-federation` already covers identity (Ed25519 keypairs), trust scoring, and `federation_send` for cross-node delegation. What it does **not** cover today:
...
- **No unified cost surface**. The `sevens-cost-tracker` plugin tracks local agent token usage but federation traffic is invisible to it.
...
| Cost tracker plugin | `plugins/sevens-cost-tracker/` | Tracks local model spend per agent / per session |
...
Wire the federation layer into `sevens-cost-tracker` so federated spend appears in the same dashboards as local spend:
...
- `sevens doctor` reports the current state of every known peer (`ACTIVE`/`SUSPENDED`/`EVICTED`) and the trailing-24h cost. A peer pinned in `SUSPENDED` for >1h shows up as a yellow warning.
...
| **Phase 3 consumer** — Cost-tracker bus event + per-peer rolling aggregation | Implemented | `plugins/sevens-cost-tracker/scripts/federation.mjs`, `skills/cost-federation/SKILL.md` | `1c0804315 feat(cost-tracker): P6 — ADR-097 Phase 3 federation_spend consumer (v0.14.0)` |
...
| **Phase 3 plugin wiring** — federation plugin adopts budget integration + ADR-097 doc | Implemented | `plugins/sevens-federation/` (v0.2.0), `docs/adrs/0001-federation-contract.md` | `b0168e4a5 feat(sevens-federation): adopt plugin contract — 3-gate alignment + ADR-097 budget integration + smoke` |
...
- `sevens doctor` shows peer states + trailing-24h cost
