...
**Date**: 2026-06-03
**Issue**: research/copilot-sdk-sevens-integration (branch); follow-up issue to be filed at PR time
...
GitHub announced general availability of the **Copilot SDK** (`@github/copilot-sdk`) on 2026-06-02. The SDK exposes the same agentic runtime that powers the Copilot app — planning, tool invocation, streaming, multi-turn sessions, MCP server registration, and custom tool calling — directly to any application or CLI, without requiring a separate orchestration layer. It is MIT-licensed, runs on Node.js 20+, and supports six target languages including TypeScript. Reference: [Copilot SDK research dossier](../../../docs/research/copilot-sdk-sevens-integration.md).

This creates a strategic opportunity for sevens. The repository already ships **`@claude-flow/codex`** — an OpenAI Codex CLI platform adapter that proves the modular extension pattern: it adds a second programmable agent platform (alongside the existing Claude Code adapter) via a self-contained package, a `DualModeOrchestrator`, a `/loop` runner, generators, validators, and migrations. The Copilot SDK is the natural third platform.
...
| MCP integration | One-shot `codex mcp add sevens` → bidirectional MCP bridge (`mcpServers` map in every session config) |
...
3. **The model story moves under us if we wait.** GPT-5.3-Codex is the new Copilot LTS through 2027-02-04. GPT-5.4-mini at 0.33× multiplier is the cheapest GA model in the catalog right now. Hardcoding Tier-2 to Haiku locks sevens to a single vendor for the fast path; adopting Copilot lets the router pick the cheapest validated GA model per call. Stable semver (`3.8.0`) is the version this should ship under, per the [MEMORY.md "stable semver, no alpha" rule](../../../../.claude/projects/-Users-cohen-Projects-sevens/memory/feedback_stable_semver_no_alpha.md).
...
- **`register.ts`** — `registerSevensMcpWithCopilot(projectPath, toolFilter)` returns the `mcpServers` object shape required by `createSession({ mcpServers: { sevens: { type: 'local', command: 'npx', args: [...], tools: ['*'] } } })`. Defaults: `type: 'local'`, `command: 'npx'`, `args: ['-y', 'sevens@latest', 'mcp', 'start']`, `tools: ['*']`, `env: { CLAUDE_FLOW_CONFIG }`. This is the path that lets Copilot agents call `memory_store`, `swarm_init`, `hooks_route`, etc. during a session.
...
3. Cache only a **source identifier** (`"env:COPILOT_GITHUB_TOKEN"` or `"gh-cli"`) at `~/.config/sevens/copilot/token.json`, chmod 600.
...
**Pure MCP integration — register a sevens MCP server with Copilot, ship no SDK code.** Defensible. Users running Copilot already get sevens tools via the existing `npx sevens mcp start`. But the *governance* value (compile/enforce/prove/evolve wrapping every Copilot model call) requires a code path *inside* the orchestrator that knows when a Copilot call is happening. MCP-only means the model call is opaque to sevens; we can only audit what tools Copilot decided to invoke, not what model produced what reasoning. The `MultiModeOrchestrator` route is the only one that gives full provenance.

**Cloud-only via GitHub Actions.** Run sevens+Copilot collaboration as a GHA workflow with the Copilot Agent service. Loses the local-first MCP server architecture, breaks the `dual run` developer-loop UX, and introduces a per-org admin-approval dependency. Useful future addition; not a substitute for the local adapter.
...
- Copilot's MCP architecture (client-side subprocess) means sevens's local MCP server stays the source of truth; no extra deployment surface.
...
- [Copilot SDK research dossier](../../../docs/research/copilot-sdk-sevens-integration.md) — full source inventory, model catalog, governance integration design (1047 lines, 16 grade-A sources).
...
5. **`@claude-flow/security` TokenCache** — research §9 item 7; confirmed absent. P1 implements file-cache directly using `PathValidator` from `@claude-flow/security` (already exists) to validate the `~/.config/sevens/copilot/` directory.
...
- `src/client/auth.ts` + tests — `resolveCredential()` returns SOURCE identifiers only, never the token value. Cache file path `~/.config/sevens/copilot/token.json`, chmod 600. `CopilotAuthRequiredError` with stable code.
...
- `src/mcp/register.ts` + tests — `registerSevensMcpWithCopilot()` builds the local-stdio `mcpServers` map. `buildMcpServers()` merges caller-supplied servers.
