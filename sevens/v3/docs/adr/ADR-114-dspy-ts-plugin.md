# ADR-114 — Adopt DSPy.ts as a Sevens plugin for declarative prompt-program optimization
...
The key observation: **`dspy.ts` is built on the same substrate sevens already ships** (`agentdb`, ReasoningBank, HNSW, RaBitQ). Integrating it is not bolting on a foreign dependency with its own storage and lifecycle — it is wiring a complementary capability onto infrastructure that is already present.

### What this would give sevens that it does not have

| Capability | Today in sevens | With a `dspy` plugin |
|---|---|---|
...
`dspy.ts` and sevens's intelligence system *both* lean on AgentDB + ReasoningBank, and both are "self-learning". Without a sharp boundary, this becomes two systems doing fuzzily-similar things, confusing both Claude (which tool to call — see ADR-112) and contributors (where does this logic live?).
...
- **Sevens intelligence = run-time, cross-task.** "Across *all* tasks this agent has ever done, what patterns predict success here?" It is always-on, ambient, and not tied to a single signature/metric.

A DSPy-compiled module can *write into* ReasoningBank (its trials are patterns), and sevens intelligence can *retrieve* those — but they are not the same loop, and the plugin must not re-implement the intelligence pipeline.

## Decision

**Adopt `dspy.ts` as an optional Sevens plugin — `@claude-flow/plugin-dspy` — that wraps the published `dspy.ts` npm package and exposes declarative prompt-program optimization through MCP tools, a slash skill, and an optional agent-loop hook. It shares sevens's AgentDB instance rather than standing up its own store.**
...
**Storage**: the plugin receives sevens's AgentDB handle via the plugin context. It uses dedicated namespaces (`dspy/signatures`, `dspy/modules`, `dspy/trials`) so its data is inspectable and prunable independently, and so the `agentdb-curator` consolidation pipeline can see DSPy trials as candidate patterns.
...
- It does **not** introduce a second LM-provider abstraction. `dspy.ts`'s `configureLM` is bridged to sevens's existing provider config (`@claude-flow/providers`) so there is one place to set keys/models.
...
- **First-party, MIT, same author.** No third-party licensing or maintenance-risk concerns; the package and sevens move together.
...
- **Another package to keep version-aligned** with the `agentdb` line (if `dspy.ts` and sevens pin different `agentdb` majors, the shared-handle story breaks). Mitigated by both being first-party.
