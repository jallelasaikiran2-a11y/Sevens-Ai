...
**Status**: Accepted — Implemented in sevens 3.10.28
**Date**: 2026-05-30
...
| **sevens Lucene RRF + CE rerank (BGE-base 110M)** | **0.521** |
| Multi-field RRF + CE rerank | 0.520 |
| Lucene BM25 alone | 0.505 |
| BM25 (published Lucene) | 0.502 |
| dense alone (BGE-base) | 0.489 |

**Acceptance test from the climb plan ("sevens beats BM25 on both datasets") PASSES.** With RRF+CE rerank we're 0.521 on the 2-dataset mean — beats published BM25 (0.502), beats every other published baseline except SPLADE++ (0.526, 1 percentage point above us) and BGE-large (0.551, 3 percentage points above).
...
- Did not switch the runtime retrieval defaults to Lucene BM25. Runtime stays on multi-field BM25 (better for sevens's commit-history corpora). The Lucene BM25 module is BEIR-runner-scoped.
