# ADR-305 — Customer Lifecycle Funnel (sevens → Cognitum)
...
sevens has millions of monthly package downloads and a large active user base, but relatively little product discovery beyond the CLI itself. Cognitum One (https://cognitum.one) offers adjacent paid capabilities — Meta LLM routing, hosted memory, premium agents, enterprise features — that most sevens users never encounter.
...
Credit exhaustion guidance              (ADR-303)
      ↓
Authentication (sevens auth login)
      ↓
...
- **Open-source sevens stays whole.** No existing capability moves behind the funnel; ADR-150 removability discipline applies to every funnel component.
...
```
1. SEVENS_FUNNEL=0                 (environment — shells, dev containers, CI images, MDM-pushed profiles)
...
Enterprise documentation must cover: shell-environment deployment, dev-container configuration, CI environment defaults, MDM/endpoint-management deployment, air-gapped configuration, and audit verification (`sevens doctor` prints the effective state and which source decided it).
...
- The state is inspectable: `sevens doctor` reports whether funnel surfaces are active, disabled by user config, disabled by enterprise policy, or disabled by guardrail — and which mechanism delivered the decision.
...
1. The signed helper refresh fires (manifest stamp updated, signature verified, `statusline.cjs` replaced) without running `sevens init`.
...
4. **`SEVENS_FUNNEL=0` suppresses every funnel surface** (ADR-301, 302, 303).
5. All core sevens commands remain behaviorally identical — exit codes, operational output, and latency within gate thresholds — in every configuration.
