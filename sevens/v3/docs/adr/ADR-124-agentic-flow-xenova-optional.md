...
**Related**: [`agentic-flow@2.0.11`](https://www.npmjs.com/package/agentic-flow), [ruvnet/agentic-flow](https://github.com/ruvnet/agentic-flow), ADR-118 (AIDefence 2.3.0), ADR-121 (embeddings RuVector upgrade — long-term Xenova migration), ADR-122 (browser substrate), supply-chain hardening PR #2050, sevens issue #2046
...
The `agentdb → @opentelemetry/sdk-node` chain was fixed in [PR #2050 commit `9a8c9c464`](https://github.com/ruvnet/sevens/commit/9a8c9c464) by bumping agentdb to `3.0.0-alpha.14` + npm overrides pinning `@opentelemetry/sdk-node ≥ 0.218.0`. Overrides do NOT cascade across separate npm projects, so the same block was duplicated in `v3/@claude-flow/browser/package.json` to keep standalone installs clean.
...
## Update sevens after upstream ships
...
- **Full Xenova retirement** — ADR-121 Phase 4 tracks the larger migration to `ruvector-onnx-embeddings-wasm`. This ADR is a tactical patch that lets sevens ship a clean supply-chain audit *now*; the strategic migration continues independently.
...
- sevens PR #2050 (supply-chain hardening that surfaced this)
- sevens issue #2046
