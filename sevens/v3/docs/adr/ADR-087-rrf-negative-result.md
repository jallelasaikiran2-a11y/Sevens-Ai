...
**Status**: Accepted — Implemented in sevens 3.10.27
**Date**: 2026-05-30
...
The gap comes from our multi-field BM25's tokenisation choices: no Snowball stemmer, smaller stopword list (~25 vs Lucene's ~120), no Lucene-style length normalisation. We optimised it for sevens commit-history retrieval (where stemming hurts) — but BEIR's medical/scientific corpora reward stemming.
