...
**Related ADRs**:
- ADR-034 (Optional MCP backends — `sevens`/`ruvector`/`agentic-flow` stdio backends)
...
**In scope**: the deployed MCP bridge (`sevens/src/ruvocal/mcp-bridge/index.js`) and its sibling (`sevens/src/mcp-bridge/index.js`); the shipping `sevens/docker-compose.yml`; the nginx reverse proxy (`sevens/src/nginx/nginx.conf`); the stdio MCP backend spawn path; the `MCP_GROUP_*` tool-group exposure model; and the `plugin-agent-federation` default bind host.
...
find sevens -name docker-compose.yml -o -name nginx.conf
grep -n 'app.post("/mcp"' sevens/src/ruvocal/mcp-bridge/index.js

# Confirm no inbound auth gate exists
grep -niE 'mcp_token|bearer|x-api-key|requireAuth|authMiddleware|MCP_AUTH' \
  sevens/src/ruvocal/mcp-bridge/index.js   # → only outbound Authorization to LLM providers

# Confirm the blocklist's sole call site is the autopilot flow
grep -n 'isBlockedTool' sevens/src/ruvocal/mcp-bridge/index.js   # def :1501, call :1633

# Confirm env inheritance to spawned backends, and listen bind
grep -n 'env: { ...process.env }\|app.listen(' sevens/src/ruvocal/mcp-bridge/index.js
```
...
### 2.1 Topology (shipping `sevens/docker-compose.yml`)
...
### 2.2 Bridge request surface (`sevens/src/ruvocal/mcp-bridge/index.js`)
...
The deployed bridge is the ruvocal variant; the sibling `sevens/src/mcp-bridge/index.js` shares the identical flaws (the report's quoted line numbers 1302/1428 match that sibling file). Both must be remediated.

| ID | Severity | Finding | Evidence (repo-relative) |
|----|----------|---------|--------------------------|
| **V1** | **Critical** | `POST /mcp` dispatches `tools/call → executeTool()` with no authentication. The only middleware is JSON + wildcard CORS. | `sevens/src/ruvocal/mcp-bridge/index.js:1102‑1138`; middleware `:1034‑1043` |
| **V2** | **Critical** | `terminal_execute` reachable unauthenticated → shell as `node` (uid 1000). Served by the `sevens` backend; `devtools` defaults on. | `executeTool` default route `:979‑989`; backend def `:259‑261`; compose `:43` |
...
| **V5** | **High** | MongoDB bound to all interfaces on `27017` with no authentication — directly reachable off-host, independent of the RCE. | `sevens/docker-compose.yml:10‑18` |
| **V6** | **High** | Bridge port `3001` bound to all interfaces by default (amplifies V1 from local to internet). `app.listen(PORT)` has no bind-host argument. | `sevens/docker-compose.yml:26‑27`; `index.js:1889` |
| **V7** | **Medium** | `Access-Control-Allow-Origin: *` on every response at both nginx and the bridge — defense-in-depth gap for browser-driven pivots. | `sevens/src/nginx/nginx.conf:20‑22`; bridge `:1037‑1040` |
...
| 2 | RCE — shell in container; OAST callback proves outbound egress | `sevens__terminal_execute` | V1, V2, V3 |
| 3 | Credential theft — `printenv | grep -iE 'KEY|TOKEN|SECRET'` | terminal_execute | V2, V4 |
| 4 | Weaponize — spin up attacker-controlled swarm on victim keys | `sevens__swarm_init`, `sevens__agent_spawn` | V1 |
| 5 | **AI poisoning** — inject malicious "compliance"/"best-practice" patterns so future generated code beacons out | `sevens__agentdb_pattern-store` | V1 |
...
Apply Layer-1–3 changes to **both** bridge files (`sevens/src/ruvocal/mcp-bridge/index.js` and `sevens/src/mcp-bridge/index.js`). Authentication is the root-cause fix; every other layer is independent so no single regression re-opens the chain.
...
**1a. Bind loopback by default everywhere (root cause of public-default exposure, V6/V8).** Was 1c. Now the leading change.
  - `sevens/docker-compose.yml`: `"3001:3001"` → `"127.0.0.1:3001:3001"`; same for Mongo `"27017:27017"` → `"127.0.0.1:27017:27017"`
  - `sevens/src/ruvocal/mcp-bridge/index.js` + `sevens/src/mcp-bridge/index.js`: `app.listen(PORT)` → `app.listen(PORT, BIND_HOST)` where `BIND_HOST = process.env.MCP_BIND_HOST || '127.0.0.1'`
...
|-------------------|---------------------|------------------------|
| Deployed bridge is the ruvocal variant | compose `build.context` read | `sevens/docker-compose.yml:22‑23` |
| `POST /mcp` dispatches `tools/call → executeTool` with no auth | File read of handler + middleware | `sevens/src/ruvocal/mcp-bridge/index.js:1102‑1138`, `:1034‑1043` |
| No inbound auth gate exists | `grep -niE 'mcp_token\|bearer\|x-api-key\|requireAuth\|authMiddleware\|MCP_AUTH'` → only outbound provider `Authorization` at `:1571`,`:1780` | `sevens/src/ruvocal/mcp-bridge/index.js` |
| `terminal_execute` served by `sevens` backend; `devtools` default-on | Backend def + group-enabled expression + compose default | `index.js:259‑261`, `:56`; `docker-compose.yml:43` |
| Blocklist enforced only in autopilot | `isBlockedTool` def `:1501`, sole call `:1633` (inside `handleAutopilot`) | `sevens/src/ruvocal/mcp-bridge/index.js` |
| Backends spawned with full env inheritance | File read of `StdioMcpClient.start()` | `sevens/src/ruvocal/mcp-bridge/index.js:135‑138` |
| `app.listen(PORT)` has no bind-host arg (→ 0.0.0.0) | File read of `main()` | `sevens/src/ruvocal/mcp-bridge/index.js:1889` |
| Bridge port `3001` + Mongo `27017` bound to all interfaces | compose `ports` read | `sevens/docker-compose.yml:15‑16`, `:26‑27` |
| MongoDB has no auth configured | No `MONGO_INITDB_ROOT_*` in compose | `sevens/docker-compose.yml:10‑18` |
| Provider keys injected into the bridge env | compose `environment` read | `sevens/docker-compose.yml:32‑53` |
| CORS wildcard at nginx and bridge | File reads | `sevens/src/nginx/nginx.conf:20‑22`; `sevens/src/ruvocal/mcp-bridge/index.js:1037‑1040` |
| Sibling bridge shares the flaw (report's 1302/1428) | `sed -n '1300,1305p;1426,1430p'` matches blocklist + `isBlockedTool` autopilot call | `sevens/src/mcp-bridge/index.js` |
...
- [ADR-029](../../../sevens/docs/adr/ADR-029-HUGGINGFACE-CHAT-UI-CLOUD-RUN.md) — documented Cloud Run deployment path
- [ADR-034](../../../sevens/docs/adr/ADR-034-OPTIONAL-MCP-BACKENDS.md) — optional stdio MCP backends (`sevens`/`ruvector`/…)
- [ADR-035](../../../sevens/docs/adr/ADR-035-MCP-TOOL-GROUPS.md) — `MCP_GROUP_*` default-on/opt-in model
- [ADR-037](../../../sevens/docs/adr/ADR-037-AUTOPILOT-CHAT-MODE.md) — autopilot flow (where the blocklist is enforced)
- [ADR-038](../../../sevens/docs/adr/ADR-038-RUVOCAL-FORK.md) — the deployed ruvocal chat UI + bridge
