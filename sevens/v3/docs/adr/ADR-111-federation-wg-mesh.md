# ADR-111 — Federation network mesh via WireGuard, governed by sevens trust + breaker
...
- Supersedes parts of: ADR-104's "tailnet provides TLS" assumption (this ADR makes sevens own the network layer optionally)
...
1. **Operational coupling to Tailscale Inc.** Most operators in our session validation used Tailscale as the connectivity layer. Tailscale is excellent but introduces an external trust/billing/availability dependency that's outside sevens's control. Headscale (self-hosted Tailscale-compatible coord server) reduces the trust dependency but still requires a dedicated control-plane service.
...
- Maps sevens trust levels to packet-layer `AllowedIPs` slices (see "Trust-graded access" below)
...
- The operator wants packet-layer access to follow sevens trust changes (no two-system drift)
...
Anyone running `node plugins/sevens-core/scripts/witness/verify.mjs --manifest .claude-flow/federation/wg-witness.md.json` can prove the mesh's history end-to-end. **This is something Tailscale fundamentally can't offer** because their coordination is server-mediated.
...
- Writes to `/etc/wireguard/sevens-fed.conf` (linux) or equivalent path on macOS
- `wg-quick up sevens-fed` invocation (with operator confirmation per CLAUDE.md "destructive actions" guidance — bringing up a network interface qualifies)
...
- On SUSPENDED: `wg set sevens-fed peer <pubkey> remove-allowed-ips`
- On EVICTED: `wg set sevens-fed peer <pubkey> remove`
