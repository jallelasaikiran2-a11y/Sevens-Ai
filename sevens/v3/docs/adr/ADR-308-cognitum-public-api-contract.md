...
- **OpenAPI specification** checked into **both** repositories (sevens: `v3/docs/api/cognitum-v1.openapi.yaml`; server repo mirrors it). CI in both repos validates against the shared spec; drift fails the build.
...
|---|---|
| Auth | Local sevens continues working fully; auth-gated capabilities degrade with a clear error |
...
- Contract tests run in sevens CI against a mock server generated from the spec — the funnel surfaces are testable with zero network and no Cognitum dependency.
...
future risk: `sevens auth` (ADR-306) was implemented against the proven `auth.cognitum.one`
