...
2. `plugins/sevens-core/hooks/hooks.json` defines only `PreToolUse`, `PostToolUse`, `PreCompact`, `Stop` — no UserPromptSubmit / SessionStart context injection.
3. No `trigram` / `jaccard` symbols in plugin/helper hooks (only one reference in `plugins/sevens-rag-memory/README.md` documenting MMR diversity reranking — different code path).
...
- CI guard: `plugins/sevens-core/scripts/test-consensus-transport.mjs` (in the `mcp-roundtrip-smoke` job) — asserts exports present (incl. `FederationTransport`), `LocalTransport` round-trips, Ed25519 verify is real (no `return true` stub regression).
...
Tracked in [#1872](https://github.com/ruvnet/sevens/issues/1872) and PR #1905.
...
- [#1896](https://github.com/ruvnet/sevens/issues/1896) — external audit response with the per-gap measurement evidence above.
