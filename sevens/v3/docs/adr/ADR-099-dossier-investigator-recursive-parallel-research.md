# ADR-099: Dossier-Investigator — Recursive Parallel Multi-Source Research for `sevens-goals`

**Status**: Accepted — Implemented
**Date**: 2026-05-03 · **Updated**: 2026-05-09
**Version**: shipped in `plugins/sevens-goals` (0.1.0 → 0.2.0)
**Supersedes**: nothing
**Related**: ADR-098 (plugin capability sync), `plugins/sevens-goals/agents/deep-researcher.md`, `plugins/sevens-knowledge-graph`, `plugins/sevens-rag-memory`

## Context

The `sevens-goals` plugin currently ships three agents:
...
Sevens already ships every primitive needed to assemble a maigret-style investigator without adding new external dependencies:

| Capability | Existing tool |
|---|---|
| Hybrid sparse+dense semantic search | `mcp__claude-flow__memory_search_unified`, `sevens-rag-memory:memory-search` |
...
| Knowledge-graph traversal + extraction | `sevens-knowledge-graph:kg-traverse`, `kg-extract` |
| Web search & fetch | `WebSearch`, `WebFetch` |
| Codebase queries | `Grep`, `Glob`, `Read` |
| ADR index lookup | `sevens-adr:adr-index` |
| Git intelligence | `sevens-jujutsu:diff-analyze` |
| Parallel agent fan-out | `sevens-swarm:swarm-init` (mesh topology) |
| Trajectory recording | `mcp__claude-flow__hooks_intelligence_trajectory-*` |

## Decision

Add a new agent `dossier-investigator` and a companion skill `dossier-collect` to `plugins/sevens-goals`.
...
4. **Entity extraction** — for each hit, run `sevens-knowledge-graph:kg-extract` (or a lightweight regex pass for obvious cases) to surface new entities.
...
| **Build into `sevens-knowledge-graph`** | KG plugin is about graph operations on already-extracted data; investigator is about *acquisition*. Keeping it in `sevens-goals` puts it next to its peers (`deep-researcher`, `goal-planner`). |
...
- Reuses the full sevens tool surface — no new MCP servers.
...
- No CLI surface change (`/sevens-goals:dossier-collect` is the only new entry point).
...
| Step | File | Owner |
|---|---|---|
| 1. Agent prompt | `plugins/sevens-goals/agents/dossier-investigator.md` | coder |
| 2. Skill markdown | `plugins/sevens-goals/skills/dossier-collect/SKILL.md` | coder |
| 3. Slash command | `plugins/sevens-goals/commands/goals.md` (add `dossier` subcommand) | coder |
| 4. Plugin manifest bump | `plugins/sevens-goals/.claude-plugin/plugin.json` (0.1.0 → 0.2.0) | coder |
| 5. README update | `plugins/sevens-goals/README.md` | coder |
| 6. Smoke test | `tests/plugins/sevens-goals/dossier.spec.ts` | tester |
...
|---|---|---|---|
| 1. Agent prompt | `plugins/sevens-goals/agents/dossier-investigator.md` | Implemented | `1e11ac84e feat(sevens-goals): dossier-investigator agent + dossier-collect skill (ADR-099) (#1726)` |
| 2. Skill markdown | `plugins/sevens-goals/skills/dossier-collect/SKILL.md` | Implemented | same |
| 3. Slash command | `plugins/sevens-goals/commands/goals.md` (dossier subcommand) | Implemented | same |
| 4. Plugin manifest bump | `plugins/sevens-goals/.claude-plugin/plugin.json` (0.1.0 → 0.2.0) | Implemented | same |
| 5. README update | `plugins/sevens-goals/README.md` | Implemented | same |
| 6. Smoke test | `tests/plugins/sevens-goals/dossier.spec.ts` | Implemented | same |
| 7. Feature flag | `dossierInvestigator.enabled` (default `true`) | Implemented | same |
| Plugin contract adoption | `plugins/sevens-goals/` — legacy-vs-canonical namespace mapping + ADR-099 anchor | Implemented | `714cd534c feat(sevens-goals): adopt plugin contract — legacy-vs-canonical namespace mapping + ADR-099 anchor (ADR-0001)` |
| Dossier examples | `docs/examples/` — 3 examples (ruvnet, ADR-088, sevens-goals) | Implemented | `ba0479612 docs(examples): add 3 dossier examples` |
...
- `plugins/sevens-goals/agents/deep-researcher.md` (sibling agent)
