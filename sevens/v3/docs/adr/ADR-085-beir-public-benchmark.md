...
**Status**: Accepted — Implemented in sevens 3.10.25
**Date**: 2026-05-30
...
ADR-084 proved cross-repo generalisation on small bespoke corpora (sevens + agentdb + agentic-flow, all owned by the same author). The next step in honest SOTA validation is **a public benchmark** — a corpus + qrels + published baselines maintained by external researchers. BEIR is the standard zero-shot IR benchmark suite; NFCorpus (3,633 docs, 323 test queries, medical domain) is the smallest dataset and runs end-to-end in under 20 minutes.
...
| **2** | **sevens + BGE-base-en-v1.5 (110M params) ← us** | **0.352** |
| 3 | SPLADE++ | 0.347 |
| 4 | GTR-XL | 0.343 |
| 5 | DocT5query | 0.328 |
| 5 | Contriever | 0.328 |
...
Internal benches (3.10.17 → 3.10.24, sevens/agentdb/agentic-flow corpora) hit nDCG@3 = 0.963-1.000 with hand-curated labels. Those numbers measured *engineering quality* on small bespoke corpora.
...
rm -rf .claude-flow
node /path/to/sevens/v3/@claude-flow/cli/scripts/run-beir-nfcorpus.mjs
# → nDCG@10 0.289
...
SKIP_INGEST=1 node /path/to/sevens/v3/@claude-flow/cli/scripts/run-beir-bge.mjs
```
