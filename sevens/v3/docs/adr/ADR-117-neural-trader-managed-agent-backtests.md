...
**Related**: ADR-115 (Claude Managed Agents as the cloud agent runtime — `managed_agent_*` MCP tools in `sevens-agent`) · `sevens-neural-trader` plugin (`neural-trader` npm — Rust/NAPI engine, 112+ MCP tools, 4 agents, 7 skills incl. the new `trader-cloud-backtest`) · ADR-026 (3-tier model routing) · ADR-112 (MCP tool discoverability) · #1931 (sevens-agent / managed agents tracking) · [Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview)
**Supersedes**: nothing

## Context

`sevens-neural-trader` wraps the `neural-trader` npm package (Rust/NAPI engine): walk-forward backtests, Monte-Carlo simulation, parameter sweeps, LSTM/Transformer/N-BEATS training, regime detection, risk metrics (VaR/CVaR, Kelly, circuit breakers). The expensive jobs — a multi-year walk-forward with thousands of Monte-Carlo paths, a parameter sweep over a strategy grid, training a Transformer on a long history — are **long-running (minutes to hours), CPU/compute-heavy, and async**. Today they run one of two places:
...
ADR-115 added a third runtime — **Anthropic Claude Managed Agents** (`managed_agent_*` in the `sevens-agent` plugin): a cloud container with pre-installed packages (`pip`/`npm`/`apt`/`cargo`/`gem`/`go`), network access, a persistent filesystem, and a managed agent loop. That is *exactly* the shape neural-trader's heavy jobs want. The `managed_agent_*` tools shipped in `3.7.0-alpha.27` and are validated end-to-end — so the building blocks exist; this ADR is about wiring neural-trader to them as a first-class flow (not just "you could call those tools").

## Decision

**Add a "cloud backtest/train" recipe to `sevens-neural-trader` that dispatches heavy `neural-trader` jobs to a Managed Agent cloud container, via the `managed_agent_*` tools — no new MCP tools, no new dependencies, just orchestration.**

### Surface

- **`plugins/sevens-neural-trader/skills/trader-cloud-backtest/SKILL.md`** — the recipe:
...
5. **Terminate eagerly** — `managed_agent_terminate` the instant the metrics + artifacts are pulled; never leave an idle billing container. A `sevens doctor` / GC check (per #1931) catches orphans.
...
- Wiring `sevens mcp start` as the cloud agent's `mcpServers` so the cloud agent can write to sevens memory / cost-tracking *directly* — needs a publicly reachable HTTP sevens MCP server (ADR-115 follow-up). Until then, sevens-memory ingestion happens *locally* after `managed_agent_events`.
- The same recipe applies to `sevens-market-data` (ingest a large feed in the cloud, vectorize OHLCV) — note it, don't build it here.

### Implementation (shipped with this ADR)

- `plugins/sevens-neural-trader/skills/trader-cloud-backtest/SKILL.md` — the recipe (provision-once · pre-flight-cheap · run · ingest-locally · terminate-eagerly), `allowed-tools` scoped to `managed_agent_*` + `memory_*` + `agentdb_pattern-store` + `Bash`/`Read` (no wildcard grant).
...
- `README.md` — overview + skills table + Architecture-Decisions + Related-Plugins cross-refs to this ADR / ADR-115 / `sevens-agent`.
...
- **Generalizes.** The "heavy-job → managed-agent recipe" pattern transfers to `sevens-market-data` and any other compute-heavy plugin.
...
- "Adopt the cloud runtime for a heavy workload" — not a new pattern, just applying ADR-115 to neural-trader. Same posture as `sevens-agent` adopting Managed Agents.
...
- `sevens-neural-trader` — `plugins/sevens-neural-trader/README.md`; `neural-trader` npm: https://www.npmjs.com/package/neural-trader
...
- #1931 — sevens-agent / managed agents tracking (orphaned-session GC, `sevens mcp start` HTTP server for the cloud-MCP combo)
