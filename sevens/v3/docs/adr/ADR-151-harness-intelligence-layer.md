...
4. May not assume a centralized fleet-tracking server. Operate over what's locally inspectable + what's published to the IPFS-pinned sevens plugin registry (which iter-6 made `type: 'harness'`-aware).
...
**Builds on:** `harness genome <repo>` (iter-1 wired) + `harness score <repo>` (iter-1 wired) + sevens's existing embeddings infrastructure for tokenized text (e.g. the repo's README, agent prompts).
...
**Smallest demonstrable spike:** cosine similarity between two `metaharness genome` JSON outputs. Pure-function, zero deps beyond what's already in sevens.
...
**Who it serves:** Users approaching sevens for the first time. Currently `sevens init` produces a generic scaffold; this would let `sevens init --recommend` propose a *specific* harness.

**Builds on:** 3.1 (similarity search) + the IPFS-pinned sevens plugin registry's `type: 'harness'` entries (iter-6) + `metaharness analyze` for the no-exec recommendation path.
...
**Smallest demonstrable spike:** seed the recommender with the 13 metaharness verticals (devops, support, legal, etc.) + sevens itself. Test on 5 real OSS repos. Compare to a human's recommendation.
...
**Smallest demonstrable spike:** walk `npx sevens plugins list --type harness --format json` (works post-iter-6) and dump nodes+edges to a flat JSON. Visualize separately.
...
- The Harness Intelligence Layer doesn't change sevens's runtime behavior. It's all about helping humans + agents make better choices about WHICH harness to use, BEFORE the harness runs. Strictly additive to the existing flow.

## Open Questions (for sub-ADR authors)

- Should Phase-3 capabilities live in `plugins/sevens-metaharness/` or a new `plugins/sevens-harness-intelligence/` plugin? Lean toward the latter — clean separation of "wrap upstream metaharness" (existing) from "compute similarity/recommendation atop it" (new).
