...
A business owner who deploys sevens today can run AI agents against discrete tasks. What they cannot do is hand over *operational continuity* to those agents. Sales pipelines go untouched between sessions. Finance reconciliation waits for a human to ask for it. Marketing copy drifts without review. Support tickets queue unreplied.

The gap is not capability — sevens's agents can already do each of these things when prompted. The gap is **perpetual, observable, overrideable automation with a cockpit the owner can actually sit in**. They need:
...
|--------|-----------------|-----|
| sevens federation (ADR-097, ADR-111) | Trust-scored peer connections, PII pipeline, budget hop enforcement, WG mesh transport | No concept of "business domain"; federation is agent-to-agent, not role-to-domain-to-human |
| Managed Agents (ADR-115) | Cloud agent execution with SSE event streaming | No persistent room concept; sessions are ephemeral |
| agentbbs (npm launcher v0.1.0; Rust workspace v0.34.9-nethack) | BBS-style web UI + TUI + SSH front door for human-agent interaction, organized into "rooms"; existing `crates/agentbbs-mcp/` MCP server with server/client/transport layers + integration test suite; existing `crates/agentbbs-federation/` crate; Docker + monitoring stack; postgres-backed CI; 7 CI workflows | Typed federation-envelope compatibility with sevens's wire format is unverified; `agentbbs-mcp` tool interface needs explicit compat check against our `MCPTool` interface; FSL-1.1-Apache-2.0 license (converts to Apache-2.0 after 2 years) has integration implications for sevens's MIT distribution; `cargo` required at first run (Rust compilation); npm launcher version vs. Rust workspace version are independent versioning tracks |
...
The integration risks are therefore not about project maturity in the general sense. The specific risks are: (a) the npm launcher version track and the Rust workspace version track are independently versioned — the integration must pin against the Rust workspace tag, not the npm package semver; (b) the FSL-1.1-Apache-2.0 license converts to Apache-2.0 after 2 years, which is compatible with sevens's MIT distribution for most commercial contexts but requires legal review before embedding agentbbs binaries in a sevens distribution; (c) `cargo` must be present on the operator's machine for the Rust binary to build at first run; (d) the `agentbbs-mcp` server's tool interface needs explicit compatibility verification against sevens's `MCPTool` interface before Phase 2. See Section 9.1 for the rewritten risk register.
...
**Adopt agentbbs as a special-tier federation peer (BBS-as-peer model) and scaffold business-domain pods as a new plugin (`sevens-bbs-federation` + `sevens-business-pods`), following the optional-dep integration pattern established by ADR-150 and exercised by the agenticow PR #2500.**
...
2. **Domain pods are the unit of execution.** A pod is a named group of specialized agents serving one BBS room, with a defined bench, schedule, PII policy, and budget cap. Pods are defined in a new plugin, `sevens-business-pods`, as typed JSON templates.
...
5. **agentbbs goes in `optionalDependencies`.** Sevens must remain fully operational with agentbbs removed. The smoke contract (`plugins/sevens-bbs-federation/scripts/smoke.sh`) must pass without agentbbs present by running in degraded mode.
...
- Replacing sevens's existing federation with a BBS-centric model. BBS rooms are *one kind* of federation peer; all existing peer types continue to work unchanged.
...
```
 ...
 └──────┬────────────────────────────────────────────────────────────────┘
        ...
        │
 ┌──────▼─────────────────────────────────────────────────────────────────┐
 │              sevens node (local or remote via WG mesh)                  │
 │                                                                         │
 │  ┌─────────────────┐   ┌──────────────────┐   ┌────────────────────┐  │
...
All four tools live in `plugins/sevens-bbs-federation/scripts/` (skill-shelled) and additionally as typed MCP handlers in `plugins/sevens-bbs-federation/mcp-tools.ts` (mirroring the `sevens-metaharness` pattern). They wrap — never bypass — the existing `FederationCoordinator`.
...
inputSchema: {
  ...
  properties: {
    ...
    domainPod: {
      type: 'string',
      description: 'Name of the sevens-business-pods template that serves this room.',
    },
    piiPolicy: {
      type: 'string',
...
//   sevens memory namespace 'bbs-rooms').
```
...
//   On BBS endpoint unavailable: falls back to sevens memory store under
...
A pod template is a typed JSON object stored in `plugins/sevens-business-pods/templates/<domain>.json`. The schema:
...
interface BusinessPodTemplate {
  ...
  agents: Array<{
    ...
    agentType: string;        // must be a known sevens agent type
    description: string;
...
```typescript
// Pseudocode — new file:
// plugins/sevens-bbs-federation/src/domain-affinity-policy.ts
...
**CLI command** (new subcommand under `sevens federation`):

```bash
sevens federation trust elevate <bbs-node-id> \
  --to TRUSTED \
...
```json
{
  ...
  "_allowedMcpTools_notes": [
    ...
    "Cloud-provider MCP servers (e.g. aws-mcp, gcp-mcp) are deployment-specific; NOT bundled. Must be registered per-installation via `sevens mcp config add`."
  ],
  "bench": {
...
### 5.1 In sevens (this repository)

#### 5.1.1 New plugin: `plugins/sevens-bbs-federation/`

Standard sevens plugin structure (mirrors `plugins/sevens-metaharness/`):

```
plugins/sevens-bbs-federation/
...
#### 5.1.2 New plugin: `plugins/sevens-business-pods/`

```
plugins/sevens-business-pods/
├── plugin.json
├── templates/
│   ├── sales.json
│   ├── marketing.json
...
├── scripts/
│   ├── smoke.sh
│   └── init-pods.mjs         # scaffold a pod from a template into the running sevens node
├── skills/
...
```typescript
// New file: plugins/sevens-bbs-federation/src/bbs-room-budget-tracker.ts
...
**File to create**: `plugins/sevens-bbs-federation/src/bbs-room-budget-tracker.ts`
...
The Ops pod requires the ability to probe external HTTP endpoints for availability monitoring. This tool does not exist in sevens today and must be created.

**Minimal contract**:

```typescript
// New tool in plugins/sevens-bbs-federation/src/mcp-tools.ts (or a shared http plugin)
{
  name: 'http_fetch',
...
#### 5.1.9 Optional dependency wiring in root package.json and sevens wrapper

`agentbbs` must be added to `optionalDependencies` in:
- `/Users/cohen/Projects/sevens/package.json` (root umbrella)
- `/Users/cohen/Projects/sevens/sevens/package.json` (sevens wrapper — same lesson as metaharness in #2112: root overrides do not propagate to the published sevens wrapper)

The graceful-degradation guard pattern (same as `plugins/sevens-metaharness/src/*.ts`):
...
The agentbbs project is a mature Rust workspace with 30+ release tags. The Phase 1 survey of `crates/agentbbs-federation/` + `crates/agentbbs-mcp/` (commit `ca3c6e0` on `main`) resolved each compatibility check below. The survey is now concrete: each sub-section names the existing primitive sevens will adapt to, or the genuine gap that needs an upstream PR.

A symmetric regression guard — [`ruvnet/AgentBBS#3`](https://github.com/ruvnet/AgentBBS/pull/3) — was opened against agentbbs and asserts on every PR + nightly cron that the four MCP tool names, four `FederationPayload` variants, and three `SevensAdapter` subcommand names this section depends on continue to exist. Drift becomes a failed nightly workflow within 24h of the upstream change landing.
...
**Sevens's adapter strategy for Phase 1 → Phase 3**:
- `federation_bbs_publish` → wraps the typed pod event inside a `ReplicateMessage(Message)` where `MessageBody.subject` carries the sevens `msgType` discriminator (`pod-status` / `task-result` / `alert` / `human-override-ack` / `bench-result`) and `MessageBody.body` is the JSON-serialized typed payload.
...
- The `hmacSignature` field in sevens's `FederationEnvelope` shape is replaced by agentbbs's Ed25519-over-canonical-bytes signature on the outer `FederationEnvelope`. Stronger than HMAC; no behavior change needed in sevens's Phase 1 layer — the agentbbs side computes + validates the signature.
- `piiScanResult` is **not** stored alongside the payload by agentbbs (PII is scrubbed and discarded). For sevens's audit trail requirement, the `piiScanResult` must be persisted in sevens's local `AuditService` (already the canonical record per §6.2) *before* the envelope is dispatched to agentbbs.

**No upstream PR required for Phase 1.** A possible Phase 3 enhancement: if richer routing per typed event becomes valuable, propose a `SevensEvent { envelope_id, msg_type, payload }` variant. Filed as a follow-up — not blocking.
...
Sevens's `federation_bbs_register` / `federation_bbs_publish` / `federation_bbs_watch` / `federation_bbs_human_join` are **sevens-owned MCP tools** that live in sevens's MCP server, not in agentbbs's. The Phase 2 wire-up calls into agentbbs via one of two paths:
...
**Phase 1**: sevens's `agentbbsBin` argument is documented as "Reserved for Phase 2 wire-up; ignored in Phase 1". Phase 1 implements the sevens-side MCP tool surface only; the spawned subprocess wiring lands in Phase 2.

**Genuine gap (filed as follow-up issue, not blocking Phase 1)**: `agentbbs-mcp` does not currently expose a `create_board` tool — board creation only happens via the direct `Bbs::create_board()` Rust API or via `agentbbs ssh` admin commands. For sevens's `federation_bbs_register` to map cleanly to MCP, either (i) sevens's Phase 2 register implementation drives `agentbbs federate …` CLI subcommands rather than MCP, or (ii) upstream adds `create_board` to the four-tool MCP surface. Filed as gap #1 below.

**No upstream PR required for Phase 1.** The web UI rendering concern previously documented here is moved to Phase 4 and re-scoped — `agentbbs-web` is out of scope until sevens's pods publish enough events to make per-message-type rendering worthwhile.

#### 5.2.3 Subscription / streaming for `federation_bbs_watch`

**Genuinely missing — but matches sevens's Phase 1 polling design.**
...
This matches sevens's `federation_bbs_watch` Phase 1 contract verbatim: the tool description says *"Phase 1 is polling — Phase 4 layers streaming on the same surface."* The Phase 2 implementation will repeatedly call `tools/call name=read_board` with the agentbbs-side message ordering (`Message.id` is content-addressed and `Store::list_messages` returns ordered) and filter by sevens's monotonic `sinceEnvelopeId`.

**Phase 4 gap (filed as follow-up issue, not blocking Phase 1)**: a streaming-subscribe path. Two design options for upstream:
- (A) Add `resources/subscribe` per the MCP spec — the server pushes `notifications/resources/updated` frames; sevens consumes them in a long-poll loop.
...
#### 5.2.4 Integration seam — `SevensAdapter` already exists (other direction)

**Surprise finding: agentbbs already drives `npx sevens federation` via `crates/agentbbs-federation/src/adapter.rs::SevensAdapter`.**

The adapter wraps a `CommandRunner` trait (production `TokioCommandRunner`, test `FakeCommandRunner`) and shells out to:

```
npx sevens federation init
npx sevens federation join <addr>
npx sevens federation status
```

This means the integration direction agentbbs anticipated is: **agentbbs → sevens as the federation control plane**. Sevens's ADR-164 goes the *opposite* direction: sevens → agentbbs as the room/event substrate. Both can coexist (asymmetric driving — sevens manages federation peers, agentbbs publishes room events).

**Implications for sevens**:
- Sevens's CLI must keep `npx sevens federation {init,join,status}` alive as a stable surface. Renaming or removing these breaks the agentbbs side silently. The regression guard at [`ruvnet/AgentBBS#3`](https://github.com/ruvnet/AgentBBS/pull/3) catches this — its third static-contract check greps for these exact subcommands in `adapter.rs` and fires if either side drifts.
- `SevensAdapter` is the seam through which agentbbs can use sevens's memory/federation layer. Sevens's Phase 2 wire-up will likely add a *reciprocal* `AgentBbsAdapter` (in sevens) that drives `agentbbs federate` / `agentbbs mcp` from the sevens side — mirroring the symmetric pattern.
- Ed25519 authentication (the previous "keypair handshake" concern) is **already implemented** in the envelope layer (§5.2.1). The `federation_bbs_human_join` single-use token validation is a sevens-side concern (token minting + JTI replay protection), not an agentbbs concern — agentbbs just stores the signed envelopes.

**No upstream PR required for Phase 1.** Reciprocal `AgentBbsAdapter` is a sevens Phase 2 deliverable (§5.1, not §5.2).
...
**Implications for sevens's `federation_bbs_watch` `sinceTs` replay**:
- Phase 1 + 2 sevens's `federation_bbs_watch` polls the most-recent window (no `sinceTs` durability requirement — the sevens-side `AuditService` is the canonical record per §6.2; agentbbs is the display projection).
...
**No existing test exercises the agentbbs ↔ sevens integration end-to-end.**

`agentbbs.yml` runs fmt + clippy + nextest + cargo-deny over the agentbbs workspace. `agentbbs-federation`'s `sevens_adapter_shells_npx` test uses `FakeCommandRunner` — it asserts the *intended* `npx sevens federation …` invocations are emitted, but never spawns the real published `sevens` CLI. Symmetric story for sevens's side (Phase 1 doesn't shell out to agentbbs at all).
...
|---|-----|--------------|----------|
| 1 | No `create_board` MCP tool — board creation is Rust-API or SSH-admin only | 2 | Low (workaround: sevens's Phase 2 `federation_bbs_register` calls `agentbbs federate` CLI) |
...
**No performance benchmarks exist for agentbbs v0.1.0.** The numbers below are estimates based on current Anthropic API pricing and observed sevens agent token usage from existing sessions.
...
For accounting purposes, one BBS publish = one spend event in the `federation-spend` memory namespace (key: `fed-spend-<roomId>-<ts>`). The cost-tracker plugin (`plugins/sevens-cost-tracker/scripts/federation.mjs`) already aggregates these; no change needed there.
...
Deliverables:
- `plugins/sevens-bbs-federation/` scaffold with the four MCP tools from Section 3.2
- Graceful-degraded behavior when agentbbs is not installed (all tools return `{ degraded: true }`)
- `plugins/sevens-bbs-federation/scripts/smoke.sh` passing with and without agentbbs
- `agentbbs` in `optionalDependencies` of root and sevens wrapper `package.json`
...
Exit criteria: `scripts/smoke.sh` exits 0 with and without agentbbs installed. Fleet meta-smoke shows sevens-bbs-federation green. No regressions in existing federation tests.
...
Deliverables:
- `plugins/sevens-business-pods/` with the sales pod template (`templates/sales.json`)
...
- Per-pod spend tracking visible in `sevens cost` dashboard
...
- `sevens doctor --component bbs-federation` reporting agentbbs version, registered rooms, pod health
- Published agentbbs integration documentation
- Migration guide for operators who want to add custom pod templates
- `sevens metaharness oia-audit` extended to include BBS-specific compliance checks
...
**Risk 9.1.b — FSL-1.1-Apache-2.0 license implications**: The Functional Source License converts to Apache-2.0 after 2 years. In the current FSL period, sevens (MIT) may distribute agentbbs as an `optionalDependency` for use by operators, but embedding agentbbs binaries in a sevens distribution that is itself sold as a product may require explicit FSL compliance review. Mitigation: legal review before Phase 3. Phase 1 and Phase 2 are unaffected (operator installs agentbbs separately; sevens does not bundle it).

**Risk 9.1.c — `cargo` required at first run**: The Rust workspace compiles from source on the operator's machine unless a pre-built binary is available. Operators without `cargo` in their PATH will see a compilation failure, not a graceful-degradation response. Mitigation: the `no-bbs-smoke.yml` CI workflow must test the absent-agentbbs path (graceful degradation), not only the present-agentbbs path. Document the `cargo` requirement prominently in `plugins/sevens-bbs-federation/README.md`. Phase 3+ may consider shipping a pre-built binary via npm for common platforms.

**Risk 9.1.d — `agentbbs-mcp` compatibility with sevens's `MCPTool` interface**: The existing `crates/agentbbs-mcp/src/server.rs` implements an MCP server, but its tool interface shape (JSON-RPC envelope, tool name conventions, parameter types) has not been verified against sevens's `MCPTool` interface. If there is a mismatch (e.g., different `inputSchema` conventions or different transport handshake), the four BBS MCP tools in §3.2 may need an adapter layer. Mitigation: Phase 1 deliverable includes a documented compatibility matrix produced by reading `crates/agentbbs-mcp/src/server.rs` against sevens's `MCPTool` type definition. Incompatibilities are resolved in Phase 1 (adapter shim) before Phase 2 proceeds.
...
If agentbbs introduces a breaking API change before Phase 3, the graceful-degraded path ensures sevens continues operating; only the BBS cockpit goes dark until the adapter is updated.
...
Rejected because: (a) federation transport is intentionally pluggable (ADR-104, ADR-120) but abstracting BBS at the transport level would couple every federation feature to agentbbs availability; (b) the mandatory graceful-degradation invariant (sevens operational without agentbbs) is much harder to satisfy at the transport layer than at the peer layer; (c) BBS-as-transport would prevent federation from using WG mesh (ADR-111) for high-security peers, which finance and HR require.

### 10.2 BBS-as-surface-only (rejected)

Another option was to treat agentbbs as a pure display layer — a read-only dashboard that subscribes to sevens memory events but has no write path back to agents.
...
Building a sevens-native business cockpit was considered. It would eliminate the dependency on an unproven v0.1.0 project.

Rejected because: (a) a BBS-style interaction paradigm is a good fit for the "rooms by business function" model and agentbbs already provides it; (b) the integration effort is substantially less than building a UI from scratch; (c) agentbbs is first-party (same author) and can be coordinated with; (d) the graceful-degradation requirement means sevens does not *depend* on agentbbs — if agentbbs fails to mature, the federation primitives built here remain useful without the BBS cockpit.
...
| ADR-111 (WG mesh transport) | Finance and HR pods must route over WG mesh when using remote sevens peers |
...
| `plugins/sevens-metaharness/scripts/smoke.sh` | Reference implementation for the smoke contract pattern |
