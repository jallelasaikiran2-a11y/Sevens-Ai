...
- The CLI's on-disk state (`~/.sevens/auth.json`, `0600`) stores only: account ID, granted scopes, access-token expiry, keychain entry reference, profile name. No token material.
...
- **Revocation:** `sevens auth logout` calls `POST /v1/auth/revoke` (ADR-308), removes the keychain entry, clears `auth.json`, and revokes the `account` consent receipt. Server-side revocation (dashboard) takes effect within one access-token lifetime.
- **Multiple accounts:** named profiles — `sevens auth login --profile work`; one default profile; `sevens auth status` lists all with scopes and expiry.
- **Offline:** cached identity metadata is readable; no refresh occurs; capabilities requiring auth fail with a clear "offline, sign-in required" error. Core sevens functionality is never affected by auth being unavailable (ADR-308 failure policy).

## Consequences

- New CLI surface: `sevens auth login|logout|status [--profile <name>]`.
...
- `sevens doctor` gains an auth component (keychain availability, token expiry, scope-vs-receipt consistency check).
...
`sevens auth login|logout|status` is implemented in `v3/@claude-flow/cli/src/auth/` +
...
- **Targets `auth.cognitum.one/oauth/{authorize,token}` + `/v1/oauth/code-exchange`, NOT
  ...
  ADR-308 addendum.
- **Reuses meta-proxy's registered `client_id=meta-proxy`, not a new `sevens`-specific
  registration.** Confirmed live 2026-07-16: `GET /oauth/authorize` with `client_id=meta-proxy`
  and an arbitrary sevens-controlled loopback `redirect_uri` returns a working consent page, not a
  ...
  not a documented contract, and could tighten later; if it ever does, `sevens auth` needs its own
...
`@claude-flow/security` is only an `optionalDependency` of the CLI — `sevens auth` degrades to a
...
Refresh remains demand-driven, not timer-driven: plain `sevens auth status` reads cached identity
metadata and performs no network request, preserving this ADR's offline behavior. `sevens auth
