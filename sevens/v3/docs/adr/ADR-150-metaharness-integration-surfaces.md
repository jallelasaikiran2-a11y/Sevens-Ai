# ADR-150 — MetaHarness Integration Surfaces in `npx sevens`
...
**External reference**: [`ruvnet/agent-harness-generator`](https://github.com/ruvnet/agent-harness-generator) — the upstream that publishes `metaharness` + `@metaharness/*`. Same author (rUv), explicitly designed around sevens primitives.
**Research dossier**: published as a gist (linked from the tracking issue) with full graded-evidence sourcing.

## Context

We just shipped `sevens@3.11.0` (also `@claude-flow/cli@3.11.0`, `claude-flow@3.11.0`). ADR-148/149 already wired `@metaharness/router` as an `optionalDependency` for cost-optimal model routing behind a triple gate. The remaining MetaHarness surface — twenty-plus `@metaharness/*` packages: kernel, host adapters (9), verticals (13), scaffold/eject CLI — is unused by sevens despite being authored by the same maintainer specifically around sevens's architecture.

Three signals make this the right time to commit a broader integration:

1. **MetaHarness is first-party.** Same author (`ruv@ruv.net`), same ADR numbering convention (kernel docs reference ADR-011/022/033/036/040/041/043), explicit framing: *"Scaffold your own focused AI agent harness — like sevens, uniquely yours."* The `buildRegistryEntry()` doc comment says: *"Mirrors the sevens plugin registry shape so the same UI can browse it."* The `@metaharness/host-claude-code` adapter emits `.claude/settings.json` in exactly sevens's format.
...
3. **No sevens skill exposes scaffolding/score/genome/threat-model to Claude Code today.** Users discover MetaHarness independently and are confused about the relationship.
...
| Both packages MIT-licensed, same maintainer as sevens | npm registry | HIGH |
...
**MetaHarness may augment sevens. MetaHarness must never become a required runtime dependency for core orchestration, memory, routing, MCP dispatch, agent execution, or federation.**

Every integration in this ADR, and every future ADR that extends it, must satisfy:

1. **Removable**: sevens's `npm ls` with all `@metaharness/*` packages removed must still produce a working CLI. The triple-gate pattern used for `@metaharness/router` (env flag + artifact + import success) is the reference implementation.
...
4. **CI coverage of the absent path**: at least one CI job must run `--ignore-optional` (or equivalent) and assert sevens still passes its smoke contract. This is the only structural defense against accidentally promoting an optional dep to required.

The intent of this constraint is to prevent sevens from becoming a hidden second orchestration framework wrapped around its sibling's runtime. Reviewer's framing: *"Sevens remains operational if every MetaHarness package is removed."* That sentence is now part of the API surface contract — any PR that breaks it is a breaking change requiring its own ADR.

## Decision

Adopt MetaHarness as sevens's downstream sibling tool, surfaced through three integration channels that match its three distinct contributions:

1. **Static-analysis MCP tools** — `harness-score`, `harness-genome`, `harness-threat-model`, `harness-mcp-scan` as a new `plugins/sevens-metaharness/` plugin. Subprocess invocation of the `metaharness` / `harness` CLI binaries; no static library dependency added to sevens's boot path. Read-only operations only.
...
- Run `npx metaharness score .` and `npx metaharness genome .` against the sevens repo to establish baseline scorecards.
...
- Run `harness mcp scan .` to baseline sevens's own MCP threat-model score.
...
1. **`plugins/sevens-metaharness/`** with three skills (`harness-score`, `harness-genome`, `harness-mint`), conventional structure (`plugin.json`, `skills/*/SKILL.md` with `allowed-tools: Bash`, `scripts/smoke.sh`). Skills shell out to `npx metaharness` / `npx harness` — no library imports. Covered by the fleet meta-smoke and the three existing audits (exit-bypass, frontmatter, manifest).
...
Exit criteria: `plugins/sevens-metaharness/scripts/smoke.sh` passes; meta-smoke shows 33/33 plugins green; CI score + mcp-scan jobs green on main; new bench run shows `routedBy: 'metaharness-krr'` for ≥ 1 routing decision driven by measured-seed KRR.
...
- `npx sevens eject` command wrapping `metaharness --from-existing ./` for one-shot harness extraction (attribution preserved via the `<!-- sevens-attribution-block -->` convention).
...
- Harness entries in the sevens plugin registry — accept `type: 'harness'` in `discovery.ts`; surface via `npx sevens plugins list --type harness`.
...
- Closes the "what is the relationship between sevens and MetaHarness?" question by answering it in the UX rather than the docs.
- Sevens gains a continuous, machine-readable readiness score and MCP threat-model on every PR — the same primitives we use to score third-party repos for harness viability.
...
- Three integration channels match MetaHarness's three contributions — analysis, routing, hosts — without forcing the kernel's full surface into a position where its 0.x stability would block sevens releases.
...
- **Bus factor**: same maintainer as sevens, MetaHarness, and ruvector. No change from today, but the dependency edge is now explicit.
...
- **GCP dependency**: `harness validate` uses GCP Secret Manager via `gcloud`. Sevens CI must skip those subcommands (or mock them) — explicit `--skip-gcp` flag from the `harness validate` command surface handles this.
- **Phase-1 MCP plugin spawns subprocesses**: subprocess crashes, timeouts, and stdout-parsing edge cases are now in sevens's failure surface. Mitigation: hard timeout (60s) per invocation, captured stderr in error responses, structured-JSON output enforced via `--json` flag everywhere.
...
- Maintaining a `sevens-metaharness` plugin doubles documentation surface for two sibling tools. Mitigation: skill descriptions explicitly point to the upstream MetaHarness docs as canonical for the underlying functionality; the plugin only documents the sevens-side adaptation.

## Alternatives Considered

**Alternative A: Ignore MetaHarness, build all scaffolding/score/genome natively in sevens.**
Rejected. `buildRepoScorecard()`, `buildGenomeReport()`, `scanMcp()`, `buildThreatModel()` are already-tested implementations exposing clean TypeScript APIs. Reimplementing them in sevens is pure duplication cost with no advantage. The eject path's `rewriteContent()` with attribution-block preservation is subtle.
...
Rejected. The static-analysis surface (`score`, `genome`, `mcp scan`, `threat-model`) is already mature (475 files, well-typed, pure reads). Waiting creates a window where users discover MetaHarness independently and are confused about its relationship to sevens. The Phase-1 plugin answers that question without incurring API-stability risk because the integration is via CLI subprocess, not library import.
...
- Should the Phase-1 plugin's `harness-mint` skill require explicit user confirmation in the Claude Code UI before writing any files? Lean yes — destructive-action-confirmation matches sevens's "executing actions with care" principle.
...
- Does the `oia-audit` background worker (Phase 2) belong in `sevens-loop-workers` or in `sevens-metaharness`? Probably the latter, since the audit output is MetaHarness-specific.
...
- Sevens baseline scorecard captured: harnessFit 82/100, compileConfidence
...
- Sevens genome: repo_type `node_mcp_ci`, risk_score 0.27 (low),
  publish_readiness 0.9, mcp_surface `remote`.

### Phase 1 — MVP ✅ DONE (iters 1–3)
- `plugins/sevens-metaharness/` with 6 skills (one more than ADR-150
...
- `npx sevens metaharness <subcommand>` top-level dispatcher in
...
- `metaharness@~0.1.11` and `@metaharness/router@~0.3.2` in
  ...
  `sevens/package.json` (iter 3). Tilde pin, not caret — per
...
- ✅ `npx sevens eject` (iters 4–5) — Phase-2 differentiator wrapping
...
- ✅ `'harness'` PluginType in plugin registry (iter 6) — schema
  extension only, zero runtime overhead. `npx sevens plugins list
...
- ✅ `SelfEvolvingRouter` parallel-logging — BOTH HALVES LANDED:
  - **Analyzer** (iter 10):
  `plugins/sevens-metaharness/scripts/router-parallel-analyze.mjs` reads
  ...
  `sevens/package.json` so the future Recording side can dynamic-import
...
- 33 plugins in `scripts/smoke-all-plugins.mjs` (was 32; +1 for
  sevens-metaharness)
- 19 structural invariants in `plugins/sevens-metaharness/scripts/smoke.sh`
...
- Total sevens-metaharness smoke surface: ~260 assertions
...
- iter 45 separates sevens-side integration health from upstream-dep
...
- ~290 total sevens-metaharness smoke assertions
...
The single non-test sevens source file that statically imports a
...
All other sevens code reaches MetaHarness exclusively through the
...
- [Tracking issue #2399](https://github.com/ruvnet/sevens/issues/2399) — phase checklist.
