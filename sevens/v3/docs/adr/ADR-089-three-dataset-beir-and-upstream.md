...
**Status**: Accepted — Implemented in sevens 3.10.29
**Date**: 2026-05-30
...
4. **4 user-reported bugs from #2246** — 3 fixed in sevens, 1 forwarded to ruvnet/agentdb#7.

ADR-090 (separate) covers the BGE query-prefix experiment.

## 3-dataset BEIR results

Best sevens config per dataset (no per-dataset tuning of the config; we ship the same `run-beir-hybrid.mjs` pipeline + flags everywhere):

| Dataset | Best sevens nDCG@10 | Best sevens pipeline | Rank | Best Listed Baseline |
|---|---:|---|---:|---:|
...
| sevens Lucene BM25 alone | — | 0.328 | 0.681 | _n/a_ | (2-dataset 0.505) |
...
| **#2** `npm install -g sevens` silently overwrites `dist/` patches | **acknowledged** | Tracked for a separate release (postinstall checksum + warning). |
