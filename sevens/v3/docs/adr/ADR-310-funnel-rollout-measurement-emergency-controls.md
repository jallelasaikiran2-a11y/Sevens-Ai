...
- Rollback ships through the same three-package publication sequence as any release: `@claude-flow/cli` → `claude-flow` → `sevens`, with all dist-tags updated and verified per the repo's publishing rules.
...
- Precedence always holds (ADR-301/305): no remote policy can override `SEVENS_FUNNEL=0`, enterprise managed policy, or user config.
...
3. `SEVENS_FUNNEL=0` suppresses every funnel surface (ADR-301, 302, 303).
4. All core sevens commands remain behaviorally identical — exit codes, operational output, and latency within gate thresholds — across every configuration above.
