...
File: `plugins/sevens-bbs-federation/src/bbs-room-budget-tracker.ts`
...
await spendReporter.reportSpend({
  ...
  success: true,
  // custom field consumed by sevens-cost-tracker:
...
The `federation_spend` events emitted here are consumed by `sevens-cost-tracker@0.14.0` (ADR-097 Phase 3). The `audit_envelope_id` on each reservation row provides a direct JOIN path:
...
If the sweeper crashes permanently (e.g., the process exits), expired rows accumulate indefinitely. They are excluded from gate checks, so correctness is maintained, but storage grows. The sweeper health is reported by `npx sevens doctor --component bbs-budget`.
...
3. Re-run `npx sevens doctor --fix --component bbs-budget` to recreate the schema.
...
The following sevens source files are the primary anchor points for implementing this ADR:
...
| `plugins/sevens-bbs-federation/src/bbs-room-budget-tracker.ts` | New file — the `AtomicBbsRoomBudgetTracker` class implementing this ADR's design |
