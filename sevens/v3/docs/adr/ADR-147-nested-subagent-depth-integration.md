...
**Status**: Proposed
**Date**: 2026-06-09
**Issue**: [ruvnet/sevens#2335](https://github.com/ruvnet/sevens/issues/2335)
...
The motivation Cherny calls out is **context management**, not just parallelism. Each subagent gets its own context window; flat fan-out only offloads one level because the lead still has to read the summaries. Nested subagents let the sub-agent itself delegate to a fresh window before its own context fills up — which is the bottleneck sevens's deepest orchestrators (`sevens-goals:dossier-investigator`, `sevens-sparc:sparc-orchestrator`, `v3-queen-coordinator`) already hit.
...
Three sub-agent types tested in a live 2.1.169 session (`general-purpose`, `claude`, `analyst`) all reported no `Agent`/`Task` tool available, and `ToolSearch({query: "select:Agent,Task"})` returned no matches. The cause: **zero sevens agent definitions declare a `tools:` field in their YAML frontmatter**, so spawned children inherit `hasTaskTool=false` regardless of which subagent type is requested. The capability is present in the binary but disabled by omission in sevens's agent registry.
...
| `sevens-sparc:sparc-orchestrator` | (plugin agents dir) | 5 SPARC phases ≈ 5 nested levels — perfect fit for depth=5 |
| `hierarchical-coordinator` | `.claude/agents/swarm/hierarchical-coordinator.md` | Coordinator pattern presumes nesting |
| `sevens-goals:dossier-investigator` | (plugin agents dir) | Recursive entity expansion — the textbook depth case |
...
Before any new spawn, the hook reads the current chain depth from P2's AgentDB row (or the OTel context if the row hasn't landed yet) and refuses spawns at or beyond `swarm.maxNestingDepth` in `claude-flow.config.json`. Default cap: `4` (one less than Anthropic's announced 5, to preserve a guard band — sevens's refusal should fire before Anthropic's does, with a clearer error). Configurable per-deployment; gated behind `CLAUDE_FLOW_STRICT_NESTING=true` (default off) to avoid regressing existing pipelines until P1 + P2 telemetry is collected.
...
**Positive**:
- The deepest sevens orchestrators (`dossier-investigator`, `sparc-orchestrator`, `v3-queen-coordinator`) gain native context-window isolation per nesting level — the bottleneck Cherny called out is exactly the one these agents already hit.
- Spawn-tree persistence (P2) unlocks accurate per-tree cost attribution in `sevens-cost-tracker`, replacing today's flat per-agent-id sum.
- Depth-aware guardrails (P3) decouple sevens's nesting policy from Anthropic's API-side cap — if Anthropic raises or lowers the depth=5 cap, sevens's behaviour stays predictable.
...
- Default cap of 4 means sevens refuses one level before the API would. Trade-off: clearer error, costs one level of headroom. Reversible via config.
...
**P1** lands with:
- New `nested-*` agent set in `plugins/sevens-agent/` (8 agents + 1 skill) — orchestrators declare `tools: [Task, ...]`, leaves explicitly do not. This is the additive shape; existing v3-queen-coordinator / sparc-orchestrator / hierarchical-coordinator agents are NOT modified in P1 (deferred until the YAML opt-in mechanism is empirically confirmed to work — see below).
...
| `claude plugin details sevens-agent` after cache-stage | All 9 components discovered; YAML parsed cleanly; `tools:` field accepted by the loader |
