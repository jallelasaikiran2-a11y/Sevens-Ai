...
Sevens 3.8.0 ships four independent graph-layer implementations. All four are live in production and have real users, but they share no schema, no query contract, and no indexing strategy. Each was added in a separate ADR and each solves a slice of the graph problem in isolation.
...
**Layer 3 — sevens-knowledge-graph plugin (v0.2.0)**
Files: `plugins/sevens-knowledge-graph/`, including `agents/graph-navigator.md`, `skills/kg-extract/SKILL.md`, `skills/kg-traverse/SKILL.md`
...
**Layer 4 — sevens-graph-intelligence plugin (0.1.0-alpha.1, ADR-123)**
Files: `plugins/sevens-graph-intelligence/src/`, including `mcp-tools/index.ts` (6 tools), `adapters/` (8 adapter modules), `application/streaming-bridge.ts`, `domain/types.ts`
...
6. **Plugin is alpha-only.** `sevens-graph-intelligence@0.1.0-alpha.1` is unpublished to npm (version string is pre-1.0 alpha, not in the marketplace). The knowledge-graph plugin is `v0.2.0` and listed but does not appear in the plugin registry's `featured` or `official` sections in `discovery.ts`.
...
- sevens-graph-intelligence adapters can read live edges from AgentDB without requiring a manual `exportAsSparseMatrix()` re-implementation per adapter.
...
agentdb_graph-query({
  ...
  mode: "k-hop"           // k-hop neighbor expansion
       ...
       | "pagerank",       // single-entry PPR via sevens-graph-intelligence
...
4. If `mode === "pagerank"`: load edges from `graph_edges` into a `SparseMatrix`, call `runPageRank` from `sevens-graph-intelligence`'s `solver-bridge.ts`. This is the integration point where the sublinear solver reads from the unified schema.

The `KnowledgeGraphSource` interface in `plugins/sevens-graph-intelligence/src/adapters/knowledge-graph-adapter.ts:17` gains a default implementation that reads from `graph_edges` via the bridge, so the `KnowledgeGraphAdapter` no longer requires callers to implement their own edge source.
...
3. `agentdb_graph-query({ nodeId: "entity:xyz", mode: "pagerank", topK: 5 })` returns ranked node list using sevens-graph-intelligence's `runPageRank`. Requires `sevens-graph-intelligence` to be importable (optional dependency; graceful error if absent).
...
The eight existing adapters in `plugins/sevens-graph-intelligence/src/adapters/` are updated to read from `graph_edges` as their primary source, falling back to their current plugin-specific sources when rows are absent (backward compatibility).
...
3. `sevens-plugin-creator` scaffold includes the `"graph_adapter"` stub (commented out) in generated `plugin.json`.
...
The `sevens-knowledge-graph` plugin's graph-navigator agent currently runs its pathfinder algorithm in its system prompt, issuing sequential `agentdb_causal-edge` and `agentdb_semantic-route` calls in a loop. This is O(depth × branching-factor) sequential MCP calls — expensive, slow, and not reproducible.
...
Implementation: expand k-hop neighbors from `graph_edges`, score each candidate node by the algorithm's per-edge cost function, prune paths below threshold, rank. This matches the pathfinder algorithm described in `plugins/sevens-knowledge-graph/agents/graph-navigator.md:26–40` for the default `personalized-pagerank` mode but executes as a single native operation instead of a prompt loop.
...
The SOTA comparator drive (PR #2124, gist `298f8c668c8859b369f91734a0e9cbbe`) measured sevens winning 3 of 5 dimensions vs LangGraph/AutoGen/CrewAI. Graph traversal latency, edge insert throughput, and k-hop query cost are not currently in the benchmark suite — they are "what we don't measure" caveats.
...
Registration via `npx sevens witness regen` after each phase lands.
...
- No code changes to `sevens-graph-intelligence@0.1.0-alpha.1` are required in Phases 1–3; the plugin is consumed, not modified.
- Phase 4 requires changes inside `plugins/sevens-graph-intelligence/src/adapters/` — bump the plugin to `0.2.0-alpha.1` and register in the plugin marketplace.
...
| sevens-graph-intelligence | Complexity-bounded reasoning | Existing, called via solver bridge |
...
| **Sevens (post-ADR-130)** | **Unified cognitive graph runtime — vector + symbolic + temporal + operational memory behind one bounded query surface** |
