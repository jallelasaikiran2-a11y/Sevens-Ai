...
**Status**: Accepted — Implemented in sevens 3.10.21
**Date**: 2026-05-30
...
Each of the 10 bench queries gets `expectedSubstrings: string[]` — case-insensitive substring matches against the pattern's name. A result is "relevant" if its name contains ANY of the labelled substrings. Hand-curated from inspection of the actual sevens commit/issue history; encoded directly in `scripts/benchmark-pretrained-retrieval.mjs` (no separate file).
