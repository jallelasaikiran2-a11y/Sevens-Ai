...
**Status**: Accepted (2026-05-12) — implemented as the `managed_agent_*` MCP tools in the `sevens-agent` plugin (see Implementation below)
**Date**: 2026-05-12
**Authors**: claude (drafted with rUv)
**Related**: `sevens-agent` plugin (renamed from `sevens-wasm`) / `wasm_agent_*` MCP tools (`@ruvector/rvagent-wasm`) · ADR-026 (3-tier model routing) · ADR-095 G2 / ADR-104 (pluggable `ConsensusTransport` / `FederationTransport`) · ADR-097 (federation peers) · ADR-112 (MCP tool discoverability) · [Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview)
**Supersedes**: nothing

## Context

Sevens's `rvagent` capability — the `sevens-agent` plugin, wrapping `@ruvector/rvagent-wasm` — is a *local, WASM-sandboxed* agent harness. Its MCP surface:
...
It also exposes `mcp_servers`, `skills`, `multiagent` (research preview), and **define-outcomes** (research preview) on the agent config — i.e. Anthropic is converging on the same primitives sevens already has (agents, MCP servers, skills, multi-agent swarms, success criteria).
...
1. **Sevens's own MCP server as a Managed Agent tool source.** Managed Agents' agent config accepts `mcp_servers`. A sevens Managed Agent can be wired to `npx sevens mcp start` (HTTP transport) → the cloud agent gets sevens's 314 MCP tools (memory, swarm, hooks, agentdb, federation, …) running in the cloud container. That is: a cloud-hosted sevens agent, no local infra.
...
4. **Skills + outcomes parity.** Managed Agents accept sevens's skills (Markdown skill files) and (research-preview) `define-outcomes` — a Managed-Agent backed `rvagent` can carry the same skill set and success criteria a local one does.

### Constraints / boundaries

- **Optional, off by default.** `runtime: "wasm"` stays the default; `runtime: "managed"` requires `ANTHROPIC_API_KEY` (or the GCP `anthropic-api-key` secret in CI/ops) and the beta header — degrade gracefully (`{ error: "managed runtime needs ANTHROPIC_API_KEY + managed-agents beta access" }`) when absent. No core package gains a hard dependency on `@anthropic-ai/sdk` for this — it's a plugin-level dep (`sevens-agent` / a new `sevens-managed-agents`), lazily imported.
...
- **Branding.** Anthropic's branding guidelines forbid presenting a Managed-Agents integration as "Claude Code"/"Claude Cowork" — sevens keeps its own branding; surface it as "sevens agent (Anthropic Managed runtime)" not "Claude Code agent".
- **Cleanup discipline.** A Managed Agent leaves an `agent` + `environment` + `session` on the org account; `wasm_agent_terminate` must delete the session (and env, if sevens created it), and there should be a `managed_agent_gc` / doctor check for orphaned sessions.
...
So `sevens-agent` would expose three runtimes behind one mental model:
...
- It **is** Claude Code's machinery, exposed as a library — and sevens already loads `.claude/skills`, `.claude/commands`, `CLAUDE.md`, plugins, and shells out to `claude -p`. The SDK is the in-process version: full control over the message stream, hooks (sevens's own hooks), subagents (sevens's 16 roles), MCP servers, permission modes, sessions.
- The "give the agent sevens's 314 tools" combo is **trivial** here (unlike Managed Agents, which needs a publicly reachable URL): pass `mcpServers: { sevens: { command: "npx", args: ["sevens", "mcp", "start"] } }` — a *local* stdio MCP server, no deployment.
- Anthropic positions it as "prototype with the Agent SDK locally → move to Managed Agents for production" — having both in `sevens-agent` gives sevens exactly that path.
...
- **One mental model, two runtimes.** Authors think "rvagent"; sevens picks local WASM (fast, free, ephemeral, no network) or Anthropic Managed (cloud, persistent, long-running, no local infra) per task — like it already picks `LocalTransport` vs `FederationTransport`.
...
- **Sevens-tools-in-the-cloud.** Wiring `npx sevens mcp start` as a Managed Agent's MCP server gives a cloud-hosted sevens with zero local setup — a strong onboarding/demo story and a real ops capability.
...
- **Convergent primitives.** Managed Agents' agent/environment/session/events/skills/mcp_servers/multiagent/outcomes are the same primitives sevens has — the integration is "speak the same nouns to a different harness", not an impedance mismatch.
...
Landed as a **separate `managed_agent_*` toolset** (rather than a `runtime:` flag on the existing `wasm_agent_*` tools — cleaner, zero risk to the WASM path; both toolsets now live in the renamed `sevens-agent` plugin):

- `plugins/sevens-wasm/` → **`plugins/sevens-agent/`** (`git mv`; `plugin.json` name/description/keywords now span both runtimes; `.claude-plugin/marketplace.json` entry + READMEs + `discover-plugins` skill + `verification/inventory.json` updated). The `wasm_agent_*` / `wasm_gallery_*` tool *names* are unchanged.
...
- **`plugins/sevens-agent/skills/managed-agent/SKILL.md`** + **`commands/managed-agent.md`** — the cloud-runtime skill/command (the WASM-vs-managed decision table, the key prereq + fallback, the `mcpServers`-reachability caveat, the cost/cleanup discipline).
- **`plugins/sevens-agent/scripts/smoke.sh`** + **`.github/workflows/sevens-agent-smoke.yml`** — structural CI guard (12 checks now: manifest is `sevens-agent` with both-runtime keywords; all 10 `wasm_*` + all 6 `managed_agent_*` referenced; managed-agent skill keeps an explicit `allowed-tools` + offers the `wasm_agent_create` fallback; ADR cross-refs). Also wired into the `audit-cli-mcp-tools` / `audit-tool-descriptions` guards (311 registered / 0 dangling / 0 no-guidance).

**Validated live** against `api.anthropic.com` with the org's Anthropic key (GCP `claude-flow/anthropic-api-key`), via `sevens mcp exec -t managed_agent_*`: `create` (haiku, custom system) → `{sessionId, agentId, environmentId, status:"idle", model:"claude-haiku-4-5-20251001"}`; `prompt` ("echo … > /tmp/r.txt && cat …") → `{finished:true, status:"idle", stopReason:"end_turn", assistantText:"Done.", toolUses:[{name:"bash", input:{command:"echo … && cat …"}}], eventCount:13}`; `terminate` → `{sessionDeleted:true, environmentDeleted:true}`. No-key path returns the graceful error. `claude --plugin-dir plugins/sevens-agent` loads the plugin and lists its skills (`managed-agent`, `wasm-agent`, `wasm-gallery`). `smoke.sh` 12/0; cli build clean.

**Not yet done (follow-ups, called out above):** `runtime:"managed"` as a flag on `wasm_agent_*` (unification); a deployed/tunneled HTTP sevens MCP server so the cloud agent gets sevens's 314 tools (the combo needs a reachable URL); `task_assign` / federation dispatch to a Managed Agent session (#1916 items 3/4 — a third executor); cost-tracking integration + a `doctor`/GC check for orphaned sessions; `multiagent` / `define-outcomes` (research-preview, feature-flagged).
...
- `@ruvector/rvagent-wasm` (the `rvagent` backend) — dep of `@claude-flow/cli`; surfaced via the `sevens-agent` plugin's `wasm_agent_*` MCP tools
...
- ADR-117 — first consumer: `sevens-neural-trader` dispatches heavy backtests / Monte-Carlo / sweeps / model training to the `managed_agent_*` runtime (the `trader-cloud-backtest` skill), with cost-optimization rules
