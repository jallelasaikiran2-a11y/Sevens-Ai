# ADR-123 — sevens Graph Intelligence Engine: real-time relationship intelligence with complexity-aware execution
...
sevens isn't shipping "a faster solver". It is committing to a new architectural stance:

> **Intelligence that understands its own computational cost.**

Traditional AI systems recompute everything. sevens only computes what changed enough to matter, only at the depth the runtime can afford, and only over the relationships that are still load-bearing.

### The sevens Intelligence Stack (after ADR-123)
...
> sevens continuously reasons across agents, memory, infrastructure, workflows, and distributed systems while automatically adapting computation to available runtime budgets.
...
The rest of this document is the technical commitment — five SOTA axes surveyed, the eleven sevens graphs catalogued, the architecture diagrammed, an eight-phase rollout, and seven open questions. Lead from the layered story above; the layers are the architecture, not just the marketing.
...
sevens is the only agent platform in the field that already ships **Ed25519-signed witness chains** (ADR-103), **portable RVF cognitive containers** (`@ruvector/rvf@0.2.1`), and a **federated peer mesh with budgeted transport** (ADR-104/105/111). Layer `sublinear-time-solver@1.7.0` on top of that substrate and sevens gains a capability that LangGraph, AutoGen, Letta, MemGPT, Mem0, HippoRAG, and every browser-agent vendor structurally cannot produce: **signed, replayable, federatable, complexity-budgeted, coherence-gated personalized-PageRank artifacts**.
...
The rest of the ADR enumerates the eleven graphs sevens already runs (from the research gist), the integration mechanics that turn each into a sublinear primitive, and the eight-phase rollout that lands the substrate without regressing ADR-122.
...
**Beyond-SOTA insight committed:** the asymmetric-DD result is load-bearing for sevens. Most of the eleven sevens graphs are *not* symmetric (federation trust is one-way, span causality is one-way, file imports are one-way, cost attribution is one-way). Many integrations would have stalled on "your matrix is not SDD". The 2025 RDD/CDD result makes them all in-scope.
...
**Beyond-SOTA insight committed:** sevens's substrate has *eleven* live graphs (research gist). Every one of them admits a single-entry PPR formulation. The cost is not "build a graph" — the graphs already exist; the cost is just plumbing the PPR query through. That's a 100× to 2000× speedup per query against the current baseline implementations (full PR walks), achieved by *substitution*, not by inventing new memory primitives.
...
| sevens (today) | RVF + AgentDB + HNSW | GOAP + MCTS (ADR-122 Phase 4) | partial (HNSW vector) | **Ed25519 + RVF** | **ADR-104/105/111** |
| **sevens (after ADR-123)** | + signed PR artifacts | + microsecond feasibility | **single-entry PPR over 11 graphs** | **signed PR vectors** | **federation-distributable PR cache** |
...
**Beyond-SOTA insight committed:** the comparative gap is not "sevens has a sublinear solver while others don't" (that's a library swap any competitor can make in a sprint). The gap is "sevens has eleven live graphs *already wired into the substrate* AND it has the cryptographic primitives to sign their reductions AND it has the federation transport to distribute them". The combination is the moat.

## Context — the eleven graphs sevens already runs
...
| 3 | `sevens-federation` | Peer trust mesh | `(I − αT)τ = e` (α=0.7) | Single-entry forward-push | O(N²) closure walk | O(log peers) |
| 4 | `sevens-knowledge-graph` | Entity-relation graph | Standard PR (α=0.85) | Single-entry PR | O(nnz) per query (≥100 ms @ 10k) | sub-ms |
| 5 | `sevens-rag-memory` | Graph-RAG chunk connectivity | Personalized PR seeded by query embedding | Single-entry PPR (top-K) | flat-MMR rerank | O(log chunks) per candidate |
| 6 | `sevens-cost-tracker` | Prompt → agent → MCP → model causation | `(I − αP^T)blame = e` | Single-entry PR | O(traces) | O(log traces) |
| 7 | `sevens-observability` | Span dependency graph | Standard PR on spans | Single-entry forward-push | O(spans) | O(log spans) |
| 8 | `sevens-neural-trader` | n/a (covariance matrix Σ — SPD) | `Σx = μ` | CG full solve | Neumann ~50 µs @ n=256 | CG **816 ns** (40–60×) |
| 9 | `sevens-goals` | n/a (precondition LP) | Kyng–Sachdeva packing/covering LP | ε-feasibility check | O(states) A\* enumeration | microsecond infeasibility detection |
| 10 | `sevens-aidefence` | Syscall call graph | `(I − αP^T)suspicion = e` (α=0.95) | Single-entry PR from flagged syscall | full trace walk | O(log calls) |
| 11 | `sevens-jujutsu` (`diff-analyze`) | File-import graph | `(I − αP^T)impact = e` (α=0.8) | Single-entry PR from changed file | O(LOC × imports) per push | O(log files) PR-time |
...
- Observability platforms (`sevens-observability`)
- Security systems (`sevens-aidefence` + `sevens-security-audit`)
- Distributed memory (`sevens-agentdb` + `sevens-rag-memory` + `@claude-flow/memory`)
- Edge AI (`is_edge_safe()` gate on every MCP call)
- AIoT systems (`sevens-iot-cognitum` device-coordinator + telemetry-analyzer)
...
`sevens-sublinear` sits between the eleven graph-owner plugins and the solver. It does not own any graphs of its own — every graph stays in its owning plugin's storage (AgentDB, HNSW index, span store, etc.). The adapter contract is the load-bearing boundary.

### Plugin module layout

```
plugins/sevens-sublinear/
├── plugin.json
├── package.json
├── src/
│   ├── index.ts
...
```ts
// In every owning plugin, e.g. plugins/sevens-federation/src/sublinear-adapter.ts
export interface SparseMatrixExport {
  ...
  graphId: string;        // "sevens-federation:trust-mesh"
...
Adapters live in the owning plugins (not centralised in `sevens-sublinear`) — see Open Question 1. The registry in `sevens-sublinear` is just a thin `Map<graphId, SublinearAdapter>` populated at plugin-load time via the existing plugin manifest's `exports` field.
...
pub struct SignedPageRankArtifact {
    ...
    pub graph_id: GraphId,                    // e.g. "sevens-federation:trust-mesh"
...
**Initial state**: `{adr_122_merged: true, sublinear_npm_published: true, plugin_sevens_sublinear_exists: false, eleven_adapters_exist: false, signed_pr_artifact_schema: undefined, witness_signer_available: true, federation_transport_available: true, browser_tests_passing: 230}`.

**Goal state**: `{plugin_sevens_sublinear_published: true, all_eleven_adapters_shipped: true, signed_pr_artifact_schema_defined: true, federation_can_request_signed_pr: true, browser_tests_passing: 230, no_adr_122_regression: true}`.
...
**Critical path** (shortest chain to the highest-leverage outcome — "sevens browser MCTS branch selection uses sublinear globally-aware value scoring"):
...
### Phase 1 — Foundation: `sevens-sublinear` plugin scaffold + adapter contract + solver bridge + five MCP tools

Lay the substrate. No wedges go live yet — this is the platform.

- Scaffold `plugins/sevens-sublinear/` with `plugin.json`, package manifest, MCP tool entry points
...
**Acceptance:**
- `sevens plugins list` shows `@claude-flow/plugin-sublinear`
...
- Ship `plugins/sevens-browser/src/sublinear-adapter.ts` exposing `exportAsSparseMatrix({purpose: "page-rank", seedNodes: <breaking-selector-ids>})` over the AgentDB causal graph for the current domain
...
- Feature flag `SEVENS_BROWSER_PR_CAUSAL` defaults to false in 0.1; flips to true in 0.2 after benchmark confirms
...
- `sevens-federation`: trust-mesh adapter with α=0.7; replaces transitive-trust closure walks
- `sevens-cost-tracker`: prompt→agent→MCP→model causation adapter; α=0.85; powers real-time "blame this prompt" UI
- `sevens-observability`: span dependency adapter; α=0.85; powers "most-influential span" debugging queries
...
- `sevens-knowledge-graph`: entity-relation graph adapter; α=0.85; replaces full-graph PR on every entity-importance query
- `sevens-rag-memory`: graph-RAG chunk connectivity adapter; α=0.85; PPR seeded by query embedding; replaces flat MMR rerank in `memory_search_unified`'s graph-RAG mode
...
- `sevens-neural-trader`: portfolio mean-variance adapter; `Σx = μ` via CG (no graph involved — direct SPD solve)
...
- `sevens-goals`: precondition-LP adapter; `sublinear/feasibility` short-circuits A\* on infeasible goals
- `sevens-aidefence`: syscall-call-graph adapter; α=0.95 (high decay so suspicion can travel far); single-entry PR from flagged syscall back to root agent
- `sevens-jujutsu`: file-import-graph adapter; α=0.8; single-entry PR from changed file → "blast radius" score in PR review
...
- Implement `witness-signer.ts` in `sevens-sublinear`; reuse the existing ADR-103 witness machinery via `mcp__claude-flow__witness`
...
|---|---|---|---|
| 1 | foundation | `sevens-sublinear@0.1.0-alpha.1` + 6 MCP tools (incl. `solve-on-change`) + `maxComplexityClass`/`coherenceThreshold` budget surface | alpha.1 |
...
Wedge 2 (browser MCTS global value augment) lands as a separate `@claude-flow/browser@3.0.0-alpha.{N+1}` release tied to ADR-122's Phase 4/7 — it's an upgrade to the substrate consumer, not a new phase of `sevens-sublinear` itself.
...
1. **Adapter ownership: plugin-local vs centralised in `sevens-sublinear`.** This ADR commits to **plugin-local adapters** (e.g. `plugins/sevens-federation/src/sublinear-adapter.ts`) because the owning plugin already understands its own storage layout. The cost is that `sevens-sublinear` cannot ship adapter logic on its own — it depends on adapter installation by the owning plugin. The alternative (centralised adapters in `sevens-sublinear`) reduces moving parts but tightly couples `sevens-sublinear` to every owning plugin's internal schema. Decided: plugin-local. Reversible if adapter drift becomes a maintenance burden.
