...
Many sevens users already run local models or use multiple providers. Managing endpoints, API keys, and routing policies individually increases friction.
...
Offer an optional local proxy during onboarding (ADR-302), on credit exhaustion (ADR-303), and on demand via `sevens proxy install` / `sevens proxy enable`.

### Architecture

```
Client (any OpenAI-compatible SDK / sevens agents)
  ↓
localhost:11435
  ↓
Meta Proxy (local process, sevens-managed)
  ↓
api.cognitum.one
  ↓
...
- **Default state after `sevens proxy install` is local-only.** The proxy routes exclusively to local backends (Ollama, vLLM, SGLang). No prompt leaves the machine, and no request is made to api.cognitum.one for inference.
- **Cloud routing requires a separate explicit step** — `sevens proxy config --cloud` — gated on the `cloud-routing` consent domain (ADR-302). Neither enrollment acceptance, `auth login`, nor proxy installation enables it.
...
- **Visible at runtime.** `sevens proxy status` and every request receipt state the data plane used (`local` vs `cloud:<provider>`), so the user can verify where any given prompt went.
- Cloud routing can be disabled at any time (`sevens proxy config --local-only`), reverting to a purely local multi-backend router and revoking the `cloud-routing` consent receipt.
...
|---|---|---|
| Audience | Internal orchestration interface for development of sevens itself | Supported, customer-facing product |
...
- **Optional and removable** (ADR-150 discipline): the proxy ships as an optional component; sevens remains fully operational with it absent or uninstalled. No `dependencies` entry — install is an explicit user action.
- **No credentials in the repo or config files**: tokens live in the OS keychain where available, else `~/.sevens/credentials` with `0600` permissions; never in project config, never committed (existing `@claude-flow/security` boundary rules apply).
...
- New CLI surface: `sevens proxy …` — full lifecycle command set (`install|start|stop|status|logs|update|uninstall`) specified in ADR-307, plus `proxy config` for routing mode.
- `sevens doctor` gains a proxy health check component (details in ADR-307).
...
## Addendum (2026-07-16) — `sevens proxy config` implemented; real TOML wire values confirmed

`sevens proxy config --cloud [--yes] | --local-only` is implemented in
...
`sevens proxy config` (no flags) reports the current plane by reading the same file, defaulting to
