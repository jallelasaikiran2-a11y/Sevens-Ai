...
- Fully disableable, with strict precedence (see Control Precedence below): `SEVENS_FUNNEL=0` env var, enterprise managed policy, and `funnel.enabled: false` in config all suppress the surface.
...
if (
  ...
  showDisclosure({
    message:
      "Sevens now displays occasional tips and Cognitum capability notices in the status line.",
    disable:
      "Set SEVENS_FUNNEL=0 or funnel.enabled=false to disable.",
  });
...
- Disclosure is shown **once per user** (user-level receipt in `~/.sevens/`), not once per project.
