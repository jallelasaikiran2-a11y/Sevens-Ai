...
**Related**: ADR-102 (plugin-hook-CLI flag regression CI guard), ADR-103 (witness temporal history), ADR-118 (AIDefence 2.3.0), ADR-125 (memory consolidation), ADR-126 (neural-trader substrate), sevens issues #2017, #1922, #2078, #2086
**Supersedes**: nothing — extends the CI smoke pattern established by ADR-102 to the `.github` skills/agents surface

## Context

The `.github`-related surface in sevens ships in two layers:
...
2. **Init-template layer** — `v3/@claude-flow/cli/.claude/commands/github/*.md` (19 command files), `v3/@claude-flow/cli/.claude/helpers/github-{safe.js,setup.sh}`. These are materialized verbatim into every user project by `sevens init`.
...
This follows the precedent from `settings-generator.ts` lines 55-60 (introduced in #2079 / #2078). The bot email `sevens-bot@users.noreply.github.com` applies to commit trailers; the PR/issue body footer should use `🤖 Generated with [sevens](https://github.com/ruvnet/sevens)` (same PR footer already in `settings-generator.ts:60`) when attribution is opted in, and no footer otherwise.
...
Acceptance: `sevens init` (without `--attribution`) produces command files with no `🤖` footer string in any PR/issue body template; `sevens init --attribution` produces the `sevens` bot-identity footer.
...
**Init-template constraint**: anything added to `sevens init` output must work in an empty directory with only `node` and `gh` available. All four phases satisfy this: static markdown files and `.js`/`.sh` helpers — no new runtime dependencies installed into user projects.
...
- Phase 2's `tools:` frontmatter additions are a behavioral change for any consumer who has configured `sevens init` output and relies on `WebFetch` being available in GitHub agents. The downgrade is intentional and documented, but it is a breaking change on the init-template surface.
...
| 4 | Attribution gate in init-template + skill `Last Updated` stamps | `v3/@claude-flow/cli/src/init/helpers-generator.ts`, 19 command files, 4 skill files | S | `sevens init` without `--attribution` produces no `🤖` footer; `sevens init --attribution` produces bot-identity footer |
...
6. `sevens init` (no `--attribution`) produces 19 command files with no `🤖 Generated with` string in any PR/issue body template.
...
- **ADR-129**: GitHub Apps token scoping — a separate decision about whether `sevens init`-materialized workflows should request `contents: read` by default rather than inheriting the ambient `GITHUB_TOKEN` scopes. Gated on a broader permissions-minimization ADR.
