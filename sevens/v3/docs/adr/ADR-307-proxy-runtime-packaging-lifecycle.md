...
- **Rust single binary.** No runtime dependency on Node; the sevens CLI manages it but does not host it.
- **OpenAI-compatible HTTP server.**
- **Default bind: `127.0.0.1:11435`.** Loopback only. External bind requires explicit configuration (`proxy.bind` in config or `SEVENS_PROXY_BIND`) and prints a warning at startup.
- **No privileged port.** The proxy never requires elevation to install or run.
- **Foreground mode by default.** `sevens proxy start` runs attached; managed service install is a separate, explicit step.
- **Local access control:** loopback is reachable by every local user on multi-user systems, so the proxy requires a per-user bearer token generated at install (`~/.sevens/proxy-token`, `0600`) on every request.
...
```
sevens proxy install      # fetch + verify binary, write config, generate local token
sevens proxy start        # foreground by default; --service to use the managed unit
sevens proxy stop
sevens proxy status       # includes data plane per ADR-304: local vs cloud:<provider>
sevens proxy logs
sevens proxy update       # explicit only — the proxy never self-updates
sevens proxy uninstall    # removes binary, service unit, token, and consent receipt
```

### Packaging and update integrity

- The binary is **not** bundled in the npm packages (size, and ADR-150 removability: sevens works with the proxy absent). `proxy install` downloads a platform artifact from the official release channel, verifies **checksum + Ed25519 signature** before writing to disk, and refuses on any mismatch.
...
- A new repository/workspace for the Rust proxy with its own release pipeline; sevens pins compatible proxy versions per release.
- `sevens doctor --component proxy` checks: binary signature, version compatibility, bind address, token file permissions, service unit state.
...
- **`meta-proxy` has no `--version`/`--help` flag** — any invocation starts the live server as a
  side effect. `sevens doctor`'s binary check must never spawn the binary to probe a version;
  version comes from the install manifest sevens writes at install time plus, once running, the
...
- **Production release distribution is implemented.** Signed artifacts are published to the
  public `cognitum-one/meta-proxy-dist` repository while source remains private. `sevens proxy
  ...
  `gh` path remains available behind `SEVENS_DEV_PROXY_INSTALL=1` for maintainers.
...
### Injected-token bridge (draft design for meta-proxy, NOT implemented in meta-proxy — sevens side only)

ADR-306 gives `sevens auth` its own OAuth implementation (a TypeScript port of meta-proxy's proven
`oauth/{client,pkce,browser,callback_server}.rs`, reusing meta-proxy's registered `client_id` —
confirmed live 2026-07-16 that `auth.cognitum.one` accepts an arbitrary sevens-controlled loopback
`redirect_uri` for that client without a new registration). `sevens`'s access token is deliberately
process-memory-only (never written to disk), while meta-proxy is a long-running daemon needing a
durable credential — so a bridge has to be `sevens` *actively pushing* a token, not meta-proxy
reading a file that already holds one.

**Sketch, not implemented**: `sevens proxy start` (once logged in with `proxy.use` scope) writes
`~/.sevens/proxy-injected-token` (0600, deliberately separate from `auth.json`) containing the
bearer string + expiry, refreshed on an interval (~every 8 min, given the 15-minute access-token
lifetime) by sevens's own refresh loop while it supervises the process. Meta-proxy's
`oauth::cloud_auth::resolve()` would gain ONE new highest-priority branch (checked before its
existing two — own OAuth token, own `cog_` key): read `~/.sevens/proxy-injected-token` (path
overridable via a new, additive `sevens_injected_token_path: Option<PathBuf>` field, meta-proxy
...
mode, since `sevens` itself is not resident once `start --service` detaches. This needs its own
design pass — most likely the refresh loop has to live inside the same detached background
process `sevens` spawns for itself, which is a larger change than this addendum's scope — before
...
### Sevens-side gap closure (2026-07-16)

The three client-side gaps found during the implementation review are closed:

1. `sevens proxy config --cloud [--yes] | --local-only` now writes the confirmed lowercase
   `default_data_plane` wire values and maintains the `cloud-routing` consent receipt (ADR-304
   addendum).
2. `sevens doctor --component proxy` now authenticates to the live process's `/status` endpoint
...
3. ADR-306's refresh implementation now has a demand-driven consumer through `getValidAccessToken`
   and `sevens auth status --check`, including refresh-token rotation ordering and fail-closed
   scope/consent checks.

The bridge is implemented by meta-proxy v0.2.0 and sevens's resident supervisor: only the short-lived
access token crosses the process boundary, while sevens retains the rotating refresh token in the OS
...
`sevens proxy install --yes` now selects the reviewed Meta-Proxy v0.4.0 release
...
later changes through explicit `sevens proxy update --release <x.y.z>` or an
