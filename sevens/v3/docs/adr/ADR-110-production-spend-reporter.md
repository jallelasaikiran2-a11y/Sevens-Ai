# ADR-110 — Production SpendReporter: sevens-memory adapter
...
The `cost-tracker` plugin's federation consumer (`plugins/sevens-cost-tracker/scripts/federation.mjs`) reads from a specific contract:
- **Namespace**: `federation-spend`
- **Key pattern**: `fed-spend-<peerId>-<ts>`
- **Storage**: sevens memory CLI (`memory store --namespace federation-spend --key ...`)
...
export interface MemorySpendReporterConfig {
  /** Memory store impl. Integrators inject the sevens memory CLI / MCP tool / direct memory client. */
...
- Federation is meant to be pluggable — some integrators run sevens memory, others run their own KV store, some use Redis or DynamoDB
...
- Production setup with `MemorySpendReporter` + sevens memory MCP tool

## Anti-goals

- **No automatic memory backend selection.** The plugin doesn't try to detect sevens memory and auto-wire — the integrator is explicit about which backend to use.
