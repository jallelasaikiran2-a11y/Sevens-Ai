...
**Related**: ADR-102 (supply-chain CI guards), ADR-103 (witness temporal history), ADR-127 (GitHub stack modernization), sevens issues #2078, #2079, #2086
...
`DEFAULT_INIT_OPTIONS.skills.core`, `.agentdb`, `.github`, and `.v3` are all `true`. On a fresh install, all four default to copying skills — none of which copy. The `copySkills()` error path pushes to `result.errors` but `executeInit()` wraps the call in a try/catch that continues on error without surfacing the failure to the CLI output. A user who runs `sevens init` on a clean machine gets no skills installed and no warning.
...
|---|---|---|---|
| `coder.md` | `agents/core/` | `sevens-core` | 460 diff lines |
| `researcher.md` | `agents/core/` | `sevens-core` | 411 diff lines |
| `reviewer.md` | `agents/core/` | `sevens-core` | 527 diff lines |
| `tester.md` | `agents/core/` | `sevens-testgen` | 527 diff lines |
| `memory-specialist.md` | `agents/v3/` | `sevens-rag-memory` | 1,049 diff lines |
| `security-auditor.md` | `agents/v3/` | `sevens-security-audit` | 785 diff lines |
| `sparc-orchestrator.md` | `agents/v3/` | `sevens-sparc` | 261 diff lines |
| `goal-planner.md` | `agents/goal/` | `sevens-goals` | 68 diff lines |
| `adr-architect.md` | `agents/v3/` | `sevens-adr` | 191 diff lines |

There is no deduplication policy. A user who runs `sevens init` and then `sevens plugins install sevens-core` ends up with two competing definitions of `coder.md` in their workspace. No collision warning is issued; the last-writer wins silently.
...
ADR-127 established one explicit init-template constraint that applies here: *"anything added to `sevens init` output must work in an empty directory with only `node` and `gh` available."* ADR-127's Phase 4 also established the opt-in pattern for init defaults: `settings-generator.ts` lines 55–60 and `#2079` showed that init defaults should be opt-out-friendly. ADR-127's plug-and-play discipline (each phase ships a runnable artifact independently) applies here too.
...
- Any agent that is the canonical definition for a plugin's own subject matter: `sevens-core` owns `coder.md`, `sevens-rag-memory` owns `memory-specialist.md`, etc.
...
When `sevens plugins install <plugin>` runs and the plugin ships a file at `.claude/agents/X.md`:
...
- `core/coder.md`, `core/researcher.md`, `core/reviewer.md` (forked from `sevens-core`)
- `core/tester.md` (forked from `sevens-testgen`)
- `v3/memory-specialist.md` (forked from `sevens-rag-memory`)
- `v3/security-auditor.md` (forked from `sevens-security-audit`)
- `v3/sparc-orchestrator.md` (forked from `sevens-sparc`)
- `goal/goal-planner.md` (forked from `sevens-goals`)
- `v3/adr-architect.md` (forked from `sevens-adr`)
...
No behavioral change for users who have the relevant plugins installed; they already have the plugin's version. Users who do not have the plugins lose those 9 agents — they are available via `sevens plugins install sevens-core sevens-rag-memory sevens-security-audit sevens-sparc sevens-goals sevens-adr sevens-testgen`.
...
- `agents/data/` (2 files) — generic enough to either delete or move to `sevens-core`.
...
**Expected outcome**: a default `sevens init` installs ~24 agents (`core:5 + consensus:7 + swarm:3 + sparc:4 + testing:2 + browser:0 = ~21`; browser dir is currently empty) rather than 98. The full 85-agent set remains accessible via `sevens init --agents=all`.

The `--all` flag and the `agents.all` key are NOT removed — `sevens init --all` remains a supported pattern.

**Acceptance**: `sevens init` (no flags) installs <= 30 agents. `sevens init --agents=all` installs all agents in the template. Existing users who relied on `agents.all: true` are informed via a deprecation notice in the `sevens migrate` command output.
...
**Init-template constraint (from ADR-127)**: anything added or modified in `sevens init` output must work in an empty directory with only `node` and `gh` available. All five phases satisfy this: bundled skill directories are static markdown files with no runtime deps; agent deletions reduce rather than add requirements; `COMMANDS_MAP` additions are code-path changes in the CLI itself.

## Consequences

### Positive

- Phase 1 closes a silent regression that has affected every user who ran `sevens init` on a machine without a prior `~/.claude/skills/` — which is every fresh CI runner and every first-time user.
...
- Phase 3 reduces the default `sevens init` command count from 88 to 88 (no change for defaults; orphaned dirs get proper keys or are deleted) and removes domain-specific clutter from the base install.
...
- Phase 2 removes 9 agents from the init template. Users who have `sevens init`-initialized projects and rely on, say, `coder.md` from the init template without having `sevens-core` installed will find those agents missing after upgrading. Mitigation: `sevens migrate` should detect removed agents and print install suggestions.
...
- Phase 4's `agents.all: false` change is a breaking default change. Users who relied on the current all-agents behavior must pass `--agents=all` explicitly. The `sevens migrate` command must warn on upgrade.
