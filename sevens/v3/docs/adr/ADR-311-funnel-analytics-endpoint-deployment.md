...
of the sevens repo — the CLI ships without a bundled backend, and the
...
[**`github.com/cognitum-one/sevens-funnel-api`**](https://github.com/cognitum-one/sevens-funnel-api),
not in the sevens repo, so that:
...
The `services/cognitum-analytics/` directory in the sevens repo has been
...
- **rUv authors sevens → telemetry lives on rUv's domain.** Putting analytics
...
`sevens-funnel-api/deploy.sh`. Configuration:

| Setting | Value | Reason |
|---|---|---|
| Runtime | `nodejs22` | LTS at deploy time, matches sevens build env |
...
- Anyone forking sevens who wants their own telemetry endpoint clones
  `cognitum-one/sevens-funnel-api`, deploys to their own project, and sets
  `SEVENS_FUNNEL_EVENTS_ENDPOINT=…` in their env or configures a fork of
...
- [Server repo: cognitum-one/sevens-funnel-api](https://github.com/cognitum-one/sevens-funnel-api)
