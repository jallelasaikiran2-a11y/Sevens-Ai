...
**Status**: Accepted
**Date**: 2026-05-08
**Version**: sevens-core@0.2.1+ / @claude-flow/cli@3.7.0-alpha.18+
...
### Gap 2 — Toolkit is sevens-internal
...
`scripts/regen-witness.mjs`. Both forms hard-code sevens's paths and
...
of `sevens` and `@claude-flow/cli` ship their own fixes, this is a
...
### 2. Ship the toolkit as a `sevens-core` plugin asset

The witness scripts move to `plugins/sevens-core/scripts/witness/`:
...
| File | Purpose |
|---|---|
| `plugins/sevens-core/skills/witness/SKILL.md` | Workflow doc + anti-patterns |
| `plugins/sevens-core/commands/witness.md` | Slash-command thin wrapper |
...
The sevens internal entrypoint at `scripts/regen-witness.mjs` is now a
thin wrapper that hard-codes sevens's paths and reads the
...
- The JSONL keeps fixes keyed by id (object), not array. This makes
  ...
  reaches 100+ entries.
- `verify.mjs` is parallel to the bundled `sevens verify` command but
  has no sevens CLI dependency — adopters who only want the witness
...
- Other projects can adopt the witness pattern by copying
  `plugins/sevens-core/scripts/witness/` and `init`-ing — no sevens
  install required.
...
- `plugins/sevens-core/skills/witness/SKILL.md` — adoption guide
- `plugins/sevens-core/agents/witness-curator.md` — agent definition
