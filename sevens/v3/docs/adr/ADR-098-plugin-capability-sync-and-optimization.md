...
**Supersedes**: nothing
**Related**: ADR-094 (transformers loader), ADR-095 (architectural gaps), ADR-096 (encryption-at-rest), ADR-097 (federation budget circuit breaker), `plugins/sevens-*` directory

## Context

The `plugins/sevens-*` tree is the user-facing surface of Sevens on Claude Code — 32 plugins distributed via the Sevens marketplace, each bundling agent prompts, skills, slash commands, and (in some cases) hooks. End users install via `/plugin install sevens-X@sevens` and immediately get the agent / commands.
...
|---|---|---|
| ADR-096 encryption-at-rest (CLAUDE_FLOW_ENCRYPT_AT_REST gate, fs-secure helpers) | `sevens-aidefence`, `sevens-security-audit`, `sevens-rag-memory`, `sevens-rvf` | None of these mention it |
| ADR-097 federation budget circuit breaker (`maxHops`, `maxTokens`, `maxUsd`) | `sevens-federation` ✅, `sevens-cost-tracker` should consume `federation_spend` events | Federation has it; cost-tracker doesn't |
| `validateEnv()` loader-hijack denylist | `sevens-aidefence`, `sevens-security-audit` (relevant for threat agents) | Not surfaced |
| `validateBudget()` / `enforceBudget()` (federation) | `sevens-cost-tracker` | Not surfaced |
| AgentDB controllers activated in 3.6.24 (G7 — gnn, rvf, mut, att, gvb) | `sevens-agentdb`, `sevens-rag-memory`, `sevens-knowledge-graph` | Skill docs don't mention them |
...
Only `sevens-federation` references ADR-096 / ADR-097 / encryption / budget concepts. The other 31 plugins don't reference any post-3.6.13 capabilities. End users installing `sevens-aidefence` or `sevens-security-audit` see no mention of the new file-mode-0600 default, the encryption-at-rest gate, or the loader-hijack denylist — even though those plugins' agent prompts are explicitly about security posture.
...
|---|---|---|---|
| `sevens-cost-tracker` | cost-analyst | 105 | Heavy command-table inlining |
| `sevens-adr` | adr-architect | 96 | Lifecycle-state machine inlined |
| `sevens-ddd` | domain-modeler | 93 | DDD vocabulary table inlined |
| `sevens-iot-cognitum` | device-coordinator | ~80 | Trust-tier table inlined |
...
- `sevens-federation/federation-coordinator` — multi-phase coordination, trust scoring, audit-grade logging.
- `sevens-neural-trader/trading-strategist` — real-money trading decisions; opus is correct.
- `sevens-security-audit/security-auditor` — debatable; security review work is sonnet-tier in practice.
...
|---|---|
| `sevens-aidefence` | `validateEnv` loader-hijack denylist; chmod 0600 file mode; encryption-at-rest gate (defense-in-depth pairing) |
| `sevens-security-audit` | Same set, plus the github-tools / update/executor shell injection patterns to scan for |
| `sevens-rag-memory`, `sevens-rvf` | Encryption-at-rest gate (memory.db wraps under `CLAUDE_FLOW_ENCRYPT_AT_REST=1`) |
| `sevens-cost-tracker` | Federation budget breaker; `federation_spend` events; per-peer rolling aggregation API (when ADR-097 P3 lands) |
| `sevens-agentdb`, `sevens-knowledge-graph` | The 5 activated G7 controllers (gnn, rvf, mut, att, gvb) and their MCP tools |
| `sevens-federation` | Already done in v0.2.0 |
...
Affected:
- `sevens-cost-tracker/agents/cost-analyst.md` (105 → ≤ 60)
- `sevens-adr/agents/adr-architect.md` (96 → ≤ 60)
- `sevens-ddd/agents/domain-modeler.md` (93 → ≤ 60)
- `sevens-iot-cognitum/agents/device-coordinator.md` (~80 → ≤ 60)
...
Change `sevens-security-audit/agents/security-auditor.md` from `model: opus` → `model: sonnet`. Justification: security review is bounded-scope analysis that sonnet handles cleanly; opus's long-context advantage isn't load-bearing here. Track for a release cycle and revert if quality drops.
...
- This ADR does **not** change runtime code in `@claude-flow/cli`. All edits are in `plugins/sevens-*/`.
...
| **Part 2** — Token diet (4 fat agent prompts → ≤60 lines) | Implemented | 4 slices: `f1bb3cf84` (iot-cognitum), `1e5a8ec89` (sevens-ddd), `85eab480e` (sevens-adr), `5addd83b4` (sevens-cost-tracker) |
...
- [ ] `sevens-security-audit/security-auditor.md` is on `model: sonnet`.
...
- [ ] No regression in the plugin marketplace install path (`/plugin install sevens-X@sevens` still resolves).
- [ ] Spot-check: `sevens doctor -c agentic-flow` and the broader doctor output stays green.
