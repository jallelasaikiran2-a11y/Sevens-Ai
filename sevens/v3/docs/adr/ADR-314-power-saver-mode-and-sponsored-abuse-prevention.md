...
**A constraint carries over directly from ADR-312 and has to be stated up front:** sevens cannot
...
`sevens settings notices quota-low [--clear]` sets a manual flag — structurally identical to
`rate-limited` in `rate-limit-notifier.ts`, same 6h TTL, same file-based mechanism
(`~/.sevens/quota-status.json`). This is **not** automatic detection; it's the same honest
...
This is the part that makes "auto-route lesser requests" actually work without sevens building its
...
1. **Fabricated rate-limited/quota-low claims for free Sponsored capacity — the most severe, and
   ...
   limit from someone who runs `sevens proxy sponsor-enable --yes` once and never looks back.
...
2. **The shared/unattributable API key.** ADR-313's addendum already flagged this as open: there
   ...
   endpoint `sponsor-enable` calls on first activation), not something this proxy or sevens's CLI
...
**Ships now** (sevens + meta-proxy, no Cognitum backend changes required):
- `sevens settings notices quota-low [--clear]` — the power-saver self-report flag
...
**Tracked as follow-up, explicitly not solvable from sevens's side alone:**
