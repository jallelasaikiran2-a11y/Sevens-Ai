# sevens Research Widget Integration Guide

## 📦 Overview

The sevens Research Widget is a standalone, embeddable component that brings Goal-Oriented Action Planning research capabilities to any website. It's fully self-contained with no external dependencies required on the host page.
...
<script>
  window.addEventListener('load', function() {
    if (window.SevensResearchWidget) {
      window.SevensResearchWidget.init('my-custom-container');
    }
  });
</script>
```
...
window.addEventListener('DOMContentLoaded', function() {
  if (window.SevensResearchWidget) {
    window.SevensResearchWidget.init('sevens-research-widget-container');
  }
});
```
...
```javascript
console.log(window.SevensResearchWidget.version); // "1.0.0"
```
...
<script>
  window.addEventListener('load', function() {
    window.SevensResearchWidget.init('widget-1');
    window.SevensResearchWidget.init('widget-2');
  });
</script>
```
...
export default function SevensResearchWidget() {
  useEffect(() => {
    // Load widget script
  ...
  return <div id="sevens-research-widget-container" />;
}
```
