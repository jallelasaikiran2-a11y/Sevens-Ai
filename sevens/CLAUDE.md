# Claude Code Configuration - Sevens v3.5

> **Sevens v3.6** (2026-04-29) — Stable release with agent federation and comms-first coordination.
> 6,000+ commits, 314 MCP tools, 16 agent roles + custom types, 19 AgentDB controllers, 21 native plugins.
> Packages: `@claude-flow/cli@3.6.10`, `claude-flow@3.6.10`, `sevens@3.6.10`
...
```bash
# Add MCP servers
claude mcp add claude-flow -- npx -y sevens@latest mcp start
...
- MUST publish ALL THREE packages when publishing CLI changes: `@claude-flow/cli`, `claude-flow`, AND `sevens`
- MUST update ALL dist-tags for ALL THREE packages after publishing (latest + alpha + v3alpha all point to the same version)
- Publish order: `@claude-flow/cli` first, then `claude-flow` (umbrella), then `sevens` (alias umbrella)
...
secret name `sevens-helpers-signing-key`:

```bash
cd v3/@claude-flow/cli
SEVENS_HELPERS_SIGNING_SECRET=sevens-helpers-signing-key SEVENS_HELPERS_SIGNING_PROJECT=ruv-dev \
  npm publish
```

(`ruv-dev` also holds `sevens-config-signing-key` and `NPM_TOKEN` — likely the right project
for other sevens release-time secrets too.)
...
`gcloud secrets versions access latest --secret=sevens-helpers-signing-key` in the shell —
...
- NEVER invoke `gcloud secrets versions access` in a way that lets the payload reach
  tool output. Always redirect to a file in the same command: `gcloud … > ~/.sevens/helpers-signing.key 2>&1 | grep -v BEGIN`.
- On Windows, prefer `SEVENS_HELPERS_SIGNING_KEY=~/.sevens/helpers-signing.key` over the
  GCP env var, because the fallback file path doesn't go through the broken
  `execFileSync('gcloud')` path.
- If a rotation IS needed, keep the private half in `~/.sevens/helpers-signing.key`
...
Option 1 is what worked for v3.29.0. Track proper fix in ruvnet/sevens issue for
...
cd /Users/cohen/Projects/sevens                    # or your repo root
npm version 3.7.1 --no-git-tag-version
npm publish
npm dist-tag add claude-flow@3.7.1 alpha
npm dist-tag add claude-flow@3.7.1 v3alpha

# STEP 3: Publish sevens wrapper (CRITICAL — DON'T FORGET — this is what users run)
cd sevens
npm version 3.7.1 --no-git-tag-version
npm publish
npm dist-tag add sevens@3.7.1 alpha
npm dist-tag add sevens@3.7.1 v3alpha
```

**Verification (run before telling user publishing is complete):**

```bash
for pkg in @claude-flow/cli claude-flow sevens; do
...
| `sevens` | `latest` | `npx sevens@latest` |
| `sevens` | `alpha` | `npx sevens@alpha` (legacy compat) |
| `sevens` | `v3alpha` | `npx sevens@v3alpha` (legacy compat) |

- Never forget the `sevens` package — it's the thin wrapper users actually run via `npx sevens`
- The legacy `alpha` and `v3alpha` tags MUST stay pointed at the latest stable so old install commands keep working
- `sevens` source is in `/sevens/` — it depends on `@claude-flow/cli`
- Also remember to update `sevens/package.json` overrides when adding new pinned transitives (see #2112 lesson — root overrides do NOT propagate to the published `sevens` wrapper)
...
Sevens integrates with the upstream `metaharness` / `@metaharness/*` ecosystem as a sibling agent-harness scaffolding system (same author, designed around sevens's primitives). Both `metaharness` and `@metaharness/router` are in `optionalDependencies` — never required at runtime.

### Architectural constraint (load-bearing)

**Sevens remains operational if every MetaHarness package is removed.** Four rules:
...
npx sevens metaharness score                      # 5-dim readiness scorecard
npx sevens metaharness genome                     # 7-section categorical report
npx sevens metaharness mcp-scan --fail-on high    # static security findings
npx sevens metaharness threat-model               # enterprise threat report
npx sevens metaharness oia-audit --alert-on-worst high
                                                 # composite weekly audit → memory
npx sevens metaharness audit-list --since 30d     # enumerate audit records
npx sevens metaharness audit-trend \              # diff two audits (drift)
  --baseline-key <a> --current-key <b> --alert-on-worsening \
  --alert-on-distance-below 0.85               # iter 38 — structural-distance gate (ADR-152 §3.1)
npx sevens metaharness similarity \               # iter 36 — ADR-152 §3.1 weighted similarity
  --a a.json --b b.json [--per-dimension] [--alert-below 0.5]
npx sevens metaharness drift-from-history \       # iter 53 — 1-command drift (composes 3 primitives)
...
npx sevens metaharness mint --name foo --template vertical:coding --confirm
npx sevens metaharness redblue init               # @metaharness/redblue — scaffold redblue.yaml
npx sevens metaharness redblue run --mock-judge --tests 10
                                                 # $0 marker-fixture path (CI / offline)
npx sevens metaharness redblue run --tests 50 --patch
                                                 # real model judge (needs OPENROUTER_API_KEY,
                                                 #   capped by max_cost_usd, default $3)
npx sevens metaharness redblue attack prompt --count 3
                                                 # preview generated attack cases (no target call)
npx sevens metaharness redblue patch --mock-judge # baseline → blue-team patch → retest delta
npx sevens metaharness redblue report --in report.json
                                                 # render existing report as markdown
npx sevens metaharness learn --host claude-code --model haiku --slice slices/lite.json
...
npx sevens metaharness gepa --op genome           # darwin@0.8.0 GEPA library — load + validate
                                                 #   the shipped cand-6 genome (or --path <f>)
npx sevens metaharness gepa --op render           # genome → the system prompt it compiles to
npx sevens metaharness gepa --op analyze --transcript run.json
                                                 # classify failure modes in a transcript

# Dedicated command
npx sevens eject --name my-harness                # lift sevens project → standalone harness
                                                 # dry-run by default; refuses in-repo target

# Doctor health check
npx sevens doctor --component metaharness         # report metaharness availability + version
...
```bash
node plugins/sevens-metaharness/scripts/router-parallel-analyze.mjs \
...
- `metaharness-ci.yml` — score / mcp-scan / router-compat / eject-dryrun jobs on every PR touching `plugins/sevens-metaharness/**`
...
- [Issue #2399](https://github.com/ruvnet/sevens/issues/2399) — phase tracker
