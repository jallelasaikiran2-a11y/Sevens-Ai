...
// VEXORA model routing: agent type → optimal tier/model assignment.
// Tier aliases are resolved to real model slugs via resolveOpenAICompatModel()
// when OpenRouter is the active provider (which is the VEXORA default).
//
...
//
// Override per-model via RUFLO_MODEL_OPUS / RUFLO_MODEL_SONNET / RUFLO_MODEL_HAIKU.
...
export const agentTools: MCPTool[] = [
  {
    ...
    handler: async (input) => {
      ...
      const response: Record<string, unknown> = {
        ...
        note: 'Agent registered for coordination. Execution paths (VEXORA priority): ' +
